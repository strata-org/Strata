...
/-!
# Editor syntax-highlighting generation from the Core DDM grammar

This module holds the *pure* generation logic that derives VSCode
(TextMate) and Emacs syntax-highlighting definitions from the `Core`
dialect grammar. `editors/GenSyntax.lean` writes the files from it, and
`StrataTest.EditorSyntaxFreshness` uses `staleSyntaxFiles` to check the
checked-in files stay in sync.
...
/-! ## Output paths and freshness check -/

/-- Path (relative to the repository root) of the generated VSCode grammar. -/
def vscodeSyntaxPath : String := "editors/vscode/syntaxes/core-st.tmLanguage.json"

/-- Path (relative to the repository root) of the generated Emacs mode. -/
def emacsSyntaxPath : String := "editors/emacs/core-st-mode.el"

/-- The syntax info extracted from the current `Core` dialect grammar. -/
def coreSyntaxInfo : SyntaxInfo := extractSyntaxInfo Core

/-- Freshly generated VSCode (TextMate) grammar for the current `Core` grammar. -/
def vscodeContent : String := generateTextMate coreSyntaxInfo

/-- Freshly generated Emacs mode for the current `Core` grammar. -/
def emacsContent : String := generateEmacs coreSyntaxInfo

/-- The editor syntax files paired with the contents the generator produces
    from the current `Core` grammar. -/
def generatedSyntaxFiles : List (String × String) :=
  [ (vscodeSyntaxPath, vscodeContent),
    (emacsSyntaxPath, emacsContent) ]

/-- Return the checked-in editor syntax files that do not match what the
    generator would produce from the current `Core` grammar. An empty result
    means every file is up to date. A missing file counts as stale. -/
def staleSyntaxFiles : IO (Array String) := do
  let mut stale := #[]
  for (path, expected) in generatedSyntaxFiles do
    let existing ← try IO.FS.readFile path catch _ => pure ""
