...
theorem eval_stmts_with_assert
  ...
  | stmts_some_sem h_stmt h_rest =>
    cases h_in with
    | head =>
      -- The assert is the first statement and it evaluated
      -- We need to extract the condition from h_stmt
      -- Since stmt = Statement.assert label expr md, we can use eval_assert_implies_condition
      sorry
    | tail _ h_in_rest =>
      -- Recurse on the rest
      exact eval_stmts_with_assert π φ _ _ _ _ _ label expr md h_rest h_in_rest
