...
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr { (@TEnv.default Unit).updateContext { types := [[("x", t[int])]] }  with functions := testIntFns}
                             esM[(λ (~Int.Add %0))]
         return (format $ ans.fst.toLMonoTy)
...
-/
#guard_msgs in
#eval do let ans ← LExpr.annotate { (@TEnv.default Unit).updateContext { types := [[("x", t[int])]] } with functions := testIntFns}
                                   esM[((~Int.Add x) (~Int.Neg #30))]
         return (format $ ans.fst)
...
-/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr { (@TEnv.default Unit).updateContext { types := [[("fn", t[∀a. %a → %a])]] } with functions := testIntFns}
                             esM[(λ (%0 (fn #true)))]
         return format ans.fst
