...
-- type check.
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default
                             esM[λλ %2]
         return (format $ ans)

/-- info: ok: (((λ (%0 : $__ty3)) : (arrow $__ty3 $__ty3)) (y : $__ty3)) : $__ty3) -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default (TEnv.default.updateContext { types := [[("y", t[∀x. %x])]] })
                            esM[((λ %0) y)]
         return (format $ ans.fst)

/-- info: error: Cannot unify differently named type constructors bool and int! -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default (TEnv.default.updateContext { types := [[("x", t[bool])]] })
...
#guard_msgs in
#eval do let ans ← LExpr.annotate LContext.default ( TEnv.default.updateContext { types := [[("succ", t[int → int])]] })
                               esM[λ(succ %0)]
         return (format $ ans.fst)

/-- info: ok: (∀(int) ((%0 : int) == (#5 : int)) : bool)) -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default esM[∀ (%0 == #5)]
         return (format $ ans.fst)

/-- info: ok: ((λ (%0 : $__ty0)) : (arrow $__ty0 $__ty0)) -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default esM[λ(%0)]
         return (format $ ans.fst)

/-- info: ok: (#5 : int) -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default esM[#5]
         return (format $ ans.fst)

/-- info: ok: int -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default esM[((λ %0) #5)]
         return (format $ ans.fst.toLMonoTy)

/-- info: ok: (arrow $__ty0 int) -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default esM[λ #5]
         return (format $ ans.fst.toLMonoTy)
...
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default esM[λλ(%0 %1)]
         return (format $ ans.fst.toLMonoTy)

/-- info: ok: (arrow (arrow int $__ty4) $__ty4) -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr LContext.default TEnv.default esM[((λλ (%0 %1)) #5)]
         return (format ans.fst.toLMonoTy)
...
-/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr {LContext.default with functions := testIntFns} TEnv.default
                             esM[~unit]
         return (format $ ans.fst)

/-- info: ok: (~unit : unit) -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr { LContext.default with functions := testIntFns, knownTypes := makeKnownTypes [t[unit].toKnownType!] } TEnv.default
                             esM[~unit]
         return (format $ ans.fst)

/-- info: ok: int -/
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr {LContext.default with functions := testIntFns}
                    ((@TEnv.default Unit).updateContext { aliases := [{typeArgs := [], name := "myInt", type := mty[int]}]})
...
#guard_msgs in
#eval do let ans ← LExprT.fromLExpr {LContext.default with functions := testIntFns} ((@TEnv.default Unit).updateContext { types := [[("x", t[int])]] })
                             esM[(λ (~Int.Add %0))]
         return (format $ ans.fst.toLMonoTy)
...
#guard_msgs in
#eval do
    let ans ← LExprT.fromLExpr LContext.default (@TEnv.default Unit) (.const "3" (some .bool))
    return (format $ ans.fst.toLMonoTy)

/-- info: ok: real -/
#guard_msgs in
#eval do
    let T ← ((@LContext.default Unit).addKnownTypeWithError ⟨ "real", 0⟩ f!"error")
    let ans ← LExprT.fromLExpr T (@TEnv.default Unit) (.const "true" (some .real))
    return (format $ ans.fst.toLMonoTy)
