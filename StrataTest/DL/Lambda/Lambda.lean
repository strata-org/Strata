...
#eval do let F ← IntBoolFactory.addFactoryFunc { name := "Int.Add",
                                                 inputs := [],
                                                 output := .tcons "int" [] }
         let ans ← typeCheckAndPartialEval (fun _ => []) TypeFactory.default F esM[((~Int.Le ((~Int.Div #300) ((~Int.Add #2) #1))) #100)]
         return format ans
...
-/
#guard_msgs in
#eval format $ typeCheckAndPartialEval (fun _ => []) TypeFactory.default  IntBoolFactory
