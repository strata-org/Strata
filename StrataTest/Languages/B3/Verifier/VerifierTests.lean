...
partial def flattenConjunction : B3AST.Expression SourceRange → List (B3AST.Expression SourceRange)
                ...
                | some diag =>
                    if !diag.diagnosedFailures.isEmpty then
                      ...
                      for failure in diag.diagnosedFailures do
                        ...
                        -- Show assumptions for this failure
                        if !failure.pathCondition.isEmpty then
                          IO.println "             under the assumptions"
                      ...
                      if !result.pathCondition.isEmpty then
                        match stmt with
                        | .check m expr | .assert m expr =>
                            let exprLoc := formatSourceLocation baseOffset m
                            let formatted := formatExpressionOnly prog expr
                            IO.println s!"  └─ {exprLoc}: could not prove {formatted}"
                            IO.println "             under the assumptions"
                            for expr in result.pathCondition.reverse do
                              -- Flatten conjunctions to show each on separate line
                              for conjunct in flattenConjunction expr do
                                let formatted := formatExpressionOnly prog conjunct
                                IO.println s!"               {formatted}"
                        | .reach m expr =>
                            let exprLoc := formatSourceLocation baseOffset m
                            let formatted := formatExpressionOnly prog expr
                            IO.println s!"  └─ {exprLoc}: unreachable {formatted}"
                            IO.println "             under the assumptions"
                            for expr in result.pathCondition.reverse do
                              -- Flatten conjunctions to show each on separate line
                              for conjunct in flattenConjunction expr do
