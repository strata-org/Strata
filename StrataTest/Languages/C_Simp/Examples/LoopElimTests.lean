...
private def nondetLoopProgram : C_Simp.Program :=
  ...
  let iLeN : Expression.Expr := .app () (.app () (.op () "Int.Le" none : C_Simp.Expression.Expr) iExpr) nExpr
  let iAddOne : Expression.Expr := .app () (.app () (.op () "Int.Add" none : C_Simp.Expression.Expr) iExpr) one
  { funcs := [{
    name := ⟨"nondetLoop", ()⟩,
    pre := .app () (.app () (.op () "Int.Ge" none : C_Simp.Expression.Expr) nExpr) zero,
    post := .true (),
    ret_ty := .tcons "int" [],
