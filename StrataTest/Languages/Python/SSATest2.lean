...
meta import Strata.Languages.Python.PythonToSSA2
...
/-! ## Tests for Python → SSA translation (v2 — strict block-argument SSA)

Same test harness as SSATest, but uses PythonToSSA2 which eliminates
cross-block references via the fwd mechanism. Dumps actual output to
`/tmp/ssa_test2_actual/` for comparison.
-/

namespace Strata.Python.SSATest2

open Strata.Python.PythonToSSA2 (translateModule)
...
private meta def runTestCase
  ...
  IO.FS.writeFile s!"/tmp/ssa_test2_actual/{testName}.actual" actual
...
#eval withPython fun pythonCmd => do
  IO.FS.createDirAll "/tmp/ssa_test2_actual"
  IO.FS.withTempDir fun tmpDir => do
    ...
    IO.println s!"SSATest2: {passed}/{tasks.size} passed"
    if errors.size > 0 then
      IO.println s!"SSATest2 differences ({errors.size}):"
      for err in errors do
        IO.println err

end Strata.Python.SSATest2
