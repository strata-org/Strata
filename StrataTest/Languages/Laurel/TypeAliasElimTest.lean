...
private def mkProc (name : String) (inputs : List Parameter) (outputs : List Parameter)
    (body : Body := .Transparent ⟨.Block [] none, none, none⟩) : Procedure :=
...
private def chainedProgram : Program :=
  { staticProcedures := [
      mkProc "test"
        ...
        (.Transparent ⟨.Return (some ⟨.Identifier (mkId "x"), none, none⟩), none, none⟩)
    ]
    staticFields := []
    types := [
...
private def procSigProgram : Program :=
  { staticProcedures := [
      mkProc "compute"
        ...
        (.Transparent ⟨.Return (some ⟨.Identifier (mkId "a"), none, none⟩), none, none⟩)
    ]
    staticFields := []
    types := [
