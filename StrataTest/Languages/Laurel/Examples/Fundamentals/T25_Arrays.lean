...
already covered by `arrayNonIntElement`; this exercises the separate
`validateProcedure → validateHighType` entry on `proc.inputs`. -/
...
#strata
program Laurel;
// A destructive write whose value disagrees with the element type is a type
// error (the value is checked against the array's element type).
procedure arrayWriteWrongElem()
  opaque
  modifies *
{
  var a: Array<int> := [1, 2, 3];
