...
// --- Procedures ---
procedure Register(id : int, value : int, registry : Map int OptionInt, count : int) returns (registry : Map int OptionInt, count : int)
spec {
...
procedure GetValue(id : int, registry : Map int OptionInt) returns (res : OptionInt)
spec {
...
procedure Main(registry : Map int OptionInt, count : int) returns (registry : Map int OptionInt, count : int)
spec {
}
{
    ...
    call registry, count := Register(101, 500, registry, count);

    var result : OptionInt;
    call result := GetValue(101, registry);
