...
postconditions: 
body: assume [Extract_requires_0] (~List..isCons $__xs0)

(procedure Test :  () → ())
modifies: []
preconditions: 
postconditions: (Test_ensures_0, #true)
body: init (xs : (List int)) := init_xs_0
xs := ((~Cons #1) ~Nil)
havoc xs
init (h : int) := init_h_1
assert [(Origin_Extract_Requires)Extract_requires_0] (~List..isCons $__xs2)
call [h] := Extract([xs])
assert [Test_ensures_0] #true

---
info: Obligation: (Origin_Extract_Requires)Extract_requires_0
Property: assert
Result: ❌ fail
Model:
($__xs2, (as Nil (List Int))
