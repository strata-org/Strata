  ...
  { name := "Alpha"
    ...
    constrs := [
      { name := ⟨"AlphaValue", .unres⟩, args := [
          (⟨"beta", .unres⟩, .tcons "Beta" []),
          (⟨"gamma", .unres⟩, .tcons "Gamma" [])
  ...
  { name := "Beta"
    ...
    constrs := [
      { name := ⟨"BetaValue", .unres⟩, args := [
          (⟨"delta", .unres⟩, .tcons "Delta" []),
          (⟨"epsilon", .unres⟩, .tcons "Epsilon" [])
...
def deltaDatatype : LDatatype Visibility :=
  { name := "Delta"
    typeArgs := []
    constrs := [
      { name := ⟨"DeltaValue", .unres⟩, args := [(⟨"zeta", .unres⟩, .tcons "Zeta" [])], testerName := "Delta..isDeltaValue" }
    ]
    constrs_ne := by decide }
...
def epsilonDatatype : LDatatype Visibility :=
  { name := "Epsilon"
    typeArgs := []
    constrs := [
      { name := ⟨"EpsilonValue", .unres⟩, args := [(⟨"zeta", .unres⟩, .tcons "Zeta" [])], testerName := "Epsilon..isEpsilonValue" }
    ]
    constrs_ne := by decide }
...
def zetaDatatype : LDatatype Visibility :=
  { name := "Zeta"
    typeArgs := []
    constrs := [
      { name := ⟨"ZetaValue", .unres⟩, args := [(⟨"value", .unres⟩, .int)], testerName := "Zeta..isZetaValue" }
    ]
    constrs_ne := by decide }

/--
info: (declare-datatype Zeta (
  (ZetaValue (Zeta..value Int))))
(declare-datatype Epsilon (
  (EpsilonValue (Epsilon..zeta Zeta))))
(declare-datatype Gamma (
  (GammaValue (Gamma..epsilon Epsilon) (Gamma..zeta Zeta))))
(declare-datatype Delta (
  (DeltaValue (Delta..zeta Zeta))))
(declare-datatype Beta (
  (BetaValue (Beta..delta Delta) (Beta..epsilon Epsilon))))
(declare-datatype Alpha (
  (AlphaValue (Alpha..beta Beta) (Alpha..gamma Gamma))))
; alphaVar
