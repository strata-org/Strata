...
Property: assert
Assumptions:
<label_ite_cond_true: (~Bool.Not (~IntList..isNil cur))>: !(IntList..isNil(xs@2))
...
Assumptions:
assume_entry_invariant_0_0: 0 + listLen(xs@2) == listLen(xs@2)
<label_ite_cond_true: (~Bool.Not (~IntList..isNil cur))>: if !(IntList..isNil(xs@2)) then !(IntList..isNil(xs@2)) else true
...
<label_ite_cond_false: !(~Bool.Not (~IntList..isNil cur))>: if if !(IntList..isNil(xs@2)) then false else true then if !(IntList..isNil(xs@2)) then false else true else true
Obligation:
