...
info: ok: [(type Foo (a : Type, b : Type);
  ...
  func Seq.empty : ∀[a]. () → (Seq a);
  func Seq.append : ∀[a]. ((s1 : (Seq a)) (s2 : (Seq a))) → (Seq a);
  func Seq.select : ∀[a]. ((s : (Seq a)) (i : int)) → a;
  func Seq.length : ∀[a]. ((s : (Seq a))) → int;
  func Seq.build : ∀[a]. ((s : (Seq a)) (v : a)) → (Seq a);
  func Seq.update : ∀[a]. ((s : (Seq a)) (i : int) (v : a)) → (Seq a);
  func Seq.contains : ∀[a]. ((s : (Seq a)) (v : a)) → bool;
  func Seq.take : ∀[a]. ((s : (Seq a)) (n : int)) → (Seq a);
  func Seq.drop : ∀[a]. ((s : (Seq a)) (n : int)) → (Seq a);
