    ...
    IO.FS.withTempDir fun strataDir => do
      ...
      | .ok (sigs, warnings) =>
        ...
        let expectedWarnings := #[
          ...
          "float arithmetic in predicates is not yet supported" -- deferred float arith
        ]
...
/-- Test that integer arithmetic operators (`+`, `-`, `*`, `//`, `%`)
    inside predicate bodies translate cleanly. -/
meta def predicateArithTestCase : IO Unit := withPython fun pythonCmd => do
  IO.FS.withTempFile fun _handle dialectFile => do
    IO.FS.writeBinFile dialectFile Strata.Python.Python.toIon
    IO.FS.withTempDir fun strataDir => do
      let r ←
        translateFile
          ...
          (dialectFile := dialectFile)
          (strataDir := strataDir)
          (pythonFile := testDir / "predicate_arith.py")
          (searchPath := testDir)
          |>.toBaseIO
      match r with
      | .ok (sigs, warnings) =>
        if sigs.isEmpty then
          throw <| IO.userError "Expected signatures from predicate_arith.py but got none"
        if !warnings.isEmpty then
          let warnStr := warnings.foldl (init := "") fun acc w => s!"{acc}\n  {w}"
