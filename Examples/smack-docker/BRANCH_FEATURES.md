...
| 8 | Callee bodies not emitted — only the entry procedure body was translated; CBMC reported `[.no-body.<callee>]` on every call. **Partial fix `ca95931be`; then call-elim-on-CBM (`w1cp91531`, 2026-06-09) lowered the `__VERIFIER_assume`/`assert_.iN` intrinsics inline to `ASSUME`/`ASSERT (arg != 0)` at the FUNCTION_CALL site — sound (polarity verified both axes via a controlled negative probe), retiring the named residual. ~108 LoC, build/tests green; patched worktree left for PR promotion (htd/smack-only — CBM/GOTO path not upstream-targetable).** **Correction to the framing: `[.no-body]` is NOT the first blocker on real programs.** The 8 real SMACK pilot programs abort *upstream* of `[.no-body]` — rc=6 on synthetic `_initialize` global-initializer param typing (`got bool expected integer` on `_exnv`; `got bool expected array` on `_M_0`) and rc=134 array-bounds — so the call-elim fix is byte-identical BEFORE==AFTER on them (no real-program verdict flip). The documented "all FAIL via [.no-body]" baseline reproduces only on a hand-built minimal pilot. **Next real CBM blocker: `_initialize` param bool-vs-int/array typing in Core→GOTO emission**, independent of call-elim. Report: `reports/callelim-on-cbm-prototype-2026-06-09.md`. | [#1184](https://github.com/strata-org/Strata/issues/1184) (open; related: missing multi-return support) | ✓ `ca95931be` + call-elim-on-CBM prototype (worktree, not yet committed) | — |
...
### 9.5 Strata `verify` runtime (5 — #17 open, #22 fixed via cherry-pick, #29 implemented-local, #30/#31 open)
...
| 29 | `evalCFGBlocks` loop-with-invariant OOM on `FunctionPreconditions_Part4/loopGuardPrecondPgm`. **Root cause confirmed (census workflow `wqlj6z95v`, 2026-06-09):** `evalCFGBlocks` (`Strata/Languages/Core/ProcedureEval.lean:133-149`) is a **fuel-only worklist with no visited-set / fixpoint / back-edge awareness**, and `evalCFGStep` (`:106-130`) forks *both* successors on a symbolic `condGoto`, so it literally **unrolls** the reducible loop back-edge (`StructuredToUnstructured.lean:132` decrease block `.goto lentry`) until fuel runs out — heap grows one `Env` per pseudo-iteration. The structured path doesn't OOM because `LoopElim` runs an acyclic havoc-and-cut on `.loop` Stmts (`LoopElim.lean:125-203`); the CFG path never does (`:249` passes `.cfg` bodies through unchanged). **NOT an irreducibility problem** — census found 0 irreducible CFGs across all 3,767 procedures in the three corpora. Original monolithic `FunctionPreconditions.lean` leaf hit OOM (exit 137) at 147s; hand-split into Part1-4 (Part1+Part2 unblocked 9 blocks; Part3 fails fast on F2 funcDecl drop; Part4 isolates `loopGuardPrecondPgm` and still hangs). **Recommended fix: a CFG-level loop-elim pre-pass** (recover the loop contract from the header `condGoto`'s `specLoopInvariant`/`specDecreases` metadata at `StructuredToUnstructured.lean:146-151`, splice `LoopElim`'s acyclic recipe to remove the back-edge before `evalCFGBody`); verification path only, not the CBMC/GOTO path. See `reports/irreducible-cfg-census-2026-06-09.md` + BACKLOG.md. **IMPLEMENTED + #29 closure measured (2026-06-10, local — uncommitted; `reports/cfg-loopelim-implementation-2026-06-10.md`):** built `Strata/Transform/CFGLoopElim.lean` (dominators → back-edge → natural-loop region → acyclic recipe with mandatory VC2), wired `cfgLoopElimPipelinePhase` into `transformPipelinePhases` after `loopElimPipelinePhase` (verification path only — CBMC/GOTO untouched). The reproducer `verifyCFG loopGuardPrecondPgm` flips **non-terminating (240 s timeout, 12.5 GB RSS) → 9 s / 847 MB, all VCs ✅**; the previously OOM-killed `FunctionPreconditions_Part4` CFGForm leaf is now a passing `#guard_msgs` test. Verdict-set is a sound subset of the structured oracle (VC1+entry-SafeDiv+VC2; missing only a benign post-havoc SafeDiv re-test — completeness gap, `n ∉ M`). Soundness gate passes: a wrong invariant (`1==0`) FAILs VC1 in both forms. Zero regressions (the 8 still-failing CFGForm leaves are the documented loop-free #26/#27/#28 ones — the phase is a no-op on loop-free CFGs). Committed regression coverage: a new tracked test `StrataTest/Languages/Core/Examples/CFGLoopElim.lean` verifies a `.cfg`-body loop end-to-end (CI-globbed; independent of the untracked CFGForm harness). **Residual:** measure (VC3/VC4) loops deliberately not re-emitted — `stmtsToCFG` already lowers the measure (header VC3 + decrease-block VC4), so naive re-emission double-counts and orphans the decrease block (tested + reverted; the 3/313 `specDecreases` procs need decrease-block rewriting, but their OOM is still closed); corpus-scale TIMEOUT-unblock sweep deferred. **Branch locality:** htd/smack-only — main has no `.cfg` body shape so `evalCFGBlocks` does not exist there. | not filed yet | ✓ `CFGLoopElim.lean` (local, uncommitted) | — |
| 30 | **`strata verify` temp-file name-too-long (errno 63)** on procedures with very long mangled names. Surfaced by the EQ end-to-end re-sweep (2026-06-10, `reports/eq-resweep-end-to-end-2026-06-10.md`): 13 of 1,253 #1331-cleared EQ procedures fail with `invalid argument (error code: 63, file name too long)`. **Deterministic and name-length-driven** — every failing proc name is 265–278 chars (deeply-mangled Java signatures like `reffile_com_alibaba_excel_converters_..._convertToExcelData__lp_...`); strata writes a per-obligation temp `.smt2` file named after the *unhashed* obligation name (`<MANGLED>_ensures_0_2.smt2`), exceeding the OS 255-byte path-component limit. Concentrated in `EQ_4yipg2qisfn_out` (5), `EQ_52uajzr2i5s_out` (4), `EQ_xcnu42auqk5_out` (4). **No soundness risk** — fails loudly (rc=3, zero spurious `goals passed`). **NOT #1331** (orthogonal to `old`-modifies; only surfaces *because* #1331 cleared the type-check gate). **Fix: hash/truncate the obligation name in strata's temp-file path.** **Branch locality:** strata-core SMT-emission path; long mangled names are an EQ/Java-corpus trait, not pilot. | not filed yet | — | (path is shared strata core; trigger is EQ-corpus-specific) |
| 31 | **Java float-array-heap SMT type-mismatch crash.** Surfaced by the same EQ re-sweep: 1 of 1,253 procedures (`EQ_knmfqqxigdy_out`, proc `...ReturnMoneyCalculator.getMoneyWithProfitOnBSide`) hits `SMT Solver Crash!` with a Z3 parse error — `Subexpressions must have the same type: (= Output_..._floatArrHeap@44 otherfile__null)`, equating a `floatArrHeap` (`Map StrataRef (Map (_ BitVec 32) Real)`) with a `null` (`StrataRef`). A type-mismatch in the SMT lowering of the Java float-array `$heap` model. **No soundness risk** — crashes loudly (rc=3, zero spurious `goals passed`). **NOT #1331.** Fix direction: the float-array heap SMT encoding (or the translator type for null-vs-array-heap). **Branch locality:** EQ/Java-corpus `$heap` model. | not filed yet | — | (EQ-corpus-specific) |
...
- **31 distinct bugs/issues** surfaced (or confirmed) on this branch (24 → 31: +#25 F1+F4 `flushCmds` empty-accum dispatch drop fixed at `437d38683`; +#26 F2 `.typeDecl`/`.funcDecl` silent drop in `stmtsToCFG`; +#27 `evalCFGBlocks` unreachable-arm obligation pruning on `Cover/reachCheckMixedPgm`; +#28 cosmetic CFG-vs-structured path-condition asymmetry; +#29 `evalCFGBlocks` loop-with-invariant OOM on `FunctionPreconditions_Part4`; +#30 `strata verify` errno-63 temp-file-name-too-long on 265–278-char mangled Java proc names; +#31 Java float-array-heap SMT type-mismatch crash — both #30/#31 surfaced by the EQ end-to-end re-sweep, `reports/eq-resweep-end-to-end-2026-06-10.md`).
...
- **7 not yet patched** (#1184, #1186, #17 deeply-nested-expr `elabExpr` rewrite, #26 F2 silent drop, #27 unreachable-arm pruning triage, #30 errno-63 long-name temp file, #31 float-heap SMT type mismatch). **#29 evalCFGBlocks loop OOM is now fixed locally** via the new CFG-native loop-elim pass (`Strata/Transform/CFGLoopElim.lean`, uncommitted) — reproducer flips non-terminating → 9 s, all VCs pass; `reports/cfg-loopelim-implementation-2026-06-10.md`.
