...
- **24 distinct bugs/issues** surfaced (or confirmed) on this branch.
- **19 fixed on htd/smack** (#18 PASS-? matrix gap fixed at `a817909fc`; #21 large-`.bpl` hang fixed at `277c468cb`; #22 long flat-list overflow fixed by 4-commit cherry-pick `95abbe567`/`7b1018e81`/`73c17b1bd`/`fab1575f8`; #23 obligation-pipeline SO fixed at `494cf1147` via `balancedNondetIte`, merged as `346f55083`).
- **1 fixed on side branch, draft-ready** (#24 SMT2 e-15 emission fixed on `htd/decimal-e15-fix` at `6f5e74fa6`; issue draft at `strata-decimal-e15-emission-bug.md` (repo root); not yet pushed, filed, or merged into htd/smack).
