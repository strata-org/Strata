...
| [irreducible-cfg-census-2026-06-09.md](irreducible-cfg-census-2026-06-09.md) | RESOLVED-OBSERVATIONAL (workflow `wqlj6z95v`, 2026-06-09; migrated from BACKLOG) — **zero irreducible CFGs** across all three corpora (EQ-200 0/3293, SMACK pilot 0/469, StrataExamples 0/5; 3,767 procs, 313 loop-bearing, all reducible single-header). SMACK/javac/`stmtsToCFG` produce reducible CFGs by construction; `StrataGenerator.cs:1329,1340` rejects irreducible flow. Load-bearing byproduct: confirmed the #29 OOM root cause (`evalCFGBlocks` fuel-only worklist unrolls reducible loop back-edges) and motivated the CFG-level loop-elim pass. | None for irreducibility (zero frequency, no payoff). Actionable output is the CFG-level loop-elim pass — now IMPLEMENTED, see row below. |
| [cfg-loopelim-effect-2026-06-09.md](cfg-loopelim-effect-2026-06-09.md) | SUPERSEDED-BY implementation report below — design-lock study (workflow `w7gh4f4nk`): Framing A (net-new CFG pass), naive cut is UNSOUND (must emit VC2), default invariant `true`. Its Measure phase died on transient 503s ×3, so #29 closure was never measured. | Superseded — the build + measurement is done; see `cfg-loopelim-implementation-2026-06-10.md`. |
| [cfg-loopelim-implementation-2026-06-10.md](cfg-loopelim-implementation-2026-06-10.md) | RESOLVED (#29 closed, local/uncommitted) — built `Strata/Transform/CFGLoopElim.lean` + wired `cfgLoopElimPipelinePhase`. The reproducer `verifyCFG loopGuardPrecondPgm` flips **non-terminating (240 s timeout, 12.5 GB RSS) → 9 s / 847 MB, all VCs ✅** (the measurement the effect study never completed). Verdict-set = sound subset of structured oracle (VC1+entry-SafeDiv+VC2; missing only a benign post-havoc SafeDiv re-test, `n ∉ M`). Wrong-invariant soundness gate passes (VC1 FAILs both forms). Zero regressions. | Commit pending. Residual: wire measure VC3/VC4 into the CFG recipe (3/313 procs); run corpus-scale TIMEOUT-unblock sweep (plan Task 5). |
...
## #1331 fix verification (`old(<unmodified-global>)`)

| Report | Status | Next action |
|---|---|---|
| [verify-1331-fix-corpus-2026-06-10.md](verify-1331-fix-corpus-2026-06-10.md) | RESOLVED-OBSERVATIONAL (type-check layer) — fix `188255668` clears the old-fvar ELAB error 24/24 = 100% on an 80-file stratified sample, 0 regressions; 67% then reach a clean type-check, 33% hit the pre-existing modifies-completeness blocker (BACKLOG follow-up). | None — superseded for end-to-end verdicts by the row below; retained for the type-check-layer measurement. |
| [eq-resweep-end-to-end-2026-06-10.md](eq-resweep-end-to-end-2026-06-10.md) | RESOLVED-OBSERVATIONAL (end-to-end SMT) — the 16 cleared files pushed past type-check into SMT, **all 1,253 procedures** verified `--split-procs`: **927 real-proof PASS (74.0%), 1,166 non-failing (93.1%), 0 #1331 regressions, 0 soundness holes.** 73 timeouts (mixed: deep VC-gen stall + hard-SMT + ≥1 masked real failure) + 14 loud errors. Surfaced two new EQ defects → BRANCH_FEATURES §9.5 #30 (errno-63 long-name temp file) + #31 (float-heap SMT type mismatch). Durable artifacts under `Examples/smack-docker/eq-resweep/`. | Optional higher-budget re-run (≥300 s/proc) to split the 73 timeouts into real-PASS / real-FAIL. The two new defects are the next blockers for clean EQ end-to-end verification. |
...
For the verification matrix history (v1 → v7) and per-program verdicts, see [`Examples/smack-docker/README.md`](../Examples/smack-docker/README.md) "Run history" and "v6 per-program detail" sections (v7 is verdict-identical to v6 — first round on the post-Jun-9-fixes binary, regression-confirmation; the v6 per-program table stands for v7).
