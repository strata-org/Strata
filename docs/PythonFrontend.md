...
produce for AWS-SDK-style tasks: fully type-annotated procedural code that
calls boto3, raises a handful of exception types, and avoids the more dynamic
corners of the language. Within that subset the aim is *sound* analysis — if
the tool reports "no bugs" then, modulo known specification gaps, there really
are none.
...
2. The `Any`-boxed value encoding that underpins everything the front-end
   produces.
3. The PySpec mechanism that describes library functions (especially boto3).
...
* `x + y` on Python `int`s becomes `from_int(as_int(x) + as_int(y))`, bracketed
  by tag checks.
...
translator: before it emits `as_int(x)` it emits an assertion of
...
2. **PySpec-modelled functions** (typically boto3) — translated using the
...
code talks to: boto3, botocore, parts of the standard library. A PySpec file
is itself a Strata program (in the `PythonSpecs` dialect) written as an
`.ion`. For boto3 these files are generated offline from Smithy models via
`ErgoSmithyPython` codegen, then compiled to `.pyspec.st.ion` by Strata.

A PySpec declaration is roughly:

```
composite S3 {
  function get_object(self : S3, **kwargs : dict(Bucket : str, Key : str, ...)) -> dict(Body : bytes, ...) {
    requires kwargs.Bucket.length >= 3
    requires regex(kwargs.Key, "^[^/]+(/.+)?$")
    ensures response.Body is_bytes
    ...
  };
  ...
}
...
1. Reads each PySpec Ion file listed under `--spec-dir`.
2. Indexes overload tables so that a Python call like `s3.get_object(...)` can
   be dispatched to the right PySpec procedure based on the argument shape
   (the Smithy models frequently have overloaded signatures).
...
Because boto3 is enormous (≈18k procedures across ≈420 services), generation
is gated on `--update-botomoog` and the result is cached in `pyspec/`.

### The `--dispatch` flag

When the spec tree contains a `boto3.pyspec.st.ion` (or
`boto3/__init__.pyspec.st.ion`), the front-end also loads dispatch tables so
that `boto3.client("s3")` resolves statically to the `S3` composite. Without
this, calls on the returned object fall back to holes.
...
* `try/except` for a known exception type, and raising exceptions by
  instantiating them.
* boto3 calls (via PySpec), including auto-generated input validation and
  result narrowing.
...
* **PySpec correctness is assumed.** When a PySpec procedure states a
  postcondition, the verifier trusts it. Specs derived from Smithy models
  inherit the same trust.
