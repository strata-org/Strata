...
- **Source artifact** — the user-supplied input that a translation
  ...
  even a binary. The source artifact is *not* in any Strata IR
  yet.
...
- **Analysis** — anything that consumes a Strata IR and produces a
  result *about* the program. Two flavors:
...
- **Transform** — a translation whose output is the same kind of
  ...
  *transform* gives you a transformed version of the input, while
  an *analysis* gives you a *fact* about the input. An analysis
...
genuinely central** — it must remain the *waist*, narrow and
...
ways that differ only in *where the function lives* and therefore
*who imports whom*:
...
| Form | Lives in | Imports |
|------|----------|---------|
| A | `IR1` module | `IR1 → IR2` (IR1 depends on IR2) |
| B | `IR2` module | `IR2 → IR1` (IR2 depends on IR1) |
| C | a third module | `IR1ToIR2 → IR1` and `IR1ToIR2 → IR2` (the module depends on both; neither IR depends on the translation) |
...
2. **Is one side a single component and the other side a
   *family* of components that all need to translate to/from
...
This is *not* a claim about how often each side changes. Core
...
open-for-extension *around* is the central IR. So translations
*out of* Core live with the consumer; translations *into* Core
live with the producer.

### 3. Knowledge ownership

Translations should live with the component that owns the
*details* of the side that varies:
...
This is *not* a claim that Core authors don't understand SMT or
...
depend on everything; it is the *only* place that may.
...
This does not imply a *total* order on dialects. A dialect like
...
Keeping translations *out of* Core means Core's source files
change only when Core's *language* changes — not when a
...
Whether such a change counts as a *breaking* change is governed
...
This means counterexample raising is needed at *every* level for
...
sits *outside* both IRs, depending inward on each. New consumers
...
- the translation logically belongs to *neither* endpoint.
...
these interfaces *physical* package boundaries:

| Package | Role |
|---------|------|
| `Strata` (main) | DL, Core, Laurel, SMT dialect, CBMC translator, deductive verifier (kept together for now) |
| `StrataDDM` | Dialect Definition Mechanism (already independent) |
...
rule for the boundaries that *do* cross packages:
...
For interfaces that remain *inside* the main `Strata` package
...
what." Laurel is one of *many* producers feeding Core (Python
...
*target* is a spoke and Core is the waist; Laurel imports Core,
...
The reasoning is the *same* as for Laurel → Core, applied
symmetrically: the SMT analysis is one of *many* consumers of
...
*shape* of the result it ingests (`VCResult`, `ProofObligation`),
...
When referring to "CBMC" in this document: the *external CBMC
binary* is the analysis tool; the *Strata-side translator* is
...
Across all six directions the test is the same — *which side has
many counterparts and which side must remain extensible-around
without being touched?* The "many" side owns the translation;
...
Three legitimate reasons to put `fromIR1` *inside* IR1:
...
2. **The target is canonical and unique.** If there will only
   ever be one external form — e.g. Ion serialization as *the*
...
1. **Apply the one-to-many test:**
   1. If one side is Core, the translation lives on the non-Core
      side.
   2. If one side is *one* and the other is *many*, the
      translation lives on the *many* side.
...
5. If the translation needs to preserve provenance (source
   ...
   side channel.
6. Distinguish *in-process analysis* placement (purely Strata
   code) from *external-tool* placement (talks to a separate
...
7. If raising (counterexamples, source-mapped diagnostics) is
   needed, plan it at *every* level — raising mirrors the
