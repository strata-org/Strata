...
def pyAnalyzeLaurelCommand : Command where
  ...
  callback := fun v pflags => do
    ...
    -- Separate prelude results from user results so the output only shows
    -- user-specific obligations.  Prelude correctness is checked by a
    -- dedicated build-time test (PreludeVerifyTest).
    ...
    if preludeFailures.size > 0 then
      ...
      for r in preludeFailures do
        IO.println s!"Assertion failed in prelude file: {r.obligation.label}: {r.formatOutcome}"
