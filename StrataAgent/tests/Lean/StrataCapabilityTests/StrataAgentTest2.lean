...
def wrapInBlock (s : Stmt P (Cmd P)) : Option (Stmt P (Cmd P)) :=
  if "wrapper" ∈ s.labels then none
  else if s.definedVars false ≠ [] then none
  else some (.block "wrapper" [s] .empty)


theorem wrapStmtInBlock_overapproximates
    (extendEval : ExtendEval P) (newPrefix : String) :
    Overapproximates (MyLang extendEval) (MyLang extendEval) wrapInBlock newPrefix := by
    sorry
