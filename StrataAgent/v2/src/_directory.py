...
def create_directory_server(agent_name: str, swarm: Any):
    ...
    async def list_agents(input: dict[str, Any]) -> dict[str, Any]:
        visible = swarm._registry.visibility_graph.get(agent_name, set())
        agents = []
        for name in sorted(visible):
            # Resolve node — for sharded agents, use first instance
            node = None
            if name in swarm._registry.nodes:
                node = swarm._registry.nodes[name]
            elif name in swarm._registry.sharded_agents:
                instance_name = f"{name}_0"
                if instance_name in swarm._registry.nodes:
                    node = swarm._registry.nodes[instance_name]
            ...
            if name in swarm._registry.results:
                status = swarm._registry.results[name].status.value
            elif name in swarm._registry.sharded_agents:
                # Sharded: running if any instance is running
                any_running = any(
                    f"{name}_{i}" in swarm._registry.tasks and not swarm._registry.tasks[f"{name}_{i}"].done()
                    for i in range(swarm._registry.sharded_agents[name].replicas)
                )
                status = "running" if any_running else "completed"
            elif name in swarm._registry.tasks:
                task = swarm._registry.tasks[name]
                if task.done():
                    status = "completed"
                else:
                    status = "running"
