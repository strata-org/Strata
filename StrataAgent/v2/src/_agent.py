...
class SwarmAgent(Generic[T]):
    def __init__(
        ...
        # Messages are now owned by the backend (self.backend.messages)
        # This property provides access for backward compat
    ...
    async def _consume_response(self, result: AgentResult[T], start_time: float) -> bool:
        ...
        while True:
            ...
            try:
                ...
                await self._emit("message", f"[Backend disconnected: {e}. Attempting reconnect...]")
                try:
                    if await self.backend.reconnect():
                        await self._emit("message", "[Reconnected successfully]")
                        return True  # Re-enter outer loop with fresh receive_messages
                except Exception:
                    pass
                # Reconnect failed — raise to outer handler for full recovery
                raise
            ...
            # Pause check — interrupt and wait
            if self.pause.is_paused:
                ...
                return True  # Re-enter outer loop after unpause
            ...
            # Backend accumulates messages internally via receive_messages()
            await self._emit_heartbeat_if_needed()

            # Emit session_id as soon as backend has it (first message from Claude)
            ...
            if message.type == "text" and message.content:
                ...
                # Record tool call in telemetry (enables mid-response nudges)
    ...
    async def _wait_for_wakeup(self, result: AgentResult[T]) -> bool:
        ...
        while not self.cancellation.is_cancelled:
            ...
            # All agents wait the same way — NudgeMonitor handles periodic wakeups
            msg = await messages_ch.receive(timeout=None)  # block until message arrives
            if msg:
                result.status = AgentStatus.RUNNING
                await self._emit("status_change", AgentStatus.RUNNING.value)
                ts = datetime.now().strftime("%H:%M:%S")
    ...
    async def _attempt_recovery(self, result: AgentResult[T], config: BackendConfig) -> bool:
        ...
        try:
            if await self.backend.reconnect():
                ...
                # Hydrate backend.messages from local JSONL history
                history = await self.backend.get_message_history()
                if history:
                    self.backend._messages = history
                    logger.info(f"[RETRY] {self.spec.name}: loaded {len(history)} messages from history")
        ...
        # No summary-based recovery — we require session IDs for resume.
        # If session is dead, the agent fails and the swarm handles it
        # (checkpoint + fresh start with workspace files intact).
        return False
    ...
    async def run(self) -> AgentResult[T]:
        ...
        try:
            logger.info(f"[START] {self.spec.name}: connecting to backend...")
            await self.backend.connect(config)
            logger.info(f"[START] {self.spec.name}: connected")
            # Emit session_id immediately after connect (backend has it from the CLI)
            if hasattr(self.backend, '_session_id') and self.backend._session_id:
                await self._emit("session_id", self.backend._session_id)
            # If resuming a session, hydrate backend.messages from local history
            if self.spec.resume_session_id:
                history = await self.backend.get_message_history()
                if history:
                    self.backend._messages = history
                    logger.info(f"[RESUME] {self.spec.name}: loaded {len(history)} messages from session history")
        except Exception as e:
        ...
        try:
            logger.info(f"[START] {self.spec.name}: sending initial query...")
            await self._emit("message", "[Sending initial query to backend...]")
            await self.backend.send_query(prompt)
            logger.info(f"[START] {self.spec.name}: initial query sent, entering response loop")
            ...
            while True:
                ...
                if has_messaging_tools:
                    ...
                    if msg:
                        ...
                        await self._emit("message", f"[Received message from '{msg.sender}', sending to backend...]")
                        await self.backend.send_query(injection)
                        await self._emit("message", injection)
                        continue
