    ...
    async with swarm_agent(
        ...
        for round_i in range(RESEARCHER_DECISION_ROUNDS):
            prompt = (
                ...
                "REASON: <one sentence>\n\n"
                f"- done: the report at {report_rel} is COMPLETE and USEFUL for its case:\n"
                "    • FALSE goal → a CONFIRMED counterexample (RECOMMENDATION: GIVE_UP).\n"
                "    • TRUE + quick → proof shape + primitives verified against the ACTUAL\n"
                "      defs (full footprint: BOTH written AND read/getVars sets, incl. the\n"
                "      free vars of every emitted assert/assume) (RECOMMENDATION: PROCEED).\n"
                "    • TRUE + hard → a `Proposed decomposition` with at least one NEW\n"
                "      concrete lemma/def statement that ADVANCES the theory beyond the\n"
                "      prior report (RECOMMENDATION: PROCEED). This is a RESEARCH problem:\n"
                "      propose the missing machinery — do NOT give up on a true goal.\n"
                "- not_done: no counterexample yet for a suspected-false goal, OR (true+hard)\n"
                "  you have not yet added a concrete next step this pass — keep digging.\n\n"
                "GIVE_UP is ONLY for a genuinely FALSE goal or one needing a human ROOT "
                "signature change — NEVER for 'hard' or 'needs new substrate'. A true goal "
                "is always PROCEED (with a decomposition if hard). Do NOT accept a done "
                "that quits on a goal you believe is true."
            )
            ...
            await researcher.run_ai(
                inp=(f"Not done yet. Keep working the report at {report_rel} (read your "
                     f"OWN prior report there and BUILD ON it — do not restart). If you "
                     f"suspect the goal is FALSE, verify against the ACTUAL definitions "
                     f"(full footprint: writes AND reads/getVars incl. assert/assume free "
                     f"vars) and give a CONFIRMED counterexample. If the goal is TRUE but "
                     f"hard, do NOT give up: PROPOSE the missing machinery — add at least "
                     f"one NEW concrete auxiliary def/lemma statement that advances the "
                     f"decomposition beyond the last pass, and RECOMMEND PROCEED. GIVE_UP "
                     f"is only for a genuinely false goal or a human-only signature change. "
                     f"Then I re-check."),
