...
class NudgeMonitor:
    ...
    def __init__(
        ...
        self._on_stalled_service = None  # async callback(swarm, agent_name) for domain-specific recovery
        self._on_mcp_restart = None  # async callback(swarm) for manual MCP restart
        ...
        if module_name:
            ...
            for candidate in [module_name, f"swarm_nudges.{module_name}"]:
                try:
                    ...
                    # Load optional hooks
                    self._on_stalled_service = getattr(mod, "ON_STALLED_SERVICE", None)
                    self._on_mcp_restart = getattr(mod, "ON_MCP_RESTART", None)
    ...
    async def _reconnect_stalled_services(self) -> None:
        """Detect ANY agent that's stalled and call recovery hook.
        Checks both reply_only agents (overdue replies) and all agents (stall events)."""
        if not self._swarm:
            return
        if not self._on_stalled_service:
            return
        registry = self._swarm._registry
        now = time.time()
        ...
        # Check reply_only agents with overdue replies >3 min
        for agent_name in list(self._reply_only_agents):
            overdue = self.pending.get_overdue(agent_name, timeout_seconds=180)
            if overdue and registry.backends.get(agent_name):
                stalled_agents.append(agent_name)

        # Check any agent that recorded a stall event recently
        for agent_name, stall_time in list(registry.stalled_agents.items()):
            if now - stall_time < 300:  # stalled in last 5 min
                if agent_name not in stalled_agents:
                    stalled_agents.append(agent_name)

        for agent_name in stalled_agents:
            # Only trigger once per 5 min per agent
            last_key = f"_stall_recover_{agent_name}"
            last_time = getattr(self, last_key, 0)
            if now - last_time < 300:
                continue
            setattr(self, last_key, now)
            logger.warning(f"[NUDGE] Agent '{agent_name}' appears stalled. Calling recovery hook.")
            try:
                await self._on_stalled_service(self._swarm, agent_name)
            except Exception as e:
                logger.error(f"[NUDGE] Recovery hook error for '{agent_name}': {e}")
