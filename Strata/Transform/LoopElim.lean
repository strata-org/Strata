...
private def removeLoopsDecls :
  ...
  | .proc proc md :: ds => do
    ...
    -- Only structured bodies are processed; CFG bodies pass through unchanged
    -- (CFG bodies have no structured loops to eliminate).
    match proc.body with
    | .structured ss =>
      if !Core.Statements.noCall ss then
        throw s!"Procedure body contains a procedure call (loop elim does not support calls)"
      let (c1, body) ← removeLoopsBlock 100 ss
      let (c2, ds) ← removeLoopsDecls ds
      return (c1 || c2, .proc { proc with body := .structured body } md :: ds)
    | .cfg _ =>
      let (changed, ds) ← removeLoopsDecls ds
      return (changed, .proc proc md :: ds)
  | d :: ds => do
    let (changed, ds) ← removeLoopsDecls ds
    return (changed, d :: ds)
