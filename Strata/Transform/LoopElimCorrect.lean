...
private theorem loop_step_hasInvFailure_false_of_or
    {ρ : Imperative.Env Expression} {hasInvFailure : Bool}
...
private theorem loop_first_step_hinv_bool
  ...
  | step_loop_enter _ hib _ _ => exact hib
...
private theorem env_or_false_eq (ρ : Imperative.Env Expression) :
    ({ ρ with hasFailure := ρ.hasFailure || Bool.false } : Imperative.Env Expression) = ρ := by
...
private theorem assert_pass_step
    ...
    (ρ : Imperative.Env Expression) (htt : ρ.eval ρ.store e = .some HasBool.tt)
...
private theorem stmts_passive_all_pass
    (ss : Statements) (ρ : Imperative.Env Expression)
...
private theorem stmts_mapIdx_assert_terminal
    (inv : List (String × Expression.Expr)) (ρ : Imperative.Env Expression)
...
private theorem havoc_targeting_single
    ...
    (ρ₀ : Imperative.Env Expression) (v_target : Expression.Expr)
...
private theorem havocs_targeting_stmts
    ...
    (ρ₀ : Imperative.Env Expression) (σ_target : SemanticStore Expression)
    (hwfvar : WellFormedSemanticEvalVar ρ₀.eval)
  ...
  | nil =>
    ...
    rw [List.map_nil]
    have h : ({ ρ₀ with store := ρ₀.store } : Imperative.Env Expression) = ρ₀ := by
      cases ρ₀; rfl
  ...
  | cons x rest ih =>
    ...
    let ρ₁ : Imperative.Env Expression := { ρ₀ with store := σ₁ }
...
private theorem havoc_block_to_target
    ...
    (ρ₀ ρ_target : Imperative.Env Expression)
    (hwfvar : WellFormedSemanticEvalVar ρ₀.eval)
    (hdef_src : ∀ x ∈ vars, (ρ₀.store x).isSome)
...
private theorem evalCommand_store_frame_stmt
    {ρ : Imperative.Env Expression} {c : Command} {σ' : CoreStore} {f : Bool}
...
private theorem step_preserves_outerInv
  ...
  | step_block | step_ite_true _ _ | step_ite_false _ _
  ...
  | step_exit | step_funcDecl | step_typeDecl
...
private theorem stmt_star_preserves_isSome
    (s : Stmt Expression Command) (ρ₀ : Imperative.Env Expression) (c₂ : CC)
...
/-- Block-level analog of `stmt_star_preserves_isSome`. -/
private theorem stmts_star_preserves_isSome
    (ss : Statements) (ρ₀ : Imperative.Env Expression) (c₂ : CC)
...
private def extendEnv
    (m : Expression.Ident) (v : Expression.Expr)
    (ρ : Imperative.Env Expression) : Imperative.Env Expression :=
...
private theorem step_cmdsIn_subset
  ...
  induction hstep with
  | step_cmd _ | step_loop_exit _ _ _ _ | step_loop_nondet_exit _ _
  | step_exit | step_funcDecl | step_typeDecl | step_stmts_nil | step_seq_exit
  | step_block_done | step_block_exit_match _ | step_block_exit_mismatch _ =>
    intro c hc; simp [Config.cmdsIn] at hc
  ...
  | step_loop_enter _ _ _ _ | step_loop_nondet_enter _ _ =>
    -- b = .seq (.block .none _ (.stmts body _)) [loop]; both halves
    -- of b's cmdsIn coincide with a's `.commandsIn body`.
    intro c hc
...
private theorem step_no_new_vars
  ...
  | step_loop_nondet_enter _ _ | step_loop_nondet_exit _ _
  | step_exit | step_typeDecl | step_stmts_nil | step_stmts_cons =>
    exact hn
  ...
  | step_block_done | step_block_exit_match _ | step_block_exit_mismatch _ =>
    -- b.store = projectStore σ_parent ρ'.store; a's outer store is σ_parent.
    obtain ⟨_, _⟩ := hinv
    simp only [Config.getEnv] at hn ⊢
...
private theorem stmt_star_preserves_reservedFresh
    (s : Stmt Expression Command) (ρ₀ ρ₁ : Imperative.Env Expression)
...
private theorem step_touchedVars_subset
  ...
  | step_block =>
    -- c₁'s and c₂'s touchedVarsSet are equal: `Block.modifiedVars ss ++ Block.definedVars ss`.
...
private theorem Stmt.getVars_loop_body_mem
    {g : ExprOrNondet Expression} {m : Option Expression.Expr}
...
private theorem step_preserves_extendCfg
  ...
  | step_loop_enter hguard hinv_bool hinv_iff hwfb =>
    rename_i inv g me body md ρ hasInvF
    have h_wf' : WellFormedSemanticEvalExprCongr ρ.eval := h_wfExpr
    -- guard, inv exprs, measure expr all need eval lifting
    ...
    exact StepStmt.step_loop_enter hguard' hinv_bool' hinv_iff' hwfb
  | step_loop_exit hguard hinv_bool hinv_iff hwfb =>
    rename_i md inv g me body ρ hasInvF
    have h_wf' : WellFormedSemanticEvalExprCongr ρ.eval := h_wfExpr
    have hg_get : m ∉ HasFvars.getFvars g := by
      intro h; apply h_notin
      simp only [Config.allVarsSet, Stmt.modifiedVars, Stmt.definedVars,
    ...
    show StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ)
      ...
      (.terminal { extendEnv m v ρ with
        hasFailure := ρ.hasFailure || hasInvF })
    exact StepStmt.step_loop_exit hguard' hinv_bool' hinv_iff' hwfb
  ...
  | step_loop_nondet_exit hinv_bool hinv_iff =>
    rename_i md inv me body ρ hasInvF
    have h_wf' : WellFormedSemanticEvalExprCongr ρ.eval := h_wfExpr
    have hinv_get : ∀ le ∈ inv, m ∉ HasFvars.getFvars le.2 := by
      intro le hle hmem
      apply h_notin
  ...
  | step_funcDecl =>
    ...
    rw [show
        ...
        ({ extendEnv m v env_ρ with
          ...
          : Imperative.Env Expression) from by
...
private theorem step_preserves_cfgWfExprCongr
  ...
  | step_cmd hcmd =>
    ...
    | cmd_sem _ | @call_sem _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ =>
      exact hwf
  | step_block | step_ite_true | step_ite_false
  | step_ite_nondet_true | step_ite_nondet_false
  | step_loop_enter | step_loop_nondet_enter =>
    exact ⟨hwf, hwf⟩
  | step_loop_exit | step_loop_nondet_exit | step_exit | step_typeDecl
  | step_stmts_nil | step_stmts_cons | step_seq_done | step_seq_exit =>
    exact hwf
...
private theorem step_preserves_store_outside_touchedVars_isNone
  ...
  | step_block_body _ ih =>
    ...
    exact ih hnone hx
  | step_block_done | step_block_exit_match _ | step_block_exit_mismatch _ =>
    -- c₂.store x = projectStore σ_parent ρ'.store x.  Both branches give
    -- `none` because (ρ'.store x).isNone (= hnone).
    simp only [Config.getEnv] at hnone ⊢
    simp only [projectStore]
    have hρ'_none : ‹Imperative.Env Expression›.store x = none :=
...
private theorem stmts_concat_terminal
    (ss₁ ss₂ : Statements) (ρ₀ ρ₁ ρ₂ : Imperative.Env Expression)
...
private theorem stmts_prefix_exiting_append
    (ss₁ ss₂ : Statements) (ρ₀ ρ' : Imperative.Env Expression) (lbl : String)
...
private theorem blockT_none_reaches_terminal
    ...
    {e_parent : SemanticEval Expression} {ρ' : Imperative.Env Expression}
...
private theorem blockT_none_reaches_exiting_some
    {inner : CC} {σ_parent : SemanticStore Expression}
    {e_parent : SemanticEval Expression} {l : String} {ρ' : Imperative.Env Expression}
    (hstar : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
      (.block .none σ_parent e_parent inner) (.exiting l ρ')) :
    ∃ (ρ_inner : Imperative.Env Expression),
      ∃ (h : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
...
private theorem seqT_reaches_exiting
    {inner : CC} {ss : Statements} {lbl : String} {ρ' : Imperative.Env Expression}
    (hstar : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
...
private theorem block_definedVars_true_subset_false (bss : Statements) (n : Expression.Ident)
  ...
  | cons s rest ih =>
    ...
    · left
      ...
      | .exit .. => simp [Stmt.definedVars] at hs
      | .funcDecl .. => simp [Stmt.definedVars] at hs
      | .typeDecl .. => simp [Stmt.definedVars] at hs
    · exact .inr (ih hr)
...
private theorem InitEnvWF.toBlock_block {reserved : List String} {l : String}
    ...
    {bss : Statements} {md : MetaData Expression} {ρ : Imperative.Env Expression}
  ...
  loopMeasuresAreIntTypes := by
    have h' := h.loopMeasuresAreIntTypes
    show Statements.measuresAreIntTypes bss
    simpa [Statement.measuresAreIntTypes] using h'
...
/-- For ite: `BlockInitEnvWF tss` follows from `InitEnvWF (.ite c tss ess md)`.
    ...
    `step_ite_true`/`step_ite_nondet_true` discriminant. -/
private theorem InitEnvWF.toBlock_ite_left {reserved : List String}
    {declaredFuncs : Expression.Ident → Bool}
    {c : ExprOrNondet Expression}
    {tss ess : Statements} {md : MetaData Expression} {ρ : Imperative.Env Expression}
    (h : InitEnvWF reserved declaredFuncs (.ite c tss ess md) ρ)
    (hcond : ∀ g, c = .det g → ρ.eval ρ.store g = some HasBool.tt) :
  ...
  loopMeasuresAreIntTypes := by
    have h' := h.loopMeasuresAreIntTypes
    show Statements.measuresAreIntTypes tss
    simp [Statement.measuresAreIntTypes] at h'
    exact h'.1
...
private theorem InitEnvWF.toBlock_ite_right {reserved : List String}
    ...
    {tss ess : Statements} {md : MetaData Expression} {ρ : Imperative.Env Expression}
    (h : InitEnvWF reserved declaredFuncs (.ite c tss ess md) ρ)
    (hcond : ∀ g, c = .det g → ρ.eval ρ.store g = some HasBool.ff) :
  ...
  loopMeasuresAreIntTypes := by
    have h' := h.loopMeasuresAreIntTypes
    show Statements.measuresAreIntTypes ess
    simp [Statement.measuresAreIntTypes] at h'
    exact h'.2
...
private theorem BlockInitEnvWF.toStmt_head {reserved : List String}
  ...
  loopMeasuresAreIntTypes := by
    have h' := h.loopMeasuresAreIntTypes
    show Statement.measuresAreIntTypes s
    simp [Statements.measuresAreIntTypes] at h'
    exact h'.1
...
private theorem cmd_definedVars_true_isSome_after
    {cmd : Command} {ρ₀ ρ₁ : Imperative.Env Expression}
...
private theorem stmt_definedVars_true_isSome_after
    {s : Statement} {ρ₀ ρ₁ : Imperative.Env Expression}
    (hstar : CoreStar π φ (.stmt s ρ₀) (.terminal ρ₁))
...
private theorem step_preserves_isNoneAnchored
  ...
  | step_ite_nondet_true | step_ite_nondet_false
  | step_loop_enter _ _ _ _ | step_loop_nondet_enter _ _ =>
    -- All these step rules introduce a `.block`/`.seq` wrapper but the
    -- inner store is unchanged, so `isNoneAnchored` lifts as `⟨hinv, hinv⟩`.
    simp only [isNoneAnchored] at hinv ⊢
    exact ⟨hinv, hinv⟩
  | step_loop_exit _ _ _ _ | step_loop_nondet_exit _ _ | step_exit
  | step_funcDecl | step_typeDecl | step_stmts_nil | step_stmts_cons
  | step_seq_done | step_seq_exit =>
    -- These rules produce a single env whose store equals the input's, so
    -- `isNoneAnchored` is preserved as-is.
    simp only [isNoneAnchored] at hinv ⊢
    exact hinv
  | step_block_done | step_block_exit_match _ | step_block_exit_mismatch _ =>
    -- After exiting a `.block`, the outer config holds
    -- `projectStore σ_parent ρ.store`, which is `none` at `n` because
    -- `σ_parent n = none` (from the `block` arm of `isNoneAnchored`).
    simp only [isNoneAnchored] at hinv ⊢
    obtain ⟨hpar, _⟩ := hinv
    simp only [projectStore]
...
private theorem stmt_terminal_preserves_isNone
  ...
  | .loop g m inv body md =>
    ...
    suffices ∀ (n_steps : Nat) (ρ₀ ρ₁ : Imperative.Env Expression),
        (ρ₀.store n).isNone →
        ∀ (h : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
...
private theorem stmt_compound_terminal_preserves_isNone
  ...
  | .funcDecl decl md | .typeDecl decl md =>
    -- step_funcDecl/step_typeDecl modify eval/typing context, not the store.
    cases hstar with
    | step _ _ _ h1 r1 =>
      cases h1
      cases r1 with
  ...
  | .block l bss md =>
    ...
    | step _ mid _ h1 r1 =>
      ...
      match block_reaches_terminal (P := Expression)
          (EvalCmd := EvalCommand π φ) (extendEval := EvalPureFunc φ) r1 with
      | .inl ⟨ρ_inner, _, heq⟩ | .inr ⟨_, ρ_inner, _, heq⟩ =>
        rw [heq]
        show (projectStore ρ₀.store ρ_inner.store n).isNone
        simp only [projectStore]
        rw [if_neg (by rw [Option.not_isSome_iff_eq_none]; exact Option.isNone_iff_eq_none.mp hnone)]
...
private theorem step_funcDecl_preserves_cfgWfInt
    ...
    (hwf_ext : WFEvalExtension φ)
    {ρ : Imperative.Env Expression} {decl : PureFunc Expression} {md : MetaData Expression}
...
private theorem BlockInitEnvWF.toBlock_tail_via_defUseOk {reserved : List String}
    {declaredFuncs : Expression.Ident → Bool}
    (hwf_ext : WFEvalExtension φ)
    {s : Statement} {ss : Statements} {ρ₀ ρ₁ : Imperative.Env Expression}
    (h : BlockInitEnvWF reserved declaredFuncs (s :: ss) ρ₀)
    (hstar : CoreStar π φ (.stmt s ρ₀) (.terminal ρ₁)) :
  ...
  loopMeasuresAreIntTypes := by
    have h' := h.loopMeasuresAreIntTypes
    show Statements.measuresAreIntTypes ss
    simp [Statements.measuresAreIntTypes] at h'
    exact h'.2
...
private theorem block_exit_case
    {σ_bss_tgt : LoopElimState}
    {l : String} {bss : Statements} {md : MetaData Expression}
    {ρ₀ ρ' : Imperative.Env Expression} {lbl : String}
    (r1 : ReflTrans (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
...
private theorem block_canfail_case
    {σ_bss_tgt : LoopElimState}
    {l : String} {bss : Statements} {md : MetaData Expression}
    {ρ₀ : Imperative.Env Expression} {cfg : CC}
    (hfail : cfg.getEnv.hasFailure = Bool.true)
    (r1 : ReflTrans (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
...
private theorem block_corr_cons_term_head_term
    ...
    {s : Statement} {ss : Statements}
    {ρ₀ ρ₁ ρ' : Imperative.Env Expression}
...
private theorem block_corr_cons_exit_head_term
    {σ_head_tgt σ_tail_tgt : LoopElimState}
    {s : Statement} {ss : Statements}
    {ρ₀ ρ₁ ρ' : Imperative.Env Expression} {lbl : String}
    (hterm_s : CoreStar π φ (.stmt s ρ₀) (.terminal ρ₁))
...
private theorem seq_block_loop_terminal_decompose
    ...
    {loop_stmt : Statement} {ρ' : Imperative.Env Expression}
    (hstar : CoreStar π φ
...
/-- Bundle of four facts that the simulation theorems extract together at
    every `loop` step: a `seq → terminal` decomposition, all-tt invariants,
    `projectStore`-identity, and eval-preservation.  Bundled here so that
    each call site is one line instead of four separate `have`s.  The body
    env at which the `seq`-trace decomposes is left as a parameter `ρ_iter`
    (typically `{ ρ₀ with hasFailure := ρ₀.hasFailure || hasInvFailure }`,
    which after `simp [Bool.or_false]` collapses to `ρ₀` in the
    no-failure branch). -/
private theorem loop_terminal_facts
    {guardE : ExprOrNondet Expression}
    {measure : Option Expression.Expr}
    {inv : List (String × Expression.Expr)}
    {body : Statements} {md : MetaData Expression}
    {ρ₀ ρ' ρ_iter : Imperative.Env Expression}
    (hreach : CoreStar π φ
        (.stmt (.loop guardE measure inv body md) ρ₀) (.terminal ρ'))
    (hreach_inner : CoreStar π φ
        (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body ρ_iter))
          [.loop guardE measure inv body md]) (.terminal ρ'))
    (hnf'' : ρ'.hasFailure = Bool.false) :
    (∃ (ρ_mid : Imperative.Env Expression),
      CoreStar π φ
        (.block .none ρ₀.store ρ₀.eval (.stmts body ρ_iter)) (.terminal ρ_mid) ∧
      CoreStar π φ (.stmts [.loop guardE measure inv body md] ρ_mid) (.terminal ρ') ∧
      ∃ (ρ_inner : Imperative.Env Expression),
        CoreStar π φ (.stmts body ρ_iter) (.terminal ρ_inner) ∧
        ρ_mid = ({ ρ_inner with store := projectStore ρ₀.store ρ_inner.store, eval := ρ₀.eval }
          : Imperative.Env Expression)) ∧
    (∀ le ∈ inv, ρ'.eval ρ'.store le.2 = some HasBool.tt) ∧
    projectStore ρ₀.store ρ'.store = ρ'.store ∧
    ρ'.eval = ρ₀.eval := by
  refine ⟨seq_block_loop_terminal_decompose π φ hreach_inner,
...
private theorem inv_eval_agree_under_projectStore
    ...
    {inv : List (String × Expression.Expr)}
    {body : Statements} {md : MetaData Expression}
    {ρ₀ ρ ρ_inner : Imperative.Env Expression}
...
private theorem body_eval_eq_init_on_expr
  ...
  have hdu_body : Block.defUseWellFormed (fun n => (ρ₀.store n).isSome) declaredFuncs body = Bool.true :=
    hdu.2
  have heta : ({ ρ₀ with hasFailure := ρ₀.hasFailure } : Imperative.Env Expression) = ρ₀ := by
    cases ρ₀; rfl
  rw [heta] at h_inner
...
private theorem body_eval_inv_preserved_from
    ...
    {inv : List (String × Expression.Expr)}
    {body : Statements} {md : MetaData Expression}
    {ρ₀ ρ_start ρ_inner : Imperative.Env Expression}
    (hswf : InitEnvWF reserved declaredFuncs (.loop guardE measure inv body md) ρ₀)
...
private theorem hall_tt_mid_of_loop_terminal
    ...
    {inv : List (String × Expression.Expr)}
    {body : Statements} {md : MetaData Expression}
    {ρ_mid ρ' : Imperative.Env Expression}
  ...
  | step _ _ _ hstep1 hrest =>
    cases hstep1 with
    | step_loop_enter _ hinvb_mid hinviff_mid _ =>
...
private theorem hall_tt_inner_of_mid
    ...
    {inv : List (String × Expression.Expr)}
    {body : Statements} {md : MetaData Expression}
    {ρ₀ ρ_mid ρ_inner : Imperative.Env Expression}
    (hwf_ext : WFEvalExtension φ)
    (hswf : InitEnvWF reserved declaredFuncs (.loop guardE measure inv body md) ρ₀)
...
/-- The havoc block, applied at `ρ` with all its target vars already
    `isSome` in `ρ.store`, runs as the identity (terminates at the same
    `ρ`).  Used 5 times in the simulation proofs. -/
private theorem havoc_block_id_at
    (havoc_label : String)
    (havoc_vars : List Expression.Ident) (md : MetaData Expression)
    (ρ : Imperative.Env Expression)
    (hwfvar : WellFormedSemanticEvalVar ρ.eval)
    (h_defined : ∀ x ∈ havoc_vars, (ρ.store x).isSome) :
    CoreStar π φ (.stmt (.block havoc_label
      (havoc_vars.map fun n => Stmt.cmd (HasHavoc.havoc n md)) ∅) ρ)
      (.terminal ρ) := by
  have h := havoc_block_to_target π φ havoc_label havoc_vars md ρ ρ hwfvar
    h_defined h_defined (fun x _ => rfl)
  simp at h; exact h

/-- `m` evaluates to the same value at `ρ_ext` as at `ρ`, given the
    stores agree on `m`'s fvars and `ρ_ext.eval = ρ.eval`. -/
private theorem m_eval_after_extend
    {ρ : Imperative.Env Expression} {ρ_ext : Imperative.Env Expression} {m : Expression.Expr} {v_m : Expression.Expr}
    (hwfExpr : WellFormedSemanticEvalExprCongr ρ.eval)
    (h_stores_agree_on_m : ∀ x ∈ HasFvars.getFvars m, ρ.store x = ρ_ext.store x)
    (h_eval_eq : ρ_ext.eval = ρ.eval)
    (hm_eval : ρ.eval ρ.store m = some v_m) :
    ρ_ext.eval ρ_ext.store m = some v_m := by
  have h := hwfExpr m ρ.store ρ_ext.store h_stores_agree_on_m
  rw [h_eval_eq, ← h]; exact hm_eval

/-- `lt (mkFvar m_old_ident) 0` evaluates the same as `lt m 0` at `ρ_ext`,
    because `mkFvar m_old_ident ↦ v_m` and `m ↦ v_m` at `ρ_ext`.  Used 7
    times in the simulation proofs. -/
private theorem lt_mold_eq_lt_m_at_ext
    {ρ_ext : Imperative.Env Expression} {m_old_ident : Expression.Ident}
    {m : Expression.Expr} {v_m : Expression.Expr}
    (hwfCong_ext : WellFormedCoreEvalCong ρ_ext.eval)
    (h_fvar_eval : ρ_ext.eval ρ_ext.store
      (HasFvar.mkFvar m_old_ident) = some v_m)
    (h_m_eval_ext : ρ_ext.eval ρ_ext.store m = some v_m) :
    ρ_ext.eval ρ_ext.store
      (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
    ρ_ext.eval ρ_ext.store (HasIntOps.lt m HasInt.zero) := by
  show ρ_ext.eval ρ_ext.store
  ...
  apply hwfCong_ext.appcongr ρ_ext.store ρ_ext.store () _ _ _ _ ?_ rfl
  apply hwfCong_ext.appcongr ρ_ext.store ρ_ext.store () _ _ _ _ rfl ?_
  rw [h_fvar_eval, h_m_eval_ext]

/-- `ρ_ext.eval ρ_ext.store (lt m 0) = ρ.eval ρ.store (lt m 0)` when the
    stores agree on `m`'s fvars and `ρ_ext.eval = ρ.eval`.  Used to lift
    a `lt m 0 = some bv` fact from `ρ` to `ρ_ext` after the m_old extension. -/
private theorem lt_m_zero_eval_after_extend
    {ρ : Imperative.Env Expression} {ρ_ext : Imperative.Env Expression} {m : Expression.Expr}
    (hwfExpr : WellFormedSemanticEvalExprCongr ρ.eval)
    (h_stores_agree_on_m : ∀ x ∈ HasFvars.getFvars m, ρ.store x = ρ_ext.store x)
    (h_eval_eq : ρ_ext.eval = ρ.eval) {bv : Core.Expression.Expr}
    (h_lt : ρ.eval ρ.store (HasIntOps.lt m HasInt.zero) = some bv) :
    ρ_ext.eval ρ_ext.store (HasIntOps.lt m HasInt.zero) = some bv := by
  have hgv : ∀ x ∈ HasFvars.getFvars
      (HasIntOps.lt m HasInt.zero : Core.Expression.Expr),
      ρ.store x = ρ_ext.store x := by
    intro x hx
  ...
  have h := hwfExpr (HasIntOps.lt m HasInt.zero) ρ.store ρ_ext.store hgv
  rw [h_eval_eq, ← h]; exact h_lt

/-- An fvar `mkFvar m_old_ident` evaluates to its store value via
    `WellFormedSemanticEvalVar`. -/
private theorem fvar_eval_of_extendStore
    {ρ_ext : Imperative.Env Expression} {m_old_ident : Expression.Ident} {v_m : Expression.Expr}
    (hwfvar : WellFormedSemanticEvalVar ρ_ext.eval)
    (h_m_old_at_ext : ρ_ext.store m_old_ident = some v_m) :
    ρ_ext.eval ρ_ext.store (HasFvar.mkFvar m_old_ident) = some v_m := by
  have hgF : HasFvar.getFvar (HasFvar.mkFvar m_old_ident :
      Core.Expression.Expr) = some m_old_ident := rfl
  have := hwfvar (HasFvar.mkFvar m_old_ident) m_old_ident ρ_ext.store hgF
  rw [this, h_m_old_at_ext]

/-- The store extended at `m_old_ident` agrees with the original on every
    fvar of `m`, since `m_old_ident` is fresh (not among `m`'s fvars). -/
private theorem stores_agree_on_m_after_extend
    (σ : SemanticStore Expression) {m_old_ident : Expression.Ident} {v_m : Expression.Expr}
    (m : Expression.Expr)
    (h_m_old_notin : m_old_ident ∉ HasFvars.getFvars m) :
    ∀ x ∈ HasFvars.getFvars m,
      σ x = extendStore m_old_ident v_m σ x := by
  intro x hx
  have hne : x ≠ m_old_ident := fun h => h_m_old_notin (h ▸ hx)
  show σ x = extendStore m_old_ident v_m σ x
  unfold extendStore
  rw [if_neg hne]

/-- `loopElimReservedPrefix` is a prefix of the m_old_ident name
    `{loopElimReservedPrefix}_measure_{loop_num}`. -/
private theorem m_old_ident_isPrefix (loop_num : String) :
    loopElimReservedPrefix.toList.isPrefixOf
      (HasIdent.ident (s!"{loopElimReservedPrefix}_measure_{loop_num}" : String)
        : Expression.Ident).name.toList := by
  show loopElimReservedPrefix.toList.isPrefixOf
      (s!"{loopElimReservedPrefix}_measure_{loop_num}").toList
  exact loopElimReservedPrefix_isPrefixOf_measure loop_num

/-- The fresh `m_old_ident` is absent from `ρ₀.store` (proved via
    `reservedFresh`).  Same-shape proof at 6 sites. -/
private theorem m_old_none_of_reserved
    {reserved : List String} {declaredFuncs : Expression.Ident → Bool}
    {s : Statement}
    {ρ₀ : Imperative.Env Expression}
    (hswf : InitEnvWF reserved declaredFuncs s ρ₀)
    (h_loop_reserved : loopElimReservedPrefix ∈ reserved)
    {m_old_ident : Expression.Ident}
    (h_m_old_isPrefix : loopElimReservedPrefix.toList.isPrefixOf
      m_old_ident.name.toList) :
    ρ₀.store m_old_ident = none := by
...
/-- Witness: `init m_old := v_m` from a pre-state where `m_old_ident` is
    `none`.  Thin wrapper around the generic `updatedStateInit`
    (Core/StatementSemanticsProps), bridging `extendStore = updatedState`. -/
private theorem m_old_initState
    (σ : SemanticStore Expression) (m_old_ident : Expression.Ident)
    (v_m : Expression.Expr)
    (h_none : σ m_old_ident = none) :
    InitState Expression σ m_old_ident v_m
      (extendStore m_old_ident v_m σ) := by
  have h := updatedStateInit (P := Expression) (σ := σ) (h := m_old_ident) (v := v_m) h_none
  have hext : extendStore m_old_ident v_m σ = updatedState σ m_old_ident v_m := by
    funext y; simp [extendStore, updatedState]
  rw [hext]; exact h

/-- The single-step trace `init m_old` from `ρ` to `ρ_ext` where `ρ_ext`
    extends `ρ`'s store with `m_old_ident ↦ v_m`.  Extracted because the
    proof is identical at 6 sites; the only varying inputs are the env
    (`ρ₀` / `ρ_k`), the value (`v_m` / `v_mk`), and the proof witnesses. -/
private theorem m_old_init_step
    {ρ : Imperative.Env Expression} {m_old_ident : Expression.Ident} {m v_m : Expression.Expr}
    {md : MetaData Expression}
    (hm_eval : ρ.eval ρ.store m = some v_m)
    (h_init_m : InitState Expression ρ.store m_old_ident v_m
      (extendStore m_old_ident v_m ρ.store))
    (hwfvar : WellFormedSemanticEvalVar ρ.eval) :
    CoreStar π φ
      (.stmt (.cmd (HasInit.init (CmdT := Command) m_old_ident HasInt.intTy (.det m) md)) ρ)
      (.terminal { ρ with store := extendStore m_old_ident v_m ρ.store }) := by
  ...
  have hρ_hf : (ρ.hasFailure || Bool.false) = ρ.hasFailure := Bool.or_false _
...
/-- The fresh `m_old_ident` (a measure-snapshot variable with prefix
    `loopElimReservedPrefix`) is not among the free variables of the source
    measure expression `m`.  This is the longest sub-proof inside the
    `m_old` setup; factored out because it appears 6 times verbatim. -/
private theorem m_old_notin_getFvars_m
    {reserved : List String} {declaredFuncs : Expression.Ident → Bool}
    {g m : Expression.Expr}
    {inv : List (String × Expression.Expr)}
    {body : Statements} {md : MetaData Expression}
    {ρ₀ : Imperative.Env Expression}
    (hswf : InitEnvWF reserved declaredFuncs
      (.loop (.det g) (some m) inv body md) ρ₀)
    (h_loop_reserved : loopElimReservedPrefix ∈ reserved)
    {m_old_ident : Expression.Ident}
    (h_m_old_isPrefix : loopElimReservedPrefix.toList.isPrefixOf
      m_old_ident.name.toList)
    (h_m_old_none₀ : ρ₀.store m_old_ident = none) :
    m_old_ident ∉ HasFvars.getFvars m := by
  intro hmem
  have hgv : m_old_ident ∈ Stmt.getVars
      (Stmt.loop (.det g) (some m) inv body md) := by
    unfold Stmt.getVars
    apply List.mem_append.mpr; left
    apply List.mem_append.mpr; left
    apply List.mem_append.mpr; right
    show m_old_ident ∈ ((some m).map
      (fun lp => HasFvars.getFvars lp)).getD []
  ...
  have hdef_none : m_old_ident ∉ Stmt.definedVars
      (Stmt.loop (.det g) (some m) inv body md) Bool.false := by
    intro hd
    exact absurd
...
private theorem simulation_loop_term_enter_case
    ...
    ∀ {hasInvFailure : Bool},
      ...
      CoreStar π φ
        (.seq (.block .none ρ₀.store ρ₀.eval
          (.stmts body
            ({ ρ₀ with hasFailure :=
              ρ₀.hasFailure || hasInvFailure } : Imperative.Env Expression)))
  ...
  | .ok (b, s') =>
    ...
    -- After case-split, three sub-cases: nondet, det+nomeasure, det+measure.
    -- For nondet and det+nomeasure we rebuild the constructive `.inr` trace
    -- (mirroring the original proof under the simplified semantics).  For
    -- det+measure the constructive proof threads `m_old` through the body
    -- trace via `coreStar_frame_fresh_var`, with case-splits on `lt m 0` at
    -- ρ₀ (assert_lb verdict) and `lt m m_old` at ρ_inner_ext (assert_decrease
    -- verdict): each `tt`/`ff` outcome maps to `.inl` (CanFail) or `.inr`
    -- (constructive trace) appropriately.
    case h_2 =>
      -- Nondet guard
      refine .inr (fun hnf_arg => ?_)
      have ⟨⟨ρ_mid, h_block_mid, h_tail_mid, ρ_inner, h_inner, heq_mid⟩,
            hall_tt', hproj_id, heval_eq⟩ :=
        loop_terminal_facts π φ hreach hreach_inner hnf''
      ...
      suffices h_then : CoreStar π φ
        ...
        have hproj_env : ({ ρ' with store := projectStore ρ₀.store ρ'.store, eval := ρ₀.eval } : Imperative.Env Expression) = ρ' := by
        ...
        have henv_eq : ({ ρ' with store := ρ'.store, eval := ρ₀.eval } : Imperative.Env Expression) = ρ' := by
          rw [← heval_eq]
      ...
      have h_arb_block : CoreStar π φ
        ...
        suffices h_body_run : CoreStar π φ (.stmts arb_facts_body ρ₀) (.terminal ρ_inner) by
          ...
          have heq_proj : ({ ρ_inner with store := projectStore ρ₀.store ρ_inner.store, eval := ρ₀.eval } : Imperative.Env Expression) = ρ_mid := by
            rw [heq_mid]
          rw [heq_proj] at h; exact h
        have hwfvar : WellFormedSemanticEvalVar ρ₀.eval := hswf.wfVar
        have h_havoc_id : CoreStar π φ
            (.stmt (.block havoc_label havoc_stmts ∅) ρ₀) (.terminal ρ₀) :=
          havoc_block_id_at π φ havoc_label havoc_vars md ρ₀ hwfvar h_havoc_vars_defined₀
        ...
        have h_body_from_ρ₀ : CoreStar π φ (.stmts body ρ₀) (.terminal ρ_inner) := by
          have heta : ({ store := ρ₀.store, eval := ρ₀.eval, hasFailure := ρ₀.hasFailure } : Imperative.Env Expression) = ρ₀ := by
            cases ρ₀; simp
      ...
      have hρ'_eq_mid_store : ({ ρ_mid with store := ρ'.store } : Imperative.Env Expression) = ρ' := by
        ...
        have h_exit_assumes : CoreStar π φ (.stmts exit_inv_assumes ρ') (.terminal ρ') := by
          have hwfb' : WellFormedSemanticEvalBool ρ'.eval := by
            have h := star_preserves_wfBool Expression (EvalCommand π φ) (EvalPureFunc φ)
              ...
              (show WellFormedSemanticEvalBool _ from hwfb)
            simpa [Config.getEnv] using h
          exact stmts_mapIdx_assume_terminal π φ inv ρ' md mkExitAssumeLabel hwfb' hall_tt'
    ...
    case h_1.isFalse =>
      ...
      | step _ _ _ hstep hrest =>
        ...
        | step_loop_exit hguard_ff hinvb hinviff _ =>
          cases hrest with
          | refl =>
            have hρ'_eq : ({ ρ₀ with hasFailure := ρ₀.hasFailure || ‹Bool› } : Imperative.Env Expression) = ρ₀ := by
        ...
        | step_loop_enter hguard_tt hinvb_enter hinviff_enter hwfbe_enter =>
          have hreach : CoreStar π φ (.stmt (.loop (.det g) none inv body md) ρ₀) (.terminal ρ') :=
            .step _ _ _ (.step_loop_enter hguard_tt hinvb_enter hinviff_enter hwfbe_enter) hrest
          have ⟨⟨ρ_mid, h_block_mid, h_tail_mid, ρ_inner, h_inner, heq_mid⟩,
                hall_tt', hproj_id, heval_eq⟩ :=
            loop_terminal_facts π φ hreach hreach_inner hnf''
          ...
          have h_exit_stmts : CoreStar π φ
            ...
            have h_exit_assumes : CoreStar π φ (.stmts exit_inv_assumes ρ') (.terminal ρ') := by
              exact stmts_mapIdx_assume_terminal π φ inv ρ' md mkExitAssumeLabel hwfb_ρ' hall_tt'
    ...
    case h_2.isFalse.isTrue =>
      -- Det+measure terminal subcase.  Mirrors det+nomeasure but threads
      -- m_old through via `coreStar_frame_fresh_var`, with a case-split on
      -- `lt m 0` derived via `wfInt.ltReduces` (replaces the old
      -- `hmeasb`/`hmeasiff` premises, which `step_loop_enter` no longer
      -- carries).  In the `tt` branch we take `.inl` (CanFail through
      -- assert_lb).  In the `ff` branch we build the constructive trace.
      rename_i _hcheck g _meas m h_m_old_fresh _h_some
      have hwfb : WellFormedSemanticEvalBool ρ₀.eval := hswf.wfBool
      have hwfvar : WellFormedSemanticEvalVar ρ₀.eval := hswf.wfVar
      ...
      | step _ _ _ hstep hrest =>
        ...
        | step_loop_exit hguard_ff hinvb_ex hinviff_ex hwfb_ex =>
          -- Zero-iteration case: parallel to det+none zero-iter.
          cases hrest with
          | refl =>
            have hρ'_eq : ({ ρ₀ with hasFailure := ρ₀.hasFailure || ‹Bool› } : Imperative.Env Expression) = ρ₀ := by
              have hinvf : (‹Bool› : Bool) = Bool.false :=
        ...
        | step_loop_enter hguard_tt hinvb_enter hinviff_enter hwfbe_enter =>
          ...
          have hnot_guard_tt : ρ'.eval ρ'.store (HasBoolOps.not g) = some HasBool.tt := by
            ...
            rw [heval_eq]
            exact this.mp hguard_ff
          -- Witness for the measure expression at ρ₀ (closed int-typed value)
          -- — derived from the syntactic `loopMeasuresAreIntTypes` field plus
          -- the evaluator's `intTyEvalClosed`.
          obtain ⟨v_m, hm_eval₀, hv_m_intTy, hv_m_noFvars⟩ :=
            hswf.measure_eval_closed_intTy
          have hv_m_numeral := hv_m_intTy
          -- Case-split on `lt m 0` via `wfInt.ltReduces`.
          have hwfInt₀ : WellFormedSemanticEvalInt ρ₀.eval := hswf.wfInt
          have hzero_val : HasVal.value (HasInt.zero : Expression.Expr) :=
            HasInt.numeralIsValue _ HasInt.zeroIsNumeral
          have hzero_eval : ρ₀.eval ρ₀.store (HasInt.zero : Expression.Expr) = some HasInt.zero :=
            hswf.wfVal.2 (HasInt.zero : Expression.Expr) ρ₀.store hzero_val
          have hzero_intTy : HasInt.isIntTy (HasInt.zero : Expression.Expr) :=
            HasInt.numeralIsIntTy _ HasInt.zeroIsNumeral
          have hzero_noFvars : HasFvars.getFvars (P := Expression) (HasInt.zero : Expression.Expr) = [] :=
            HasInt.numeralHasNoFvars _ HasInt.zeroIsNumeral
          rcases hwfInt₀.ltReduces ρ₀.store m HasInt.zero v_m HasInt.zero
              hm_eval₀ hv_m_intTy hv_m_noFvars hzero_eval hzero_intTy hzero_noFvars
              with hlt_tt | hmeas_ff
          · -- Failure path: lt m 0 = tt at ρ₀ → assert_lb fails.
            -- Build a CanFail trace through outer block / fib_block (passes) /
            -- ite-true / arb_facts_block / havoc-id / assumes / init / assert_lb (fails).
            refine .inl ?_
            let loop_num := (StringGenState.gen "loop" σ.gen).fst
            let mkAssertLabel : Nat → String → String := fun i lbl =>
              s!"{loopElimAssertPrefix}{loop_num}_entry_invariant_{invSuffix i lbl}"
            ...
            have h_assumes_block : CoreStar π φ
              ...
              rw [projectStore_self_env] at h; exact h
            -- Set up ρ₀_ext for init_m_old.
            let m_old_ident : Expression.Ident := HasIdent.ident
              (s!"{loopElimReservedPrefix}_measure_{loop_num}" : String)
            have h_m_old_isPrefix : loopElimReservedPrefix.toList.isPrefixOf
                m_old_ident.name.toList :=
              m_old_ident_isPrefix loop_num
            have h_m_old_none₀ : ρ₀.store m_old_ident = none :=
              m_old_none_of_reserved hswf h_loop_reserved h_m_old_isPrefix
            have h_init_m : InitState Expression ρ₀.store m_old_ident v_m
                (extendStore m_old_ident v_m ρ₀.store) :=
              m_old_initState ρ₀.store m_old_ident v_m h_m_old_none₀
            let ρ₀_ext : Imperative.Env Expression :=
            ...
            have h_init_step : CoreStar π φ (.stmt init_m_old ρ₀) (.terminal ρ₀_ext) :=
              m_old_init_step π φ hm_eval₀ h_init_m hwfvar
            -- assert_lb fails at ρ₀_ext.
            ...
            have h_fvar_eval : ρ₀_ext.eval ρ₀_ext.store
                (HasFvar.mkFvar m_old_ident) = some v_m :=
              fvar_eval_of_extendStore hwfvar h_m_old_0ext
            have h_m_old_notin_getVars_m : m_old_ident ∉ HasFvars.getFvars m :=
              m_old_notin_getFvars_m hswf h_loop_reserved h_m_old_isPrefix h_m_old_none₀
            have h_stores_agree_on_m : ∀ x ∈ HasFvars.getFvars m,
                ρ₀.store x = ρ₀_ext.store x :=
              stores_agree_on_m_after_extend ρ₀.store m h_m_old_notin_getVars_m
            have h_m_eval_0ext : ρ₀_ext.eval ρ₀_ext.store m = some v_m :=
              m_eval_after_extend hwfExpr h_stores_agree_on_m heval_0ext hm_eval₀
            have h_lt_m_0ext : ρ₀_ext.eval ρ₀_ext.store
                (HasIntOps.lt m HasInt.zero) = some HasBool.tt :=
              lt_m_zero_eval_after_extend hwfExpr h_stores_agree_on_m heval_0ext hlt_tt
            have h_lt_mold_eq_lt_m_0ext :
                ρ₀_ext.eval ρ₀_ext.store
                  (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
                ρ₀_ext.eval ρ₀_ext.store (HasIntOps.lt m HasInt.zero) :=
              lt_mold_eq_lt_m_at_ext hwfCong_0ext h_fvar_eval h_m_eval_0ext
            have h_lt_mold_0ext : ρ₀_ext.eval ρ₀_ext.store
                (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
                some HasBool.tt := by rw [h_lt_mold_eq_lt_m_0ext]; exact h_lt_m_0ext
            have h_not_lt_ff_0ext : ρ₀_ext.eval ρ₀_ext.store
            ...
            have h_assert_fail :
                CoreStar π φ (.stmt assert_lb ρ₀_ext)
                  (.terminal { ρ₀_ext with hasFailure := ρ₀_ext.hasFailure || Bool.true }) :=
              .step _ _ _ (.step_cmd (EvalCommand.cmd_sem
                (.eval_assert_fail h_not_lt_ff_0ext hwfb_0ext))) (.refl _)
            let ρ_f : Imperative.Env Expression :=
              { ρ₀_ext with hasFailure := ρ₀_ext.hasFailure || Bool.true }
            have hρf_fail : ρ_f.hasFailure = Bool.true := by
              show ρ₀_ext.hasFailure || Bool.true = Bool.true
              simp
            -- Build the prefix4 trace ρ₀ → terminal ρ_f.
            let mkMaintainLabel : Nat → String → String := fun i lbl =>
              s!"{loopElimAssertPrefix}{loop_num}_arbitrary_iter_maintain_invariant_{invSuffix i lbl}"
            let maintain_stmts : Statements :=
              inv.mapIdx fun i le => Stmt.cmd (HasPassiveCmds.assert (mkMaintainLabel i le.1) le.2 md)
            ...
            let arb_facts_label := s!"{loopElimBlockPrefix}arbitrary_iter_facts_{loop_num}"
            -- prefix4 ρ₀ → terminal ρ_f via stmts_cons_step.
            have h_assert_fail_run : CoreStar π φ
                (.stmts [assert_lb] ρ₀_ext) (.terminal ρ_f) :=
              ReflTrans_Transitive _ _ _ _
                (stmts_cons_step Expression (EvalCommand π φ) (EvalPureFunc φ)
                  _ _ ρ₀_ext ρ_f h_assert_fail)
                (.step _ _ _ .step_stmts_nil (.refl _))
            have h_prefix4_fail : CoreStar π φ
                (.stmts [.block havoc_label havoc_stmts ∅,
                  .block arb_assumes_label arb_assumes_body md,
              ...
              have h3 : CoreStar π φ
                  (.stmts [init_m_old, assert_lb] ρ₀)
                  (.stmts [assert_lb] ρ₀_ext) :=
                stmts_cons_step Expression (EvalCommand π φ) (EvalPureFunc φ)
                  _ _ ρ₀ ρ₀_ext h_init_step
              exact ReflTrans_Transitive _ _ _ _ h1
                (ReflTrans_Transitive _ _ _ _ h2
                  (ReflTrans_Transitive _ _ _ _ h3 h_assert_fail_run))
            -- Build a CanFail witness for prefix4 itself, then extend through suffix.
            have h_prefix4_cf : CanFailBlock (EvalCommand π φ) (EvalPureFunc φ)
                [.block havoc_label havoc_stmts ∅,
                  .block arb_assumes_label arb_assumes_body md,
                  init_m_old, assert_lb] ρ₀ :=
              ⟨_, by simp [Config.getEnv]; exact hρf_fail, h_prefix4_fail⟩
            -- Extend prefix4's CanFail through suffix = body ++ maintain_stmts ++ [assert_decrease].
            have h_arb_cf : CanFailBlock (EvalCommand π φ) (EvalPureFunc φ)
              ...
              have hlist_eq :
                ...
                show _ = _
                simp [arb_facts_body, List.cons_append, List.append_assoc]
              rw [hlist_eq]
          ...
          · -- Success path for assert_lb: lt m 0 = ff at ρ₀.
            -- Still need to case-split on lt m m_old at ρ_inner_ext for assert_decrease.
            have heval_mid : ρ_mid.eval = ρ₀.eval := by rw [heq_mid]
            ...
            have h_m_eval_0ext : ρ₀_ext.eval ρ₀_ext.store m = some v_m :=
              m_eval_after_extend hwfExpr h_stores_agree_on_m heval_0ext hm_eval₀
            have h_lt_m_0ext : ρ₀_ext.eval ρ₀_ext.store
                (HasIntOps.lt m HasInt.zero) = some HasBool.ff :=
              lt_m_zero_eval_after_extend hwfExpr h_stores_agree_on_m heval_0ext hmeas_ff
            have h_lt_mold_eq_lt_m_0ext :
                ρ₀_ext.eval ρ₀_ext.store
                  (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
                ρ₀_ext.eval ρ₀_ext.store (HasIntOps.lt m HasInt.zero) :=
            ...
            have h_lt_mold_0ext : ρ₀_ext.eval ρ₀_ext.store
                (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
                some HasBool.ff := by rw [h_lt_mold_eq_lt_m_0ext]; exact h_lt_m_0ext
            ...
            -- Body lift via coreStar_frame_fresh_var.
            have heta : ({ store := ρ₀.store, eval := ρ₀.eval, hasFailure := ρ₀.hasFailure }
                : Imperative.Env Expression) = ρ₀ := by cases ρ₀; simp
            have h_body_from_ρ₀ : CoreStar π φ (.stmts body ρ₀) (.terminal ρ_inner) := by
              rw [← heta]; exact h_inner
            ...
            let ρ_inner_ext : Imperative.Env Expression := extendEnv m_old_ident v_m ρ_inner
            ...
            -- hall_tt at ρ_mid (from h_tail_mid's first step).
            have h_loop_mid : CoreStar π φ
                (.stmt (.loop (.det g) (some m) inv body md) ρ_mid)
                (.terminal ρ') := by
              cases h_tail_mid with
            ...
            -- maintain_stmts at ρ_inner_ext → ρ_inner_ext.
            let mkMaintainLabel : Nat → String → String := fun i lbl =>
              s!"{loopElimAssertPrefix}{loop_num}_arbitrary_iter_maintain_invariant_{invSuffix i lbl}"
            let maintain_stmts : Statements :=
              inv.mapIdx fun i le => Stmt.cmd (HasPassiveCmds.assert (mkMaintainLabel i le.1) le.2 md)
              ...
              exact havoc_block_to_target π φ havoc_label havoc_vars md ρ_mid ρ' hwfvar_mid
                ...
                (by intro x hx_not_havoc
                    ...
                    · have hx_not_mod : x ∉ Block.modifiedVars body := by
                        ...
                        have hisNone := hswf.defsUndefined x (by
                          show x ∈ Stmt.definedVars
                            (.loop (ExprOrNondet.det g) (some m) inv body md) Bool.false
                          simp [Stmt.definedVars]; exact h_not_filter)
                        exact absurd hsome (by simp [Option.isNone_iff_eq_none] at hisNone; simp [hisNone])
                      have hx_not_def : x ∉ Block.definedVars body Bool.false := by
                        intro hdef
                      ...
                      have hx_not_touched_loop : x ∉ Config.touchedVarsSet
                          (Config.stmt
                            (.loop (ExprOrNondet.det g) (some m) inv body md) ρ₀) := by
                        simp only [Config.touchedVarsSet, Stmt.modifiedVars,
                          Stmt.definedVars, List.mem_append]
                        exact fun h => h.elim hx_not_mod hx_not_def
                      have h_rho'_eq : ρ'.store x = ρ₀.store x := by
            ...
            have hwfb_ρ' : WellFormedSemanticEvalBool ρ'.eval := by
              have h := star_preserves_wfBool Expression (EvalCommand π φ) (EvalPureFunc φ)
                hwf_ext.toWFEvalExtension
                (s := (.loop (.det g) (some m) inv body md : Statement)) (ρ := ρ₀) hreach
                (show WellFormedSemanticEvalBool _ from hwfb)
              simpa [Config.getEnv] using h
            have h_not_guard_assume : CoreStar π φ
                (.stmt (.cmd (HasPassiveCmds.assume
            ...
            -- Build assert_decrease pass at ρ_inner_ext.
            let assert_decrease : Statement := .cmd (HasPassiveCmds.assert
              s!"{loopElimAssertPrefix}{loop_num}_measure_decrease"
              (HasIntOps.lt m (HasFvar.mkFvar m_old_ident)) md)
            -- Step A: extract closed int-typed evaluation of m at ρ_mid via
            -- the new `intTyEvalClosed` field, applied at ρ_mid.store.
            -- `ρ_mid.eval = ρ₀.eval` (loop iteration doesn't modify eval) so
            -- `WellFormedSemanticEvalInt` lifts directly.  `m`'s fvars are
            -- isSome at ρ_mid.store because they were isSome at ρ₀.store
            -- (defUseOk) and small-step traces preserve isSome.
            have hmIsIntTy : HasInt.isIntTy m := by
              have hm := hswf.loopMeasuresAreIntTypes
              show HasInt.isIntTy m
              simp [Statement.measuresAreIntTypes] at hm
              exact hm.1
            have h_m_vars_isSome_ρ₀ : ∀ x ∈ HasFvars.getFvars m, (ρ₀.store x).isSome := by
              intro x hx
              have hdu := hswf.defUseOk
              simp only [Stmt.defUseWellFormed, Bool.and_eq_true] at hdu
              obtain ⟨⟨⟨⟨⟨⟨_, _⟩, hmeasOk⟩, _⟩, _⟩, _⟩, _⟩ := hdu
              have hmem : x ∈ ((some m).map
                  (fun lp => HasFvars.getFvars lp)).getD [] := by
                simp only [Option.map, Option.getD]; exact hx
              exact (List.all_eq_true.mp hmeasOk) x hmem
            have h_m_vars_isSome_ρ_mid : ∀ x ∈ HasFvars.getFvars m, (ρ_mid.store x).isSome := by
              intro x hx
              -- ρ_mid.store = projectStore ρ₀.store ρ_inner.store
              rw [heq_mid]
              show (projectStore ρ₀.store ρ_inner.store x).isSome
              have hsome := h_m_vars_isSome_ρ₀ x hx
              -- ρ_inner.store x is also isSome because the body trace preserves it.
              have h_inner_isSome : (ρ_inner.store x).isSome := by
                have h_body_star : CoreStar π φ (.stmts body ρ₀) (.terminal ρ_inner) :=
                  h_body_from_ρ₀
                exact stmts_star_preserves_isSome (π := π) (φ := φ) body ρ₀ _ h_body_star x hsome
              show (if (ρ₀.store x).isSome then ρ_inner.store x else none).isSome
              simp [hsome, h_inner_isSome]
            have hwfInt_mid : WellFormedSemanticEvalInt ρ_mid.eval := by
              show WellFormedSemanticEvalInt ρ_mid.eval
              rw [heval_mid]; exact hswf.wfInt
            have hwfVal_mid : WellFormedSemanticEvalVal ρ_mid.eval := by
              show WellFormedSemanticEvalVal ρ_mid.eval
              rw [heval_mid]; exact hswf.wfVal
            obtain ⟨v_m_mid, hm_eval_mid, hv_m_mid_intTy⟩ :=
              hwfInt_mid.intTyEvalSome ρ_mid.store m hmIsIntTy h_m_vars_isSome_ρ_mid
            have hv_m_mid_value : (@HasVal.value Expression) v_m_mid :=
              hwfVal_mid.1 m v_m_mid ρ_mid.store hm_eval_mid
            have hv_m_mid_noFvars : HasFvars.getFvars v_m_mid = [] :=
              HasInt.intValueIsClosed v_m_mid hv_m_mid_value hv_m_mid_intTy
            have hv_m_mid_numeral : HasInt.isIntTy v_m_mid := hv_m_mid_intTy
            ...
            have h_m_ops_declared : ∀ x ∈ HasOps.getOps (P := Expression) m,
              ...
              have hmeasOps :
                  (((some m).map
                    (fun lp => HasOps.getOps (P := Expression) lp)).getD []).all
                    (fun n => declaredFuncs n) = Bool.true :=
                hdu.1.1.1.2
              have hmem : x ∈ ((some m).map
                  (fun lp => HasOps.getOps (P := Expression) lp)).getD [] := by
                simp only [Option.map, Option.getD]; exact hx
            ...
            have hlt_disj : ρ_inner_ext.eval ρ_inner_ext.store
              ...
              hwfLt_inner_ext.ltReduces ρ_inner_ext.store m (HasFvar.mkFvar m_old_ident)
                v_m_mid v_m h_m_at_inner_ext hv_m_mid_intTy hv_m_mid_noFvars
                h_fvar_eval_inner_ext hv_m_intTy hv_m_noFvars
            -- Case-split on assert_decrease's verdict.  `tt` → constructive
            -- `.inr`; `ff` → `.inl` (CanFail through assert_decrease).
            ...
            · refine .inr (fun _ => ?_)
              ...
              -- Build arb_facts_body trace ρ₀ → ρ_inner_ext.
              let arb_facts_body : Statements :=
                [.block havoc_label havoc_stmts ∅,
                 .block arb_assumes_label arb_assumes_body md,
                 init_m_old, assert_lb] ++
              ...
              have h_arb_block : CoreStar π φ
                ...
                have hmid_alt : (Imperative.Env.mk (projectStore ρ₀.store ρ_inner_ext.store) ρ₀.eval
                    ρ_inner_ext.hasFailure : Imperative.Env Expression) = ρ_mid := by
                  rw [heq_mid, hstore_eq]
                  show (Imperative.Env.mk (projectStore ρ₀.store ρ_inner.store) ρ₀.eval
                      ρ_inner_ext.hasFailure : Imperative.Env Expression) =
                    Imperative.Env.mk (projectStore ρ₀.store ρ_inner.store) ρ₀.eval ρ_inner.hasFailure
                  rfl
                have hh : ({ ρ_inner_ext with store := projectStore ρ₀.store ρ_inner_ext.store, eval := ρ₀.eval } : Imperative.Env Expression) = Imperative.Env.mk (projectStore ρ₀.store ρ_inner_ext.store) ρ₀.eval ρ_inner_ext.hasFailure := rfl
                rw [hh, hmid_alt] at h
                exact h
              -- Build then-branch trace.
              ...
              have henv_eq : ({ ρ' with store := ρ'.store, eval := ρ₀.eval }
                  : Imperative.Env Expression) = ρ' := by rw [← heval_eq]
              rw [henv_eq] at h_outer
              exact h_outer
            · -- assert_decrease fails at ρ_inner_ext.  Take `.inl` (CanFail).
              refine .inl ?_
              ...
              exact Transform.canFailBlock_to_canFail_block (EvalCommand π φ) (EvalPureFunc φ)
                coreIsAtAssert s!"{loopElimBlockPrefix}loop_{loop_num}" _ ∅ ρ₀
                h_outer_stmts_canFail
/-- Mutual size-induction helper: `exitsCoveredByBlocks` is monotone in the
    label set.  Both branches in one statement so we only run the structural
    induction once. -/
private theorem exitsCoveredByBlocks_weaken_aux :
    ∀ (m : Nat) (labels labels' : List String),
      (∀ x ∈ labels, x ∈ labels') →
      (∀ s : Statement, Stmt.sizeOf s ≤ m →
        Stmt.exitsCoveredByBlocks labels s →
        Stmt.exitsCoveredByBlocks labels' s) ∧
      (∀ ss : Statements, Block.sizeOf ss ≤ m →
  ...
  induction m with
  | zero =>
    intro labels labels' _
    refine ⟨fun s hsz _ => ?_, fun ss hsz hss => ?_⟩
    · cases s <;> simp [Stmt.sizeOf] at hsz
    · cases ss with
      | nil => trivial
      | cons s rest => simp [Block.sizeOf] at hsz
  | succ n ih =>
    intro labels labels' hsub
    refine ⟨fun s hsz hs => ?_, fun ss hsz hss => ?_⟩
    · cases s with
      | cmd _ => trivial
      | exit l _ => exact hsub _ hs
      | funcDecl _ _ => trivial
      | typeDecl _ _ => trivial
      | block lbl bss md =>
        have hbss_sz : Block.sizeOf bss ≤ n := by simp [Stmt.sizeOf] at hsz; omega
        change Stmt.exitsCoveredByBlocks.Block.exitsCoveredByBlocks (lbl :: labels') bss
        exact (ih (lbl :: labels) (lbl :: labels')
          (fun x hx => by
            cases List.mem_cons.mp hx with
            | inl h => exact List.mem_cons.mpr (.inl h)
            | inr h => exact List.mem_cons.mpr (.inr (hsub x h)))).2 bss hbss_sz hs
      | ite c tss ess md =>
        have htss_sz : Block.sizeOf tss ≤ n := by simp [Stmt.sizeOf] at hsz; omega
        have hess_sz : Block.sizeOf ess ≤ n := by simp [Stmt.sizeOf] at hsz; omega
        obtain ⟨ht, he⟩ := hs
        exact ⟨(ih labels labels' hsub).2 tss htss_sz ht,
               (ih labels labels' hsub).2 ess hess_sz he⟩
      | loop _ _ _ bdy _ =>
        have hbdy_sz : Block.sizeOf bdy ≤ n := by simp [Stmt.sizeOf] at hsz; omega
        exact (ih labels labels' hsub).2 bdy hbdy_sz hs
    · cases ss with
      | nil => trivial
      | cons s rest =>
        have hsz_s : Stmt.sizeOf s ≤ n := by simp [Block.sizeOf] at hsz; omega
        have hsz_rest : Block.sizeOf rest ≤ n := by simp [Block.sizeOf] at hsz; omega
        obtain ⟨h1, h2⟩ := hss
        refine ⟨?_, ?_⟩
        · exact (ih labels labels' hsub).1 s hsz_s h1
        · exact (ih labels labels' hsub).2 rest hsz_rest h2

private theorem exitsCoveredByBlocks_weaken
    (labels labels' : List String)
    (hsub : ∀ x ∈ labels, x ∈ labels') :
    (∀ s : Statement, Stmt.exitsCoveredByBlocks labels s →
      Stmt.exitsCoveredByBlocks labels' s) ∧
    (∀ ss : Statements, Stmt.exitsCoveredByBlocks.Block.exitsCoveredByBlocks labels ss →
      Stmt.exitsCoveredByBlocks.Block.exitsCoveredByBlocks labels' ss) :=
  ⟨fun s hs => (exitsCoveredByBlocks_weaken_aux (Stmt.sizeOf s) _ _ hsub).1 s
    (Nat.le_refl _) hs,
   fun ss hss => (exitsCoveredByBlocks_weaken_aux (Block.sizeOf ss) _ _ hsub).2 ss
    (Nat.le_refl _) hss⟩
...
private theorem stmtsT_singleton_exiting
    {s : Statement} {ρ₀ ρ' : Imperative.Env Expression} {l : String}
    (hstar : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
...
/-- Predicate: cfg has the specific loop with env ρ at its at-loop position.
    Stated as a `Prop` matching `atLoopEnv`'s structure but with full loop
    constituents (rather than only env). -/
@[expose] private def Config.atOurLoop
    (loop : Statement) (ρ : Imperative.Env Expression) : CC → Prop
  | .stmt s ρ_s => s = loop ∧ ρ_s = ρ
  | .stmts (s :: _) ρ_s => s = loop ∧ ρ_s = ρ
  | .block _ _ _ inner => Config.atOurLoop loop ρ inner
  | .seq inner _ => Config.atOurLoop loop ρ inner
  | _ => False

/-- Helper: lift a body-iteration trace through a cfg whose at-loop position
    matches our loop with env ρ.  The new cfg has env ρ_next at its at-loop
    position.  Used to extend `cfg_init` traces in the loop iteration extract
    proofs as we recurse over loop iterations. -/
private theorem extend_atOurLoop_via_body_iter
    {loop : Statement} {ρ ρ_next : Imperative.Env Expression}
    (h_body_iter : CoreStar π φ (.stmt loop ρ) (.stmts [loop] ρ_next)) :
    ∀ (cfg_init : CC),
      Config.atOurLoop loop ρ cfg_init →
      ∃ cfg_init_next, CoreStar π φ cfg_init cfg_init_next ∧
        Config.atOurLoop loop ρ_next cfg_init_next := by
  intro cfg_init
  induction cfg_init with
  | stmt s ρ_s =>
    intro hmatch
    obtain ⟨hs, hρ⟩ := hmatch
    subst hs; subst hρ
    refine ⟨_, h_body_iter, ?_⟩
    exact ⟨rfl, rfl⟩
  | stmts bss ρ_s =>
    intro hmatch
    match bss, hmatch with
    | s :: rest, ⟨hs, hρ⟩ =>
      cases hs; cases hρ
      have h1 : CoreStar π φ
          (.stmts (loop :: rest) ρ)
          (.seq (.stmt loop ρ) rest) :=
        .step _ _ _ .step_stmts_cons (.refl _)
      have h2 : CoreStar π φ
          (.seq (.stmt loop ρ) rest)
          (.seq (.stmts [loop] ρ_next) rest) :=
        seq_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
          _ _ rest h_body_iter
      refine ⟨_, ReflTrans_Transitive _ _ _ _ h1 h2, ?_⟩
      exact ⟨rfl, rfl⟩
  | block label σ_p e_p inner ih =>
    intro hmatch
    simp only [Config.atOurLoop] at hmatch
    obtain ⟨inner_next, h_inner, hmatch_next⟩ := ih hmatch
    refine ⟨.block label σ_p e_p inner_next, ?_, ?_⟩
    · exact block_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
        _ _ label σ_p e_p h_inner
    · simp only [Config.atOurLoop]; exact hmatch_next
  | seq inner rest ih =>
    intro hmatch
    simp only [Config.atOurLoop] at hmatch
    obtain ⟨inner_next, h_inner, hmatch_next⟩ := ih hmatch
    refine ⟨.seq inner_next rest, ?_, ?_⟩
    · exact seq_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
        _ _ rest h_inner
    · simp only [Config.atOurLoop]; exact hmatch_next
  | terminal _ => intro hmatch; exact hmatch.elim
  | exiting _ _ => intro hmatch; exact hmatch.elim
...
private theorem loop_iteration_extract_exit
    ...
    {inv : List (String × Expression.Expr)}
    {body : Statements} {md : MetaData Expression}
    {ρ₀ ρ_start ρ' : Imperative.Env Expression} {lbl : String}
    (hswf : InitEnvWF reserved declaredFuncs (.loop guardE measure inv body md) ρ₀)
    (hstart_eval : ρ_start.eval = ρ₀.eval)
    ...
    (hreach_pre : CoreStar π φ
      (.stmt (.loop guardE measure inv body md) ρ₀)
      (.stmts [.loop guardE measure inv body md] ρ_start))
    (hreach : CoreStar π φ (.stmt (.loop guardE measure inv body md) ρ_start)
      (.exiting lbl ρ'))
    (hnf' : ρ'.hasFailure = Bool.false) :
    ∃ (ρ_k ρ_inner : Imperative.Env Expression),
      ρ_k.eval = ρ₀.eval ∧
      ...
      --   (1) witness `v` such that `ρ_k.eval ρ_k.store me = some v`
      --       (ensures init_m_old can fire).
      -- Vacuous in the nondet case.
      (∀ g me, guardE = .det g → measure = some me →
        ∃ v, ρ_k.eval ρ_k.store me = some v ∧ HasInt.isIntTy v ∧
          HasFvars.getFvars (P := Expression) v = []) := by
  -- Use length induction on the trace, mirroring `loop_cf_iteration_extract`.
  have hstarT := reflTrans_to_T hreach
  suffices ∀ (k : Nat) (ρ : Imperative.Env Expression),
      ρ.eval = ρ₀.eval →
      ρ.hasFailure = Bool.false →
      (∀ le ∈ inv, ρ.eval ρ.store le.2 = some HasBool.tt) →
      (∀ x, (ρ.store x).isSome ↔ (ρ₀.store x).isSome) →
      ∀ (cfg_init : CC),
      CoreStar π φ (.stmt (.loop guardE measure inv body md) ρ₀) cfg_init →
      Config.atOurLoop (.loop guardE measure inv body md) ρ cfg_init →
      ∀ (ρ' : Imperative.Env Expression)
        (hnf' : ρ'.hasFailure = Bool.false)
        (h : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
        ...
        ∃ (ρ_k ρ_inner : Imperative.Env Expression),
          ...
          (∀ g me, guardE = .det g → measure = some me →
            ∃ v, ρ_k.eval ρ_k.store me = some v ∧ HasInt.isIntTy v ∧
          HasFvars.getFvars (P := Expression) v = []) by
    have hatOurLoop_init :
        Config.atOurLoop (.loop guardE measure inv body md) ρ_start
          (.stmts [.loop guardE measure inv body md] ρ_start : CC) :=
      ⟨rfl, rfl⟩
    exact this hstarT.len ρ_start hstart_eval hstart_nf hstart_tt hstart_dom
      _ hreach_pre hatOurLoop_init ρ' hnf' hstarT (Nat.le_refl _)
  clear hreach hstarT hstart_eval hstart_nf hstart_tt hstart_dom hreach_pre ρ' hnf' ρ_start
  intro k
  induction k with
  | zero =>
    intro ρ _ _ _ _ _ _ _ ρ' _ hT hlen
    cases hT with
    | step _ _ _ _ _ => simp [ReflTransT.len] at hlen
  | succ k' ih =>
    intro ρ heval_ρ hnf_ρ hall_tt_ρ hsame_ρ cfg_init hreach_init hatOurLoop_init ρ' hnf' hT hlen
    have hmeas_ρ : ∀ g me, guardE = .det g → measure = some me →
        ∃ v, ρ.eval ρ.store me = some v ∧ HasInt.isIntTy v ∧
          HasFvars.getFvars (P := Expression) v = [] := by
      intro _ me _ hme
      -- Use the syntactic-field helper at ρ₀ (the loop's source env), then
      -- transport to ρ via ρ.eval = ρ₀.eval and store-agreement on m's fvars.
      have hmIsIntTy : HasInt.isIntTy me := by
        have hm := hswf.loopMeasuresAreIntTypes
        show HasInt.isIntTy me
        cases measure with
        | none => cases hme
        | some m =>
          have heq : m = me := by injection hme
          subst heq
          simp [Statement.measuresAreIntTypes] at hm
          exact hm.1
      have h_me_vars_isSome_ρ : ∀ x ∈ HasFvars.getFvars me, (ρ.store x).isSome := by
        intro x hx
        have h_me_vars_isSome_ρ₀ : (ρ₀.store x).isSome := by
          have hdu := hswf.defUseOk
          simp only [Stmt.defUseWellFormed, Bool.and_eq_true] at hdu
          obtain ⟨⟨⟨⟨⟨⟨_, _⟩, hmeasOk⟩, _⟩, _⟩, _⟩, _⟩ := hdu
          have hmem : x ∈ ((measure).map
              (fun lp => HasFvars.getFvars lp)).getD [] := by
            rw [hme]; simp only [Option.map, Option.getD]; exact hx
          exact (List.all_eq_true.mp hmeasOk) x hmem
        exact (hsame_ρ x).mpr h_me_vars_isSome_ρ₀
      have hwfInt_ρ : WellFormedSemanticEvalInt ρ.eval := heval_ρ ▸ hswf.wfInt
      have hwfVal_ρ : WellFormedSemanticEvalVal ρ.eval := heval_ρ ▸ hswf.wfVal
      obtain ⟨v, hv_eval, hv_intTy⟩ :=
        hwfInt_ρ.intTyEvalSome ρ.store me hmIsIntTy h_me_vars_isSome_ρ
      have hv_value : (@HasVal.value Expression) v := hwfVal_ρ.1 me v ρ.store hv_eval
    ...
    | step _ _ _ hstep1 hrest =>
      ...
      | step_loop_nondet_exit _ _ =>
        cases hrest with
        | step _ _ _ h _ => exact nomatch h
      | @step_loop_enter _ g_var _ _ _ _ hasInvFailure_var
                          hgt _ hinv_iff _ =>
        ...
        have hρ_eq : ({ ρ with hasFailure := ρ.hasFailure || Bool.false } : Imperative.Env Expression) = ρ := by
          cases ρ with
        ...
        | .inl ⟨h_block_exit, _⟩ =>
          ...
          refine ⟨ρ, ρ_inner, heval_ρ, hnf_ρ, hall_tt_ρ,
            reflTransT_to_prop h_body_exit_ρ, heq', hsame_ρ, ?_, ?_, ?_⟩
          · intro g' heq_g
            cases heq_g; exact hgt
          · intro x _ _ _; rfl
          · intro g' me heq_g hme; exact hmeas_ρ g' me heq_g hme
        | .inr ⟨ρ_mid, h_block_term, h_tail_exit, hlen_sum⟩ =>
          ...
          have hno_ff_inner : ¬∃ le ∈ inv, ρ_inner.eval ρ_inner.store le.2 = some HasBool.ff := by
            ...
            | step _ _ _ hstep_next hrest_next =>
              ...
              | step_loop_exit _ _ hinv_iff_next _ =>
                ...
                | step _ _ _ h _ => exact nomatch h
              | step_loop_enter _ _ hinv_iff_next _ =>
          ...
          have hall_tt_inner : ∀ le ∈ inv, ρ_inner.eval ρ_inner.store le.2 = some HasBool.tt := by
            ...
            | step _ _ _ hstep_next _ =>
              have hbool_mid : ∀ le ∈ inv,
                ...
                | step_loop_enter _ hinv_b _ _ => exact hinv_b
          ...
          -- Build body iter trace and extend cfg_init via helper
          have h_inv_b : ∀ le ∈ inv,
              ρ.eval ρ.store le.2 = .some HasBool.tt ∨
              ρ.eval ρ.store le.2 = .some HasBool.ff :=
            fun le hle => Or.inl (hall_tt_ρ le hle)
          have h_block_term_ρ : ReflTransT (StepStmt Expression
              (EvalCommand π φ) (EvalPureFunc φ))
              (.block .none ρ.store ρ.eval (.stmts body ρ)) (.terminal ρ_mid) :=
            hρ_eq ▸ h_block_term
          have h_seq_lift : CoreStar π φ
              (.seq
                (.block .none ρ.store ρ.eval (.stmts body ρ))
                [.loop (.det g_var) measure inv body md])
              (.seq (.terminal ρ_mid)
                [.loop (.det g_var) measure inv body md]) :=
            seq_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
              _ _ _ (reflTransT_to_prop h_block_term_ρ)
          have h_body_iter : CoreStar π φ
              (.stmt (.loop (.det g_var) measure inv body md) ρ)
              (.stmts [.loop (.det g_var) measure inv body md] ρ_mid) := by
            refine ReflTrans.step
              (Config.stmt (.loop (.det g_var) measure inv body md) ρ : CC)
              (Config.seq
                (Config.block .none ρ.store ρ.eval (Config.stmts body
                  ({ ρ with hasFailure := ρ.hasFailure || Bool.false } : Imperative.Env Expression)) : CC)
                [.loop (.det g_var) measure inv body md])
              _ ?_ ?_
            · exact .step_loop_enter (hasInvFailure := Bool.false) hgt h_inv_b
                (by simp only [Bool.false_eq_true, false_iff]; exact fun hex => hno_ff hex)
                (heval_ρ ▸ hswf.wfBool)
            · simp only [hρ_eq]
              refine ReflTrans_Transitive _ _ _ _ ?_
                (.step _ _ _ .step_seq_done (.refl _))
              exact h_seq_lift
          obtain ⟨cfg_init_mid, hreach_init_mid, hatOurLoop_init_mid⟩ :=
            extend_atOurLoop_via_body_iter (π := π) (φ := φ)
              h_body_iter cfg_init hatOurLoop_init
          obtain ⟨ρ_k, ρ_inner_k, h1, h2, h3, h4, h5, h6, h7, h_pres_mid, hk_meas_eval⟩ :=
            ih ρ_mid heval_mid hnf_mid hall_tt_mid hsame_mid
              cfg_init_mid
              (ReflTrans_Transitive _ _ _ _ hreach_init hreach_init_mid)
              hatOurLoop_init_mid
              ρ' hnf' h_loop_T hlen_loop
          refine ⟨ρ_k, ρ_inner_k, h1, h2, h3, h4, h5, h6, h7, ?_, hk_meas_eval⟩
      ...
      | step_loop_nondet_enter hinv_b hinv_iff =>
        ...
        | .inr ⟨ρ_mid, h_block_term, h_tail_exit, hlen_sum⟩ =>
          ...
          have hall_tt_mid : ∀ le ∈ inv, ρ_mid.eval ρ_mid.store le.2 = some HasBool.tt := by
            ...
            exact (body_eval_inv_preserved_init π φ hwf_ext hswf hsame_ρ heval_ρ
              h_body_term_ρ_p hle ρ_inner.store HasBool.tt).mpr (hall_tt_inner le hle)
          -- Build body iter trace for nondet case
          have h_inv_b_n : ∀ le ∈ inv,
              ρ.eval ρ.store le.2 = .some HasBool.tt ∨
              ρ.eval ρ.store le.2 = .some HasBool.ff :=
            fun le hle => Or.inl (hall_tt_ρ le hle)
          have h_block_term_ρ_n : ReflTransT (StepStmt Expression
              (EvalCommand π φ) (EvalPureFunc φ))
              (.block .none ρ.store ρ.eval (.stmts body ρ)) (.terminal ρ_mid) :=
            hρ_eq ▸ h_block_term
          have h_seq_lift_n : CoreStar π φ
              (.seq
                (.block .none ρ.store ρ.eval (.stmts body ρ))
              ...
              (.seq (.terminal ρ_mid)
                [.loop .nondet measure inv body md]) :=
            seq_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
              _ _ _ (reflTransT_to_prop h_block_term_ρ_n)
          have h_body_iter_n : CoreStar π φ
              (.stmt (.loop .nondet measure inv body md) ρ)
              (.stmts [.loop .nondet measure inv body md] ρ_mid) := by
            refine ReflTrans.step
              (Config.stmt (.loop .nondet measure inv body md) ρ : CC)
              (Config.seq
                (Config.block .none ρ.store ρ.eval (Config.stmts body
                  ({ ρ with hasFailure := ρ.hasFailure || Bool.false } : Imperative.Env Expression)) : CC)
                [.loop .nondet measure inv body md])
              _ ?_ ?_
            · exact .step_loop_nondet_enter (hasInvFailure := Bool.false) h_inv_b_n
                (by simp only [Bool.false_eq_true, false_iff]; exact fun hex => hno_ff hex)
            · simp only [hρ_eq]
              refine ReflTrans_Transitive _ _ _ _ ?_
                (.step _ _ _ .step_seq_done (.refl _))
              exact h_seq_lift_n
          obtain ⟨cfg_init_mid_n, hreach_init_mid_n, hatOurLoop_init_mid_n⟩ :=
            extend_atOurLoop_via_body_iter (π := π) (φ := φ)
              h_body_iter_n cfg_init hatOurLoop_init
          obtain ⟨ρ_k, ρ_inner_k, h1, h2, h3, h4, h5, h6, h7, h_pres_mid, hk_meas_eval⟩ :=
            ih ρ_mid heval_mid hnf_mid hall_tt_mid hsame_mid
              cfg_init_mid_n
              (ReflTrans_Transitive _ _ _ _ hreach_init hreach_init_mid_n)
              hatOurLoop_init_mid_n
              ρ' hnf' h_loop_T hlen_loop
          refine ⟨ρ_k, ρ_inner_k, h1, h2, h3, h4, h5, h6, h7, ?_, hk_meas_eval⟩
          intro x hx_mod hx_def hx_some
          have hx_not_touched : x ∉ Config.touchedVarsSet (Config.stmts body ρ) := by
...
private theorem simulation_loop_exit_enter_case
    ...
    (hmeas_eval_init : ∀ g, guardE = .det g → ∀ me, measure = .some me →
      ∃ v, ρ₀.eval ρ₀.store me = .some v ∧ HasInt.isIntTy v ∧
          HasFvars.getFvars (P := Expression) v = []) :
    Transform.CanFail (LangCore π φ)
  ...
  | .ok (b, s') =>
    ...
    case h_2 =>
      ...
      have h_havoc_vars_defined₀ : ∀ x ∈ havoc_vars, (ρ₀.store x).isSome :=
        havoc_vars_defined_of_init hswf havoc_vars rfl
      -- Extract uniform ρ_k for both sub-cases of seq_reaches_exiting.
      have h_extract :
          ∃ (ρ_k ρ_inner_k : Imperative.Env Expression),
            ρ_k.eval = ρ₀.eval ∧
            ρ_k.hasFailure = Bool.false ∧
            (∀ le ∈ inv, ρ_k.eval ρ_k.store le.2 = some HasBool.tt) ∧
            CoreStar π φ (.stmts body ρ_k) (.exiting lbl ρ_inner_k) ∧
            ρ' = ({ ρ_inner_k with store := projectStore ρ_k.store ρ_inner_k.store, eval := ρ_k.eval } : Imperative.Env Expression) ∧
            (∀ x, (ρ_k.store x).isSome ↔ (ρ₀.store x).isSome) ∧
            (∀ x, x ∉ Block.modifiedVars body → x ∉ Block.definedVars body false →
        ...
        | .inl h_block_exit =>
          ...
          rw [heta] at h_body_exit
          refine ⟨ρ₀, ρ_inner, rfl, hnf₀, hall_tt, h_body_exit, heq_ρ',
                  fun _ => Iff.rfl, ?_⟩
          intros _ _ _ _; rfl
            ...
            | step _ _ _ hstep_next hrest_next =>
              ...
              have hno_ff : ¬∃ le' ∈ inv,
                ...
                | step_loop_nondet_enter _ hinv_iff =>
                  ...
                  have hnf_succ : (Config.seq (.block .none ρ_mid.store ρ_mid.eval (.stmts body
                      { ρ_mid with hasFailure := ρ_mid.hasFailure || true }))
                      [.loop ExprOrNondet.nondet measure inv body md]).getEnv.hasFailure
                      = Bool.true := by simp [Config.getEnv]
                  have hnf_ρ' : ρ'.hasFailure = Bool.true := by
                    have := StepStmtStar_hasFailure_monotone (P := Expression)
                      (EvalCmd := EvalCommand π φ) (extendEval := EvalPureFunc φ)
                      (reflTransT_to_prop hrest_next) hnf_succ
              ...
              · exact htt
              · exact absurd ⟨le, hle, hff⟩ hno_ff
          -- Build hreach_pre for nondet case: (.stmt loop ρ₀) →* (.stmts [.loop ...] ρ_mid)
          have hbody_star_n : CoreStar π φ
              (.stmts body ρ₀) (.terminal ρ_inner_first) := by
            rw [← heta]; exact h_body_first
          have hblock_done_n : CoreStar π φ
              (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
              (.terminal ρ_mid) := by
            apply ReflTrans_Transitive _ _ _ _
              (block_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
                _ _ .none ρ₀.store ρ₀.eval hbody_star_n)
            refine .step _ _ _ .step_block_done ?_
            rw [heq_mid]; exact .refl _
          have hseq_done_n : CoreStar π φ
              (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
                [.loop ExprOrNondet.nondet measure inv body md])
              (.stmts [.loop ExprOrNondet.nondet measure inv body md] ρ_mid) := by
            apply ReflTrans_Transitive _ _ _ _
              (seq_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
                _ _ _ hblock_done_n)
            exact .step _ _ _ .step_seq_done (.refl _)
          have hloop_step_n : Imperative.StepStmt Expression
              (EvalCommand π φ) (EvalPureFunc φ)
              (.stmt (.loop ExprOrNondet.nondet measure inv body md) ρ₀)
              (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
                [.loop ExprOrNondet.nondet measure inv body md]) := by
            have heta_ρ₀ : ({ ρ₀ with hasFailure := ρ₀.hasFailure || Bool.false }
                : Imperative.Env Expression) = ρ₀ := by
              cases ρ₀; simp_all
            have hinviff_false : (Bool.false = Bool.true ↔
                ∃ le, le ∈ inv ∧ ρ₀.eval ρ₀.store le.2 = some HasBool.ff) := by
              constructor
              · intro h; cases h
              · intro ⟨le, hle, hff⟩
                have htt := hall_tt le hle
                rw [htt] at hff; cases hff
            have h : Imperative.StepStmt Expression
                (EvalCommand π φ) (EvalPureFunc φ)
                (.stmt (.loop ExprOrNondet.nondet measure inv body md) ρ₀)
                (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body
                  { ρ₀ with hasFailure := ρ₀.hasFailure || Bool.false }))
                  [.loop ExprOrNondet.nondet measure inv body md]) :=
              Imperative.StepStmt.step_loop_nondet_enter (hasInvFailure := Bool.false)
                (fun le hle => Or.inl (hall_tt le hle))
                hinviff_false
            rw [heta_ρ₀] at h; exact h
          have hreach_pre_n : CoreStar π φ
              (.stmt (.loop ExprOrNondet.nondet measure inv body md) ρ₀)
              (.stmts [.loop ExprOrNondet.nondet measure inv body md] ρ_mid) :=
            .step _ _ _ hloop_step_n hseq_done_n
          obtain ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
                  hsame_k, _h_guard_k, h_pres_k, _⟩ :=
            loop_iteration_extract_exit (π := π) (φ := φ) hwf_ext reserved hswf
              heval_mid hnf_mid hsame_mid hall_tt_mid hreach_pre_n
              h_loop hnf''
          refine ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
                  hsame_k, ?_⟩
          intro x hx_mod hx_def hx_some
          have hx_not_touched : x ∉ Config.touchedVarsSet (Config.stmts body ρ₀) := by
            simp only [Config.touchedVarsSet, List.mem_append]
      ...
      have hk_env_eq : ({ ρ₀ with store := ρ_k.store } : Imperative.Env Expression) = ρ_k := by
        cases ρ_k with
        | mk sk ek fk =>
          cases ρ₀ with
          | mk s₀ e₀ f₀ =>
      ...
      -- Assumes block at ρ_k (only inv assumes for nondet).
      have h_assumes_block : CoreStar π φ
        ...
        have h := block_wrap_terminal (EvalCommand π φ) (EvalPureFunc φ)
          arb_assumes_label arb_assumes_body md ρ_k ρ_k h_assumes_run
        rw [projectStore_self_env] at h; exact h
      have h_arb_body_exit : CoreStar π φ (.stmts arb_facts_body ρ₀)
          (.exiting lbl ρ_inner_k) := by
        show CoreStar π φ (.stmts
          ([.block havoc_label havoc_stmts ∅,
            .block arb_assumes_label arb_assumes_body md] ++ [] ++ body ++
            maintain_stmts ++ []) ρ₀) (.exiting lbl ρ_inner_k)
        simp only [List.append_nil]
        exact ReflTrans_Transitive _ _ _ _
          (stmts_prefix_terminal_append Expression (EvalCommand π φ) (EvalPureFunc φ)
            ...
            (ReflTrans_Transitive _ _ _ _
              ...
              (ReflTrans_Transitive _ _ _ _
                ...
                (.step _ _ _ .step_stmts_nil (.refl _)))))
          (stmts_prefix_exiting_append π φ body maintain_stmts ρ_k ρ_inner_k lbl
            h_body_k_exit)
      have h_arb_block_exit : CoreStar π φ
          (.stmt (.block arb_facts_label arb_facts_body ∅) ρ₀)
          (.exiting lbl ({ ρ_inner_k with store := projectStore ρ₀.store ρ_inner_k.store, eval := ρ₀.eval } : Imperative.Env Expression)) :=
        block_wrap_exiting_mismatch (EvalCommand π φ) (EvalPureFunc φ) arb_facts_label
          arb_facts_body ∅ lbl ρ₀ ρ_inner_k hne_arb h_arb_body_exit
      ...
      have h_ite_inner_exit : CoreStar π φ
          (.block .none ρ₀.store ρ₀.eval (.stmts
            ((.block arb_facts_label arb_facts_body ∅) ::
              ([.block havoc_label havoc_stmts ∅] ++ [] ++ exit_inv_assumes)) ρ₀))
          (.exiting lbl ({ ρ_inner_k with store := projectStore ρ₀.store ρ_inner_k.store, eval := ρ₀.eval } : Imperative.Env Expression)) := by
      ...
      have h_outer_exit := block_wrap_exiting_mismatch (EvalCommand π φ) (EvalPureFunc φ)
        s!"{loopElimBlockPrefix}loop_{loop_num}" _ ∅ lbl ρ₀ _ hne_loop h_stmts_exit
      rw [hproj_idem] at h_outer_exit
      have hproj_eq : projectStore ρ₀.store ρ_inner_k.store =
        ...
        · have hsome_k : (ρ_k.store x).isSome := (hsame_k x).mpr (by simp [hh])
          simp [hsome_k]
      have hρ'_eq : ρ' = ({ ρ_inner_k with store := projectStore ρ₀.store ρ_inner_k.store, eval := ρ₀.eval } : Imperative.Env Expression) := by
        rw [heq_ρ', heval_k, hproj_eq]
      rw [← hρ'_eq] at h_outer_exit
      exact h_outer_exit
    case h_1.isFalse =>
      -- Det guard, no measure.
      rename_i _ _ g _ _
      refine .inr (fun _ => ?_)
      ...
      have h_extract :
        ...
        | .inr ⟨ρ_mid, h_block_term, h_tail_exit⟩ =>
            ...
            | step _ _ _ hstep_next hrest_next =>
              ...
              have hno_ff : ¬∃ le' ∈ inv,
                ...
                | step_loop_exit _ _ hinv_iff _ =>
                  ...
                  cases hrest_next with
                  | step _ _ _ h _ => exact nomatch h
                | step_loop_enter _ _ hinv_iff _ =>
                  have hinvF_true := hinv_iff.mpr ⟨le', hle', hff⟩
                  rw [hinvF_true] at hrest_next
                  have hnf_ρ' : ρ'.hasFailure = Bool.true := by
                    have := StepStmtStar_hasFailure_monotone (P := Expression)
              ...
              · exact htt
              · exact absurd ⟨le, hle, hff⟩ hno_ff
          -- Build hreach_pre for det+nomeasure: trace from (.stmt loop ρ₀) to (.stmts [.loop ...] ρ_mid)
          have hbody_star_dn : CoreStar π φ
              (.stmts body ρ₀) (.terminal ρ_inner_first) := by
            rw [← heta]; exact h_body_first
          have hblock_done_dn : CoreStar π φ
              (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
              (.terminal ρ_mid) := by
            apply ReflTrans_Transitive _ _ _ _
              (block_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
                _ _ .none ρ₀.store ρ₀.eval hbody_star_dn)
            refine .step _ _ _ .step_block_done ?_
            rw [heq_mid]; exact .refl _
          have hseq_done_dn : CoreStar π φ
              (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
                [.loop (.det g) (none : Option Expression.Expr) inv body md])
              (.stmts [.loop (.det g) (none : Option Expression.Expr) inv body md] ρ_mid) := by
            apply ReflTrans_Transitive _ _ _ _
              (seq_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
                _ _ _ hblock_done_dn)
            exact .step _ _ _ .step_seq_done (.refl _)
          have hloop_step_dn : Imperative.StepStmt Expression
              (EvalCommand π φ) (EvalPureFunc φ)
              (.stmt (.loop (.det g) (none : Option Expression.Expr) inv body md) ρ₀)
              (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
                [.loop (.det g) (none : Option Expression.Expr) inv body md]) := by
            have heta_ρ₀ : ({ ρ₀ with hasFailure := ρ₀.hasFailure || Bool.false }
                : Imperative.Env Expression) = ρ₀ := by
              cases ρ₀; simp_all
            have hinviff_false : (Bool.false = Bool.true ↔
            ...
            have h : Imperative.StepStmt Expression
                ...
                (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body
                  { ρ₀ with hasFailure := ρ₀.hasFailure || Bool.false }))
                  [.loop (.det g) (none : Option Expression.Expr) inv body md]) :=
              Imperative.StepStmt.step_loop_enter (hasInvFailure := Bool.false)
                hguard_tt
                (fun le hle => Or.inl (hall_tt le hle))
                hinviff_false hwfb
            rw [heta_ρ₀] at h; exact h
          have hreach_pre_dn : CoreStar π φ
              (.stmt (.loop (.det g) (none : Option Expression.Expr) inv body md) ρ₀)
              (.stmts [.loop (.det g) (none : Option Expression.Expr) inv body md] ρ_mid) :=
            .step _ _ _ hloop_step_dn hseq_done_dn
          obtain ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
                  hsame_k, h_guard_k, h_pres_k, _⟩ :=
            loop_iteration_extract_exit (π := π) (φ := φ) hwf_ext reserved hswf
              heval_mid hnf_mid hsame_mid hall_tt_mid hreach_pre_dn
              h_loop hnf''
          have hguard_k_tt : ρ_k.eval ρ_k.store g = some HasBool.tt := h_guard_k g rfl
          refine ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
                  hsame_k, hguard_k_tt, ?_⟩
          intro x hx_mod hx_def hx_some
          have hx_not_touched : x ∉ Config.touchedVarsSet (Config.stmts body ρ₀) := by
            simp only [Config.touchedVarsSet, List.mem_append]
            exact fun h => h.elim hx_mod hx_def
          ...
          rw [h_k_mid, h_mid_eq]
      obtain ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
              hsame_k, hguard_k_tt, h_pres_k⟩ := h_extract
      have hwfb_k : WellFormedSemanticEvalBool ρ_k.eval := by rw [heval_k]; exact hwfb
      -- havoc to ρ_k.
      have h_havoc_vars_defined_k : ∀ x ∈ havoc_vars, (ρ_k.store x).isSome := fun x hx =>
        (hsame_k x).mpr (h_havoc_vars_defined₀ x hx)
      ...
      have hk_env_eq : ({ ρ₀ with store := ρ_k.store } : Imperative.Env Expression) = ρ_k := by
        ...
        | mk sk ek fk =>
          ...
          | mk s₀ e₀ f₀ =>
            ...
            simp [heval_k, hnf_k, hnf₀]
      rw [hk_env_eq] at h_havoc_to_k
      -- Assumes block at ρ_k (guard assume + inv assumes for det).
      have h_assumes_block : CoreStar π φ
          (.stmt (.block arb_assumes_label arb_assumes_body md) ρ_k) (.terminal ρ_k) := by
        have h_assumes_run : CoreStar π φ (.stmts arb_assumes_body ρ_k) (.terminal ρ_k) := by
          simp only [arb_assumes_body, List.cons_append, List.nil_append]
      ...
      exact h_outer_exit
    case h_2.isFalse.isTrue =>
      -- Det guard, measure = some m.
      rename_i _hcheck g _meas m h_m_old_fresh _h_some
      have hguard_tt : ρ₀.eval ρ₀.store g = some HasBool.tt := hguard_det g rfl
      obtain ⟨v_m, hm_eval₀, hv_m_intTy, hv_m_noFvars⟩ := hmeas_eval_init g rfl m rfl
      have hv_m_numeral := hv_m_intTy
      have heta : ({ store := ρ₀.store, eval := ρ₀.eval, hasFailure := ρ₀.hasFailure }
        : Imperative.Env Expression) = ρ₀ := by cases ρ₀; simp
      have hwfb : WellFormedSemanticEvalBool ρ₀.eval := hswf.wfBool
      ...
      have h_havoc_vars_defined₀ : ∀ x ∈ havoc_vars, (ρ₀.store x).isSome :=
        havoc_vars_defined_of_init hswf havoc_vars rfl
      -- Case-split on `lt m 0` at ρ₀ via wfInt.ltReduces.
      have hzero_val : HasVal.value (HasInt.zero : Expression.Expr) :=
        HasInt.numeralIsValue _ HasInt.zeroIsNumeral
      have hzero_eval : ρ₀.eval ρ₀.store (HasInt.zero : Expression.Expr) = some HasInt.zero :=
        hswf.wfVal.2 (HasInt.zero : Expression.Expr) ρ₀.store hzero_val
      ...
      have hzero_noFvars : HasFvars.getFvars (P := Expression) (HasInt.zero : Expression.Expr) = [] :=
        HasInt.numeralHasNoFvars _ HasInt.zeroIsNumeral
      rcases hwfInt₀.ltReduces ρ₀.store m HasInt.zero v_m HasInt.zero
          hm_eval₀ hv_m_intTy hv_m_noFvars hzero_eval hzero_intTy hzero_noFvars with hlt_tt | hmeas_ff
      · -- Failure path: lt m 0 = tt at ρ₀ → assert_lb fails. Take .inl (CanFail).
        refine .inl ?_
        have h_havoc_id : CoreStar π φ
            (.stmt (.block havoc_label havoc_stmts ∅) ρ₀) (.terminal ρ₀) :=
          havoc_block_id_at π φ havoc_label havoc_vars md ρ₀ hwfvar h_havoc_vars_defined₀
        ...
        exact Transform.canFailBlock_to_canFail_block (EvalCommand π φ) (EvalPureFunc φ)
          coreIsAtAssert s!"{loopElimBlockPrefix}loop_{loop_num}" _ ∅ ρ₀
          h_outer_stmts_canFail
      · -- Non-failure path at ρ₀: extract uniform ρ_k.  Inside, case-split on
        -- `lt m 0` at ρ_k to decide CanFail vs constructive trace.
        have h_extract :
            ∃ (ρ_k ρ_inner_k : Imperative.Env Expression),
              ρ_k.eval = ρ₀.eval ∧
              ρ_k.hasFailure = Bool.false ∧
              ...
              (∀ x, x ∉ Block.modifiedVars body → x ∉ Block.definedVars body false →
                (ρ₀.store x).isSome → ρ_k.store x = ρ₀.store x) ∧
              (∃ v, ρ_k.eval ρ_k.store m = some v ∧ HasInt.isIntTy v ∧
          HasFvars.getFvars (P := Expression) v = []) := by
          match seq_reaches_exiting (P := Expression)
            (EvalCmd := EvalCommand π φ) (extendEval := EvalPureFunc φ) hreach_inner with
          | .inl h_block_exit =>
            ...
            rw [heta] at h_body_exit
            refine ⟨ρ₀, ρ_inner, rfl, hnf₀, hall_tt, h_body_exit, heq_ρ',
                    fun _ => Iff.rfl, hguard_tt, ?_, v_m, hm_eval₀, hv_m_intTy, hv_m_noFvars⟩
            intros _ _ _ _; rfl
          | .inr ⟨ρ_mid, h_block_term, h_tail_exit⟩ =>
            rw [← heta] at h_block_term
            rw [heta] at h_block_term
            ...
            obtain ⟨h_loop_T, _⟩ := stmtsT_singleton_exiting π φ h_tail_T
            have h_loop : CoreStar π φ
                (.stmt (.loop (ExprOrNondet.det g) (some m) inv body md) ρ_mid)
                (.exiting lbl ρ') :=
              reflTransT_to_prop h_loop_T
            have hall_tt_mid : ∀ le ∈ inv, ρ_mid.eval ρ_mid.store le.2 = some HasBool.tt := by
              intro le hle
              ...
              | step _ _ _ hstep_next hrest_next =>
                ...
                · exact htt
                · exact absurd ⟨le, hle, hff⟩ hno_ff
            -- Build trace from (.stmt loop ρ₀) to (.stmts [.loop ...] ρ_mid)
            -- via step_loop_enter + body + block_done + seq_done.
            have hbody_star : CoreStar π φ
                (.stmts body ρ₀) (.terminal ρ_inner_first) := by
              rw [← heta]; exact h_body_first
            have hblock_done_main : CoreStar π φ
                (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
                (.terminal ρ_mid) := by
              apply ReflTrans_Transitive _ _ _ _
                (block_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
                  _ _ .none ρ₀.store ρ₀.eval hbody_star)
              refine .step _ _ _ .step_block_done ?_
              rw [heq_mid]; exact .refl _
            have hseq_done_main : CoreStar π φ
                (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
                  [.loop (.det g) (some m) inv body md])
                (.stmts [.loop (.det g) (some m) inv body md] ρ_mid) := by
              apply ReflTrans_Transitive _ _ _ _
                (seq_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
                  _ _ _ hblock_done_main)
              exact .step _ _ _ .step_seq_done (.refl _)
            have hloop_step_main : Imperative.StepStmt Expression
                (EvalCommand π φ) (EvalPureFunc φ)
                (.stmt (.loop (.det g) (some m) inv body md) ρ₀)
                (.seq (.block .none ρ₀.store ρ₀.eval (.stmts body ρ₀))
                  [.loop (.det g) (some m) inv body md]) := by
              have heta_ρ₀ : ({ ρ₀ with hasFailure := ρ₀.hasFailure || Bool.false }
                  : Imperative.Env Expression) = ρ₀ := by
                cases ρ₀; simp_all
              have hinviff_false : (Bool.false = Bool.true ↔
              ...
              have h : Imperative.StepStmt Expression
                ...
                Imperative.StepStmt.step_loop_enter (hasInvFailure := Bool.false)
                  ...
                  hinviff_false hwfb
              rw [heta_ρ₀] at h; exact h
            have hreach_main : CoreStar π φ
                (.stmt (.loop (.det g) (some m) inv body md) ρ₀)
                (.stmts [.loop (.det g) (some m) inv body md] ρ_mid) :=
              .step _ _ _ hloop_step_main hseq_done_main
            obtain ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
                    hsame_k, h_guard_k, h_pres_k, h_meas_k_eval⟩ :=
              loop_iteration_extract_exit (π := π) (φ := φ) hwf_ext reserved hswf
                heval_mid hnf_mid hsame_mid hall_tt_mid hreach_main h_loop hnf''
            have hguard_k_tt : ρ_k.eval ρ_k.store g = some HasBool.tt := h_guard_k g rfl
            have h_pres_k_ρ₀ : ∀ x, x ∉ Block.modifiedVars body →
              ...
              have h_k_mid : ρ_k.store x = ρ_mid.store x :=
                h_pres_k x hx_mod hx_def h_mid_some
              rw [h_k_mid, h_mid_eq]
            obtain ⟨v_mk, hm_eval_k, hv_mk_intTy, hv_mk_noFvars⟩ :=
              h_meas_k_eval g m rfl rfl
            refine ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
                    hsame_k, hguard_k_tt, h_pres_k_ρ₀,
                    v_mk, hm_eval_k, hv_mk_intTy, hv_mk_noFvars⟩
        obtain ⟨ρ_k, ρ_inner_k, heval_k, hnf_k, hall_tt_k, h_body_k_exit, heq_ρ',
                hsame_k, hguard_k_tt, h_pres_k, v_mk, hm_eval_k,
                hv_mk_intTy, hv_mk_noFvars⟩ := h_extract
        have hwfb_k : WellFormedSemanticEvalBool ρ_k.eval := by rw [heval_k]; exact hwfb
        ...
        have hwfInt_k : WellFormedSemanticEvalInt ρ_k.eval := by rw [heval_k]; exact hwfInt₀
        have hzero_eval_k : ρ_k.eval ρ_k.store (HasInt.zero : Expression.Expr) = some HasInt.zero :=
          (by rw [heval_k]; exact hswf.wfVal.2 (HasInt.zero : Expression.Expr) ρ_k.store hzero_val)
        -- Case-split on `lt m 0` at ρ_k for assert_lb's verdict.
        have hzero_intTy_k : HasInt.isIntTy (HasInt.zero : Expression.Expr) :=
          HasInt.numeralIsIntTy _ HasInt.zeroIsNumeral
        have hzero_noFvars_k : HasFvars.getFvars (P := Expression) (HasInt.zero : Expression.Expr) = [] :=
          HasInt.numeralHasNoFvars _ HasInt.zeroIsNumeral
        rcases hwfInt_k.ltReduces ρ_k.store m HasInt.zero v_mk HasInt.zero
            hm_eval_k hv_mk_intTy hv_mk_noFvars hzero_eval_k hzero_intTy_k hzero_noFvars_k
            with hk_lt_tt | hk_meas_ff
        · -- assert_lb fails at ρ_k_ext.  Take .inl (CanFail).
          refine .inl ?_
          have h_havoc_vars_defined_k : ∀ x ∈ havoc_vars, (ρ_k.store x).isSome := fun x hx =>
            (hsame_k x).mpr (h_havoc_vars_defined₀ x hx)
          have h_k_eq_ρ₀_outside : ∀ x, x ∉ havoc_vars → ρ_k.store x = ρ₀.store x := by
          ...
          have h_init_m : InitState Expression ρ_k.store m_old_ident v_mk
              (extendStore m_old_ident v_mk ρ_k.store) :=
            m_old_initState ρ_k.store m_old_ident v_mk h_m_old_none_k
          let ρ_k_ext : Imperative.Env Expression :=
            { ρ_k with store := extendStore m_old_ident v_mk ρ_k.store }
          have h_init_step : CoreStar π φ (.stmt init_m_old ρ_k) (.terminal ρ_k_ext) :=
            m_old_init_step π φ hm_eval_k h_init_m hwfvar_k
          ...
          have h_m_old_kext : ρ_k_ext.store m_old_ident = some v_mk := by
            show extendStore m_old_ident v_mk ρ_k.store m_old_ident = some v_mk
            simp [extendStore]
          have h_fvar_eval : ρ_k_ext.eval ρ_k_ext.store
              (HasFvar.mkFvar m_old_ident) = some v_mk :=
            fvar_eval_of_extendStore hwfvar_k h_m_old_kext
          have h_m_old_notin_getVars_m : m_old_ident ∉ HasFvars.getFvars m :=
            m_old_notin_getFvars_m hswf h_loop_reserved h_m_old_isPrefix h_m_old_none₀
          have h_stores_agree_on_m : ∀ x ∈ HasFvars.getFvars m,
              ρ_k.store x = ρ_k_ext.store x :=
            stores_agree_on_m_after_extend ρ_k.store m h_m_old_notin_getVars_m
          have h_m_eval_kext : ρ_k_ext.eval ρ_k_ext.store m = some v_mk :=
            m_eval_after_extend hwfExpr_k h_stores_agree_on_m heval_kext hm_eval_k
          have h_lt_m_kext : ρ_k_ext.eval ρ_k_ext.store
              (HasIntOps.lt m HasInt.zero) = some HasBool.tt :=
            lt_m_zero_eval_after_extend hwfExpr_k h_stores_agree_on_m heval_kext hk_lt_tt
          have h_lt_mold_eq_lt_m_kext :
              ρ_k_ext.eval ρ_k_ext.store
                (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
              ρ_k_ext.eval ρ_k_ext.store (HasIntOps.lt m HasInt.zero) :=
            lt_mold_eq_lt_m_at_ext hwfCong_kext h_fvar_eval h_m_eval_kext
          have h_lt_mold_kext : ρ_k_ext.eval ρ_k_ext.store
              (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
              some HasBool.tt := by rw [h_lt_mold_eq_lt_m_kext]; exact h_lt_m_kext
          have h_not_lt_ff_kext : ρ_k_ext.eval ρ_k_ext.store
              (HasBoolOps.not (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero)) =
              some HasBool.ff :=
          ...
          have h_assert_fail :
            ...
            .step _ _ _ (.step_cmd (EvalCommand.cmd_sem
              (.eval_assert_fail h_not_lt_ff_kext hwfb_kext))) (.refl _)
          let ρ_f : Imperative.Env Expression :=
            { ρ_k_ext with hasFailure := ρ_k_ext.hasFailure || Bool.true }
          have hρf_fail : ρ_f.hasFailure = Bool.true := by
            show ρ_k_ext.hasFailure || Bool.true = Bool.true
            simp
          have h_assert_fail_run : CoreStar π φ
              (.stmts [assert_lb] ρ_k_ext) (.terminal ρ_f) :=
            ReflTrans_Transitive _ _ _ _
              (stmts_cons_step Expression (EvalCommand π φ) (EvalPureFunc φ)
          ...
          exact Transform.canFailBlock_to_canFail_block (EvalCommand π φ) (EvalPureFunc φ)
            coreIsAtAssert s!"{loopElimBlockPrefix}loop_{loop_num}" _ ∅ ρ₀
            h_outer_stmts_canFail
        -- ff branch at ρ_k: build constructive trace.
        -- havoc to ρ_k.
        have h_havoc_vars_defined_k : ∀ x ∈ havoc_vars, (ρ_k.store x).isSome := fun x hx =>
          (hsame_k x).mpr (h_havoc_vars_defined₀ x hx)
        have h_k_eq_ρ₀_outside : ∀ x, x ∉ havoc_vars → ρ_k.store x = ρ₀.store x := by
        ...
        have hk_env_eq : ({ ρ₀ with store := ρ_k.store } : Imperative.Env Expression) = ρ_k := by
          ...
          | mk sk ek fk =>
            ...
            | mk s₀ e₀ f₀ =>
              ...
              simp [heval_k, hnf_k, hnf₀]
        rw [hk_env_eq] at h_havoc_to_k
        -- Assumes block at ρ_k (guard assume + inv assumes).
        have h_assumes_block : CoreStar π φ
            (.stmt (.block arb_assumes_label arb_assumes_body md) ρ_k) (.terminal ρ_k) := by
          have h_assumes_run : CoreStar π φ (.stmts arb_assumes_body ρ_k) (.terminal ρ_k) := by
            simp only [arb_assumes_body, List.cons_append, List.nil_append]
        ...
        have h_lt_m_kext : ρ_k_ext.eval ρ_k_ext.store
            (HasIntOps.lt m HasInt.zero) = some HasBool.ff :=
          lt_m_zero_eval_after_extend hwfExpr_k h_stores_agree_on_m heval_kext hk_meas_ff
        have h_lt_mold_eq_lt_m_kext :
            ρ_k_ext.eval ρ_k_ext.store
              (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
            ρ_k_ext.eval ρ_k_ext.store (HasIntOps.lt m HasInt.zero) :=
        ...
        -- Lift the body exit trace from ρ_k to ρ_k_ext via fresh-var frame.
        have hr_ext : CoreStar π φ
            (extendCfg_fn m_old_ident v_mk (.stmts body ρ_k))
            (extendCfg_fn m_old_ident v_mk (.exiting lbl ρ_inner_k)) :=
          coreStar_frame_fresh_var (π := π) (φ := φ)
            (m := m_old_ident) (v := v_mk) hwf_ext h_body_k_exit
            h_m_old_notin_allVarsSet
            h_no_call_body h_wfExpr_body
        let ρ_inner_kext : Imperative.Env Expression := extendEnv m_old_ident v_mk ρ_inner_k
        have h_body_kext_exit : CoreStar π φ
            (.stmts body ρ_k_ext) (.exiting lbl ρ_inner_kext) := by
          show CoreStar π φ
              (.stmts body (extendEnv m_old_ident v_mk ρ_k))
              (.exiting lbl (extendEnv m_old_ident v_mk ρ_inner_k))
          exact hr_ext
        -- maintain_stmts and assert_decrease are after `body`, but `body` exits
        -- before reaching them — so the exit propagates through these via
        -- stmts_prefix_exiting_append (no need to evaluate them at ρ_inner_kext).
        -- Build the full prefix → exit trace.
        have h_prefix4_term : CoreStar π φ
            (.stmts ([.block havoc_label havoc_stmts ∅,
                      .block arb_assumes_label arb_assumes_body md,
                      init_m_old, assert_lb]) ρ₀)
            (.terminal ρ_k_ext) := by
          ...
          exact ReflTrans_Transitive _ _ _ _ h1
            (ReflTrans_Transitive _ _ _ _ h2
              (ReflTrans_Transitive _ _ _ _ h3 h4))
        -- arb_facts_body exits at ρ_inner_kext.
        have h_arb_body_exit : CoreStar π φ (.stmts arb_facts_body ρ₀)
          ...
          rw [hlist_eq]
          have h_body_then_rest : CoreStar π φ
          ...
          exact ReflTrans_Transitive _ _ _ _
            (stmts_prefix_terminal_append Expression (EvalCommand π φ) (EvalPureFunc φ)
              _ _ ρ₀ ρ_k_ext h_prefix4_term)
            h_body_then_rest
        have h_arb_block_exit : CoreStar π φ
            (.stmt (.block arb_facts_label arb_facts_body ∅) ρ₀)
            (.exiting lbl ({ ρ_inner_kext with store := projectStore ρ₀.store ρ_inner_kext.store, eval := ρ₀.eval } : Imperative.Env Expression)) :=
          block_wrap_exiting_mismatch (EvalCommand π φ) (EvalPureFunc φ) arb_facts_label
            arb_facts_body ∅ lbl ρ₀ ρ_inner_kext hne_arb h_arb_body_exit
        have h_then_stmts_exit : CoreStar π φ
            (.stmts ((.block arb_facts_label arb_facts_body ∅) ::
              ([.block havoc_label havoc_stmts ∅] ++
               [Stmt.cmd (HasPassiveCmds.assume
              ...
              ((.block arb_facts_label arb_facts_body ∅) ::
                ([.block havoc_label havoc_stmts ∅] ++
                 [Stmt.cmd (HasPassiveCmds.assume
                   s!"{loopElimAssumePrefix}{loop_num}_not_guard" (HasBoolOps.not g) md)] ++
                 exit_inv_assumes)) ρ₀))
            (.exiting lbl ({ ρ_inner_kext with store := projectStore ρ₀.store ρ_inner_kext.store, eval := ρ₀.eval } : Imperative.Env Expression)) := by
          have h := ReflTrans_Transitive _ _ _ _
            (block_inner_star Expression (EvalCommand π φ) (EvalPureFunc φ)
              _ _ .none ρ₀.store ρ₀.eval h_then_stmts_exit)
            (.step _ _ _ (.step_block_exit_mismatch (by simp)) (.refl _))
        ...
        have h_proj_kext_eq_proj_k :
          ...
          · have hsome_k : (ρ_k.store x).isSome := (hsame_k x).mpr (by simp [hh])
            ...
            show extendStore m_old_ident v_mk ρ_inner_k.store x = ρ_inner_k.store x
            unfold extendStore
            rcases Decidable.em (x = m_old_ident) with hxm | hxm
            · exfalso; rw [hxm] at hh; rw [h_m_old_none₀] at hh; cases hh
            · rw [if_neg hxm]
        have hρ'_eq : ρ' = ({ ρ_inner_kext with store := projectStore ρ₀.store ρ_inner_kext.store, eval := ρ₀.eval } : Imperative.Env Expression) := by
          rw [heq_ρ', heval_k]
...
private theorem simulation_loop_exit_case
    ...
    (lbl : String) (ρ' : Imperative.Env Expression)
    (hreach : CoreStar π φ
      ...
      | step _ _ _ h1 hrest =>
        ...
        | step_loop_nondet_enter hinvb_ne hinviff_ne =>
          ...
          exact simulation_loop_exit_enter_case π φ hwf_ext reserved h_loop_reserved
            ...
            hswf lbl hnf'' hnf₀ hall_tt hnif hrest
            (fun _ heq => nomatch heq) (fun _ heq _ _ => nomatch heq)
...
private theorem loop_cf_iteration_extract
    ...
    ∃ (ρ_k : Imperative.Env Expression),
      ...
      ((∃ g, guardE = .det g ∧ ∃ me, measure = some me ∧
          ρ_k.eval ρ_k.store (HasIntOps.lt me HasInt.zero) = some HasBool.tt) ∨
       ((∀ g me, guardE = .det g → measure = some me →
          ρ_k.eval ρ_k.store (HasIntOps.lt me HasInt.zero) = some HasBool.ff) ∧
      ...
      -- `ρ_k.eval ρ_k.store me = some v`.  This is fed by
      ...
      ∀ (cfg : CC) (hfail : cfg.getEnv.hasFailure = Bool.true)
        ...
        ∃ (ρ_k : Imperative.Env Expression),
          ...
          (∀ g me, guardE = .det g → measure = some me →
            ∃ v, ρ_k.eval ρ_k.store me = some v ∧ HasInt.isIntTy v ∧
          HasFvars.getFvars (P := Expression) v = []) by
    have hatOurLoop_init : Config.atOurLoop (.loop guardE measure inv body md) ρ₀
        (.stmt (.loop guardE measure inv body md) ρ₀ : CC) := ⟨rfl, rfl⟩
    obtain ⟨ρ_k, h1, h2, h3, h4, h5, h6, h7, h8⟩ :=
      this hstarT.len ρ₀ rfl hnf₀ hall_tt (fun _ => Iff.rfl) _ (.refl _) hatOurLoop_init cfg hfail
  ...
  induction k with
  | zero =>
    intro ρ _ hnf_ρ _ _ _ _ _ cfg hfail hT hlen
    match hT with
    | .refl _ => exact absurd hfail (by simp [Config.getEnv, hnf_ρ])
    | .step _ _ _ _ _ => simp [ReflTransT.len] at hlen
  | succ k' ih =>
    intro ρ heval_ρ hnf_ρ hall_tt_ρ hsame_ρ cfg_init hreach_init hatOurLoop_init cfg hfail hT hlen
    have hmeas_eval : ∀ me, measure = some me →
        ∃ v, ρ.eval ρ.store me = some v ∧ HasInt.isIntTy v ∧
          HasFvars.getFvars (P := Expression) v = [] := by
      intro me hme
      -- Use the syntactic-field helper, transported from ρ₀ to ρ via
      -- ρ.eval = ρ₀.eval and store-domain agreement on m's fvars.
      have hmIsIntTy : HasInt.isIntTy me := by
        have hm := hswf.loopMeasuresAreIntTypes
        show HasInt.isIntTy me
        cases measure with
    ...
    | .step _ _ _ hstep1 hrest =>
      ...
      | @step_loop_enter _ g_var _ _ _ _ hasInvFailure_var
        ...
        have hif_false : hasInvFailure_var = Bool.false := Bool.eq_false_iff.mpr hnot_true
        subst hif_false
        -- Case-split on `lt me 0` using `wfInt.ltReduces` whenever `measure = some me`.
        -- This replaces the old `hmeas_b`/`hmeas_iff` premises.
        by_cases h_meas_lt_tt :
            ∃ me, measure = some me ∧
              ρ.eval ρ.store (HasIntOps.lt me HasInt.zero) = some HasBool.tt
        · -- Some measure expr is negative at ρ → take case_c (measure-failure witness).
          obtain ⟨me, hme, htt⟩ := h_meas_lt_tt
        ...
        · have hrest_len : hrest.len ≤ k' := by
          ...
          have hρ_eq : ({ ρ with hasFailure := ρ.hasFailure || Bool.false } : Imperative.Env Expression) = ρ := by
            cases ρ with
            | mk store eval hf => simp at hnf_ρ; subst hnf_ρ; rfl
          -- Negate: for all me with measure = some me, lt me 0 ≠ tt.
          have hmeas_ne_tt : ∀ me, measure = some me →
              ρ.eval ρ.store (HasIntOps.lt me HasInt.zero) ≠ some HasBool.tt := by
            intro me hme htt
            exact h_meas_lt_tt ⟨me, hme, htt⟩
          -- Use wfInt.ltReduces to derive `lt me 0 = tt ∨ lt me 0 = ff`, then take ff.
          have hmeas_ok_ρ_raw : ∀ me, measure = some me →
              ρ.eval ρ.store (HasIntOps.lt me HasInt.zero) = some HasBool.ff := by
            intro me hme
            obtain ⟨v, hv_eval, hv_intTy, hv_noFvars⟩ := hmeas_eval me hme
            have hwfVal_ρ : WellFormedSemanticEvalVal ρ.eval := heval_ρ ▸ hswf.wfVal
            have hzero_val : HasVal.value (HasInt.zero : Expression.Expr) :=
              HasInt.numeralIsValue _ HasInt.zeroIsNumeral
            have hzero_eval : ρ.eval ρ.store (HasInt.zero : Expression.Expr) = some HasInt.zero :=
              hwfVal_ρ.2 (HasInt.zero : Expression.Expr) ρ.store hzero_val
            have hzero_intTy : HasInt.isIntTy (HasInt.zero : Expression.Expr) :=
              HasInt.numeralIsIntTy _ HasInt.zeroIsNumeral
            have hzero_noFvars : HasFvars.getFvars (P := Expression) (HasInt.zero : Expression.Expr) = [] :=
              HasInt.numeralHasNoFvars _ HasInt.zeroIsNumeral
            rcases hwfInt_ρ.ltReduces ρ.store me HasInt.zero v HasInt.zero
              hv_eval hv_intTy hv_noFvars hzero_eval hzero_intTy hzero_noFvars with htt | hff
            · exact absurd htt (hmeas_ne_tt me hme)
            · exact hff
          ...
          | .inr ⟨ρ_mid, h_block_term, h_tail_fail, hlen_sum⟩ =>
            ...
            · have hnf_inner' : ρ_inner.hasFailure = Bool.false := by
              ...
              · have h_body_term_p : CoreStar π φ (.stmts body ρ) (.terminal ρ_inner) :=
                ...
                -- Build body-iteration trace:
                -- (.stmt loop ρ) →* (.stmts [loop] ρ_mid)
                have h_inv_b : ∀ le ∈ inv,
                    ρ.eval ρ.store le.2 = .some HasBool.tt ∨
                    ρ.eval ρ.store le.2 = .some HasBool.ff :=
                  fun le hle => Or.inl (hall_tt_ρ le hle)
              ...
              · -- Some invariant is ff at ρ_inner → ρ is witness
                ...
                have hinvb_mid : ∀ le ∈ inv,
                  ...
                  | step _ _ _ hstep _ =>
                    ...
                    | step_loop_enter _ hinvb _ _ => exact hinvb
      ...
      | step_loop_nondet_enter _ hinv_iff =>
        ...
        | .inl ⟨cfg', h_block_fail, hf_block, _⟩ =>
          ...
          have hmeas_vac : ∀ g' me, (.nondet : ExprOrNondet Expression) = .det g' →
              measure = some me →
              ρ.eval ρ.store (HasIntOps.lt me HasInt.zero) = some HasBool.ff :=
        ...
        | .inr ⟨ρ_mid, h_block_term, h_tail_fail, hlen_sum⟩ =>
          ...
          · have hnf_inner' : ρ_inner.hasFailure = Bool.false := by
            ...
            · have h_body_term_p : CoreStar π φ (.stmts body ρ) (.terminal ρ_inner) :=
              ...
              have hlen_bound : h_loop_mid.len ≤ k' := by
                have := hrest_len; omega
              -- Build body iter trace for nondet
              have h_inv_b_n : ∀ le ∈ inv,
                  ρ.eval ρ.store le.2 = .some HasBool.tt ∨
                  ρ.eval ρ.store le.2 = .some HasBool.ff :=
                fun le hle => Or.inl (hall_tt_ρ le hle)
...
private theorem simulation_loop_cf_all_tt_det_nomeasure
  ...
  intro loop_num havoc_vars havoc_stmts havoc_label arb_assumes_label
...
private theorem simulation_loop_cf_all_tt
  ...
  suffices hthen_cf : CanFailBlock (EvalCommand π φ) (EvalPureFunc φ) then_branch ρ₀ by
    ...
    | det g =>
      ...
      have hguard_tt : ρ₀.eval ρ₀.store g = some HasBool.tt := by
        ...
        | step _ _ _ h1 _ =>
          cases h1 with
          | step_loop_enter hgt _ _ _ => exact hgt
          | step_loop_exit _ _ _ _ =>
  ...
  | ok p =>
    ...
    case h_2.isFalse.isTrue =>
      -- Det guard + measure: build CanFail trace.  Uses `loop_cf_iteration_extract`
      -- to find a failing iteration ρ_k.  The new disjunction in the iteration
      -- extract has case_c (lt m 0 = tt → assert_lb fails) and the measure_ok
      -- branch (lt m 0 = ff → assert_lb passes; failure follows in body or maintain).
      rename_i _hcheck g0 _meas m h_m_old_fresh _h_some
      have htb := htgt_eq
      ...
      obtain ⟨v_m, hm_eval_k, hv_m_intTy, hv_m_noFvars⟩ := hk_meas_eval g0 m rfl rfl
      have hv_m_numeral := hv_m_intTy
      ...
      have h_init_m : InitState Expression ρ_k.store m_old_ident v_m
          (extendStore m_old_ident v_m ρ_k.store) :=
        m_old_initState ρ_k.store m_old_ident v_m h_m_old_none_k
      let ρ_k_ext : Imperative.Env Expression := { ρ_k with store := extendStore m_old_ident v_m ρ_k.store }
      have h_init_step : CoreStar π φ (.stmt init_m_old ρ_k) (.terminal ρ_k_ext) :=
        m_old_init_step π φ hm_eval_k h_init_m hwfvar_k
      have heval_kext : ρ_k_ext.eval = ρ_k.eval := rfl
      have hwfb_kext : WellFormedSemanticEvalBool ρ_k_ext.eval := by
      ...
      have h_m_old_kext : ρ_k_ext.store m_old_ident = some v_m := by
        ...
        simp [extendStore]
      have h_fvar_eval : ρ_k_ext.eval ρ_k_ext.store (HasFvar.mkFvar m_old_ident) = some v_m :=
        fvar_eval_of_extendStore hwfvar_k h_m_old_kext
      ...
      have hwfCong_kext : WellFormedCoreEvalCong ρ_k_ext.eval := by
        rw [heval_kext, heval_k]; exact hswf.evalCong
      have h_m_eval_kext : ρ_k_ext.eval ρ_k_ext.store m = some v_m :=
        m_eval_after_extend hwfExpr_k h_stores_agree_on_m heval_kext hm_eval_k
      ...
      · -- case_c: lt me 0 = tt at ρ_k.  Then assert_lb fails since
        ...
        have h_lt_m_kext : ρ_k_ext.eval ρ_k_ext.store
            (HasIntOps.lt m HasInt.zero) = some HasBool.tt :=
          lt_m_zero_eval_after_extend hwfExpr_k h_stores_agree_on_m heval_kext hlt_tt
        have h_lt_mold_eq_lt_m_kext :
            ρ_k_ext.eval ρ_k_ext.store
              (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
            ρ_k_ext.eval ρ_k_ext.store (HasIntOps.lt m HasInt.zero) :=
        ...
        let ρ_f : Imperative.Env Expression := { ρ_k_ext with hasFailure := ρ_k_ext.hasFailure || Bool.true }
        have hρf_fail : ρ_f.hasFailure = Bool.true := by
          show ρ_k_ext.hasFailure || Bool.true = Bool.true
          simp
        ...
        exact ReflTrans_Transitive _ _ _ _ h1
          (ReflTrans_Transitive _ _ _ _ h2
            (ReflTrans_Transitive _ _ _ _ h3 h4))
      · -- measure_ok branch: lt m 0 = ff at ρ_k.  Failure is in body or maintain.
        have hmeas_ff_k : ρ_k.eval ρ_k.store (HasIntOps.lt m HasInt.zero) = some HasBool.ff :=
          hmeas_ok g0 m rfl rfl
        have h_m_old_notin_mod : m_old_ident ∉ Block.modifiedVars body := by
          intro hmem
          apply h_m_old_fresh
          show m_old_ident ∈ Block.modifiedVars body ++ Block.definedVars body true ++
        ...
        have h_m_old_notin_defF : m_old_ident ∉ Block.definedVars body Bool.false := by
          intro hmem
          have hdef_loop : m_old_ident ∈ Stmt.definedVars
              (Stmt.loop (.det g0) (some m) inv body md) Bool.false := by
            simp [Stmt.definedVars]; exact hmem
          have := hswf.definedVarsNotReserved m_old_ident hdef_loop
            loopElimReservedPrefix h_loop_reserved h_m_old_isPrefix
          exact absurd this (by simp)
        ...
        have h_lt_m_kext : ρ_k_ext.eval ρ_k_ext.store
            (HasIntOps.lt m HasInt.zero) = some HasBool.ff :=
          lt_m_zero_eval_after_extend hwfExpr_k h_stores_agree_on_m heval_kext hmeas_ff_k
        have h_lt_mold_eq_lt_m_kext :
            ρ_k_ext.eval ρ_k_ext.store
              (HasIntOps.lt (HasFvar.mkFvar m_old_ident) HasInt.zero) =
            ρ_k_ext.eval ρ_k_ext.store (HasIntOps.lt m HasInt.zero) :=
        ...
        · obtain ⟨ρ_inner, h_body_term, hnf_inner, hinv_bool_inner, hsome_ff⟩ := hbody_term
          ...
          have h_m_old_notin_getVars_inv : ∀ le ∈ inv,
            ...
            intro le hle hmem
            have hgv : m_old_ident ∈ Stmt.getVars
                (Stmt.loop (.det g0) (some m) inv body md) := by
              unfold Stmt.getVars
              apply List.mem_append.mpr; left
              apply List.mem_append.mpr; right
...
private theorem stmt_corr_step
  ...
  case term_branch =>
    ...
    | .block l bss md =>
      ...
      have hsim_bss := ih.2.1 σ bss hsz_bss (stmtOk_block_inner hok) hno_call_bss
        ρ₀ declaredFuncs (InitEnvWF.toBlock_block hswf)
      cases hreach with
      | step _ _ _ h1 r1 =>
        cases h1 with
    ...
    | .ite c tss ess md =>
      ...
      | step _ _ _ h1 r1 =>
        cases h1 with
        | step_ite_true hcond hwfb' =>
          have hsim_tss := ih.2.1 σ tss hsz_tss (stmtOk_ite_left hok) hno_call_tss.1
            ρ₀
            declaredFuncs (InitEnvWF.toBlock_ite_left hswf
              (fun g heq => by cases heq; exact hcond))
        ...
        | step_ite_false hcond hwfb' =>
          have hsim_ess := ih.2.1 (blockPostState σ tss) ess hsz_ess
            (stmtOk_ite_right hok) hno_call_tss.2 ρ₀ declaredFuncs
            (InitEnvWF.toBlock_ite_right hswf
              (fun g heq => by cases heq; exact hcond))
          ...
          | step _ _ _ h1 hrest =>
            ...
            | step_loop_enter hgt hinvb hinviff hwfbe =>
              ...
              exact simulation_loop_term_enter_case π φ hwf_ext reserved h_loop_reserved
                ...
                hswf (.step _ _ _ (.step_loop_enter hgt hinvb hinviff hwfbe) hrest)
    ...
    | s :: ss =>
      ...
      | step _ _ _ h1 r1 =>
        ...
        | step_stmts_cons =>
          ...
          have hsim_s := ih.1 σ s hsz_s (blockOk_cons_left hok) hno_call_split.1
            ρ₀ declaredFuncs hswf_s
          ...
          exact block_corr_cons_term_head_term π φ hterm_s hreach_ss
            (fun h => hsim_s.1 ρ₁ h)
            (fun h => ih.2.2.1 σ s hsz_s (blockOk_cons_left hok) hno_call_split.1
              ρ₀ declaredFuncs hswf_s h)
            hsim_ss.1
  case bexit =>
    intro lbl ρ' hreach
    match bss with
...
private theorem stmt_cf_step
  ...
  | .block l bss md =>
    ...
    have hcf_inner : CanFailBlock (EvalCommand π φ) (EvalPureFunc φ) bss ρ₀ → CanFailBlock (EvalCommand π φ) (EvalPureFunc φ) (blockResult σ bss) ρ₀ :=
      fun h => ih.2.2.2 σ bss hsz_bss (stmtOk_block_inner hok) hno_call_bss
        ρ₀ declaredFuncs (InitEnvWF.toBlock_block hswf) h
    cases hreach with
  ...
  | .ite c tss ess md =>
    ...
    | step _ _ _ h1 r1 => cases h1 with
      | step_ite_true hcond hwfb' =>
        have hcf_tss : CanFailBlock (EvalCommand π φ) (EvalPureFunc φ) tss ρ₀ →
            CanFailBlock (EvalCommand π φ) (EvalPureFunc φ) (blockResult σ tss) ρ₀ :=
          fun h => ih.2.2.2 σ tss hsz_tss (stmtOk_ite_left hok) hno_call_split.1
            ρ₀ declaredFuncs
            (InitEnvWF.toBlock_ite_left hswf
              (fun g heq => by cases heq; exact hcond)) h
      ...
      | step_ite_false hcond hwfb' =>
        have hcf_ess : CanFailBlock (EvalCommand π φ) (EvalPureFunc φ) ess ρ₀ →
          ...
          fun h => ih.2.2.2 (blockPostState σ tss) ess hsz_ess
            (stmtOk_ite_right hok) hno_call_split.2 ρ₀ declaredFuncs
            (InitEnvWF.toBlock_ite_right hswf
              (fun g heq => by cases heq; exact hcond)) h
      ...
      | step_stmts_cons =>
        ...
        | .inl ⟨cfg', hf', hstar'⟩ =>
          have ⟨kcfg, hkf, hkstar⟩ := ih.2.2.1 σ s hsz_s
            (blockOk_cons_left hok) hno_call_split.1 ρ₀ declaredFuncs
            (BlockInitEnvWF.toStmt_head hswf)
            ⟨cfg', hf', hstar'⟩
...
private theorem mem_getOps_not_subset
  ...
  match g, hm with
  | .const _ (.boolConst Bool.true), hm =>
      simp [Lambda.LExpr.getOps] at hm
  | .const _ (.boolConst Bool.false), hm =>
      simp [Lambda.LExpr.getOps] at hm
  | .const _ (.intConst _), hm =>
  ...
  repeat (first
    | (cases h
       ...
       simp only [HasVarsImp.definedVars, HasVarsImp.modifiedVars,
         ...
         HasPassiveCmds.assert, HasPassiveCmds.assume,
         HasHavoc.havoc,
         Command.definedVars, Command.modifiedVars,
         Cmd.definedVars, Cmd.modifiedVars,
         ExprOrNondet.getVars,
...
private theorem mem_touchedVars_stmtResult_loop
  ...
  | .ok (b, s') =>
    ...
    have body_gv_to_src : ∀ m, m ∈ Block.getVars body →
      ...
      show m ∈ guard.getVars ++ (measure.map HasFvars.getFvars).getD [] ++
                (inv.flatMap fun lp => HasFvars.getFvars lp.2) ++
                Block.getVars body
    ...
    have measure_to_src : ∀ m, m ∈ (measure.map HasFvars.getFvars).getD [] →
        m ∈ Stmt.touchedVars (.loop guard measure inv body md) := by
      intro m hm
      apply List.mem_append_right
      show m ∈ guard.getVars ++ (measure.map HasFvars.getFvars).getD [] ++
    ...
    have measure_some_to_src : ∀ (m_e : Expression.Expr) (m : Expression.Ident),
        measure = some m_e → m ∈ HasFvars.getFvars m_e →
        m ∈ Stmt.touchedVars (.loop guard measure inv body md) := by
      intro m_e m heq hm
            ...
            rw [defUseWellFormed_block_congr
                  ...
                  (fun n => by
                    ...
                    simp only [Block.definedVars, Stmt.definedVars,
                      ↓reduceIte, h_pre_def_true_eq, List.append_nil,
...
private theorem defUseWellFormed_singleton_assume
  ...
  · unfold Stmt.defUseWellFormed
    simp only [HasFvars.getFvars, HasVarsImp.modifiedVars,
      ...
      HasVarsPure.getVars, List.all_nil, Bool.and_true,
      Bool.and_eq_true]
    refine ⟨?_, ?_⟩
    ...
    case h_2.isFalse.isTrue =>
      ...
      apply defUseWellFormed_loop_output_detSome _ inv body md outer declared
        ...
        (fun n hn => h_meas_all n
          (by show n ∈ (Option.getD ((some m0).map HasFvars.getFvars) [])
              simpa using hn))
        (fun n hn => h_meas_ops n
          (by show n ∈ (Option.getD ((some m0).map (HasOps.getOps (P := Expression))) [])
              simpa using hn))
...
/-! ## `measuresAreIntTypes` is preserved by `stmtResult` / `blockResult`

The transform copies loop bodies verbatim into its output, so the syntactic
"every loop's measure is int-typed" property survives the rewrite.  This is
the structural lemma that discharges the output's `loopMeasuresAreIntTypes`
field at the top-level theorem. -/

/-- Generic `mapIdx`-of-`Statement` form, for entries already at statement type. -/
private theorem Statements.measuresAreIntTypes_mapIdx_stmt
    {α : Type _} (xs : List α)
    (f : Nat → α → Statement)
    (hf : ∀ i x, Statement.measuresAreIntTypes (f i x)) :
    Statements.measuresAreIntTypes (xs.mapIdx f) := by
  induction xs generalizing f with
  | nil => exact True.intro
  | cons hd tl ih =>
    rw [List.mapIdx_cons]
    refine ⟨hf 0 hd, ?_⟩
    exact ih (f := fun i x => f (i + 1) x) (fun i x => hf (i + 1) x)

private theorem Statements.measuresAreIntTypes_map_cmd
    {α : Type _} (xs : List α) (f : α → Command) :
    Statements.measuresAreIntTypes (xs.map (fun x => Stmt.cmd (f x))) := by
  induction xs with
  | nil => exact True.intro
  | cons hd tl ih =>
    refine ⟨True.intro, ?_⟩
...
private theorem Statements.measuresAreIntTypes_append (xs ys : Statements) :
    Statements.measuresAreIntTypes (xs ++ ys) ↔
      (Statements.measuresAreIntTypes xs ∧ Statements.measuresAreIntTypes ys) := by
  induction xs with
  | nil =>
    simp [Statements.measuresAreIntTypes]
  | cons hd tl ih =>
    show Statements.measuresAreIntTypes (hd :: (tl ++ ys)) ↔ _
    constructor
    · rintro ⟨hhd, htlys⟩
      have ⟨htl, hys⟩ := ih.mp htlys
      exact ⟨⟨hhd, htl⟩, hys⟩
    · rintro ⟨⟨hhd, htl⟩, hys⟩
      exact ⟨hhd, ih.mpr ⟨htl, hys⟩⟩

/-- The loop-elim output preserves int-typed measures: the transform inlines
    the source body verbatim into the output, so any `.loop` left in the
    output (from nested loops in the source body) still has its original
    int-typed measure. -/
private theorem loop_output_measures_intTy (σ : LoopElimState)
    (guard : ExprOrNondet Expression)
    (measure : Option Expression.Expr)
    (inv : List (String × Expression.Expr))
    (body : Statements) (md : MetaData Expression)
    (hok : stmtOk σ (.loop guard measure inv body md))
    (h_body_intTy : Statements.measuresAreIntTypes body) :
    Statement.measuresAreIntTypes
      (stmtResult σ (.loop guard measure inv body md)) := by
  show Statement.measuresAreIntTypes
    (match (stmtRun σ (.loop guard measure inv body md)).fst with
      | .ok (_, s') => s' | .error _ => default)
  have hok' := hok
  unfold stmtOk at hok'
  ...
  | .ok (b, s') =>
    ...
    dsimp only [stmtRun, StateT.run, ExceptT.run, Stmt.removeLoopsM,
      removeLoopsLoopCase, buildLoopOutput,
      buildTerminationStmtsSome,
      hasLabelConflict, invSuffix, measureOldIdent,
      bind, pure, ExceptT.bind, ExceptT.pure, ExceptT.mk, ExceptT.bindCont,
      ExceptT.lift, StateT.bind, StateT.pure,
    ...
    all_goals (first | contradiction | (
      ...
      all_goals (first
        ...
        | (obtain ⟨_, rfl⟩ := h))))
    all_goals (
      simp only [Statement.measuresAreIntTypes, Statements.measuresAreIntTypes,
        Statements.measuresAreIntTypes_append]
      repeat' (first
        | trivial
        | (refine ⟨?_, ?_⟩)
        | (apply Statements.measuresAreIntTypes_mapIdx_stmt; intros; trivial)
        | (apply Statements.measuresAreIntTypes_map_cmd)
        | (exact h_body_intTy)))

mutual
private theorem Statement.measuresAreIntTypes_stmtResult
    (σ : LoopElimState) (s : Statement) (hok : stmtOk σ s)
    (hint : Statement.measuresAreIntTypes s) :
    Statement.measuresAreIntTypes (stmtResult σ s) := by
  match s with
  | .cmd c =>
    rw [stmtResult_cmd]; exact True.intro
  | .exit l md =>
    rw [stmtResult_exit]; exact True.intro
  | .funcDecl d md =>
    rw [stmtResult_funcDecl]; exact True.intro
  | .typeDecl tc md =>
    rw [stmtResult_typeDecl]; exact True.intro
  | .block l bss md =>
    rw [stmtResult_block (hok := hok)]
    have hint_bss : Statements.measuresAreIntTypes bss := hint
    have ih := Statements.measuresAreIntTypes_blockResult σ bss (stmtOk_block_inner hok) hint_bss
    show Statements.measuresAreIntTypes (blockResult σ bss)
    exact ih
  | .ite c tss ess md =>
    rw [stmtResult_ite (hok := hok)]
    have hint_split : Statements.measuresAreIntTypes tss ∧
                      Statements.measuresAreIntTypes ess := hint
    have ih_t := Statements.measuresAreIntTypes_blockResult σ tss (stmtOk_ite_left hok) hint_split.1
    have ih_e := Statements.measuresAreIntTypes_blockResult (blockPostState σ tss) ess
                   (stmtOk_ite_right hok) hint_split.2
    exact ⟨ih_t, ih_e⟩
  | .loop guard measure inv body md =>
    -- Body's measures are int-typed (recursively); the transform copies the
    -- body verbatim into the output, so the output retains int-typedness.
    have h_body_intTy : Statements.measuresAreIntTypes body := by
      cases measure with
      | none => exact hint
      | some m => exact hint.2
    show Statement.measuresAreIntTypes (stmtResult σ (.loop guard measure inv body md))
    exact loop_output_measures_intTy σ guard measure inv body md hok h_body_intTy
  termination_by Stmt.sizeOf s

private theorem Statements.measuresAreIntTypes_blockResult
    (σ : LoopElimState) (ss : Statements) (hok : blockOk σ ss)
    (hint : Statements.measuresAreIntTypes ss) :
    Statements.measuresAreIntTypes (blockResult σ ss) := by
  match ss with
  | [] => rw [blockResult_nil]; exact True.intro
  | s :: rest =>
    rw [blockResult_cons (hok := hok)]
    have hint_split : Statement.measuresAreIntTypes s ∧
                      Statements.measuresAreIntTypes rest := hint
    have ih_s := Statement.measuresAreIntTypes_stmtResult σ s (blockOk_cons_left hok) hint_split.1
    have ih_r := Statements.measuresAreIntTypes_blockResult (stmtPostState σ s) rest
                   (blockOk_cons_right hok) hint_split.2
    exact ⟨ih_s, ih_r⟩
...
theorem loopElim_overapproximatesAggressive
  ...
  have h_loop_reserved' : loopElimReservedPrefix ∈ reserved := h_loop_reserved _ rfl
  have hpre_noCall : Statement.noCall st = Bool.true := hpre
  have hsim := (simulation π φ hwf_ext (Stmt.sizeOf st) reserved h_loop_reserved').1
    σ st (Nat.le_refl _) hok hpre_noCall ρ₀ declaredFuncs hswf'
  refine ⟨?_, ?_, ?_, ?_⟩
  ...
  · intro ⟨cfg, hfail, hreach⟩
    ...
    · exact (canfail_simulation π φ hwf_ext (Stmt.sizeOf st) reserved h_loop_reserved').1
        σ st (Nat.le_refl _) hok hpre_noCall ρ₀ declaredFuncs hswf' ⟨cfg, hfail, hreach⟩
  · -- Show `InitEnvWF reserved (stmtResult σ st) ρ₀` from `InitEnvWF reserved st ρ₀`.
    ...
    refine
      { readWritesDefined := ?readWritesDefined,
        ...
        loopMeasuresAreIntTypes := by
          -- The transform copies loop bodies verbatim into its output, so
          -- if every loop in `st` has an int-typed measure, every loop in
          -- `stmtResult σ st` also does.
          exact Statement.measuresAreIntTypes_stmtResult σ st hok
            hswf.loopMeasuresAreIntTypes }
    ...
    case definedVarsNotReserved =>
      ...
      · -- n has `loopElimReservedPrefix` as prefix.  Suppose for contradiction
        ...
        intro h_p_prefix
        -- `h_pd` gives `PrefixDisjoint loopElimReservedPrefix (reserved.erase loopElimReservedPrefix)`.
        have h_pd_loop := h_pd loopElimReservedPrefix rfl
        have ⟨h_pd_left, h_pd_right⟩ := h_pd_loop p hp_mem
