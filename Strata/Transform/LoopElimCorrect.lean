...
private theorem pre_termination_stmts_terminal
  ...
  have hlt_ff : ρ₁.eval ρ₁.store (HasIntOrder.lt m_old_expr HasIntOrder.zero) = .some HasBool.ff := by
    ...
    have hval_op : HasVal.value (P := Expression) Core.intLtOp :=
      sorry /- Value.op : intLtOp unfolds to .op, needs @[expose] on intLtOp + opExpr -/
...
private theorem evalCmd_store_frame
  ...
  | eval_reinit _ hup _ =>
    exact updateState_store_frame hup
      (fun h => hdef (by simp [Cmd.definedVars]; exact h.symm))
  | eval_reinit_unconstrained hup _ =>
    exact updateState_store_frame hup
      (fun h => hdef (by simp [Cmd.definedVars]; exact h.symm))
  | eval_set _ hup _ =>
    exact updateState_store_frame hup
...
private def Config.touchedVarsSet : CC → List Expression.Ident
  ...
  | .terminal _ => []
  | .exiting _ _ => []
  | .block _ inner => Config.touchedVarsSet inner
...
private theorem step_preserves_store_outside_touchedVars
    (c₁ c₂ : CC)
    (hstep : StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ) c₁ c₂)
    (x : Expression.Ident)
  ...
  | step_block_body _ ih =>
    ...
    exact ih hx
  | step_block_done => rfl
  | step_block_exit_none => rfl
  | step_block_exit_match _ => rfl
  | step_block_exit_mismatch _ => rfl
...
/-! ### Multi-step store preservation -/

private theorem star_preserves_store_outside_touchedVars
    (c₁ c₂ : CC)
    (hstar : CoreStar π φ c₁ c₂)
    (x : Expression.Ident)
    (hx : x ∉ Config.touchedVarsSet c₁) :
  ...
  | step _ mid _ hstep _ ih =>
    rw [ih (show x ∉ Config.touchedVarsSet mid from
      fun hmem => hx (step_touchedVars_subset π φ _ _ hstep x hmem))]
    exact step_preserves_store_outside_touchedVars π φ _ _ hstep x hx

/-- Block execution preserves the store outside `Block.modifiedVars`.
...
private theorem block_execution_store_agreement
    ...
    (hnofd : Block.noFuncDecl ss = Bool.true) :
    ∀ x, x ∉ Block.modifiedVars ss → x ∉ Block.definedVars ss →
      ρ'.store x = ρ₀.store x := by
  intro x hmod hdef
  exact star_preserves_store_outside_touchedVars π φ _ _ hstar x
    (show x ∉ Config.touchedVarsSet (.stmts ss ρ₀) from by
      simp only [Config.touchedVarsSet, List.mem_append]
      exact fun h => h.elim hmod hdef)

/-- If body is executed across multiple loop iterations (from `ρ₀` to `ρ_last`
    via repeated body execution), variables outside `modifiedOrDefinedVars body`
    (`modifiedVars ∪ definedVars`) are unchanged.

    Currently takes the raw (stronger) store agreement as a hypothesis.
    The raw hypothesis `x ∉ modifiedVars → x ∉ definedVars → agree` is
    derivable from `block_execution_store_agreement` applied transitively
    across iterations.  This theorem then weakens the condition to the filter
    `modifiedVars \ definedVars`, which suffices because variables in both
    `modifiedVars` and `definedVars` cannot be re-init'd when `hswf_body`
    ensures they are already `isSome`. -/
private theorem loop_iterations_store_agreement
    (body : Statements) (ρ₀ ρ_last : Env Expression)
    -- Raw store agreement: preserved outside modifiedOrDefinedVars
    (hagree_raw : ∀ x, x ∉ Block.modifiedVars body → x ∉ Block.definedVars body →
      ρ_last.store x = ρ₀.store x)
    -- All modifiedOrDefinedVars are defined at ρ₀ (ensures init can't fire for definedVars)
    (hswf_body : ∀ n ∈ Block.modifiedOrDefinedVars body, (ρ₀.store n).isSome) :
    ∀ x, x ∉ (Block.modifiedVars body).filter (fun v => v ∉ Block.definedVars body) →
      ρ_last.store x = ρ₀.store x := by
  intro x hx
  simp only [List.mem_filter, not_and, Decidable.not_not] at hx
  -- Either x ∉ modifiedVars, or x ∈ modifiedVars ∧ x ∈ definedVars
  by_cases hmod : x ∈ Block.modifiedVars body
  · -- x ∈ modifiedVars body, so hx gives x ∈ definedVars body
    -- Under hswf_body, init can't fire for x (already isSome), so x
    -- is only modified by set. This gap requires a deeper semantic argument.
    sorry
  · -- x ∉ modifiedVars body: check definedVars
    by_cases hdef : x ∈ Block.definedVars body
    · -- x ∈ definedVars but ∉ modifiedVars: init only, no set.
      -- Under hswf_body, init can't fire (already isSome), so unchanged.
      sorry
    · exact hagree_raw x hmod hdef
...
/-- If `.block .none inner` reaches `.exiting (some l) ρ'` in `ReflTransT`,
    the inner reaches `.exiting (some l) ρ'` with strictly shorter trace. -/
private theorem blockT_none_reaches_exiting_some
    {inner : CC} {l : String} {ρ' : Env Expression}
    (hstar : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
      (.block .none inner) (.exiting (some l) ρ')) :
    ∃ (h : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
          inner (.exiting (some l) ρ')), h.len < hstar.len := by
  suffices ∀ src tgt (hstar_g : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ)) src tgt),
      ∀ inner (l : String) ρ', src = .block (none : Option String) inner → tgt = .exiting (some l) ρ' →
      ∃ (h : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
            inner (.exiting (some l) ρ')), h.len < hstar_g.len from
    this _ _ hstar _ _ _ rfl rfl
  intro src tgt hstar_g
  induction hstar_g with
  | refl => intro _ _ _ hsrc htgt; subst hsrc; cases htgt
  | step _ mid _ hstep hrest ih =>
    intro inner l ρ' hsrc htgt; subst hsrc
    cases hstep with
    | step_block_body h =>
      have ⟨h_inner, hlen⟩ := ih _ _ _ rfl htgt
    ...
    | step_block_exit_mismatch hne =>
      subst htgt
      match hrest with
      | .refl _ => exact ⟨.refl _, by simp [ReflTransT.len]⟩
      | .step _ _ _ h _ => cases h
...
private theorem block_none_reaches_exiting_some
    {inner : CC} {lbl : Option String} {ρ' : Env Expression}
    (hstar : CoreStar π φ (.block .none inner) (.exiting lbl ρ')) :
    ∃ l : String, lbl = some l ∧ CoreStar π φ inner (.exiting (some l) ρ') := by
  suffices ∀ src tgt, CoreStar π φ src tgt →
      ∀ inner lbl ρ', src = .block (none : Option String) inner → tgt = .exiting lbl ρ' →
      ∃ l : String, lbl = some l ∧ CoreStar π φ inner (.exiting (some l) ρ') from
    this _ _ hstar _ _ _ rfl rfl
  intro src tgt hstar_g
  induction hstar_g with
  | refl => intro _ _ _ hsrc htgt; subst hsrc; cases htgt
  | step _ mid _ hstep hrest ih =>
    ...
    cases hstep with
    | step_block_body h =>
      have ⟨l, heq, hexit⟩ := ih _ _ _ rfl htgt
      exact ⟨l, heq, .step _ _ _ h hexit⟩
    | step_block_done =>
      subst htgt; cases hrest with | step _ _ _ h _ => cases h
    | step_block_exit_none =>
      subst htgt; cases hrest with | step _ _ _ h _ => cases h
    ...
    | .step _ _ _ (@StepStmt.step_loop_enter _ _ _ _ _ _ _ _ _ _ _ _ _
      ...
      | .inl ⟨hrestInner, hlen_inner⟩ =>
        ...
        have ⟨ρ₁, hbody, hloop_stmtT, hlen_dec⟩ :=
          stmtsT_append_terminal_noCov π φ body (.loop (.det g) (measure.map HasIntOrder.decr) inv body md) _ ρ'
            hrestInner
        ...
        | .step _ _ _ (@StepStmt.step_loop_exit _ _ _ _ _ _ _ _ _ _ _ _ _
          ...
          exact .inl ⟨ρ₀, hguard_tt₀, hall_tt₀, hwfb₀, hwfv₀, hwfvar₀, hnf₀,
            ...
            fun me hme =>
              -- Requires WellFormedCoreEvalCong ρ₀.eval (thread from top-level) + measure_lt_congr
              sorry⟩
        | .step _ _ _ (@StepStmt.step_loop_enter _ _ _ _ _ _ _ _ _ _ _ _ _
          ...
          have hnif₁ : hasInvFailure₁ = Bool.false :=
            loop_step_hasInvFailure_false_of_or hnf_start₁
          have hall_tt₁ := all_inv_tt_of_hasInvFailure_false hinv_bool₁ hff_iff₁ hnif₁
          have hswf₁ : ∀ n ∈ Block.modifiedOrDefinedVars body, (ρ₁.store n).isSome :=
            fun n hn => star_preserves_isDefined_single (π := π) (φ := φ) hbody' (hswf₀ n hn)
          have hstore₁ : ∀ x, x ∉ Block.modifiedVars body → x ∉ Block.definedVars body →
              ρ₁.store x = ρ₀.store x :=
            block_execution_store_agreement π φ body ρ₀ ρ₁ hbody' hnofd
          have ih_result := ih ρ₁ (measure.map HasIntOrder.decr) hg_tt₁ hall_tt₁ hwfb₁ hwfv₁ hwfvar₁ hswf₁
            (.step _ _ _ (.step_loop_enter hg_tt₁ hinv_bool₁ hff_iff₁ ‹_› ‹_›) hrest₁)
            hloop_len
          -- Adjust ρ_last.eval = ρ₁.eval to ρ_last.eval = ρ₀.eval via heval_eq
          cases ih_result with
          | inl hsuccess =>
            obtain ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8, h8g, h9swf, h9store, h9, h9meas⟩ := hsuccess
            exact .inl ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8, h8g, h9swf,
              fun x hmod hdef => (h9store x hmod hdef).trans (hstore₁ x hmod hdef),
              h9.trans heval_eq,
              sorry /- measure non-negativity at ρ_last: needs conversion from IH's decremented measure -/⟩
          | inr hbreak =>
            obtain ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8swf, h8store, h8⟩ := hbreak
            exact .inr ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8swf,
              fun x hmod hdef => (h8store x hmod hdef).trans (hstore₁ x hmod hdef),
              h8.trans heval_eq⟩
      | .inr ⟨hrestExit, _⟩ =>
        ...
        have hrest_exit_prop : CoreStar π φ
            (.stmts (body ++ [.loop (.det g) (measure.map HasIntOrder.decr) inv body md])
              { ρ₀ with hasFailure := ρ₀.hasFailure || Bool.false })
            (.exiting .none ρ') := reflTransT_to_prop hrestExit
        have hrest_exit_ρ₀ : CoreStar π φ
            (.stmts (body ++ [.loop (.det g) (measure.map HasIntOrder.decr) inv body md]) ρ₀)
        ...
        have hbody_exit : CoreStar π φ (.stmts body ρ₀) (.exiting .none ρ') := by
          ...
          suffices ∀ (pfx : Statements) (ρ_cur : Env Expression),
              CoreStar π φ (.stmts (pfx ++ [.loop (.det g) (measure.map HasIntOrder.decr) inv body md]) ρ_cur)
                (.exiting .none ρ') →
          ...
          | nil =>
            ...
            exfalso
            have h_nil_eq : ([] : Statements) ++ [Stmt.loop (.det g) (measure.map HasIntOrder.decr) inv body md] =
                [Stmt.loop (.det g) (measure.map HasIntOrder.decr) inv body md] := rfl
            ...
            | .step _ _ _ .step_stmts_cons hrest_seq =>
              ...
              | .inl hloop_exit =>
                ...
                exact loop_det_cannot_exit_none π φ g (measure.map HasIntOrder.decr) inv body md ρ_cur ρ' hloop_exit
          ...
          | cons s rest ih_pfx =>
            ...
            have h_eq : (s :: rest) ++ [Stmt.loop (.det g) (measure.map HasIntOrder.decr) inv body md] =
                s :: (rest ++ [Stmt.loop (.det g) (measure.map HasIntOrder.decr) inv body md]) := rfl
...
private theorem loop_invariant_dichotomy_nondet
  ...
  suffices ∀ (n : Nat) (ρ₀ : Env Expression),
      ...
      (∃ ρ_last,
        ...
        (∀ x, x ∉ Block.modifiedVars body → x ∉ Block.definedVars body → ρ_last.store x = ρ₀.store x) ∧
        ρ_last.eval = ρ₀.eval) by
    exact this hblockT.len ρ₀ hall_tt hwfb hwfv hwfvar hswf hblockT (Nat.le_refl _)
  intro n
  induction n with
  | zero =>
    ...
    | .step _ _ _ _ _, hlen => simp [ReflTransT.len] at hlen
  | succ n ih =>
    intro ρ₀ hall_tt₀ hwfb₀ hwfv₀ hwfvar₀ hswf₀ hstarT hlen
    have hnf₀' : ρ₀.hasFailure = Bool.false :=
    ...
    | .inl ⟨hrestInner, hlen_inner⟩ =>
      ...
      | .step _ _ _ (@StepStmt.step_loop_nondet_enter _ _ _ _ _ _ _ _ _ _ _
        ...
        have hrest₁_len : hrest₁.len ≤ n := by
          ...
          omega
        have ih_result := ih ρ₁ hall_tt₁ hwfb₁ hwfv₁ hwfvar₁ hswf₁ (hρ₁_eq ▸ hrest₁)
          (by
            suffices ∀ (ρ : Env Expression)
                (h : ReflTransT (StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ))
                  (.block .none (.stmts (body ++ [.loop .nondet .none inv body md]) ρ))
                  (.terminal ρ'))
                (heq : ρ = ρ₁),
                (heq ▸ h).len = h.len by
        ...
        cases ih_result with
        | inl hsuccess =>
          obtain ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10⟩ := hsuccess
          exact .inl ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8,
            fun x hx1 hx2 => (h9 x hx1 hx2).trans (hstore₁ x hx1 hx2), h10.trans heval_eq⟩
        | inr hbreak =>
          obtain ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8, h9⟩ := hbreak
          exact .inr ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7,
            fun x hx1 hx2 => (h8 x hx1 hx2).trans (hstore₁ x hx1 hx2), h9.trans heval_eq⟩
...
private theorem loop_det_exiting_body_exit
  ...
  suffices ∀ (n : Nat) (ρ₀ : Env Expression) (measure : Option Expression.Expr),
      ...
      ∃ ρ_last,
        ...
        CoreStar π φ (.stmts body ρ_last) (.exiting (some l) ρ') ∧
        ρ_last.eval = ρ₀.eval by
    exact this hloopT.len ρ₀ measure hguard_tt hall_tt hwfb hwfv hwfvar hloopT (Nat.le_refl _)
  intro n
  induction n with
  | zero =>
    ...
    | .step _ _ _ _ _, hlen => simp [ReflTransT.len] at hlen
  | succ n ih =>
    intro ρ₀ measure hguard_tt₀ hall_tt₀ hwfb₀ hwfv₀ hwfvar₀ hstarT hlen
    match hstarT, hlen with
    | .step _ _ _ (@StepStmt.step_loop_exit _ _ _ _ _ _ _ _ _ _ _ _ _
        hasInvFailure hg_ff _ _ _) _, _ =>
      exfalso; rw [hguard_tt₀] at hg_ff; cases hg_ff
    | .step _ _ _ (@StepStmt.step_loop_enter _ _ _ _ _ _ _ _ _ _ _ _ _
      ...
      have hρ₀_eq :
          ({ ρ₀ with hasFailure := ρ₀.hasFailure || Bool.false } : Env Expression) = ρ₀ := by
        cases ρ₀; simp [Bool.or_false]
      -- Extract inner with length bound from block .none
      -- Note: hrest has env {ρ₀ with hasFailure := ρ₀.hasFailure || false}
      -- We work with this env in the decomposition to preserve lengths.
      have ⟨hinner_T, hlen_inner⟩ :=
      ...
      match stmtsT_append_reaches_exiting π φ body
          (.loop (.det g) (measure.map HasIntOrder.decr) inv body md) (some l) _ ρ' hinner_T with
      | .inl hbody_exit =>
        -- Body exits (with ugly env). Transport to ρ₀.
        exact ⟨ρ₀, hguard_tt₀, hall_tt₀, hwfb₀, hwfv₀, hwfvar₀, hnf₀,
          hρ₀_eq ▸ hbody_exit, rfl⟩
      | .inr ⟨ρ₁, hbody_term, hloop_exitT, hlen_loop⟩ =>
        have hbody_term' : CoreStar π φ (.stmts body ρ₀) (.terminal ρ₁) :=
          hρ₀_eq ▸ hbody_term
        have heval_eq : ρ₁.eval = ρ₀.eval :=
          smallStep_noFuncDecl_preserves_eval_block Expression (EvalCommand π φ) (EvalPureFunc φ)
        ...
        | .step _ _ _ (@StepStmt.step_loop_enter _ _ _ _ _ _ _ _ _ _ _ _ _
          ...
          -- hlen_loop is now about the destructured form
          -- We need to bound it
          have hloop_len : (1 + r1_loop.len) ≤ n := by
            simp [ReflTransT.len] at hlen hlen_loop hlen_inner ⊢
            omega
          -- Apply IH with inline trace; coerce length bound via simp
          ...
          have ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7, h8⟩ :=
            ih ρ₁ (measure.map HasIntOrder.decr) hg_tt_1 hall_tt₁ hwfb₁ hwfv₁ hwfvar₁ hloop_ρ₁_T hloop_len'
...
private theorem loop_nondet_exiting_body_exit
  ...
  -- Extract inner with length bound
  have ⟨hinner_T, hlen_inner⟩ :=
    blockT_none_reaches_exiting_some (π := π) (φ := φ) henterT
  -- Strong induction on inner stmts trace length
  suffices ∀ (n : Nat) (ρ_cur : Env Expression),
    ...
    exact this hinner_T.len ρ₀ hall_tt hwfb hwfv hwfvar hnf₀ hinner_T (Nat.le_refl _)
  intro n
  induction n with
  | zero =>
    intro ρ_cur _ _ _ _ _ hstarT hlen
    match hstarT, hlen with
    | .step _ _ _ _ _, hlen => simp [ReflTransT.len] at hlen
  | succ n ih =>
    intro ρ_cur hall_tt_cur hwfb_cur hwfv_cur hwfvar_cur hnf_cur hstarT hlen
    match stmtsT_append_reaches_exiting π φ body
    ...
    | .inr ⟨ρ₁, hbody_term, hloop_exitT, hlen_loop⟩ =>
      ...
      have hnf₁ : ρ₁.hasFailure = Bool.false :=
        hasFailure_false_backwards (π := π) (φ := φ) (reflTransT_to_prop hloop_exitT) hnf
      -- Loop exits with (some l): must enter (exit→terminal)
      match hloop_exitT with
      | .step _ _ _ (.step_loop_nondet_exit _ _) r1_loop =>
        match r1_loop with | .step _ _ _ h _ => cases h
      | .step _ _ _ (@StepStmt.step_loop_nondet_enter _ _ _ _ _ _ _ _ _ _ _
          hasInvFailure₁ hinv_eval₁ hff_iff₁) r1_loop =>
        have hnf_start₁ : (ρ₁.hasFailure || hasInvFailure₁) = Bool.false :=
          hasFailure_false_backwards (π := π) (φ := φ) (reflTransT_to_prop r1_loop) hnf
        have hnif₁ : hasInvFailure₁ = Bool.false :=
          loop_step_hasInvFailure_false_of_or hnf_start₁
        have hall_tt₁ := all_inv_tt_of_hasInvFailure_false hinv_eval₁ hff_iff₁ hnif₁
        have hρ₁_eq : ({ ρ₁ with hasFailure := ρ₁.hasFailure || hasInvFailure₁ } : Env Expression) = ρ₁ := by
          rw [hnif₁]; exact env_or_false_eq ρ₁
        -- Extract inner stmts trace from r1_loop (block .none wrapper)
        have ⟨hinner_r1, hlen_inner_r1⟩ :=
          blockT_none_reaches_exiting_some (π := π) (φ := φ) r1_loop
        -- Length bound: inner trace < r1_loop.len, and r1_loop fits in overall bound
        have hlen_bound : hinner_r1.len ≤ n := by
          simp [ReflTransT.len] at hlen hlen_loop ⊢
          omega
        -- Apply IH at {ρ₁ with hasFailure := ...} (= ρ₁ by hρ₁_eq)
        have ⟨ρ_last, h1, h2, h3, h4, h5, h6, h7⟩ :=
          ih { ρ₁ with hasFailure := ρ₁.hasFailure || hasInvFailure₁ }
            (fun le hle => by simp [hnif₁, Bool.or_false]; exact hall_tt₁ le hle)
            (by simp [hnif₁, Bool.or_false]; exact hwfb₁)
            (by simp [hnif₁, Bool.or_false]; exact hwfv₁)
            (by simp [hnif₁, Bool.or_false]; exact hwfvar₁)
            (by simp [hnif₁, Bool.or_false]; exact hnf₁)
            hinner_r1 hlen_bound
        exact ⟨ρ_last, h1, h2, h3, h4, h5, h6,
          h7.trans (by simp [hnif₁, Bool.or_false]; exact heval_eq)⟩
...
private theorem env_with_store_eq (ρ₀ ρ_target : Env Expression)
  ...
  cases ρ₀; cases ρ_target; simp_all [Env.mk.injEq]
...
flag, and store values at `Block.modifiedOrDefinedVars ss`.  Two environments that agree
on these produce executions with the same hasFailure behavior and the same
store values on `modifiedOrDefinedVars` at each step.
...
`ConfigSE`).  The mathematical justification:
- Commands read the store only through `eval store expr` (where
  `getVars expr ⊆ modifiedOrDefinedVars` of the containing statement) and
  direct lookups `store x` for `x ∈ modifiedVars ∪ definedVars ⊆ modifiedOrDefinedVars`.
- `WellFormedCoreEvalCong` + `WellFormedSemanticEvalVar` together imply that
  `eval` depends only on `store` values at `getVars expr`.
- `UpdateState`/`InitState` write to `x ∈ modifiedOrDefinedVars`, preserving agreement
  on modifiedOrDefinedVars for the next step.
- Structural steps (block, seq, ite, loop) don't access the store at all.

The theorems below are marked `private` and used to fill proof obligations in
the loop-elimination correctness argument. -/

/-- Transfer `CanFailBlock` across environments that agree on `Block.touchedVars ss`
    (all vars read or written by ss) with same eval and hasFailure.
    Requires `WellFormedSemanticEvalExprCongr` to ensure expression evaluation
    depends only on store values at `getVars expr`. -/
private theorem canFailBlock_touchedVars_transfer
    (ss : Statements) (ρ₁ ρ₂ : Env Expression)
    (heval : ρ₂.eval = ρ₁.eval)
    (hfail : ρ₂.hasFailure = ρ₁.hasFailure)
    (hagree : ∀ n ∈ Block.touchedVars ss, ρ₂.store n = ρ₁.store n)
    (hcf : CanFailBlock π φ ss ρ₁) :
    CanFailBlock π φ ss ρ₂ := by
  obtain ⟨cfg, hfail_cfg, hstar⟩ := hcf
  -- If ρ₁.hasFailure is already true, ρ₂.hasFailure = true and the starting
  -- config (.stmts ss ρ₂) already has hasFailure = true, so .refl witnesses it.
  by_cases h : ρ₁.hasFailure = Bool.true
  · exact ⟨.stmts ss ρ₂, hfail ▸ h, .refl _⟩
  · -- ρ₁.hasFailure = false. Execution from ρ₁ eventually sets hasFailure.
    -- Use the full-store simulation: since ρ₂ agrees with ρ₁ on touchedVars and
    -- differs only outside, we observe that EvalStmtsSmall_hasFailure_irrel
    -- generalizes to partial store agreement when WellFormedSemanticEvalExprCongr holds.
    -- Key fact: any var accessed during execution of ss is in Block.touchedVars ss,
    -- so the stores are effectively equal for the purpose of the execution.
    -- The proof applies coreStar_stmts_touchedVars_transfer and then uses hasFailure
    -- monotonicity to show some intermediate config has hasFailure = true.
    -- Direct approach: partial-store simulation (see module docstring above).
    sorry

/-- Transfer `CoreStar` termination across environments that agree on
    `Block.touchedVars ss` with same eval and hasFailure.
    The terminal from ρ₂ agrees with ρ₁' on touchedVars and has same eval/hasFailure.
    Requires `WellFormedSemanticEvalExprCongr`. -/
private theorem coreStar_stmts_touchedVars_transfer
    (ss : Statements) (ρ₁ ρ₂ ρ₁' : Env Expression)
    (heval : ρ₂.eval = ρ₁.eval)
    (hfail : ρ₂.hasFailure = ρ₁.hasFailure)
    (hagree : ∀ n ∈ Block.touchedVars ss, ρ₂.store n = ρ₁.store n)
    (hstar : CoreStar π φ (.stmts ss ρ₁) (.terminal ρ₁')) :
    ∃ ρ₂', CoreStar π φ (.stmts ss ρ₂) (.terminal ρ₂') ∧
      ρ₂'.eval = ρ₁'.eval ∧ ρ₂'.hasFailure = ρ₁'.hasFailure ∧
      (∀ n ∈ Block.touchedVars ss, ρ₂'.store n = ρ₁'.store n) := by
  -- Same partial-store simulation as canFailBlock_touchedVars_transfer.
  -- Given ρ₂ agrees with ρ₁ on touchedVars ss (all accessed vars) and has same eval,
  -- execution from ρ₂ follows an identical path to terminal.
  -- The terminal ρ₂' agrees with ρ₁' on touchedVars (writes are to modifiedOrDefinedVars
  -- ⊆ touchedVars, and reads used the same values due to agreement + congruence).
  -- Proof: induction on hstar with ConfigPSE invariant (partial-store equivalence).
  -- Each step preserves the invariant:
  --  - step_cmd: EvalCmd reads getVars(expr) ⊆ touchedVars, writes modifiedOrDefinedVars ⊆ touchedVars
  --  - step_ite: reads guard vars ⊆ touchedVars
  --  - step_loop: reads guard + inv vars ⊆ touchedVars
  --  - structural steps: don't access store
  -- At terminal, the invariant gives the conclusion.
  -- Requires WellFormedSemanticEvalExprCongr (available at all call sites).
  sorry
...
private theorem route_body_canfail_through_target
    ...
    (hmeas_wf : ∀ me, measure = .some me →
      ρ₀.eval ρ₀.store (HasIntOrder.lt me HasIntOrder.zero) = .some HasBool.ff)
    (hcf : CanFailBlock π φ body ρ₀)
    (hok : stmtOk σ (.loop guard measure inv body md)) :
    Transform.CanFail (LangCore π φ) (stmtResult σ (.loop guard measure inv body md)) ρ₀ := by
  -- Get the detailed target structure (with prefix termination)
  have ⟨ll, fil, arb_lbl, fib, prefix_stmts, suffix_stmts, exit_state_stmts,
    maintain_invariants, body_statements, heq_stmt, heq_body, _heq_maintain, heq_fib,
    ρ_pf, hprefix_term, _heval_pf, _hfail_pf, _hagree_pf⟩ :=
    stmtResult_loop_with_prefix_term π φ σ guard measure inv body md ρ₀
      hwfb hwfvar hnf hwfc hwfv hall_tt hguard_ok hswf_body hmeas_wf hok
  rw [heq_stmt]
  -- 1. First-iter block (fib) terminates at ρ₀: asserts + assumes all pass
  ...
  -- 2. body_statements = blockResult body, which canfails by hcf
  rw [← heq_body] at hcf
  -- 3. CanFail for body_statements ++ maintain ++ suffix (body' canfails, rest is appended)
  -- Transfer body canfail from ρ₀ to ρ_pf: they have same eval/hasFailure, and ρ_pf agrees
  -- with ρ₀ on modifiedOrDefinedVars body. The prefix only modifies assigned_vars ⊆ modifiedOrDefinedVars body
  -- (via havoc_block_identity) plus assumes (no store change). In det+some case, prefix also
  -- does init m_old (fresh var not in body). Full store equality follows from prefix not
  -- modifying vars outside modifiedOrDefinedVars body ∪ {m_old_ident} and agreement on modifiedOrDefinedVars body.
  have hcf_suffix : CanFailBlock π φ
      (body_statements ++ (maintain_invariants ++ suffix_stmts)) ρ_pf :=
    canFailBlock_append_left π φ body_statements (maintain_invariants ++ suffix_stmts) ρ_pf
      (canFailBlock_touchedVars_transfer π φ body_statements ρ₀ ρ_pf
        _heval_pf _hfail_pf (heq_body ▸ _hagree_pf) hcf)
  -- 4. CanFail for the full arb_iter_facts inner list
  have hcf_arb_inner : CanFailBlock π φ
      (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) ρ₀ := by
    have hassoc : prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts =
        prefix_stmts ++ (body_statements ++ (maintain_invariants ++ suffix_stmts)) := by
      simp [List.append_assoc]
    rw [hassoc]
    obtain ⟨cfg, hfail, hreach⟩ := hcf_suffix
    exact ⟨cfg, hfail,
      ReflTrans_Transitive _ _ _ _
        (stmts_prefix_terminal_append Expression (EvalCommand π φ) (EvalPureFunc φ)
          prefix_stmts _ ρ₀ ρ_pf hprefix_term)
        hreach⟩
  -- 5. CanFail for arb_block :: exit_state_stmts (lift through block wrapper)
  have hcf_then : CanFailBlock π φ
      (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
        :: exit_state_stmts) ρ₀ :=
    canFail_head_to_block π φ _ _ ρ₀
      (canFailBlock_to_canFail_block π φ arb_lbl _ _ ρ₀ hcf_arb_inner)
  -- 6. Route through ite: take the then-branch
  have hcf_ite : Transform.CanFail (LangCore π φ)
      (.ite guard
        (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
          :: exit_state_stmts) [] {}) ρ₀ := by
    obtain ⟨cfg, hfail, hreach⟩ := hcf_then
    cases guard with
    | det g =>
      exact ⟨cfg, hfail, .step _ _ _ (.step_ite_true hguard_ok hwfb) hreach⟩
    | nondet =>
      exact ⟨cfg, hfail, .step _ _ _ .step_ite_nondet_true hreach⟩
  -- 7. Lift: CanFail for ite → CanFailBlock for [ite] → CanFailBlock for [first_iter, ite] → CanFail for outer block
  have hcf_ite_block : CanFailBlock π φ
      [.ite guard
        (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
          :: exit_state_stmts) [] {}] ρ₀ :=
    canFail_head_to_block π φ _ [] ρ₀ hcf_ite
  have hcf_inner : CanFailBlock π φ
      [.block fil fib {}, .ite guard
        (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
          :: exit_state_stmts) [] {}] ρ₀ :=
    canFail_tail_to_block π φ (.block fil fib {})
      [.ite guard _ [] {}] ρ₀ ρ₀ hfirst_iter_term hcf_ite_block
  exact canFailBlock_to_canFail_block π φ ll _ {} ρ₀ hcf_inner
...
private theorem loop_enter_canfail_simulation
    ...
    (σ : LoopElimState)
    (guard : ExprOrNondet Expression)
    (measure_cont : Option Expression.Expr)
    (inv : List (String × Expression.Expr))
    (body : Statements) (md : MetaData Expression)
    (ρ₀ : Env Expression)
    (hwfb : WellFormedSemanticEvalBool ρ₀.eval)
    ...
    (hcf : ∃ cfg : CC, cfg.getEnv.hasFailure = Bool.true ∧
      CoreStar π φ (.block .none (.stmts (body ++ [.loop guard measure_cont inv body md]) ρ₀)) cfg)
    -- Body-level IHs (for strictly smaller size)
    (ih_body_term : ∀ ρ ρ',
      WellFormedSemanticEvalBool ρ.eval → WellFormedSemanticEvalVal ρ.eval →
      WellFormedSemanticEvalVar ρ.eval → WellFormedCoreEvalCong ρ.eval →
      (∀ n ∈ Block.modifiedOrDefinedVars body, (ρ.store n).isSome) →
    ...
    (ih_body_cf : ∀ ρ,
      ...
      CanFailBlock π φ body ρ →
      CanFailBlock π φ body ρ)
    (measure_tgt : Option Expression.Expr)
    (hmeas_wf_tgt : ∀ me, measure_tgt = .some me →
      ρ₀.eval ρ₀.store (HasIntOrder.lt me HasIntOrder.zero) = .some HasBool.ff)
    (hok : stmtOk σ (.loop guard measure_tgt inv body md)) :
    Transform.CanFail (LangCore π φ)
      (stmtResult σ (.loop guard measure_tgt inv body md)) ρ₀ := by
  ...
  match stmtsT_append_canfail body (.loop guard measure_cont inv body md) ρ₀ hinner_T hinner_fail with
  | .inl ⟨cfg_body, hfail_body, hstar_body⟩ =>
    -- Body canfails from ρ₀. By IH, target body also canfails.
    have htgt_body_cf := ih_body_cf ρ₀ hwfb hwfv hwfvar hwfc hswf_body ⟨cfg_body, hfail_body, hstar_body⟩
    have hinv_bool : ∀ le ∈ inv, ρ₀.eval ρ₀.store le.2 = .some HasBool.tt ∨
        ρ₀.eval ρ₀.store le.2 = .some HasBool.ff :=
      fun le hle => .inl (hall_tt le hle)
    exact route_body_canfail_through_target π φ σ guard measure_tgt inv body md ρ₀
      hwfb hwfvar hnf hwfc hwfv hall_tt hinv_bool hguard_ok hswf_body hmeas_wf_tgt htgt_body_cf hok
  | .inr ⟨ρ₁, hterm_body_prop, cfg_loop, hloop_T, hloop_fail, _⟩ =>
    ...
    · -- ρ₁.hasFailure=true: body produced a failing terminal → body canfails
      ...
      have htgt_body_cf := ih_body_cf ρ₀ hwfb hwfv hwfvar hwfc hswf_body hcf_body
      exact route_body_canfail_through_target π φ σ guard measure_tgt inv body md ρ₀
        hwfb hwfvar hnf hwfc hwfv hall_tt hinv_bool hguard_ok hswf_body hmeas_wf_tgt htgt_body_cf hok
    · -- ρ₁.hasFailure=false: loop canfails from ρ₁.
      ...
      match ih_body_term ρ₀ ρ₁ hwfb hwfv hwfvar hwfc hswf_body hterm_body with
      | .inl htgt_cf =>
        exact route_body_canfail_through_target π φ σ guard measure_tgt inv body md ρ₀
          hwfb hwfvar hnf hwfc hwfv hall_tt hinv_bool hguard_ok hswf_body hmeas_wf_tgt htgt_cf hok
      | .inr htgt_ok =>
        ...
        have heval_eq₁ : ρ₁.eval = ρ₀.eval :=
          smallStep_noFuncDecl_preserves_eval_block Expression (EvalCommand π φ) (EvalPureFunc φ)
            body ρ₀ ρ₁ hnofd hterm_body
        have hwfb₁ : WellFormedSemanticEvalBool ρ₁.eval := heval_eq₁ ▸ hwfb
        suffices route_maintain_cf :
            (∀ le ∈ inv, ρ₁.eval ρ₁.store le.2 = .some HasBool.tt ∨
                         ρ₁.eval ρ₁.store le.2 = .some HasBool.ff) →
            (∃ le ∈ inv, ρ₁.eval ρ₁.store le.2 = .some HasBool.ff) →
            Transform.CanFail (LangCore π φ)
              (stmtResult σ (.loop guard measure_tgt inv body md)) ρ₀ by
          -- The source loop canfails from ρ₁ (via hloop_T/hloop_fail).
          -- Decompose the loop's first step to extract invariant evaluation info:
          -- step_loop_enter/step_loop_exit both require hinv_bool₁ and provide hff_iff.
          -- If hasInvFailure=true (some inv is ff): apply route_maintain_cf.
          -- If hasInvFailure=false (all invs tt): loop enters body, need recursive
          -- decomposition of the loop canfail to find the eventual failure.
          -- This requires a well-founded induction on the loop execution length.
          sorry
        -- Proof of route_maintain_cf
        intro hinv_eval₁ hsome_ff₁
        have ⟨ll, fil, arb_lbl, fib, prefix_stmts, suffix_stmts, exit_state_stmts,
          maintain_invariants, body_statements, heq_stmt₂, heq_body₂, heq_maintain₂,
          heq_fib₂, ρ_pf₂, hprefix_term₂, _heval_pf₂, _hfail_pf₂, _hagree_pf₂⟩ :=
          stmtResult_loop_with_prefix_term π φ σ guard measure_tgt inv body md ρ₀
            hwfb hwfvar hnf hwfc hwfv hall_tt hguard_ok hswf_body hmeas_wf_tgt
            hok
        let mkMaintainLbl : Nat → String → String := fun i lbl =>
          s!"{loopElimAssertPrefix}{(StringGenState.gen "loop" σ.gen).fst}_arbitrary_iter_maintain_invariant_{if lbl.isEmpty then toString i else s!"{i}_{lbl}"}"
        -- Transfer body termination from ρ₀ to ρ_pf₂ (touchedVars agreement)
        have ⟨ρ_body_end, hbody_s, heval_end, hfail_end, hagree_end⟩ :=
          coreStar_stmts_touchedVars_transfer π φ body_statements ρ₀ ρ_pf₂ ρ₁
            _heval_pf₂ _hfail_pf₂ (heq_body₂ ▸ _hagree_pf₂) (heq_body₂ ▸ htgt_body_term)
        -- maintain_invariants canfails from ρ₁; transfer to ρ_body_end
        have hmaint_cf : CanFailBlock π φ maintain_invariants ρ_body_end := by
          rw [heq_maintain₂]
          have hmaint_at_rho1 := stmts_mapIdx_maintain_assert_canFail π φ inv ρ₁ md mkMaintainLbl
            hwfb₁ hnf₁' hinv_eval₁ hsome_ff₁
          -- ρ_body_end agrees with ρ₁ on touchedVars body (from hagree_end) which
          -- includes the invariant expression vars (since inv vars ⊆ loop touchedVars
          -- and loop body touchedVars includes them for well-formed loops).
          -- The maintain stmts are asserts whose touchedVars = getVars(inv exprs).
          exact canFailBlock_touchedVars_transfer π φ _ ρ₁ ρ_body_end
            heval_end hfail_end
            (fun n hn => sorry /- n ∈ touchedVars(maintain) ⊆ touchedVars(body): inv vars
               are accessed by body (well-formedness of the loop invariant). -/)
            hmaint_at_rho1
        have hcf_arb : CanFailBlock π φ
            (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) ρ₀ := by
          rw [show prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts =
              prefix_stmts ++ (body_statements ++ (maintain_invariants ++ suffix_stmts))
            from by simp [List.append_assoc]]
          exact canFailBlock_prefix_terminal_suffix π φ prefix_stmts _ ρ₀ ρ_pf₂ hprefix_term₂
            (canFailBlock_prefix_terminal_suffix π φ body_statements _ ρ_pf₂ ρ_body_end hbody_s
              (canFailBlock_append_left π φ maintain_invariants suffix_stmts ρ_body_end hmaint_cf))
        rw [heq_stmt₂]
        -- Steps 5-7: route hcf_arb through block/ite/outer (same pattern as route_body_canfail_through_target)
        let mkAssertLbl₂ : Nat → String → String := fun i lbl =>
          s!"{loopElimAssertPrefix}{(StringGenState.gen "loop" σ.gen).fst}_entry_invariant_{if lbl.isEmpty then toString i else s!"{i}_{lbl}"}"
        let mkAssumeLbl₂ : Nat → String → String := fun i lbl =>
          s!"{loopElimAssumePrefix}{(StringGenState.gen "loop" σ.gen).fst}_entry_invariant_{if lbl.isEmpty then toString i else s!"{i}_{lbl}"}"
        ...
        have hcf_then₂ : CanFailBlock π φ
            (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
              :: exit_state_stmts) ρ₀ :=
          canFail_head_to_block π φ _ _ ρ₀
            (canFailBlock_to_canFail_block π φ arb_lbl _ _ ρ₀ hcf_arb)
        have hcf_ite₂ : Transform.CanFail (LangCore π φ)
            (.ite guard
              (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
                :: exit_state_stmts) [] {}) ρ₀ := by
          obtain ⟨cfg, hfail, hreach⟩ := hcf_then₂
          cases guard with
          | det g => exact ⟨cfg, hfail, .step _ _ _ (.step_ite_true hguard_ok hwfb) hreach⟩
          | nondet => exact ⟨cfg, hfail, .step _ _ _ .step_ite_nondet_true hreach⟩
        have hcf_ite_block₂ : CanFailBlock π φ
            [.ite guard
              (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
                :: exit_state_stmts) [] {}] ρ₀ :=
...
/-! ## Det loop success: target reaches ρ' -/

/-- When the det loop's last iteration from `ρ_last` terminates at `ρ'`
    (guard ff at ρ', invs tt at ρ'), and body' simulation is available,
    the full target structure reaches `ρ'` from `ρ₀`.

    This is the core simulation lemma for the "success" (terminal, last
    iteration body terminates) case of `step_loop_enter`.  The target
    execution path is:

    1. first-iter block (asserts + assumes at ρ₀) → terminates at ρ₀
    2. ite guard tt → then-branch
    3. arb-block inner:
       a. havoc assigned_vars targeting ρ_last → { ρ₀ with store := ρ_last.store } = ρ_last
       b. assume guard tt ∧ invs tt at ρ_last → passes
       c. body' from ρ_last → terminates at ρ' (by IH)
       d. maintain_invariants (assert invs at ρ') → passes
       e. suffix (post_termination) → terminates
    4. exit_state:
       a. havoc assigned_vars at ρ' (identity, all defined) → ρ'
       b. assume ¬g at ρ' → passes (guard ff)
...
private theorem det_loop_success_target_simulation
    ...
    (hswf_body : ∀ n ∈ Block.modifiedOrDefinedVars body, (ρ₀.store n).isSome)
    -- ρ_last conditions
    (hg_last : ρ_last.eval ρ_last.store g = some HasBool.tt)
    (hinv_last : ∀ le ∈ inv, ρ_last.eval ρ_last.store le.2 = some HasBool.tt)
    (heval_last : ρ_last.eval = ρ₀.eval)
    (hnf_last : ρ_last.hasFailure = Bool.false)
    (hwfb_last : WellFormedSemanticEvalBool ρ_last.eval)
    (hwfv_last : WellFormedSemanticEvalVal ρ_last.eval)
    (hwfvar_last : WellFormedSemanticEvalVar ρ_last.eval)
    (hswf_last : ∀ n ∈ Block.modifiedOrDefinedVars body, (ρ_last.store n).isSome)
    -- Store agreement: ρ_last agrees with ρ₀ outside assigned vars
    (hstore_agree : ∀ x,
      x ∉ (Block.modifiedVars body).filter (fun v => v ∉ Block.definedVars body) →
      ρ_last.store x = ρ₀.store x)
    -- Source body last iteration
    (hbody_last : CoreStar π φ (.stmts body ρ_last) (.terminal ρ'))
    -- ρ' conditions
    (hall_tt_post : ∀ le ∈ inv, ρ'.eval ρ'.store le.2 = some HasBool.tt)
    (hg_ff_post : ρ'.eval ρ'.store g = some HasBool.ff)
    -- Guard/invs at ρ₀
    (hguard_tt₀ : ρ₀.eval ρ₀.store g = some HasBool.tt)
    (hall_tt₀ : ∀ le ∈ inv, ρ₀.eval ρ₀.store le.2 = some HasBool.tt)
    (hmeas_wf_last : ∀ me, measure = .some me →
      ρ_last.eval ρ_last.store (HasIntOrder.lt me HasIntOrder.zero) = .some HasBool.ff)
    -- IH: body' terminates at ρ' from ρ_last
    (ih_body_ok :
      ρ'.hasFailure = Bool.false →
  ...
  have ⟨ll, fil, arb_lbl, fib, prefix_stmts, suffix_stmts, exit_state_stmts,
    maintain_invariants, body_statements, heq_stmt, heq_body, heq_maintain, heq_fib,
    ⟨ρ_pf_tgt, hprefix_targeting, _heval_pf_tgt, _hfail_pf_tgt, _hagree_pf_tgt⟩,
    heq_exit, hsuffix_info⟩ :=
  ...
  have heval_post : ρ'.eval = ρ₀.eval := by
    have := smallStep_noFuncDecl_preserves_eval_block Expression (EvalCommand π φ) (EvalPureFunc φ)
      body ρ_last ρ' hnofd hbody_last
    exact this.trans heval_last
  have hwfb_post : WellFormedSemanticEvalBool ρ'.eval := heval_post ▸ hwfb
  -- 1. First-iter block terminates at ρ₀
  let mkAssertLbl : Nat → String → String := fun i lbl =>
    s!"{loopElimAssertPrefix}{(StringGenState.gen "loop" σ.gen).fst}_entry_invariant_{if lbl.isEmpty then toString i else s!"{i}_{lbl}"}"
  let mkAssumeLbl : Nat → String → String := fun i lbl =>
    s!"{loopElimAssumePrefix}{(StringGenState.gen "loop" σ.gen).fst}_entry_invariant_{if lbl.isEmpty then toString i else s!"{i}_{lbl}"}"
  have hfib_term : CoreStar π φ (.stmts fib ρ₀) (.terminal ρ₀) := by
    ...
    exact ReflTrans_Transitive _ _ _ _
      ...
      (stmts_mapIdx_assume_terminal π φ inv ρ₀ md mkAssumeLbl hwfb hall_tt₀)
  have hfirst_iter_term := block_wrap_terminal π φ fil fib {} ρ₀ ρ₀ hfib_term
  -- Body' simulation: either canfails or terminates at ρ'
  -- 3d. maintain_invariants (assert invs) pass at ρ'
  have hmaint_term : CoreStar π φ (.stmts maintain_invariants ρ') (.terminal ρ') := by
    rw [heq_maintain]
    let mkMaintainLbl : Nat → String → String := fun i lbl =>
      s!"{loopElimAssertPrefix}{(StringGenState.gen "loop" σ.gen).fst}_arbitrary_iter_maintain_invariant_{if lbl.isEmpty then toString i else s!"{i}_{lbl}"}"
    exact stmts_mapIdx_assert_terminal π φ inv ρ' md mkMaintainLbl hwfb_post hall_tt_post
  -- Transfer body termination from ρ_last to ρ_pf_tgt (touchedVars agreement)
  have hbody_term : CoreStar π φ (.stmts body_statements ρ_pf_tgt) (.terminal ρ') := by
    have ⟨ρ_body_end, hstar_end, heval_end, hfail_end, hagree_end⟩ :=
      coreStar_stmts_touchedVars_transfer π φ body_statements ρ_last ρ_pf_tgt ρ'
        _heval_pf_tgt _hfail_pf_tgt (heq_body ▸ _hagree_pf_tgt)
        (heq_body ▸ ih_body_ok hnf)
    -- ρ_body_end = ρ': they agree on touchedVars, and vars outside touchedVars are
    -- preserved from ρ_pf_tgt (which agrees with ρ_last outside touchedVars body).
    -- Since ρ' is reached from ρ_last by executing body (which only modifies
    -- modifiedOrDefinedVars ⊆ touchedVars), and ρ_pf_tgt agrees with ρ_last on
    -- touchedVars, ρ_body_end = ρ'.
    sorry
  -- 3. Arb-block inner execution from ρ₀:
  -- prefix → body' → maintain → suffix
  -- prefix targeting ρ_pf_tgt (from stmtResult_loop_with_prefix_targeting_det)
  have hprefix_to_last : CoreStar π φ (.stmts prefix_stmts ρ₀) (.terminal ρ_pf_tgt) :=
    hprefix_targeting
  -- suffix terminates at ρ' (post_termination)
  have hsuffix_term : CoreStar π φ (.stmts suffix_stmts ρ') (.terminal ρ') := by
    revert hsuffix_info hmeas_wf_last
    cases measure with
    | none =>
      intro hmeas_wf_last' hsuffix_info'
  ...
  have harb_inner_term : CoreStar π φ
      (.stmts (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) ρ₀)
      (.terminal ρ') := by
    rw [show prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts =
        prefix_stmts ++ (body_statements ++ (maintain_invariants ++ suffix_stmts))
      from by simp [List.append_assoc]]
    exact ReflTrans_Transitive _ _ _ _
      (stmts_prefix_terminal_append Expression (EvalCommand π φ) (EvalPureFunc φ) _ _ ρ₀ ρ_pf_tgt
        hprefix_to_last)
      (ReflTrans_Transitive _ _ _ _
        (stmts_prefix_terminal_append Expression (EvalCommand π φ) (EvalPureFunc φ) _ _ ρ_pf_tgt ρ'
          hbody_term)
        (ReflTrans_Transitive _ _ _ _
          (stmts_prefix_terminal_append Expression (EvalCommand π φ) (EvalPureFunc φ) _ _ ρ' ρ'
            hmaint_term)
          hsuffix_term))
  have harb_block_term := block_wrap_terminal π φ arb_lbl _ {} ρ₀ ρ' harb_inner_term
  -- 4. exit_state from ρ': havoc (identity) + assume ¬g + assume invs
  have hexit_term : CoreStar π φ (.stmts exit_state_stmts ρ') (.terminal ρ') := by
    rw [heq_exit]
    let loop_num := (StringGenState.gen "loop" σ.gen).fst
    let invSuffix : Nat → String → String := fun i lbl =>
      if lbl.isEmpty then toString i else s!"{i}_{lbl}"
    let assigned_vars :=
      (Block.modifiedVars body).filter (fun v => v ∉ Block.definedVars body)
    -- Definedness of assigned_vars at ρ' (via star_preserves_isDefined_single)
    have hdef_assigned_post : ∀ n ∈ assigned_vars, (ρ'.store n).isSome := by
      intro n hn
      simp only [assigned_vars, List.mem_filter] at hn
      have hdef_last := hswf_last n
        ((modifiedVars_subset_modifiedOrDefinedVars (Block.sizeOf body)).2 body (Nat.le_refl _) n hn.1)
      exact star_preserves_isDefined_single (π := π) (φ := φ) hbody_last hdef_last
    have hwfvar_post : WellFormedSemanticEvalVar ρ'.eval := heval_post ▸ hwfvar
    have hnf_post : ρ'.hasFailure = Bool.false := hnf
    -- 4a. havoc identity at ρ'
    have hhavoc_id_post := havoc_block_identity π φ
      s!"{loopElimBlockPrefix}loop_havoc_{loop_num}" assigned_vars md ρ'
      hdef_assigned_post hwfvar_post hnf_post
    -- 4b. assume ¬g at ρ' (guard ff → not g is tt)
    have hnot_g_tt : ρ'.eval ρ'.store (HasNot.not g) = some HasBool.tt :=
      (hwfb_post ρ'.store g).2.mp hg_ff_post
    have hassume_not_guard := assume_pass_step π φ
      s!"{loopElimAssumePrefix}{loop_num}_not_guard" (HasNot.not g) md ρ' hnot_g_tt hwfb_post
    -- 4c. invariant assumes at ρ'
    have hinv_assumes_term := stmts_mapIdx_assume_terminal π φ inv ρ' md
      (fun i lbl => s!"{loopElimAssumePrefix}{loop_num}_exit_invariant_{invSuffix i lbl}")
      hwfb_post hall_tt_post
    -- Combine: [havocd, assume_not_guard] ++ inv_assumes
    exact stmts_concat_terminal π φ _ _ ρ' ρ' ρ'
      (stmts_pair_terminal π φ _ _ ρ' ρ' ρ' hhavoc_id_post hassume_not_guard)
      hinv_assumes_term
  -- Combine: [arb_block, exit_state_stmts...]
  have hthen_inner_term : CoreStar π φ
      (.stmts (.block arb_lbl (prefix_stmts ++ body_statements ++ maintain_invariants ++ suffix_stmts) {}
        :: exit_state_stmts) ρ₀) (.terminal ρ') :=
    ReflTrans_Transitive _ _ _ _
      (stmts_cons_step Expression (EvalCommand π φ) (EvalPureFunc φ)
        (.block arb_lbl _ {}) exit_state_stmts ρ₀ ρ' harb_block_term)
      hexit_term
  -- 2. ite guard tt → then-branch
  have hite_term : CoreStar π φ
      (.stmt (.ite (.det g) _ [] {}) ρ₀) (.terminal ρ') :=
    .step _ _ _ (.step_ite_true hguard_tt₀ hwfb) hthen_inner_term
  -- Combine: [first_iter, ite]
  have hinner_term : CoreStar π φ
      (.stmts [.block fil fib {}, .ite (.det g) _ [] {}] ρ₀) (.terminal ρ') :=
    ReflTrans_Transitive _ _ _ _
      (stmts_cons_step Expression (EvalCommand π φ) (EvalPureFunc φ)
        (.block fil fib {}) [.ite (.det g) _ [] {}] ρ₀ ρ₀ hfirst_iter_term)
      (ReflTrans_Transitive _ _ _ _
        (.step _ _ _ .step_stmts_cons (.refl _))
        (ReflTrans_Transitive _ _ _ _
          ...
          (.step _ _ _ .step_seq_done (.step _ _ _ .step_stmts_nil (.refl _)))))
  -- Wrap in outer block
  exact block_wrap_terminal π φ ll _ {} ρ₀ ρ' hinner_term
...
private theorem simulation
  ...
  induction sz with
  | zero =>
    refine ⟨fun σ st hsz _ _ => ?_, fun σ bss hsz _ _ => ?_, fun σ st hsz _ _ => ?_, fun σ bss hsz _ _ => ?_⟩
    · match st with
      | .cmd _ => exact absurd hsz (by simp [Stmt.sizeOf])
      | .block .. => exact absurd hsz (by simp [Stmt.sizeOf])
      | .ite .. => exact absurd hsz (by simp [Stmt.sizeOf])
  ...
  | succ n ih =>
    refine ⟨?_, ?_, ?_, ?_⟩
    -- === Statement case ===
    · intro σ st hsz hnofd hok ρ₀ hwfb hwfv hwfvar hwfc hswf
      match st with
      | .cmd c =>
        rw [stmtResult_cmd]
      ...
      | .block l bss md =>
        ...
        have blk_ih := ih.2.1 σ bss hsz_bss hnofd_bss (stmtOk_block_inner hok) ρ₀ hwfb hwfv hwfvar hwfc (by exact hswf)
        constructor
        · intro ρ' hstar
          cases hstar with
      ...
      | .ite c tss ess md =>
        ...
        · intro ρ' hstar
          ...
          | step _ _ _ h1 r1 => cases h1 with
            | step_ite_true hcond hwfb' =>
              match (ih.2.1 σ tss hsz_tss hnofd_tss (stmtOk_ite_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn))).1 ρ' r1 with
              | .inl hcf =>
            ...
            | step_ite_false hcond hwfb' =>
              match (ih.2.1 (blockPostState σ tss) ess hsz_ess hnofd_ess (stmtOk_ite_right hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_right _ hn))).1 ρ' r1 with
              | .inl hcf =>
                left; obtain ⟨cfg, hfail, hreach⟩ := hcf
        ...
        · intro lbl ρ' hstar
          ...
          | step _ _ _ h1 r1 => cases h1 with
            | step_ite_true hcond hwfb' =>
              match (ih.2.1 σ tss hsz_tss hnofd_tss (stmtOk_ite_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn))).2 lbl ρ' r1 with
              | .inl hcf =>
                left; obtain ⟨cfg, hfail, hreach⟩ := hcf
                exact ⟨cfg, hfail, ReflTrans_Transitive _ _ _ _ (.step _ _ _ (.step_ite_true hcond hwfb') (.refl _)) hreach⟩
              | .inr hok => exact .inr (fun hnf => .step _ _ _ (.step_ite_true hcond hwfb') (hok hnf))
            | step_ite_false hcond hwfb' =>
              match (ih.2.1 (blockPostState σ tss) ess hsz_ess hnofd_ess (stmtOk_ite_right hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_right _ hn))).2 lbl ρ' r1 with
              | .inl hcf =>
                left; obtain ⟨cfg, hfail, hreach⟩ := hcf
                exact ⟨cfg, hfail, ReflTrans_Transitive _ _ _ _ (.step _ _ _ (.step_ite_false hcond hwfb') (.refl _)) hreach⟩
              | .inr hok => exact .inr (fun hnf => .step _ _ _ (.step_ite_false hcond hwfb') (hok hnf))
      ...
      | .loop guard measure inv body md =>
        ...
        · intro ρ' hstar
          ...
          · -- Source loop reaches terminal ρ' with hasFailure = false
            ...
            | step _ _ _ h1 r1 => cases h1 with
              ...
              | step_loop_enter hguard_tt hinv_eval hff_iff hwfb' hmeas_src =>
                rename_i hasInvFailure
                have hnf_start : (ρ₀.hasFailure || hasInvFailure) = Bool.false :=
                ...
                have hsz_body : Block.sizeOf body ≤ n := by
                  simp [Stmt.sizeOf] at hsz; omega
                -- Terminal det enter: source loop from ρ₀ (guard tt, all invs tt)
                -- terminates at ρ' with hasFailure=false.
                -- Apply loop_invariant_dichotomy_det to decompose into 3 cases.
                -- Reconstruct the full loop trace
                have hstep : StepStmt Expression (EvalCommand π φ) (EvalPureFunc φ)
                    (.stmt (Stmt.loop (.det _) measure inv body md) ρ₀)
                    (.block .none (.stmts (body ++ [Stmt.loop (.det _) (measure.map HasIntOrder.decr) inv body md])
                      { ρ₀ with hasFailure := ρ₀.hasFailure || hasInvFailure })) :=
                  .step_loop_enter hguard_tt hinv_eval hff_iff hwfb' ‹_›
                have hloop_src : CoreStar π φ
                    (.stmt (Stmt.loop (.det _) measure inv body md) ρ₀)
                    (.terminal ρ') :=
                  ReflTrans.step _ _ _ hstep (henv_eq ▸ r1)
                match loop_invariant_dichotomy_det π φ hwf_ext _ measure inv body md ρ₀ ρ'
                    hguard_tt hall_tt hwfb hwfv hwfvar hnofd_body
                    (loop_modifiedOrDefinedVars_implies_body_modifiedOrDefinedVars hswf) hnf hloop_src with
                | .inl ⟨ρ_last, hg_last, hinv_last, hwfb_last, hwfv_last,
                    hwfvar_last, hnf_last, hbody_last, hall_tt_post, hg_ff_post,
                    hswf_last, hstore_agree_last, heval_last, hmeas_wf_last⟩ =>
                  -- Success: body terminates at ρ', invs tt, guard ff.
                  have hswf_body₂ : ∀ n₂ ∈ Block.modifiedOrDefinedVars body, (ρ₀.store n₂).isSome :=
                    loop_modifiedOrDefinedVars_implies_body_modifiedOrDefinedVars hswf
                  have hbody_ok : ρ'.hasFailure = Bool.false →
                      CoreStar π φ (.stmts body ρ_last) (.terminal ρ') :=
                    fun _ => hbody_last
                  exact .inr (fun _ => heq_stmt ▸
                      det_loop_success_target_simulation π φ hwf_ext σ _ measure inv
                        body md ρ₀ ρ_last ρ' hwfb hwfv hwfvar hwfc hnofd_body hnf₀ hnf
                        hswf_body₂ hg_last hinv_last heval_last hnf_last
                        hwfb_last hwfv_last hwfvar_last
                        hswf_last
                ...
                | .inr ⟨ρ_last, hg_last, hinv_last, hwfb_last, hwfv_last,
                  ...
                  -- Break: body from ρ_last exits with .none reaching ρ'.
                  -- Target: havoc → assumes → body' exits → propagate up.
                  sorry
              ...
              | step_loop_nondet_enter hinv_eval hff_iff =>
                ...
                have henv_eq :
                  ...
                  rw [hnif]; exact env_or_false_eq ρ₀
                -- Terminal nondet enter: same structure as det enter.
                have hsz_body : Block.sizeOf body ≤ n := by
                  simp [Stmt.sizeOf] at hsz; omega
                have henter : CoreStar π φ
                    (.block .none (.stmts (body ++ [Stmt.loop .nondet .none inv body md]) ρ₀)) (.terminal ρ') :=
                  henv_eq ▸ r1
                have hloop_src : CoreStar π φ
                    (.stmt (Stmt.loop .nondet .none inv body md) ρ₀) (.terminal ρ') :=
                  ReflTrans.step _ _ _ (.step_loop_nondet_enter hinv_eval hff_iff) (henv_eq ▸ r1)
                match loop_invariant_dichotomy_nondet π φ hwf_ext .none inv body md ρ₀ ρ'
                    hall_tt hwfb hwfv hwfvar hnofd_body (loop_modifiedOrDefinedVars_implies_body_modifiedOrDefinedVars hswf) hnf hloop_src henter with
                | .inl ⟨ρ_last, hinv_last, hwfb_last, hwfv_last,
                    hwfvar_last, hnf_last, hbody_last, hall_tt_post, hswf_last, hstore_agree_last, heval_last⟩ =>
                  -- Success: same as det case but without guard.
                  -- Need to show target block simulates to ρ' via:
                  --   first-iter block (assert+assume invs) → ite nondet true →
                  --   arb block (havoc targeting ρ_last + assume invs + body + assert invs) →
                  --   exit state (havoc + assume invs at ρ')
                  have hswf_body₂ : ∀ n₂ ∈ Block.modifiedOrDefinedVars body, (ρ₀.store n₂).isSome :=
                    loop_modifiedOrDefinedVars_implies_body_modifiedOrDefinedVars hswf
                  exact .inr (fun _ => sorry /- nondet_loop_success: needs body replay from havoc-targeted state (mirror of det_loop_success_target_simulation) -/)
                | .inr ⟨ρ_last, hinv_last, hwfb_last, hwfv_last,
                    hwfvar_last, hnf_last, hbody_exit, hswf_last, hstore_agree_last, heval_last⟩ =>
                  -- Break: same as det case but without guard.
                  sorry
        ...
        · intro lbl ρ' hstar
          ...
          | step _ _ _ h1 r1 => cases h1 with
            ...
            | step_loop_enter hguard_tt hinv_eval hff_iff hwfb' hmeas_src =>
              -- Exiting det enter: source loop body exits with label lbl.
              -- The block .none wrapping body++[loop] reached .exiting lbl ρ'.
              rename_i hasInvFailure
              exact .inr (fun hnf => by
                ...
                have henv_eq :
                    ({ ρ₀ with hasFailure := ρ₀.hasFailure || hasInvFailure } : Env Expression) = ρ₀ := by
                  rw [hnif]; exact env_or_false_eq ρ₀
                -- From block .none reaching .exiting lbl: lbl = some l
                have ⟨l, hlbl_eq, hinner_exit⟩ :=
                  block_none_reaches_exiting_some (π := π) (φ := φ) r1
                subst hlbl_eq
                -- Reconstruct full loop trace for loop_det_exiting_body_exit
                have hloop_src : CoreStar π φ
                    (.stmt (Stmt.loop (.det _) measure inv body md) ρ₀)
                    (.exiting (some l) ρ') :=
                  ReflTrans.step _ _ _
                    (.step_loop_enter hguard_tt hinv_eval hff_iff hwfb' hmeas_src) r1
                -- Extract ρ_last where body exits with (some l) from ρ_last
                have ⟨ρ_last, _hg_last, hinv_last, hwfb_last, hwfv_last,
                    hwfvar_last, hnf_last, hbody_exit_last, heval_last⟩ :=
                  loop_det_exiting_body_exit π φ hwf_ext _ measure inv body md ρ₀ ρ' l
                    hguard_tt hall_tt hwfb hwfv hwfvar hnofd_body hnf hloop_src
                -- Target routing: need to show
                --   CoreStar (.stmt (.block ll [.block fil fib {}, .ite (.det _) tb [] {}] {}) ρ₀)
                --     (.exiting (some l) ρ')
                -- Strategy:
                --   1. first_iter terminates at ρ₀ (asserts+assumes with hall_tt)
                --   2. ite guard tt → enters then-branch
                --   3. then-branch: block arb_lbl (prefix ++ body ++ maintain ++ suffix) {} :: exit_stmts
                --   4. prefix terminates targeting ρ_last
                --   5. body from ρ_last exits with (some l)
                --   6. exit propagates through: stmts seq → arb_block (needs l ≠ arb_lbl)
                --      → stmts seq → ite → stmts seq → outer block ll (needs l ≠ ll)
            ...
            | step_loop_nondet_enter hinv_eval hff_iff =>
              ...
              exact .inr (fun hnf => by
                ...
                have ⟨l, hlbl_eq, hinner_exit⟩ :=
                  block_none_reaches_exiting_some (π := π) (φ := φ) r1
                subst hlbl_eq
                -- Use loop_nondet_exiting_body_exit with the block trace
                have henter : CoreStar π φ
                    (.block .none (.stmts (body ++ [Stmt.loop .nondet .none inv body md]) ρ₀))
                    (.exiting (some l) ρ') :=
                  henv_eq ▸ r1
                have ⟨ρ_last, hinv_last, hwfb_last, hwfv_last,
                    hwfvar_last, hnf_last, hbody_exit_last, heval_last⟩ :=
                  loop_nondet_exiting_body_exit π φ hwf_ext .none inv body md ρ₀ ρ' l
        ...
        · intro ρ' hstar
          ...
          | step _ _ _ h1 r1 => cases h1 with
            | step_stmts_cons =>
              ...
              have s_ih := ih.1 σ s hsz_s hnofd_s (blockOk_cons_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn))
              ...
              | .inr hok_s =>
                ...
                have ss_ih := ih.2.1 (stmtPostState σ s) ss hsz_ss hnofd_ss (blockOk_cons_right hok) ρ₁ hwfb₁ hwfv₁ hwfvar₁ ((smallStep_noFuncDecl_preserves_eval Expression (EvalCommand π φ) (EvalPureFunc φ) s ρ₀ ρ₁ hnofd_s hterm_s) ▸ hwfc) (fun n hn => star_preserves_isDefined_single (π := π) (φ := φ) hterm_s (hswf n (List.mem_append_right _ hn)))
        ...
        · intro lbl ρ' hstar
          ...
          | step _ _ _ h1 r1 => cases h1 with
            | step_stmts_cons =>
              ...
              | .inl hexit_s =>
                match (ih.1 σ s hsz_s hnofd_s (blockOk_cons_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn))).2 lbl ρ' hexit_s with
                | .inl hcf =>
                  exact .inl (canFail_head_to_block π φ (stmtResult σ s) (blockResult (stmtPostState σ s) ss) ρ₀ hcf)
                | .inr hok =>
                  exact .inr (fun hnf =>
              ...
              | .inr ⟨ρ₁, hterm_s, hexit_ss⟩ =>
                match (ih.1 σ s hsz_s hnofd_s (blockOk_cons_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn))).1 ρ₁ hterm_s with
                | .inl hcf =>
                  exact .inl (canFail_head_to_block π φ (stmtResult σ s) (blockResult (stmtPostState σ s) ss) ρ₀ hcf)
                | .inr hok_s =>
                  have hwfb₁ := star_preserves_wf (π := π) (φ := φ) hwf_ext hterm_s hwfb
                  ...
                  | .inr hok_ss =>
                    exact .inr (fun hnf => by
                      ...
                      exact ReflTrans_Transitive _ _ _ _
                        ...
                        (hok_ss hnf))
    -- === Statement CanFail ===
    · intro σ st hsz hnofd hok ρ₀ hwfb hwfv hwfvar hwfc hswf hcf
      ...
      | .block l bss md =>
        ...
        | step _ _ _ h1 r1 => cases h1 with
          | step_block =>
            ...
            have hsz_bss : Block.sizeOf bss ≤ n := by
              simp [Stmt.sizeOf] at hsz; omega
            have ⟨cfg', hfail'', hreach'⟩ := ih.2.2.2 σ bss hsz_bss hnofd_bss (stmtOk_block_inner hok) ρ₀ hwfb hwfv hwfvar hwfc hswf ⟨inner_cfg, hfail', hinner⟩
      ...
      | .ite c tss ess md =>
        ...
        | step _ _ _ h1 r1 => cases h1 with
          | step_ite_true hcond hwfb' =>
            have ⟨cfg', hfail', hreach'⟩ := ih.2.2.2 σ tss hsz_tss hnofd_tss (stmtOk_ite_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn)) ⟨cfg, hfail, r1⟩
            exact ⟨cfg', hfail', .step _ _ _ (.step_ite_true hcond hwfb') hreach'⟩
          | step_ite_false hcond hwfb' =>
            have ⟨cfg', hfail', hreach'⟩ := ih.2.2.2 (blockPostState σ tss) ess hsz_ess hnofd_ess (stmtOk_ite_right hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_right _ hn)) ⟨cfg, hfail, r1⟩
            ...
            | step_loop_enter hguard_tt hinv_eval hff_iff hwfb' hmeas_src₂ =>
              ...
              · -- All invs tt, hasInvFailure=false. Delegate to master canfail lemma.
                ...
                have henv_eq : ({ ρ₀ with hasFailure := ρ₀.hasFailure || hasInvFailure_det_cf } : Env Expression) = ρ₀ := by
                  rw [hnif]; exact env_or_false_eq ρ₀
                rw [← heq_stmt]
                exact loop_enter_canfail_simulation π φ hwf_ext σ (.det _) (measure.map HasIntOrder.decr) inv body md ρ₀
                  hwfb hwfv hwfvar hwfc hnf₀' hall_tt hguard_tt
                  (loop_modifiedOrDefinedVars_implies_body_modifiedOrDefinedVars hswf) hnofd_body
                  ⟨cfg, hfail, henv_eq ▸ r1⟩
                  (fun _ _ _ _ _ _ _ hstar => .inr (fun _ => hstar))
                  (fun _ _ _ _ _ _ hcf => hcf)
                  measure (fun me hme =>
                    -- Requires WellFormedCoreEvalCong ρ₀.eval (thread from top-level) + measure_lt_congr
                    sorry)
                  hok
            ...
            | step_loop_nondet_enter hinv_eval hff_iff =>
              ...
              · -- All invs tt, hasInvFailure=false. Delegate to master canfail lemma.
                ...
                have henv_eq : ({ ρ₀ with hasFailure := ρ₀.hasFailure || hasInvFailure_nd_cf } : Env Expression) = ρ₀ := by
                  rw [hnif]; exact env_or_false_eq ρ₀
                rw [← heq_stmt]
                exact loop_enter_canfail_simulation π φ hwf_ext σ .nondet .none inv body md ρ₀
                  hwfb hwfv hwfvar hwfc hnf₀' hall_tt trivial
                  (loop_modifiedOrDefinedVars_implies_body_modifiedOrDefinedVars hswf) hnofd_body
                  ⟨cfg, hfail, henv_eq ▸ r1⟩
                  (fun _ _ _ _ _ _ _ hstar => .inr (fun _ => hstar))
                  (fun _ _ _ _ _ _ hcf => hcf)
                  .none (fun _ h => nomatch h)
                  hok
              · have hsome_ff := not_all_tt_implies_some_ff inv ρ₀ hinv_eval hall_tt
                have hcf := canFailBlock_append_left π φ
                  (inv.mapIdx fun i le => Stmt.cmd (HasPassiveCmds.assert (mkLbl i le.1) le.2 md))
                ...
                exact canFailBlock_to_canFail_block π φ ll _ _ ρ₀
                  (canFail_head_to_block π φ _ _ ρ₀ (canFailBlock_to_canFail_block π φ fil _ _ ρ₀ hcf))
    -- === Block CanFail ===
    · intro σ bss hsz hnofd_blk hok ρ₀ hwfb hwfv hwfvar hwfc hswf hcf
      obtain ⟨cfg, hfail, hreach⟩ := hcf
      match bss with
      ...
      | s :: ss =>
        ...
        | step _ _ _ h1 r1 => cases h1 with
          | step_stmts_cons =>
            ...
            | .inl ⟨cfg', hf', hstar'⟩ =>
              have ⟨kcfg, hkf, hkstar⟩ := ih.2.2.1 σ s hsz_s hnofd_s (blockOk_cons_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn)) ⟨cfg', hf', hstar'⟩
            ...
            | .inr ⟨ρ₁, hterm_s, cfg', hf', hstar_rest⟩ =>
              ...
              have hsim_s := ih.1 σ s hsz_s hnofd_s (blockOk_cons_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn))
              ...
              | .inr hok_s =>
                ...
                · have hcf_src_s : Transform.CanFail (LangCore π φ) s ρ₀ :=
                  ...
                  have hcf_tgt_s := ih.2.2.1 σ s hsz_s hnofd_s (blockOk_cons_left hok) ρ₀ hwfb hwfv hwfvar hwfc (fun n hn => hswf n (List.mem_append_left _ hn)) hcf_src_s
                ...
                · have hnf₁' : ρ₁.hasFailure = Bool.false := by cases h : ρ₁.hasFailure <;> simp_all
                  ...
                  have ⟨kcfg2, hkf2, hkstar2⟩ := ih.2.2.2 (stmtPostState σ s) ss hsz_ss hnofd_ss (blockOk_cons_right hok) ρ₁
                    hwfb₁ hwfv₁ hwfvar₁ ((smallStep_noFuncDecl_preserves_eval Expression (EvalCommand π φ) (EvalPureFunc φ) s ρ₀ ρ₁ hnofd_s hterm_s) ▸ hwfc) (fun n hn => star_preserves_isDefined_single (π := π) (φ := φ) hterm_s (hswf n (List.mem_append_right _ hn))) ⟨cfg', hf', hstar_rest⟩
...
theorem loopElim_overapproximatesAggressive
  ...
  simp at ht; subst ht
  have hswf_mod : ∀ n ∈ Stmt.modifiedOrDefinedVars st, (ρ₀.store n).isSome := by
    intro n hn; exact hswf.1 n (List.mem_append_left _ hn)
  have hsim := (simulation π φ hwf_ext (Stmt.sizeOf st)).1
    σ st (Nat.le_refl _) (sorry /- noFuncDecl -/) (sorry /- ok -/) ρ₀ hwfb hwfv hwfvar hswf.2.1 hswf_mod
  ...
  · intro ⟨cfg, hfail, hreach⟩
    ...
    · exact (canfail_simulation π φ hwf_ext (Stmt.sizeOf st)).1
        σ st (Nat.le_refl _) (sorry /- ok -/) (sorry /- noFuncDecl: needs top-level assumption -/) ρ₀ hwfb hwfv hwfvar hswf.2.1 hswf_mod ⟨cfg, hfail, hreach⟩
