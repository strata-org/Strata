...
/-- Compute the axioms irrelevant to `functions` using precomputed `cache`.
  This is the efficient per-goal counterpart of `Program.getIrrelevantAxioms`:
  it uses the same transitive fixed-point algorithm (`computeRelevantAxioms`)
  but reuses the prebuilt call graph and axiom map instead of reconstructing
  them on every call.
-/
