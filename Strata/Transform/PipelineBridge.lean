...
external store `σ_ext` that overapproximates the source initial store
(`StoreAgreement ρ₀.store σ_ext`): `σ_ext` agrees on every source-defined
variable and may bind strictly more.

This is the constructive flip that makes the pipeline an
`OverapproximatesUptoWhen`-shaped statement feeding a *further* transform: the
downstream consumer universally quantifies the target initial env over all
`R`-related (overapproximating) envs, and this theorem supports exactly that
quantification instead of dropping it.

The structured passes (`nondetElim`, `hoistLoopPrefixInits`) still run from
`ρ₀` — they are source-to-source and never touch the CFG store — so they keep
their original `ρ₀`-freshness preconditions.  Only the final `stmtsToCFG` pass
runs the CFG, so only it consumes the `σ_ext` agreement and the `σ_ext`-stated
freshness: the source `initVars`, the `ndelimKind` *and* `hoistKind` *and*
`s2uKind` minted names must all be undefined in `σ_ext`.  The `ndelimKind`
freshness on `σ_ext` is irreducibly required: `nondetElim` mints fresh-named
`init` targets that become part of the CFG (S2U input) program's `initVars`, so
`σ_ext` must leave them undefined for those CFG inits to fire.  (The `s2uKind`
*source*-store mint hypothesis is no longer needed and is dropped.) -/
theorem pipeline_sound_terminal_compositional
    ...
    -- ρ₀-freshness for the *structured* passes (nondetElim / hoist), unchanged:
    ...
    -- Track A: agreement + σ_ext-freshness for the *CFG* (S2U) pass:
  ...
  have h_step3_unique : Block.uniqueInits (Block.hoistLoopPrefixInits (Block.nondetElim ss)) :=
    h_hoist_iv_cls.2
  -- σ_ext analogue of the transformed-program initVars freshness (was ρ₀.store).
  have h_out_undef_ext : ∀ y ∈ Block.initVars (Block.nondetElim ss), σ_ext y = none := by
    intro y hy
    rcases Block.nondetElimM_initVars_classified ss StringGenState.emp y hy with
      h_src | ⟨str, h_eq, h_nd⟩
    · exact h_store_inits_ext y h_src
    · -- ndelim-minted init targets reach the CFG (S2U) input program's initVars,
      -- so σ_ext must leave them undefined too: discharged by h_store_mints_ndelim_ext.
  ...
  have h_step3_mv : Strata.Transform.GenSuffix.NoGenSuffix
    ...
    · rw [List.mem_append] at h_src
      ...
      · rcases Block.nondetElimM_initVars_classified ss StringGenState.emp _ h_iv with
        ...
        · exact ndelimKind_not_s2uKind h_nd (LawfulHasIdent.ident_inj h_eq2 ▸ hk)
    · exact hoistKind_not_s2uKind h_hoistk (LawfulHasIdent.ident_inj h_eq ▸ hk)
  -- === STEP 3: stmtsToCFG via the COMPOSITIONAL S2U spike (CFG from σ_ext) ===
  obtain ⟨σ_cfg, h_run3, h_agree3⟩ :=
    StructuredToUnstructuredCorrect.stmtsToCFG_terminal_compositional
...
/-- **Compositional-input pipeline soundness (exiting outcome).**  The exiting
companion of `pipeline_sound_terminal_compositional`: an *escaping* source run of
`ss` from `ρ₀` to a top-level uncaught exit `.exiting lbl ρ'` is matched by a CFG
run of `pipeline ss` started from an *arbitrary* overapproximating external store
`σ_ext` (`StoreAgreement ρ₀.store σ_ext`) reaching the *same* `.exiting lbl
σ_cfg` with an agreeing final store.

As in the terminal compositional theorem, the structured passes (`nondetElim`,
`hoistLoopPrefixInits`) still run from `ρ₀` — source-to-source, never touching
the CFG store — and keep their `ρ₀`-freshness preconditions; only the final
`stmtsToCFG` pass consumes the `σ_ext` agreement and the `σ_ext`-stated freshness
(source `initVars` and all three minted kinds undefined in `σ_ext`).  This is the
exiting arm the up-to-relation overapproximation's conjunct-1 needs, since `Upto`
runs the target from the `R`-related `ρ₀'`. -/
theorem pipeline_sound_exiting_compositional
    [HasFvar P] [HasNot P] [HasVal P] [HasBoolVal P]
    [HasIdent P] [HasIntOrder P] [HasVarsPure P P.Expr] [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P] [LawfulHasIntOrder P]
    [LawfulHasNot P] [HasSubstFvar P] [LawfulHasSubstFvar P]
    ...
    ∃ σ_cfg, StructuredToUnstructuredCorrect.StepDetCFGStar extendEval (pipeline ss)
      ...
      ∧ StoreAgreement ρ'.store σ_cfg := by
  -- === STEP 1: nondetElim (exiting, from ρ₀) ===
  obtain ⟨ρ_out, h_run1, h_agree1, h_hf1⟩ :=
    nondetElim_sound_kind_exit extendEval ss ρ₀ ρ'
      hwfb hwfv (hwfdef' ρ₀) (hwfcongr' ρ₀) (hwfvar' ρ₀)
      h_store_mints_ndelim h_ndelim_writes h_nofd h_lhni lbl h_exit
  have h_out_unique : Block.uniqueInits (Block.nondetElim ss) :=
    (Block.nondetElimM_initVars_nodup ss StringGenState.emp StringGenState.wf_emp
      h_unique (fun str hk => h_disj_initVars str (Or.inl hk))).2
  have h_out_iv_sf : ∀ str : String, hoistKind str →
  ...
  have h_out_undef : ∀ y ∈ Block.initVars (Block.nondetElim ss), ρ₀.store y = none := by
    ...
    · exact h_store_inits y h_src
    · rw [h_eq]; exact h_store_mints_ndelim str h_nd
  -- === STEP 2: hoist (exiting, from ρ₀) ===
  obtain ⟨ρ_h', h_run2, h_agree2, h_hf2⟩ :=
    hoistLoopPrefixInits_preserves_kind_exit (Q := hoistKind) hoistKind_gen
      (extendEval := extendEval) (Block.nondetElim ss)
      (nondetElim_containsNondetLoop ss)
      (nondetElim_containsFuncDecl ss h_nofd)
      (nondetElim_loopHasNoInvariants ss h_lhni)
      (by rw [Block.loopMeasureNone_eq_noMeasureLoops]; exact nondetElim_noMeasureLoops ss h_nml)
      h_out_exprs_sf h_out_unique h_out_fresh
      h_out_iv_sf h_out_mv_sf h_out_undef h_store_mints_hoist lbl h_run1
      hwfvar' hwfcongr' hwfsubst' hwfdef'
  -- === Direction-B S2U preconds on the hoist output, at Q := s2uKind ===
  have h_hoist_iv_cls :=
    LoopInitHoistLoopArmWF.Block.hoistLoopPrefixInitsM_initVars_classified (Q := hoistKind) hoistKind_gen
  ...
  have h_step3_mv : Strata.Transform.GenSuffix.NoGenSuffix
    ...
    · rw [List.mem_append] at h_src
      ...
      · rcases Block.nondetElimM_initVars_classified ss StringGenState.emp _ h_iv with
        ...
        · exact ndelimKind_not_s2uKind h_nd (LawfulHasIdent.ident_inj h_eq2 ▸ hk)
    · exact hoistKind_not_s2uKind h_hoistk (LawfulHasIdent.ident_inj h_eq ▸ hk)
  -- === STEP 3: stmtsToCFG via the COMPOSITIONAL exiting wrap (CFG from σ_ext) ===
  obtain ⟨σ_cfg, h_run3, h_agree3⟩ :=
    StructuredToUnstructuredCorrect.stmtsToCFG_exiting_compositional
      (Q := StructuredToUnstructuredCorrect.s2uKind)
      extendEval (Block.hoistLoopPrefixInits (Block.nondetElim ss)) ρ₀ ρ_h' σ_ext
      hwfb hwfv (hwfdef' ρ₀) (hwfcongr' ρ₀) (hwfvar' ρ₀)
      (hoist_noFuncDecl _ (nondetElim_noFuncDecl ss h_nofd))
...
final store (source on the left).  A real composition of the three terminal
...
not carry). -/
private theorem pipeline_sound_terminal [HasFvar P] [HasNot P] [HasVal P] [HasBoolVal P]
    [HasIdent P] [HasIntOrder P] [HasVarsPure P P.Expr] [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P] [LawfulHasIntOrder P]
    [LawfulHasNot P] [HasSubstFvar P] [LawfulHasSubstFvar P]
    (extendEval : ExtendEval P)
...
/-- **Pipeline soundness, exiting outcome (helper).** The exiting half of the
dual `pipeline_sound`: an *escaping* source run of `ss` from a clean initial
store `ρ₀` to a top-level uncaught exit `.exiting lbl ρ'` is matched by a CFG
run of `pipeline ss` reaching the *same* `.exiting lbl σ_cfg`, agreeing on the
final store (source on the left).  A real composition of the three
`_sound_kind_exit` theorems via `StoreAgreement.trans`, threading the escaping
...
make `StepCFG` non-deterministic in its outcome kind). -/
private theorem pipeline_sound_exiting [HasFvar P] [HasNot P] [HasVal P] [HasBoolVal P]
    [HasIdent P] [HasIntOrder P] [HasVarsPure P P.Expr] [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P] [LawfulHasIntOrder P]
    [LawfulHasNot P] [HasSubstFvar P] [LawfulHasSubstFvar P]
    (extendEval : ExtendEval P)
  ...
  have h_out_undef : ∀ y ∈ Block.initVars (Block.nondetElim ss), ρ₀.store y = none := by
    ...
    · exact h_store_inits y h_src
    · rw [h_eq]; exact h_store_mints_ndelim str h_nd
  -- === STEP 2: hoist (exiting; input `nondetElim ss`, source run = Step 1's) ===
  obtain ⟨ρ_h', h_run2, h_agree2, h_hf2⟩ :=
    hoistLoopPrefixInits_preserves_kind_exit (Q := hoistKind) hoistKind_gen
      (extendEval := extendEval) (Block.nondetElim ss)
      (nondetElim_containsNondetLoop ss)
  ...
  have h_step3_mv : Strata.Transform.GenSuffix.NoGenSuffix
    ...
    · rw [List.mem_append] at h_src
      ...
      · rcases Block.nondetElimM_initVars_classified ss StringGenState.emp _ h_iv with
        ...
        · exact ndelimKind_not_s2uKind h_nd (LawfulHasIdent.ident_inj h_eq2 ▸ hk)
    · exact hoistKind_not_s2uKind h_hoistk (LawfulHasIdent.ident_inj h_eq ▸ hk)
  -- === STEP 3: stmtsToCFG (exiting; input `hoist (nondetElim ss)`, run = Step 2's) ===
  obtain ⟨σ_cfg, h_run3, h_agree3⟩ :=
    StructuredToUnstructuredCorrect.structuredToUnstructured_sound_kind_exit
      (Q := StructuredToUnstructuredCorrect.s2uKind)
      StructuredToUnstructuredCorrect.s2uKind_gen
      extendEval (Block.hoistLoopPrefixInits (Block.nondetElim ss)) ρ₀ ρ_h'
      hwfb hwfv (hwfdef' ρ₀) (hwfcongr' ρ₀) (hwfvar' ρ₀)
...
/-- **Pipeline soundness (dual outcome).** Every source run of `ss` from a clean
initial store `ρ₀` is matched by a run of the unstructured CFG `pipeline ss`
reaching the *matching* outcome with a final store agreeing with the source's
(source on the left).  The source outcome selects the disjunct:

  * a *terminating* source run (`.terminal ρ'`) is matched by a CFG run reaching
    `.terminal σ_cfg`; and
  * an *escaping* source run (`.exiting lbl ρ'` — an uncaught top-level exit, as
    every label is uncaught at the top level where `exitConts = []`) is matched
    by a CFG run reaching the *same* `.exiting lbl σ_cfg`.

A real composition of the three `_sound_kind`/`_sound_kind_exit` theorems via
`StoreAgreement.trans`: no hypothesis is vacuous or false — each precondition is
satisfiable by a clean initial store and a shape-restricted, kind-free user
program.  Each pass preserves the outcome *kind* over `StoreAgreement`, so the
disjunct chosen at the source is carried verbatim to the CFG conclusion: the
terminal arm composes the unchanged terminal lemmas, the exiting arm the banked
`_sound_kind_exit` siblings that surface the sum-typed loop-body drivers. -/
theorem pipeline_sound [HasFvar P] [HasNot P] [HasVal P] [HasBoolVal P] [HasIdent P]
    ...
    (h_disj_modVars : ∀ str : String,
      (hoistKind str ∨ StructuredToUnstructuredCorrect.s2uKind str) →
      HasIdent.ident (P := P) str ∉ Block.modifiedVars ss)
    (h_run : StepStmtStar P (EvalCmd P) extendEval (.stmts ss ρ₀) (.terminal ρ')
      ∨ ∃ lbl, StepStmtStar P (EvalCmd P) extendEval (.stmts ss ρ₀) (.exiting lbl ρ')) :
    (∃ σ_cfg, StructuredToUnstructuredCorrect.StepDetCFGStar extendEval (pipeline ss)
        (.atBlock (pipeline ss).entry ρ₀.store ρ₀.hasFailure)
        (.terminal σ_cfg ρ'.hasFailure)
      ∧ StoreAgreement ρ'.store σ_cfg)
    ∨ (∃ lbl σ_cfg, StructuredToUnstructuredCorrect.StepDetCFGStar extendEval (pipeline ss)
        (.atBlock (pipeline ss).entry ρ₀.store ρ₀.hasFailure)
        (.exiting lbl σ_cfg ρ'.hasFailure)
      ∧ StoreAgreement ρ'.store σ_cfg) := by
  rcases h_run with h_term | ⟨lbl, h_exit⟩
  -- =========================================================================
  ...
  case inr =>
    ...
    obtain ⟨σ_cfg, h_run, h_agree⟩ :=
      pipeline_sound_exiting extendEval ss ρ₀ ρ'
        ...
        h_store_inits h_store_mints_ndelim h_store_mints_hoist h_store_mints_s2u
        h_nofd h_lhni h_nml h_unique h_fresh h_disj
        h_ndelim_writes h_ndelim_exprs h_hoist_exprs h_disj_initVars h_disj_modVars
        lbl h_exit
    exact ⟨lbl, σ_cfg, h_run, h_agree⟩
  -- =========================================================================
  ...
  -- =========================================================================
  left
  exact pipeline_sound_terminal extendEval ss ρ₀ ρ'
    hwfb hwfv hwfvar' hwfcongr' hwfsubst' hwfdef'
    h_store_inits h_store_mints_ndelim h_store_mints_hoist h_store_mints_s2u
    h_nofd h_lhni h_nml h_unique h_fresh h_disj
    h_ndelim_writes h_ndelim_exprs h_hoist_exprs h_disj_initVars h_disj_modVars h_term

/-- The bundled precondition under which `pipeline ss` refines `ss`.

Its fields are exactly the dual `pipeline_sound`'s hypothesis list — the
well-formed evaluator bundle, the clean-initial-store conditions
(`h_store_inits`, `h_store_mints`), the source shape restrictions, and the
per-pass minted-name kind-freedom conditions.  No covered-exit condition is
required: the refinement discharges *both* outcomes (terminal and escaping
exit) non-vacuously from the dual soundness helpers, so a top-level escaping
exit is matched, not assumed away. -/
...
@[expose] def StructuredPassFailingBridge
    ...
    [HasIdent P] [HasIntOrder P] [HasVarsPure P P.Expr] [DecidableEq P.Ident]
    [HasSubstFvar P]
    (extendEval : ExtendEval P) (ss : List (Stmt P (Cmd P))) (ρ₀ : Env P) : Prop :=
  ∀ c : Config P (Cmd P),
...
flow through `nondetElim`/`hoist` postconditions identically. -/
theorem pipeline_to_fail
    [HasFvar P] [HasNot P] [HasVal P] [HasBoolVal P]
    [HasIdent P] [HasIntOrder P] [HasVarsPure P P.Expr] [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P] [LawfulHasIntOrder P]
    [LawfulHasNot P] [HasSubstFvar P] [LawfulHasSubstFvar P]
    ...
    ∃ d : CFGConfig String (Cmd P) P,
      StructuredToUnstructuredCorrect.StepDetCFGStar extendEval (pipeline ss)
  ...
  have h_step3_mv : Strata.Transform.GenSuffix.NoGenSuffix
    ...
    · rw [List.mem_append] at h_src
      ...
      · rcases Block.nondetElimM_initVars_classified ss StringGenState.emp _ h_iv with
        ...
        · exact ndelimKind_not_s2uKind h_nd (LawfulHasIdent.ident_inj h_eq2 ▸ hk)
    · exact hoistKind_not_s2uKind h_hoistk (LawfulHasIdent.ident_inj h_eq ▸ hk)
  -- === FINAL S2U PASS: transport the failing config of `hoist (nondetElim ss)`
  --     to a failing CFG config, from the external store `σ_ext`. ===
  exact StructuredToUnstructuredCorrect.structuredToUnstructured_sound_kind_fail
    (Q := StructuredToUnstructuredCorrect.s2uKind)
    StructuredToUnstructuredCorrect.s2uKind_gen
    extendEval (Block.hoistLoopPrefixInits (Block.nondetElim ss)) ρ₀ c' σ_ext
    h_ρ₀_nofail hwfb hwfv (hwfdef' ρ₀) (hwfcongr' ρ₀) (hwfvar' ρ₀)
    (hoist_noFuncDecl _ (nondetElim_noFuncDecl ss h_nofd))
    (hoist_simpleShape _ (nondetElim_simpleShape ss))
    h_step3_unique
    (hoist_loopBodyNoInits _)
