...
/-- Target-store freshness invariant relative to a generator state `σ`:
every gen-shaped (`HasUnderscoreDigitSuffix`) string not yet generated by `σ`
is undefined in `σ_tgt`. -/
@[expose] def GenFreshStore {P : PureExpr} [HasIdent P]
    (σ : StringGenState) (σ_tgt : SemanticStore P) : Prop :=
  ∀ s, String.HasUnderscoreDigitSuffix s → s ∉ σ.stringGens →
...
theorem GenFreshStore.gen_slot_none {P : PureExpr} [HasIdent P]
    ...
    (pf : String) (h_fresh : GenFreshStore σ σ_tgt) (hwf : StringGenState.WF σ) :
    σ_tgt (HasIdent.ident (P := P) (StringGenState.gen pf σ).1) = none :=
  h_fresh _ (StringGenState.gen_hasUnderscoreDigitSuffix pf σ)
    (StringGenState.stringGens_gen_not_in pf σ hwf)
...
theorem GenFreshStore.storeWith_gen {P : PureExpr} [HasIdent P] [DecidableEq P.Ident]
    [LawfulHasIdent P]
    {σ : StringGenState} {σ_tgt : SemanticStore P}
    (pf : String) (b : P.Expr) (h_fresh : GenFreshStore σ σ_tgt) :
    GenFreshStore (StringGenState.gen pf σ).2
...
theorem GenFreshStore.mono {P : PureExpr} [HasIdent P]
    ...
    (h_fresh : GenFreshStore σ σ_tgt) :
    GenFreshStore σ' σ_tgt := by
  intro s h_suf h_nin
...
only for strings `s` that are *not* gen-shaped (no underscore-digit suffix). -/
...
ever defines or modifies a gen-shaped variable. -/
@[expose] def SrcNoGenWrites {P : PureExpr} [HasIdent P] [HasVarsPure P P.Expr]
    (ss : List (Stmt P (Cmd P))) : Prop :=
  NoGenSuffix (P := P) (Block.definedVars ss ++ Block.modifiedVars ss)

/-- A single `EvalCmd` whose command writes no gen-shaped variable preserves the
"no gen-shaped slot is defined" invariant on its store. -/
theorem evalCmd_preserves_src_fresh {P : PureExpr} [HasFvar P] [HasBool P]
    ...
    {δ : SemanticEval P} {σ σ' : SemanticStore P} {c : Cmd P} {haf : Bool}
    (h : EvalCmd P δ σ c σ' haf)
    (h_src_fresh : ∀ s, String.HasUnderscoreDigitSuffix s →
      σ (HasIdent.ident (P := P) s) = none)
    (h_no_writes : NoGenSuffix (P := P) (Cmd.definedVars c ++ Cmd.modifiedVars c)) :
...
@[expose] def AgreeOffGen {P : PureExpr} [HasIdent P]
  ...
  ∀ x : P.Ident,
    (∀ s : String, x = HasIdent.ident (P := P) s → ¬ String.HasUnderscoreDigitSuffix s) →
    σ_tgt x = σ_src x

/-- `AgreeOffGen` is reflexive. -/
theorem AgreeOffGen.refl {P : PureExpr} [HasIdent P] (σ : SemanticStore P) :
    AgreeOffGen σ σ := fun _ _ => rfl
...
theorem AgreeOffGen.storeWith_gen {P : PureExpr} [HasIdent P] [LawfulHasIdent P]
    ...
    (h_off : AgreeOffGen σ_src σ_tgt)
    (h_gen : String.HasUnderscoreDigitSuffix g) :
    AgreeOffGen σ_src (storeWith σ_tgt (HasIdent.ident (P := P) g) b) := by
  intro x h_nongen
...
theorem cmd_replay_agreement_offgen {P : PureExpr} [HasFvar P] [HasBool P] [HasNot P]
    ...
    (h_off : AgreeOffGen ρ_src.store ρ_tgt.store)
    ...
    (h_tgt_fresh : GenFreshStore σ ρ_tgt.store)
    (h_no_writes : NoGenSuffix (P := P) (Cmd.definedVars c ++ Cmd.modifiedVars c))
    ...
    (∀ s, String.HasUnderscoreDigitSuffix s →
      ...
      ∧ ∃ ρ_tgt', StepStmtStar P (EvalCmd P) extendEval
        ...
        ∧ AgreeOffGen ρ_src'.store ρ_tgt'.store
        ∧ ρ_tgt'.hasFailure = ρ_src'.hasFailure
        ∧ ρ_tgt'.eval = ρ_src'.eval
        ∧ GenFreshStore σ ρ_tgt'.store := by
  ...
  have h_src'_fresh : ∀ s, String.HasUnderscoreDigitSuffix s →
...
private theorem nondetElim_loop_det_sim_iteration {P : PureExpr} [HasFvar P] [HasNot P]
    ...
    (h_body_sim : ∀ (oc_b : Option String) (ρb_src ρb' ρb_tgt : Env P),
      ...
      AgreeOffGen ρb_src.store ρb_tgt.store →
      ...
      StringGenState.WF σ →
      (∀ t, String.HasUnderscoreDigitSuffix t →
        ρb_src.store (HasIdent.ident (P := P) t) = none) →
      GenFreshStore σ ρb_tgt.store →
      ...
      (∀ t, String.HasUnderscoreDigitSuffix t →
        ...
        ∧ ∃ ρb_out, StepStmtStar P (EvalCmd P) extendEval
          ...
          ∧ AgreeOffGen ρb'.store ρb_out.store
          ∧ ρb_out.hasFailure = ρb'.hasFailure
          ∧ ρb_out.eval = ρb'.eval
          ∧ GenFreshStore σ_out ρb_out.store)
    ...
    (h_eval_eq : ρ_tgt.eval = ρ_src.eval)
    (h_fail_eq : ρ_tgt.hasFailure = ρ_src.hasFailure)
    (h_agree : AgreeOffGen ρ_src.store ρ_tgt.store)
    ...
    (h_src_fresh : ∀ t, String.HasUnderscoreDigitSuffix t →
    ...
    (∀ t, String.HasUnderscoreDigitSuffix t →
      ...
      ∧ ∃ ρ_out, StepStmtStar P (EvalCmd P) extendEval
        ...
        ∧ AgreeOffGen ρ'.store ρ_out.store
        ∧ ρ_out.hasFailure = ρ'.hasFailure
        ∧ ρ_out.eval = ρ'.eval
        ∧ GenFreshStore σ ρ_out.store := by
  ...
  | succ n ih =>
    ...
    · -- EXIT: guard reads `ff`; loop terminates.  The loop exit reaches
      ...
      | none =>
        ...
        -- Target: guard reads ff too (AgreeOffGen on guard's source-defined vars).
    ...
    · -- ENTER: guard reads `tt`; run one body iteration then recurse (terminal
      ...
      | none =>
        ...
        -- AgreeOffGen survives projecting both stores through their agreeing parents.
        have h_agree_next : AgreeOffGen ρ_src_next.store ρ_tgt_next.store := by
          intro x h_nongen
        ...
        have h_src_fresh_next : ∀ t, String.HasUnderscoreDigitSuffix t →
        ...
        have h_tgt_fresh_next : GenFreshStore σ ρ_tgt_next.store := by
      ...
      | some lbl =>
        ...
        · -- Body-block exits with `lbl`: the body exits, propagated past the loop.
          ...
          · -- AgreeOffGen at the projected stores.
            intro x h_nongen
            show projectStore ρ_tgt.store ρ_inner_tgt.store x
              = projectStore ρ_src.store ρ_inner.store x
            show (if (ρ_tgt.store x).isSome then ρ_inner_tgt.store x else none)
          ...
          · exact h_fail_inner
          · exact h_eval_inner
          · -- GenFreshStore at the projected store: from the parent `h_tgt_fresh`.
            intro s h_suf h_notin
            show projectStore ρ_tgt.store ρ_inner_tgt.store (HasIdent.ident (P := P) s) = none
            show (if (ρ_tgt.store (HasIdent.ident (P := P) s)).isSome
                then ρ_inner_tgt.store (HasIdent.ident (P := P) s) else none) = none
    ...
    (h_body_sim : ∀ (oc_b : Option String) (ρb_src ρb' ρb_tgt : Env P),
      ...
      (∀ t, String.HasUnderscoreDigitSuffix t →
        ...
        ∧ ∃ ρb_out, StepStmtStar P (EvalCmd P) extendEval
          ...
          ∧ ρb_out.eval = ρb'.eval
          ∧ GenFreshStore σ_out ρb_out.store)
    (h_g_gen : String.HasUnderscoreDigitSuffix g)
...
Invariants threaded:
- `AgreeOffGen ρ_src.store ρ_tgt.store`: target = source off the guard slots
  (so a fresh user var stays fresh in the target — the `.cmd`/`init` arm);
- `GenFreshStore σ ρ_tgt.store`: the target store has no *ungenerated* gen-shaped
...
The conclusion strengthens to `AgreeOffGen ρ'.store ρ_out.store` so the inductive
...
private theorem nondetElim_stmt_gen {P : PureExpr} [HasFvar P] [HasNot P]
    ...
    (h_src_fresh : ∀ t, String.HasUnderscoreDigitSuffix t →
      ρ_src.store (HasIdent.ident (P := P) t) = none)
    (h_tgt_fresh : GenFreshStore σ ρ_tgt.store)
    (h_no_writes : NoGenSuffix (P := P) (Stmt.definedVars s ++ Stmt.modifiedVars s))
    ...
    (∀ t, String.HasUnderscoreDigitSuffix t →
      ...
      ∧ ∃ ρ_out, StepStmtStar P (EvalCmd P) extendEval
        ...
        ∧ ρ_out.hasFailure = ρ'.hasFailure
        ∧ ρ_out.eval = ρ'.eval
        ∧ GenFreshStore (Stmt.nondetElimM s σ).2 ρ_out.store := by
  ...
  | .cmd c, h_no_writes, _, _, oc, h_term =>
    ...
    have h_no_writes_c : NoGenSuffix (P := P) (Cmd.definedVars c ++ Cmd.modifiedVars c) := by
  ...
  | .block lbl bss md, h_no_writes, h_nofd, h_lhni, oc, h_term =>
    ...
    | step_block =>
      ...
      have h_mv : Stmt.modifiedVars (P := P) (.block lbl bss md) = Block.modifiedVars bss := by
        with_unfolding_all rfl
      have h_no_writes_bss : SrcNoGenWrites (P := P) bss := by
        show NoGenSuffix (P := P) (Block.definedVars bss ++ Block.modifiedVars bss)
        rw [h_dv, h_mv] at h_no_writes; exact h_no_writes
      ...
      have wrap : ∀ (oc_inner : Option String) (ρ_inner : Env P),
          ...
          (∀ t, String.HasUnderscoreDigitSuffix t →
            ...
            ∧ ∃ ρ_out, StepStmtStar P (EvalCmd P) extendEval
              ...
              ∧ ρ_out.hasFailure = ρ'.hasFailure
              ∧ ρ_out.eval = ρ'.eval
              ∧ GenFreshStore (Stmt.nondetElimM (.block lbl bss md) σ).2 ρ_out.store := by
        ...
        obtain ⟨h_fresh_inner, ρ_out_inner, h_run_inner, h_off_inner, h_fail_inner,
          ...
          nondetElim_simulation_gen extendEval bss σ ρ_src ρ_inner ρ_tgt
        ...
        · -- ρ'.store is fresh: ρ' = ρ_inner projected through ρ_src.store.
          ...
          · rw [if_neg hp]
        · -- AgreeOffGen survives projecting both stores through agreeing parents.
          subst h_ρ'_eq
          intro x h_nongen
        ...
        · -- GenFreshStore at the block output state = (Block.nondetElimM bss σ).2.
  ...
  | .ite (.det e) tss ess md, h_no_writes, h_nofd, h_lhni, oc, h_term =>
    -- The deterministic guard reads the same value in the target (AgreeOffGen on
    ...
    | step_ite_true h_cond hwfb_s =>
      ...
      have h_no_writes_t : SrcNoGenWrites (P := P) tss := by
        intro x hx t heq
      ...
      obtain ⟨h_fresh', ρ_out, h_run, h_off', h_fail', h_eval', h_fresh_out⟩ :=
        nondetElim_simulation_gen extendEval tss σ ρ_src ρ' ρ_tgt
          h_eval_eq h_fail_eq h_agree hwfb hwfv hwf_def hwf_congr hwf_var
    ...
    | step_ite_false h_cond hwfb_s =>
      ...
      have h_tgt_fresh₁ : GenFreshStore (Block.nondetElimM tss σ).2 ρ_tgt.store :=
        GenFreshStore.mono (Block.nondetElimM_genStep tss σ) h_tgt_fresh
      obtain ⟨h_fresh', ρ_out, h_run, h_off', h_fail', h_eval', h_fresh_out⟩ :=
        nondetElim_simulation_gen extendEval ess (Block.nondetElimM tss σ).2 ρ_src ρ' ρ_tgt
          h_eval_eq h_fail_eq h_agree hwfb hwfv hwf_def hwf_congr hwf_var
  ...
  | .ite .nondet tss ess md, h_no_writes, h_nofd, h_lhni, oc, h_term =>
    ...
    have h_g_gen : String.HasUnderscoreDigitSuffix g := by
      have := StringGenState.gen_hasUnderscoreDigitSuffix ndelimItePrefix σ
      rw [hgen] at this; exact this
    have h_tgt_g_none : ρ_tgt.store (HasIdent.ident (P := P) g) = none := by
      have := GenFreshStore.gen_slot_none ndelimItePrefix h_tgt_fresh h_wf_gen
      rw [hgen] at this; exact this
    have hwf_var_t : WellFormedSemanticEvalVar ρ_tgt.eval := h_eval_eq ▸ hwf_var
    ...
    · -- True branch fired: choose havoc value tt.
      ...
      have h_fresh_g : GenFreshStore σ₁
        ...
        rw [hgen] at this; exact this
      obtain ⟨h_fresh', ρ_out, h_run, h_off', h_fail', h_eval', h_fresh_out⟩ :=
        nondetElim_simulation_gen extendEval tss σ₁
      ...
      · simp only [Stmt.nondetElimM, hgen]
        ...
        have h_step23 : StringGenState.GenStep σ₂ σ₃ := by
          have := Block.nondetElimM_genStep ess σ₂; rw [h₂] at this; exact this
        have h_fresh_σ₂ : GenFreshStore σ₂ ρ_out.store := by rw [h_eq2]; exact h_fresh_out
    ...
    · -- False branch fired: choose havoc value ff.  The output's else branch is
      ...
      have h_off_g : AgreeOffGen ρ_src.store
        ...
        AgreeOffGen.storeWith_gen h_agree h_g_gen
      have h_fresh_g1 : GenFreshStore σ₁
        ...
        rw [hgen] at this; exact this
      have h_fresh_g : GenFreshStore (Block.nondetElimM tss σ₁).2
          (storeWith ρ_tgt.store (HasIdent.ident (P := P) g) HasBool.ff) :=
        GenFreshStore.mono h_step12 h_fresh_g1
      obtain ⟨h_fresh', ρ_out, h_run, h_off', h_fail', h_eval', h_fresh_out⟩ :=
        nondetElim_simulation_gen extendEval ess (Block.nondetElimM tss σ₁).2
  ...
  | .loop (.det e) m inv body md, h_no_writes, h_nofd, h_lhni, oc, h_term =>
    ...
    have h_nofd_body : Block.noFuncDecl body = true := by
      simpa only [Stmt.noFuncDecl] using h_nofd
    have h_no_writes_body : SrcNoGenWrites (P := P) body := by
      ...
      show NoGenSuffix (P := P) (Block.definedVars body ++ Block.modifiedVars body)
      rw [h_dv, h_mv] at h_no_writes; exact h_no_writes
    ...
    have h_body_sim : ∀ (oc_b : Option String) (ρb_src ρb' ρb_tgt : Env P),
        ...
        (∀ t, String.HasUnderscoreDigitSuffix t →
          ...
          ∧ ∃ ρb_out, StepStmtStar P (EvalCmd P) extendEval
            ...
            ∧ ρb_out.hasFailure = ρb'.hasFailure
            ∧ ρb_out.eval = ρb'.eval
            ∧ GenFreshStore (Block.nondetElimM body σ).2 ρb_out.store :=
      fun oc_b ρb_src ρb' ρb_tgt h_ev h_fl h_ag hb hv hd hc hvar hwfg hsf htf hrun =>
        nondetElim_simulation_gen extendEval body σ ρb_src ρb' ρb_tgt
  ...
  | .loop .nondet m inv body md, h_no_writes, h_nofd, h_lhni, oc, h_term =>
    ...
    have h_g_gen : String.HasUnderscoreDigitSuffix g := by
      have := StringGenState.gen_hasUnderscoreDigitSuffix ndelimLoopPrefix σ
      rw [hgen] at this; exact this
    ...
    have h_wf₁ : StringGenState.WF σ₁ := h_step01.wf_mono h_wf_gen
    have h_tgt_g_none : ρ_tgt.store (HasIdent.ident (P := P) g) = none := by
      have := GenFreshStore.gen_slot_none ndelimLoopPrefix h_tgt_fresh h_wf_gen
      rw [hgen] at this; exact this
    have hwf_var_t : WellFormedSemanticEvalVar ρ_tgt.eval := h_eval_eq ▸ hwf_var
    ...
    have finish : ∀ (entering : Bool) (b : P.Expr)
        ...
        (∀ t, String.HasUnderscoreDigitSuffix t →
          ...
          ∧ ∃ ρ_out, StepStmtStar P (EvalCmd P) extendEval
            ...
            ∧ ρ_out.hasFailure = ρ'.hasFailure
            ∧ ρ_out.eval = ρ'.eval
            ∧ GenFreshStore (Stmt.nondetElimM (.loop .nondet m ([] : List (String × P.Expr)) body md) σ).2 ρ_out.store := by
...
private theorem nondetElim_simulation_gen {P : PureExpr} [HasFvar P] [HasNot P]
    ...
    (h_src_fresh : ∀ t, String.HasUnderscoreDigitSuffix t →
      ρ_src.store (HasIdent.ident (P := P) t) = none)
    (h_tgt_fresh : GenFreshStore σ ρ_tgt.store)
    (h_no_writes : SrcNoGenWrites (P := P) ss)
    ...
    (∀ t, String.HasUnderscoreDigitSuffix t →
      ...
      ∧ ∃ ρ_out, StepStmtStar P (EvalCmd P) extendEval
        ...
        ∧ ρ_out.hasFailure = ρ'.hasFailure
        ∧ ρ_out.eval = ρ'.eval
        ∧ GenFreshStore (Block.nondetElimM ss σ).2 ρ_out.store := by
  ...
  | s :: rest, h_no_writes, h_nofd, oc, h_term =>
    ...
    · -- Head exits: recurse on `s` with the exiting outcome, then `step_seq_exit`
      ...
      have h_no_writes_s_e : NoGenSuffix (P := P) (Stmt.definedVars s ++ Stmt.modifiedVars s) := by
        intro x hx t heq
        rcases List.mem_append.mp hx with hd | hm
      ...
      obtain ⟨h_fresh', ρ_out, h_s_tgt, h_off', h_fail', h_eval', h_fresh_s⟩ :=
        nondetElim_stmt_gen extendEval s σ ρ_src ρ' ρ_tgt
          h_eval_eq h_fail_eq h_agree hwfb hwfv hwf_def hwf_congr hwf_var
      ...
      · -- GenFreshStore at `(Block.nondetElimM (s :: rest) σ).2 ⊇ (Stmt.nondetElimM s σ).2`.
    ...
    · -- Head terminates: original sequencing argument, now threading the outcome.
      have h_s_run' : StepStmtStar P (EvalCmd P) extendEval (.stmt s ρ_src) (.terminal ρ_mid) :=
        h_s_run
      -- Source-shape splits.
      have h_no_writes_s : NoGenSuffix (P := P) (Stmt.definedVars s ++ Stmt.modifiedVars s) := by
        intro x hx t heq
        rcases List.mem_append.mp hx with hd | hm
        · exact h_no_writes x (List.mem_append_left _ (List.mem_append_left _ hd)) t heq
        · exact h_no_writes x (List.mem_append_right _ (List.mem_append_left _ hm)) t heq
      have h_no_writes_rest : SrcNoGenWrites (P := P) rest := by
        intro x hx t heq
        rcases List.mem_append.mp hx with hd | hm
      ...
      obtain ⟨h_mid_fresh, ρ_mid_tgt, h_s_tgt, h_off_mid, h_fail_mid, h_eval_mid, h_fresh_mid⟩ :=
        nondetElim_stmt_gen extendEval s σ ρ_src ρ_mid ρ_tgt
          h_eval_eq h_fail_eq h_agree hwfb hwfv hwf_def hwf_congr hwf_var
      ...
      obtain ⟨h_fresh', ρ_out, h_rest_tgt, h_off', h_fail', h_eval', h_fresh_out⟩ :=
        nondetElim_simulation_gen extendEval rest (Stmt.nondetElimM s σ).2 ρ_mid ρ' ρ_mid_tgt
...
private theorem nondetElim_simulation {P : PureExpr} [HasFvar P] [HasNot P]
  ...
  have h_tgt_fresh : GenFreshStore StringGenState.emp ρ₀.store := by
    intro s h_suf _; exact h_no_gen_suffix s h_suf
  obtain ⟨h_fresh', ρ_out, h_run, h_off, h_fl, _, _⟩ :=
    nondetElim_simulation_gen extendEval ss StringGenState.emp ρ₀ ρ' ρ₀
...
theorem nondetElim_sound {P : PureExpr} [HasFvar P] [HasNot P]
    ...
    ∃ ρ_out, StepStmtStar P (EvalCmd P) extendEval
      ...
      ∧ StoreAgreement ρ'.store ρ_out.store
      ∧ ρ_out.hasFailure = ρ'.hasFailure :=
  nondetElim_simulation extendEval ss ρ₀ ρ'
