...
private def mkAdtRankLt
  ...
  let rank (t: LMonoTy) (e: Expression.Expr) : Expression.Expr :=
    .app termMeta (.op termMeta (adtRankFuncName (adtNameOf t)) (.some (.arrow t .int))) e
  let ltTy : LMonoTy := LMonoTy.arrow .int (LMonoTy.arrow .int .bool)
  LExpr.mkApp termMeta (.op termMeta "Int.Lt" (.some ltTy)) [rank calleeAdtTy callArg, rank callerAdtTy (.fvar termMeta callerParam (.some callerAdtTy))]
  ...
  go (e : Expression.Expr) (implications : List (ExpressionMetadata × Expression.Expr))
    ...
    | .ite _ c t f => do
      ...
      let notC : Expression.Expr :=
        LExpr.mkApp termMeta (.op termMeta "Bool.Not" (.some (LMonoTy.arrow .bool .bool))) [c]
