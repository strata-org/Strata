...
-- nosourcerange-file: variable references synthesized during inlining (fresh names, output copies)
-- carry ExprSourceLoc.none because they are generated by the transform, not parsed from source.
-/
...
private def renameAllLocalNames (c:Procedure)
  ...
  let new_body := List.map (fun (s0:Statement) =>
    var_map.foldl (fun (s:Statement) (old_id,new_id) =>
        let s := Statement.substFvar s old_id (.fvar ExprSourceLoc.none new_id .none)
    ...
    match cmd with
      | .call procName callArgs md =>
        ...
        let outputSetStmts :=
          ...
          List.map
            (fun (lhs_var,out_var) =>
              Statement.set lhs_var (.fvar ExprSourceLoc.none out_var (.none)) md)
            outs_lhs_and_sig
