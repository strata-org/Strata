...
private theorem PrefixStepsOK_det_init_cons
    ...
    (h_ne : oldG ≠ id) :
    PrefixStepsOK π φ
      (Statement.init oldG ty (.det (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none)) #[] :: rest) ρ := by
  constructor
  · exact h_rest
  · refine ⟨_, rfl, (prefixInitEnv rest ρ).store, ?_, rfl⟩
    have h_none : (prefixInitEnv (Statement.init oldG ty (.det (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none)) #[] :: rest) ρ).store oldG = none :=
      prefixInitEnv_store_init _ _ _ _ rfl
    have h_id_val : (prefixInitEnv (Statement.init oldG ty (.det (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none)) #[] :: rest) ρ).store id =
    ...
    have h_getFvar : HasFvar.getFvar (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none : Expression.Expr) = some id := by
      simp [HasFvar.getFvar]
    have h_eval : ρ.eval (prefixInitEnv (Statement.init oldG ty (.det (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none)) #[] :: rest) ρ).store
        (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none) = some v := by
    ...
    : PrefixStepsOK π φ
        (entries.map fun (id, ty) =>
          Statement.init (CoreIdent.mkOld id.name) (Lambda.LTy.forAll [] ty)
            (.det (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none)) #[]) ρ := by
  induction entries with
  | nil => exact trivial
  | cons e rest ih =>
    obtain ⟨id, ty⟩ := e
...
theorem procToVerifyStmt_structure
  ...
  let oldInoutInits := proc.header.getInoutParams.toList.map fun (id, ty) =>
    Statement.init (CoreIdent.mkOld id.name) (Lambda.LTy.forAll [] ty)
      (.det (LExpr.fvar (ExprSourceLoc.synthesized "proc-body-verify") id none)) #[]
...
theorem procBodyVerify_procedureCorrect
  ...
  · ----- Part 2: Postconditions + hasFailure on termination -----
    ...
    intro h_wf_proc ρ₀ h_wf σ' δ' failed h_body_exec
