...
/-! ## Abbreviations -/
abbrev StepDetCFGStar {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P] [HasFvars P]
    (extendEval : ExtendEval P)
    (cfg : CFG String (DetBlock String (Cmd P) P)) :=
  @StepCFGStar String (Cmd P) _ P (EvalCmd P) extendEval _ _ cfg

theorem StepDetCFGStar_trans {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P] [HasFvars P]
    {extendEval : ExtendEval P}
    {cfg : CFG String (DetBlock String (Cmd P) P)}
...
theorem EvalCmds_to_StepCFG_chain {P : PureExpr} [HasFvar P] [HasBoolOps P] [HasFvars P]
    {extendEval : ExtendEval P}
    {cfg : CFG String (DetBlock String (Cmd P) P)}
...
theorem run_block_goto_true {P : PureExpr} [HasFvar P] [HasBoolOps P] [HasFvars P]
    {extendEval : ExtendEval P}
    {cfg : CFG String (DetBlock String (Cmd P) P)}
    {δ : SemanticEval P} {σ σ' : SemanticStore P}
...
theorem run_block_goto_false {P : PureExpr} [HasFvar P] [HasBoolOps P] [HasFvars P]
    {extendEval : ExtendEval P}
    {cfg : CFG String (DetBlock String (Cmd P) P)}
    {δ : SemanticEval P} {σ σ' : SemanticStore P}
    {cs : List (Cmd P)} {c : P.Expr} {tlbl elbl : String} {md : MetaData P}
...
+ finish. -/
theorem run_block_finish {P : PureExpr} [HasFvar P] [HasBoolOps P] [HasFvars P]
    {extendEval : ExtendEval P}
    {cfg : CFG String (DetBlock String (Cmd P) P)}
    {δ : SemanticEval P} {σ σ' : SemanticStore P}
...
theorem stmts_nil_terminal {P : PureExpr} [HasBool P] [HasBoolOps P]
    {CmdT : Type}
    (EvalCmdR : EvalCmdParam P CmdT)
    (extendEval : ExtendEval P)
    (ρ₀ ρ' : Env P)
...
theorem EvalCmds_snoc {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P] [HasFvars P]
...
theorem EvalCmds_inv {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P] [HasFvars P]
...
private theorem store_agreement_pointwise_on_expr_vars
    {P : PureExpr} [HasFvars P]
    (σ_struct σ_cfg : SemanticStore P) (e : P.Expr)
    (h_agree : StoreAgreement σ_struct σ_cfg)
    (h_def : isDefined σ_struct (HasFvars.getFvars e)) :
    ∀ x ∈ HasFvars.getFvars e, σ_struct x = σ_cfg x := by
  intro x hx
...
private theorem EvalCmd_under_agreement {P : PureExpr}
    [HasFvar P] [HasBool P] [HasBoolOps P] [HasFvars P] [DecidableEq P.Ident]
  ...
  | eval_init heval hinit hwfvar hwfcongr =>
    ...
    have h_def_e : isDefined σ_struct₀ (HasFvars.getFvars e) :=
      h_wf_def e v σ_struct₀ heval
    have h_pointwise :
        ∀ y ∈ HasFvars.getFvars e, σ_struct₀ y = σ_cfg₀ y :=
...
theorem single_cmd_eval {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P] [HasFvars P]
    (extendEval : ExtendEval P)
    (c : Cmd P) (ρ₀ ρ₁ : Env P)
...
private theorem StepStmtStar_wfb_preserved {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    [HasIdent P] [HasFvars P] [HasOps P] [HasInt P] [HasIntOps P]
    (extendEval : ExtendEval P)
...
private theorem StepStmtStar_wfv_preserved {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    [HasIdent P] [HasFvars P] [HasOps P] [HasInt P] [HasIntOps P]
    (extendEval : ExtendEval P)
    (ss : List (Stmt P (Cmd P))) (ρ₀ ρ' : Env P)
    (h : StepStmtStar P (EvalCmd P) extendEval (.stmts ss ρ₀) (.terminal ρ'))
...
private theorem flushCmds_condGoto_agree {P : PureExpr} [HasFvar P] [HasBoolOps P]
    [HasFvars P]
    (extendEval : ExtendEval P)
    (b : Bool)
    (accum : List (Cmd P))
  ...
  have h_def_e : isDefined ρ₀.store (HasFvars.getFvars e) :=
    h_wf_def e _ ρ₀.store h_cond
  have h_pointwise :
      ∀ y ∈ HasFvars.getFvars e, ρ₀.store y = σ_cfg_after y :=
...
/-- Inversion of a `.none`-labelled block reaching terminal (`StepStmtStar`
variant).  An anonymous block (the wrapper introduced by `ite`/loop bodies)
never catches an exit (`step_block_exit_match` requires `.some`), so the only
way it reaches `.terminal` is via its inner body terminating; the resulting
store is projected and the evaluator restored to `e_parent`. -/
private theorem block_none_reaches_terminal {P : PureExpr} {CmdT : Type}
    [HasBool P] [HasBoolOps P]
    {EvalCmd : EvalCmdParam P CmdT} {extendEval : ExtendEval P}
  ...
  suffices h_gen : ∀ src tgt, StepStmtStar P EvalCmd extendEval src tgt →
      ∀ inner ρ', src = .block .none σ_parent e_parent inner → tgt = .terminal ρ' →
      ∃ ρ_inner, StepStmtStar P EvalCmd extendEval inner (.terminal ρ_inner) ∧
        ρ' = { ρ_inner with store := projectStore σ_parent ρ_inner.store, eval := e_parent } from
  ...
  | step _ mid _ hstep hrest ih =>
    ...
    cases hstep with
    | step_block_body h =>
      have ⟨ρ_inner, hterm, heq⟩ := ih _ _ rfl htgt
      exact ⟨ρ_inner, .step _ _ _ h hterm, heq⟩
    | step_block_done =>
...
private theorem store_no_gens_lift_after_accum {P : PureExpr}
    [HasFvar P] [HasBool P] [HasBoolOps P] [HasFvars P] [HasIdent P] [DecidableEq P.Ident]
...
private theorem store_no_gens_upper_lift_through_subsim {P : PureExpr}
    ...
    (h_sub_no_gen_suffix : NoGenSuffix (P := P) sub_init) :
    ∀ x : String,
        String.HasUnderscoreDigitSuffix x →
        x ∉ StringGenState.stringGens genUpperBound →
...
private theorem cmd_arm_combined_lemmas {P : PureExpr}
    ...
    (h_no_d  : NoGenSuffix (P := P) (Cmds.definedVars accum.reverse ++ Block.initVars (.cmd c :: rest)))
    (h_no_m  : NoGenSuffix (P := P) (Cmds.modifiedVars accum.reverse ++ transformBlockModVars (.cmd c :: rest))) :
    Cmds.definedVars (accum.reverse ++ [c]) = Cmds.definedVars accum.reverse ++ Cmd.definedVars c
      ...
      ∧ (NoGenSuffix (P := P) (Cmds.definedVars (c :: accum).reverse ++ Block.initVars rest))
      ∧ (NoGenSuffix (P := P) (Cmds.modifiedVars (c :: accum).reverse ++ transformBlockModVars rest)) := by
...
private theorem fresh_rest_inits_body_step {P : PureExpr} [HasIdent P]
    ...
    (h_preserve_body : ∀ x : P.Ident,
        ...
        (∀ s : String, x = HasIdent.ident (P := P) s →
            ...
            s ∉ StringGenState.stringGens gen_b) →
        σ_cfg_body x = none)
    (h_wf_b : StringGenState.WF gen_b)
    (h_rest_no_gen_suffix :
        NoGenSuffix (P := P) (Cmds.definedVars ([] : List (Cmd P)).reverse ++ Block.initVars rest))
...
private theorem typeDecl_arm_combined_lemmas {P : PureExpr}
    ...
    (h_no_d  : NoGenSuffix (P := P) (Cmds.definedVars accum.reverse ++ Block.initVars (.typeDecl tc md :: rest)))
    (h_no_m  : NoGenSuffix (P := P) (Cmds.modifiedVars accum.reverse ++ transformBlockModVars (.typeDecl tc md :: rest))) :
    (∀ x ∈ Cmds.definedVars accum.reverse ++ Block.initVars rest, σ_base x = none)
      ...
      ∧ (NoGenSuffix (P := P) (Cmds.definedVars accum.reverse ++ Block.initVars rest))
      ∧ (NoGenSuffix (P := P) (Cmds.modifiedVars accum.reverse ++ transformBlockModVars rest)) := by
...
private theorem seqT_reaches_terminal' {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    [HasFvars P]
    (extendEval : ExtendEval P)
...
private theorem stmtsT_cons_terminal' {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    [HasFvars P]
    (extendEval : ExtendEval P)
...
private theorem seqT_reaches_exiting' {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    [HasFvars P]
    (extendEval : ExtendEval P)
...
private theorem stmtsT_cons_exiting' {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    [HasFvars P]
    (extendEval : ExtendEval P)
    {s : Stmt P (Cmd P)} {rest : List (Stmt P (Cmd P))}
...
theorem loop_det_decompose_h_gen
    {P : PureExpr} [HasFvar P] [HasBoolOps P]
    [HasIdent P] [HasFvars P] [HasOps P] [HasInt P] [HasIntOps P] [DecidableEq P.Ident]
...
private theorem lentry_condGoto {P : PureExpr} [HasFvar P] [HasBoolOps P]
    [HasFvars P]
    (extendEval : ExtendEval P)
    (b : Bool)
    (cfg : CFG String (DetBlock String (Cmd P) P))
  ...
  have h_def_g : isDefined σ_struct (HasFvars.getFvars g) :=
    h_wf_def g _ σ_struct h_cond
  have h_pointwise : ∀ y ∈ HasFvars.getFvars g, σ_struct y = σ_cfg y :=
...
private theorem peel_off_one_iteration_det {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    ...
    ∃ (ρ_inner : Env P) (ρ_block : Env P),
      StepStmtStar P (EvalCmd P) extendEval
        (.stmts body { ρ_pre with hasFailure := ρ_pre.hasFailure || false })
        (.terminal ρ_inner) ∧
      ρ_block = ({ ρ_inner with store := projectStore ρ_pre.store ρ_inner.store, eval := ρ_pre.eval } : Env P) ∧
...
private theorem peel_off_one_iteration_to_cont_det {P : PureExpr} [HasFvar P] [HasBool P] [HasBoolOps P]
    ...
    (∃ ρ_body_exit,
      ...
      ρ_post_loop = { ρ_body_exit with
        store := projectStore ρ_pre.store ρ_body_exit.store, eval := ρ_pre.eval }) ∨
    (∃ (ρ_inner : Env P) (ρ_block : Env P),
      StepStmtStar P (EvalCmd P) extendEval
        (.stmts body { ρ_pre with hasFailure := ρ_pre.hasFailure || false })
        (.terminal ρ_inner) ∧
...
private theorem stmtsToBlocks_simulation {P : PureExpr} [HasFvar P] [HasBoolOps P]
    [HasIdent P] [HasFvars P] [HasOps P] [HasInt P] [HasIntOps P]
    [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P]
    [LawfulHasIntOrder P] [LawfulHasNot P]
    ...
    (h_combined_no_gen_suffix :
        NoGenSuffix (P := P) (Cmds.definedVars accum.reverse ++ Block.initVars ss))
    (h_combined_no_gen_suffix_mod :
        NoGenSuffix (P := P) (Cmds.modifiedVars accum.reverse ++ transformBlockModVars ss))
  ...
  | .ite (.det e) thenBranch elseBranch md :: rest =>
    ...
    -- Each branch is now wrapped in a `.block .none ρ₀.store ρ₀.eval (.stmts _ ρ₀)`
    -- (scoping); invert the block to recover the raw branch terminal `ρ_inner`
    -- together with the projection equation for `ρ₁`.
    have h_ite_inv : (∃ ρ_inner, StepStmtStar P (EvalCmd P) extendEval
        ...
        | step_stmts_cons =>
          ...
          | step _ _ _ hstep2 hrest2 =>
            ...
            | step_ite_true h_eval_tt _ =>
              -- `hrest2 : .block .none ρ₀.store ρ₀.eval (.stmts thenBranch ρ₀) → .terminal ρ₁`
              have ⟨ρ_inner, h_term, h_eq⟩ :=
                block_none_reaches_terminal (P := P) (EvalCmd := EvalCmd P) hrest2
              exact Or.inl ⟨ρ_inner, h_term, h_eq, h_eval_tt⟩
            | step_ite_false h_eval_ff _ =>
              have ⟨ρ_inner, h_term, h_eq⟩ :=
                block_none_reaches_terminal (P := P) (EvalCmd := EvalCmd P) hrest2
              exact Or.inr ⟨ρ_inner, h_term, h_eq, h_eval_ff⟩
    ...
    -- Eval well-formedness preservation through ite branch: the branch's block
    -- wrapper restores `ρ₀.eval` on exit, so `ρ₁.eval = ρ₀.eval` directly.
    have h_eval_eq : ρ₁.eval = ρ₀.eval := by
      rcases h_ite_inv with ⟨_, _, h_eq, _⟩ | ⟨_, _, h_eq, _⟩ <;> rw [h_eq]
    ...
    have h_then_no_gen_suffix :
        NoGenSuffix (P := P) (Cmds.definedVars [].reverse ++ Block.initVars thenBranch) := fun x hx s heq =>
    ...
    have h_else_no_gen_suffix :
        NoGenSuffix (P := P) (Cmds.definedVars [].reverse ++ Block.initVars elseBranch) := fun x hx s heq =>
      h_combined_no_gen_suffix x (List.mem_append_right _ (h_initvars_eq ▸
        List.mem_append_left _ (List.mem_append_right _ (by simpa [Cmds.definedVars] using hx)))) s heq
    have h_rest_no_gen_suffix :
        NoGenSuffix (P := P) (Cmds.definedVars [].reverse ++ Block.initVars rest) := fun x hx s heq =>
      h_combined_no_gen_suffix x (List.mem_append_right _ (h_initvars_eq ▸
    ...
    have h_then_no_gen_suffix_mod :
        NoGenSuffix (P := P) (Cmds.modifiedVars [].reverse ++ transformBlockModVars thenBranch) := fun x hx s heq =>
    ...
    have h_else_no_gen_suffix_mod :
        NoGenSuffix (P := P) (Cmds.modifiedVars [].reverse ++ transformBlockModVars elseBranch) := fun x hx s heq =>
      h_combined_no_gen_suffix_mod x (List.mem_append_right _ (h_modvars_eq ▸
        List.mem_append_left _ (List.mem_append_right _ (by simpa [Cmds.modifiedVars] using hx)))) s heq
    have h_rest_no_gen_suffix_mod :
        NoGenSuffix (P := P) (Cmds.modifiedVars [].reverse ++ transformBlockModVars rest) := fun x hx s heq =>
      h_combined_no_gen_suffix_mod x (List.mem_append_right _ (h_modvars_eq ▸
        List.mem_append_right _ (by simpa [Cmds.modifiedVars] using hx))) s heq
    rcases h_ite_inv with h_true | h_false
    · obtain ⟨ρ_then_inner, h_then_term, h_ρ₁_eq, h_cond_tt⟩ := h_true
      ...
      -- Recurse on thenBranch (terminating at the raw inner env `ρ_then_inner`).
      ...
      -- Bridge the raw-inner store agreement through the block projection
      -- to the post-ite env `ρ₁`.
      have h_agree_then : StoreAgreement ρ₁.store σ_branch :=
        StoreAgreement.through_projectStore h_ρ₁_eq h_agree_then_inner
      ...
      have h_fresh_rest_inits_branch :
        ...
        exact h_preserve_then x h_σ_after_x this h_x_not_then
          (fun s heq => Or.inr
            (StringGenState.not_mem_stringGens_of_not_hasUnderscoreDigitSuffix h_wf_t
              (h_rest_no_gen_suffix x (by simp [Cmds.definedVars]; exact hx) s heq)))
    ...
    · obtain ⟨ρ_else_inner, h_else_term, h_ρ₁_eq, h_cond_ff⟩ := h_false
      ...
      have h_flush_sim : StepDetCFGStar extendEval cfg
        ...
        flushCmds_condGoto_agree extendEval false accum e tl fl md l_ite gen_e gen_f
          ...
          h_cfg_accum h_lookup
      -- Recurse on elseBranch (terminating at the raw inner env `ρ_else_inner`).
      ...
      have h_agree_else : StoreAgreement ρ₁.store σ_branch :=
        StoreAgreement.through_projectStore h_ρ₁_eq h_agree_else_inner
      -- Freshness of rest's inits at σ_branch.
      have h_fresh_rest_inits_branch :
          ∀ x ∈ Block.initVars rest, σ_branch x = none := by
        intro x hx
        ...
        exact h_preserve_else x h_σ_after_x this h_x_not_else
          (fun s heq => Or.inr
            (StringGenState.not_mem_stringGens_of_not_hasUnderscoreDigitSuffix h_wf_e
              (h_rest_no_gen_suffix x (by simp [Cmds.definedVars]; exact hx) s heq)))
      -- Combined freshness for rest recursion.
      have h_combined_rest :
          ∀ x ∈ Cmds.definedVars [].reverse ++ Block.initVars rest,
  ...
  | .loop guard measure invariants body md :: rest =>
    ...
    have h_body_no_gen_suffix :
        NoGenSuffix (P := P) (Cmds.definedVars [].reverse ++ Block.initVars body) := by
    ...
    have h_body_no_gen_suffix_mod :
        NoGenSuffix (P := P) (Cmds.modifiedVars [].reverse ++ transformBlockModVars body) :=
    ...
    have h_body_sim_at :
      ...
      have h_sng_iter : ∀ x : String, String.HasUnderscoreDigitSuffix x →
          x ∉ StringGenState.stringGens genUpperBound →
      ...
      have ⟨σ_cfg_after_body, h_step_body, h_agree_body, h_preserve_body⟩ :=
        stmtsToBlocks_simulation extendEval lentry body ((.none, kNext) :: exitConts) []
          ...
          genUpperBound h_outer_upper_b h_sng_iter
    ...
    have h_rest_no_gen_suffix_mod :
        NoGenSuffix (P := P) (Cmds.modifiedVars [].reverse ++ transformBlockModVars rest) :=
      fun x hx s heq => h_combined_no_gen_suffix_mod x
        (List.mem_append_right _ (by
          rw [transformBlockModVars_cons, transformStmtModVars_loop]
    ...
    have ⟨σ_cfg, h_rest_sim, h_agree_rest, h_preserve_rest⟩ :=
      stmtsToBlocks_simulation extendEval k rest exitConts [] gen gen_r kNext bsRest
        ...
        h_wf_gen h_rest_no_gen_suffix h_rest_no_gen_suffix_mod
        genUpperBound h_outer_upper_r h_sng_loop
  ...
  | .block label body md :: rest =>
    ...
    have h_block_inv :
        ...
        (∃ ρ_inner, StepStmtStar P (EvalCmd P) extendEval
          ...
          ρ_blk = { ρ_inner with store := projectStore ρ₀.store ρ_inner.store, eval := ρ₀.eval }) := by
    ...
    have h_body_no_gen_suffix :
        NoGenSuffix (P := P) (Cmds.definedVars [].reverse ++ Block.initVars body) := fun x hx s heq =>
      h_combined_no_gen_suffix x (List.mem_append_right _ (h_initvars_eq ▸
    ...
    have h_body_no_gen_suffix_mod :
        NoGenSuffix (P := P) (Cmds.modifiedVars [].reverse ++ transformBlockModVars body) := fun x hx s heq =>
      h_combined_no_gen_suffix_mod x (List.mem_append_right _ (h_modvars_eq ▸
    ...
    · -- Case l = bl: blocks = accumBlocks ++ bbs ++ bsNext, entry = accumEntry.
      ...
      · -- Body terminates with ρ_inner; use that for the sim.
        ...
        -- Eval well-formedness preservation through body: the block wrapper
        -- restores `ρ₀.eval` on exit, so this holds directly from `h_ρ_blk_eq`.
...
private theorem stmtsToBlocks_simulation_to_cont {P : PureExpr} [HasFvar P] [HasBoolOps P]
    [HasIdent P] [HasFvars P] [HasOps P] [HasInt P] [HasIntOps P]
    [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P]
    [LawfulHasIntOrder P] [LawfulHasNot P]
    ...
    have h_body_sim_at :
      ...
      have ⟨σ_cfg_after_body, h_step_body, h_agree_body, h_preserve_body⟩ :=
        stmtsToBlocks_simulation extendEval lentry body ((.none, kNext) :: exitConts) []
          ...
          h_combined_body h_unique_combined_body h_hf_iter
          h_wf_le h_body_no_gen_suffix h_body_no_gen_suffix_mod
          genUpperBound h_outer_upper_b (h_sng_of_inv σ_cfg_iter h_inv_iter)
          cfg h_cfg_bbs h_cfg_nodup
      refine ⟨σ_cfg_after_body, h_step_body, h_agree_body, ?_⟩
      intro x h_keep h_after_x
      have h_iter_x : σ_cfg_iter x = none := h_inv_iter x h_keep h_after_x
...
private theorem end_block_terminal {P : PureExpr} [HasFvar P] [HasBoolOps P] [HasFvars P]
    (extendEval : ExtendEval P)
    (cfg : CFG String (DetBlock String (Cmd P) P))
...
theorem stmtsToCFG_terminal {P : PureExpr} [HasFvar P] [HasBoolOps P]
    [HasIdent P] [HasFvars P] [HasOps P] [HasInt P] [HasIntOps P]
    [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P]
    [LawfulHasIntOrder P] [LawfulHasNot P]
    ...
    (h_input_no_gen_suffix : NoGenSuffix (P := P) (Block.initVars ss))
    (h_input_no_gen_suffix_mod : NoGenSuffix (P := P) (transformBlockModVars ss))
    (h_term : StepStmtStar P (EvalCmd P) extendEval
      (.stmts ss ρ₀) (.terminal ρ')) :
  ...
  have h_combined_no_gen_suffix :
      NoGenSuffix (P := P) (Cmds.definedVars [].reverse ++ Block.initVars ss) := by
    intro x hx s heq
    simp [Cmds.definedVars] at hx
    exact h_input_no_gen_suffix x hx s heq
  have h_combined_no_gen_suffix_mod :
      NoGenSuffix (P := P) (Cmds.modifiedVars [].reverse ++ transformBlockModVars ss) := by
    intro x hx s heq
...
theorem structuredToUnstructured_sound {P : PureExpr} [HasFvar P] [HasBoolOps P]
    [HasIdent P] [HasFvars P] [HasOps P] [HasInt P] [HasIntOps P]
    [DecidableEq P.Ident]
    [LawfulHasFvar P] [LawfulHasBool P] [LawfulHasIdent P]
    [LawfulHasIntOrder P] [LawfulHasNot P]
