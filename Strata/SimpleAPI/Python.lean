...
public import Strata.Languages.Python.Specs
...
public def readDispatchOverloads (specRoot : System.FilePath)
...
public def readPythonIonModule (strataPath : String) (bytes : ByteArray) : Except String PythonStmts :=
  PythonStmts.mk <$> Python.readPythonStrataBytes strataPath bytes
...
/-- Translate a combined Laurel program (from `pyAnalyzeLaurel`)
...
public instance : ToString PyAnalyzeError where
  toString
    | .userCode _ msg => s!"User code error: {msg}"
    | .internal msg => msg
...
/-- Derive a ModuleName and spec root from a `.pyspec.st.ion` file path. -/
private def ofIonFile (ionFile : System.FilePath)
    : Except String (Python.Specs.ModuleName × System.FilePath) := do
  ...
  pure (← Python.Specs.ModuleName.ofString modName, root)
