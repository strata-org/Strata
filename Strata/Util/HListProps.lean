...
/-!
# Properties of heterogeneous lists (`HList`)

## Key theorems

* `HList.get_cons_zero`, `HList.get_cons_succ` — `get` reduces on `cons`
* `HList.cast_cast` — `cast` composes
* `HList.get_cast` — `get` commutes with `cast`
* `HList.get_append_left`, `HList.get_append_cons_self`, `HList.get_append_right`
  — `get` on an appended `HList`
-/
