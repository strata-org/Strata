...
/-- `find?` of an `ofList` over an append: the second list's binding wins (`ofList`
    resolves a key to its last occurrence), else the first's. Mirrors `find?_union`. -/
public theorem find?_ofList_append [LawfulBEq α] [LawfulHashable α]
    (l1 l2 : List (α × β)) (k : α) :
    HMap.find? (HMap.ofList (l1 ++ l2)) k
      = (HMap.find? (HMap.ofList l2) k).or (HMap.find? (HMap.ofList l1) k) := by
  simp only [find?, ofList, Std.HashMap.get?_eq_getElem?, Std.HashMap.ofList_eq_insertMany_empty,
    Std.HashMap.getElem?_insertMany_list, Std.HashMap.getElem?_empty, Option.or_none]
  rw [List.findSomeRev?_eq_findSome?_reverse, List.findSomeRev?_eq_findSome?_reverse,
    List.findSomeRev?_eq_findSome?_reverse, List.reverse_append, List.findSome?_append]
...
/-- A found key/value pair in `HMap.ofList l` is a genuine entry of `l`. Needs no
    distinctness hypothesis: the found value always comes from some list entry. -/
public theorem find?_ofList_mem [LawfulBEq α] [LawfulHashable α]
    (l : List (α × β)) (k : α) (v : β)
    (h : HMap.find? (HMap.ofList l) k = some v) : (k, v) ∈ l := by
  simp only [find?, ofList, Std.HashMap.get?_eq_getElem?,
    Std.HashMap.ofList_eq_insertMany_empty, Std.HashMap.getElem?_insertMany_list,
    Std.HashMap.getElem?_empty, Option.or_none] at h
  rw [List.findSomeRev?_eq_findSome?_reverse] at h
  obtain ⟨p, hp_mem, hp_eq⟩ := List.exists_of_findSome?_eq_some h
  obtain ⟨a, b⟩ := p; simp only at hp_eq
  split at hp_eq
  · rename_i hbeq; injection hp_eq with hbv; subst hbv
    rw [beq_iff_eq] at hbeq; subst hbeq; exact List.mem_reverse.mp hp_mem
  · exact absurd hp_eq (by simp)

/-- `ofList` lookup of a distinct-keyed list returns the paired value. -/
public theorem find?_ofList_of_mem [LawfulBEq α] [LawfulHashable α]
    (l : List (α × β)) (k : α) (v : β)
    (h_distinct : List.Pairwise (fun a b => (a.fst == b.fst) = false) l)
    (h_mem : (k, v) ∈ l) : HMap.find? (HMap.ofList l) k = some v := by
  simp only [find?, ofList, Std.HashMap.get?_eq_getElem?]
  rw [Std.HashMap.getElem?_ofList_of_mem (k_beq := BEq.refl k) h_distinct h_mem]

/-- For a distinct-keyed list, the `toList ∘ ofList` round-trip preserves entry membership.
    (`ofList` keeps one binding per key; distinct keys means no entry is dropped, and `toList`
    re-exposes exactly the stored bindings.) -/
public theorem mem_toList_ofList_iff [LawfulBEq α] [LawfulHashable α]
    (l : List (α × β)) (h_distinct : List.Pairwise (fun a b => (a.fst == b.fst) = false) l)
    (k : α) (v : β) :
    (k, v) ∈ (HMap.ofList l).toList ↔ (k, v) ∈ l := by
  have h_mem_find : ∀ (m : HMap α β), (k, v) ∈ m.toList ↔ m.find? k = some v := by
    intro m
    simp only [toList, find?, Std.HashMap.get?_eq_getElem?]
    exact Std.HashMap.mem_toList_iff_getElem?_eq_some
  rw [h_mem_find]
  constructor
  · exact fun h => find?_ofList_mem l k v h
  · exact fun h => find?_ofList_of_mem l k v h_distinct h

/-- Two lists with the same entry-membership agree on `find?` at every key, provided the *second*
    is distinct-keyed. (The first needs no distinctness: the `some` case reads its found entry off
    membership, and the `none` case only needs that a hit in `l2` would force a key in `l1`.) So the
    `toList ∘ ofList` round-trip — which reorders but preserves entries — is invisible to `find?`. -/
public theorem find?_ofList_congr_of_mem [LawfulBEq α] [LawfulHashable α]
    (l1 l2 : List (α × β))
    (h2 : List.Pairwise (fun a b => (a.fst == b.fst) = false) l2)
    (h_mem : ∀ p : α × β, p ∈ l1 ↔ p ∈ l2) (k : α) :
    HMap.find? (HMap.ofList l1) k = HMap.find? (HMap.ofList l2) k := by
  cases h1' : HMap.find? (HMap.ofList l1) k with
  | none =>
    cases h2' : HMap.find? (HMap.ofList l2) k with
    | none => rfl
    | some v2 =>
      obtain ⟨v1, h_v1⟩ := find?_ofList_of_mem_keys l1 k
        (List.mem_map_of_mem ((h_mem (k, v2)).mpr (find?_ofList_mem l2 k v2 h2')))
      rw [h_v1] at h1'; exact absurd h1' (by simp)
  | some v1 =>
    have h_in2 : (k, v1) ∈ l2 := (h_mem (k, v1)).mp (find?_ofList_mem l1 k v1 h1')
    rw [find?_ofList_of_mem l2 k v1 h2 h_in2]
...
/-- Fusing two `mapValues` into one, up to `Equiv`. -/
public theorem mapValues_mapValues [LawfulBEq α] [LawfulHashable α]
    (f : γ → δ) (g : β → γ) (m : HMap α β) :
    Equiv ((m.mapValues g).mapValues f) (m.mapValues (fun v => f (g v))) := by
  intro k
  rw [find?_mapValues, find?_mapValues, find?_mapValues, Option.map_map]
  rfl
...
public theorem mapValues_congr [LawfulBEq α] [LawfulHashable α]
    {f g : β → γ} (m : HMap α β) (h : ∀ v ∈ m.values, f v = g v) :
    Equiv (m.mapValues f) (m.mapValues g) := by
  intro k
  rw [find?_mapValues, find?_mapValues]
  cases hk : m.find? k with
  | none => rfl
  | some v => simp only [Option.map_some]; rw [h v (find?_mem_values m hk)]
...
public theorem mapValues_insert [LawfulBEq α] [LawfulHashable α]
    (f : β → γ) (m : HMap α β) (x : α) (v : β) :
    Equiv ((m.insert x v).mapValues f) ((m.mapValues f).insert x (f v)) := by
  intro k
  by_cases hk : k = x
  · subst hk
    rw [find?_mapValues, find?_insert_self, find?_insert_self, Option.map_some]
  · rw [find?_mapValues, find?_insert_ne _ _ _ _ (by simp [bne, hk]),
        find?_insert_ne _ _ _ _ (by simp [bne, hk]), find?_mapValues]
