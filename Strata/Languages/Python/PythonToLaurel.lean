...
private def mkHavocStmtsForUnmodeledCall (ctx : TranslationContext)
    ...
    (source : Option FileRange) : List StmtExprMd :=
  if let .Call _ funccall args kwords := callExpr then
    let holeExceptHavoc := [mkStmtExprMdWithLoc (StmtExpr.Assign [maybeExceptVar] (mkStmtExprMd (.Hole false none))) source]
        ...
        if let .Name _ n _ := base then
          ...
          | some (varName, ty) =>
            if isCompositeType ctx ty then ([], true)
              ...
              ([mkStmtExprMdWithLoc (StmtExpr.Assign [freeVar varName] (mkStmtExprMd (.Hole false none))) source], false)
          | _ => ([], false)
        else ([], false)
      else ([], false)
    ...
    let heapHavoc := if !involveHeap then [] else
        [mkStmtExprMdWithLoc (StmtExpr.Assign [freeVar "$heap"] (mkStmtExprMd (.Hole false none))) source]
...
partial def translateAssign  (ctx : TranslationContext)
  ...
  {
    let havocStmts := mkHavocStmtsForUnmodeledCall ctx rhs source
    match lhs with
    | .Name _ n _ =>
