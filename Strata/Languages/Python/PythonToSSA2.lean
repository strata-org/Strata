...
/-!
# PythonToSSA v2: Strict Block-Argument SSA Construction

Direct AST traversal (no BlockTree) with forwarding context (`fwd`) for
expression-level value threading. All cross-block references are eliminated:
...
- **Statement blocks**: conservative `allVars` params (same as v1).
...
namespace Strata.Python.PythonToSSA2
...
end Strata.Python.PythonToSSA2
