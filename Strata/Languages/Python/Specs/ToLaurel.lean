...
private def mkStmt (e : StmtExpr) (md : Imperative.MetaData Core.Expression := .empty) : StmtExprMd :=
  { val := e, md := md }
...
partial def specExprToLaurel (e : SpecExpr) (md : Imperative.MetaData Core.Expression := .empty)
    : ToLaurelM (Option StmtExprMd) :=
  let mk s := mkStmt s md
  match e with
  | .placeholder => do
    reportError default "Placeholder expression not translatable"
    return none
  | .var name => return some (mk (.Identifier (mkId name)))
  | .intLit v => return some (mk (.StaticCall (mkId "from_int") [mk (.LiteralInt v)]))
  | .floatLit _ => do
    reportError default "Float literals not yet supported in preconditions"
    return none
  | .getIndex subject field =>
    match subject with
    | .var "kwargs" => return some (mk (.Identifier (mkId field)))
    | _ => do
      let s? ← specExprToLaurel subject md
      return s?.map fun s => mk (.FieldSelect s (mkId field))
  ...
  | .len subject => do
    ...
    let s? ← specExprToLaurel subject md
    return s?.map fun s =>
      let unwrapped := mk (.StaticCall (mkId "Any..as_string!") [s])
      mk (.StaticCall (mkId "from_int") [mk (.StaticCall (mkId "Str.Length") [unwrapped])])
  | .intGe subject bound => do
    let s? ← specExprToLaurel subject md; let b? ← specExprToLaurel bound md
    return do
      let s ← s?; let b ← b?
      some (mk (.PrimitiveOp .Geq [mk (.StaticCall (mkId "Any..as_int!") [s]), mk (.StaticCall (mkId "Any..as_int!") [b])]))
  | .intLe subject bound => do
    let s? ← specExprToLaurel subject md; let b? ← specExprToLaurel bound md
    return do
      let s ← s?; let b ← b?
      some (mk (.PrimitiveOp .Leq [mk (.StaticCall (mkId "Any..as_int!") [s]), mk (.StaticCall (mkId "Any..as_int!") [b])]))
  | .floatGe subject bound => do
    let s? ← specExprToLaurel subject md; let b? ← specExprToLaurel bound md
    return do
      let s ← s?; let b ← b?
      let sF := mk (.StaticCall (mkId "Any..as_float!") [s])
      let bF := mk (.StaticCall (mkId "Any..as_float!") [b])
      some (mk (.PrimitiveOp .Geq [sF, bF]))
  | .floatLe subject bound => do
    ...
    return do
      ...
      let sF := mk (.StaticCall (mkId "Any..as_float!") [s])
      let bF := mk (.StaticCall (mkId "Any..as_float!") [b])
      some (mk (.PrimitiveOp .Leq [sF, bF]))
  | .not inner => do
    let i? ← specExprToLaurel inner md
    return i?.map fun i => mk (.PrimitiveOp .Not [i])
  | .implies cond body => do
    let c? ← specExprToLaurel cond md; let b? ← specExprToLaurel body md
    return do let c ← c?; let b ← b?; some (mk (.PrimitiveOp .Implies [c, b]))
  | .enumMember subject values => do
    let s? ← specExprToLaurel subject md
    return s?.map fun s =>
      let sStr := mk (.StaticCall (mkId "Any..as_string!") [s])
      let eqs := values.toList.map fun v =>
        mk (.PrimitiveOp .Eq [sStr, mk (.LiteralString v)])
      eqs.foldl (init := mk (.LiteralBool false)) fun acc eq =>
        mk (.PrimitiveOp .Or [acc, eq])
  | .containsKey container key => do
    match container with
    | .var "kwargs" =>
      -- containsKey(kwargs, "key") → parameter was provided (not None)
      return some (mk (.PrimitiveOp .Not
        [mk (.StaticCall (mkId "Any..isfrom_none") [mk (.Identifier (mkId key))])]))
    | _ =>
      let c? ← specExprToLaurel container md
      return c?.map fun c =>
        let unwrapped := mk (.StaticCall (mkId "Any..as_Dict!") [c])
        mk (.StaticCall (mkId "DictStrAny_contains") [unwrapped, mk (.LiteralString key)])
  | .regexMatch subject pattern => do
    let s? ← specExprToLaurel subject md
    return s?.map fun s =>
      let sStr := mk (.StaticCall (mkId "Any..as_string!") [s])
      mk (.StaticCall (mkId "re_search_bool") [mk (.LiteralString pattern), sStr])
  | .forallList _ _ _ => do
...
def buildSpecBody (preconditions : Array Assertion) (requiredParams : Array String := #[])
  ...
  for assertion in preconditions do
    ...
    match ← specExprToLaurel assertion.formula fileMd with
    | some condExpr =>
    ...
    | none =>
      ...
      let assertStmt ← mkStmtWithLoc (.Assert (mk .Hole)) default msg
      stmts := assertStmt :: stmts
  let body := mk (.Block stmts.reverse none)
