...
partial def initTmpParam (translation_ctx: TranslationContext) (p: Python.expr SourceRange × String) : List Core.Statement :=
  ...
  | .Call _ f args _ =>
    ...
    | .Name _ n _ =>
      match n.val with
      | "json_dumps" => [(.init p.snd t[string] (.det (.strConst pySynthLoc "")) md), .call "json_dumps" ([.inArg (.app pySynthLoc (.op pySynthLoc "DictStrAny_mk" none) (.strConst pySynthLoc "DefaultDict")), .inArg (Strata.Python.TypeStrToCoreExpr "IntOrNone")] ++ [.outArg p.snd, .outArg "maybe_except"]) md]
      | "str" =>
        assert! args.val.size == 1
