...
import all Strata.Languages.GOTO.Semantics
...
This is the meta-theory library for the GOTO semantics; the lemmas are the
deliverable. Each falls into one of three usage categories (an audit prompted by
review feedback — none is accidental dead code):

- **`simp` automation.** The `Store` rewrite lemmas tagged `@[simp]`
  (`Store.update_same`, `Store.declare_same`, `Store.kill_same`,
  `Store.update_update`) fire automatically in the determinism/progress proofs.
- **Internal proof composition.** `instrType_some_lt` and
  `StepInstr_deterministic_no_nondet` feed `ExecProg_deterministic`; the
  per-instruction `progress_*` lemmas compose into `progress_wellformed`.
- **Exported meta-theory.** The remaining results — the `*_other` store lemmas,
  `assume_preserves_store`/`assert_preserves_store`, `reachable_trans`/
  `reachable_single`/`safe_no_assert_fail`, `exec_empty_prog`, and the top-level
  `ExecProg_deterministic`/`progress_wellformed` — are the phase-1 guarantees and
  the building blocks reserved for the Phase-2 Core⟹GOTO simulation, which is
  where the store-operation and reachability facts get consumed.
...
@[simp] theorem Store.declare_same (σ : Store) (name : String) :
    (σ.declare name) name = some .undefined := by
  simp [Store.declare]
...
/-- Reachability is transitive (inherited from `ReflTrans`). -/
...
/-! ## Alignment with `Specification.lean`

`Strata.Transform.Specification` states its transform-correctness framework over
`Lang P` executions, with
`CanFail L s ρ₀ := ∃ cfg, (L.getEnv cfg).hasFailure = true ∧ L.star (L.stmtCfg s ρ₀) cfg`.
The GOTO-IL notions here line up with that shape one-for-one — `Reachable` plays
the role of `L.star`, and `AssertFails` the role of `hasFailure = true` — so the
GOTO analog of `CanFail` and the fact that `ProgramSafe` is exactly its negation
are provable *now*, at phase 1, with no dependency on `Specification.lean`. The
only piece still missing is the Phase-2 Core⟹GOTO simulation that instantiates
`Specification.lean`'s `Lang` with GOTO and transports these facts across it. -/

/-- GOTO-level analog of `Specification.CanFail`: some reachable configuration
    has a failing assertion. -/
def CanFail (callResult : CallResultRel) (eval : ExprEval) (fenv : FuncEnv)
    (prog : Program) (σ₀ : Store) : Prop :=
  ∃ pc σ, Reachable callResult eval fenv prog 0 σ₀ pc σ ∧ AssertFails eval prog pc σ

/-- `ProgramSafe` is exactly the negation of `CanFail`, confirming the phase-1
    GOTO safety notion aligns structurally with `Specification.lean`'s
    `CanFail`/overapproximation framework. -/
theorem programSafe_iff_not_canFail
    {callResult : CallResultRel} {eval : ExprEval} {fenv : FuncEnv}
    {prog : Program} {σ₀ : Store} :
    ProgramSafe callResult eval fenv prog σ₀ ↔
    ¬ CanFail callResult eval fenv prog σ₀ := by
  constructor
  · rintro hsafe ⟨pc, σ, hreach, hfail⟩
    exact hsafe pc σ hreach hfail
  · intro hnf pc σ hreach hfail
    exact hnf ⟨pc, σ, hreach, hfail⟩
...
theorem progress_assign
  ...
  | inr hnd =>
    exact ⟨pc + 1, σ.update name .undefined, .assign_nondet hty hname hrhs_eq hnd⟩
...
theorem progress_assert
    ...
    (hguard : ∃ b, (instrGuard prog pc).bind (eval σ ·) = some (.bool b)) :
    ∃ pc' σ', StepInstr callResult eval fenv prog pc σ pc' σ' := by
  obtain ⟨b, hb⟩ := hguard
  cases b with
...
/-- Per-instruction well-formedness of the instruction at `(pc, σ)`, bundling the
    obligations `progress_wellformed` needs into one predicate (so the progress
    statement takes a single hypothesis rather than one per instruction
    category). All fields hold for programs Strata's Core→GOTO translation
    emits. -/
structure WellFormedAt (callResult : CallResultRel) (eval : ExprEval)
    (fenv : FuncEnv) (prog : Program) (pc : Nat) (σ : Store) : Prop where
  /-- The instruction is one Strata's translation emits. -/
  supported : ∃ ty, instrType prog pc = some ty ∧ IsSupportedInstrType ty
  /-- Any guard evaluates to a boolean. -/
  guardBool : ∃ b, (instrGuard prog pc).bind (eval σ ·) = some (.bool b)
  /-- ASSIGN has an lhs symbol and an rhs that evaluates (or is nondet). -/
  assign : instrType prog pc = some .ASSIGN →
    (∃ name, (instrCode prog pc).bind getAssignLhs = some name) ∧
    (∃ rhs, (instrCode prog pc).bind getAssignRhs = some rhs) ∧
    (∀ rhs, (instrCode prog pc).bind getAssignRhs = some rhs →
      (∃ v, eval σ rhs = some v) ∨ rhs.id = .side_effect .Nondet)
  /-- DECL names a symbol. -/
  decl : instrType prog pc = some .DECL →
    ∃ name, (instrCode prog pc).bind getSymbolName = some name
  /-- DEAD names a symbol. -/
  dead : instrType prog pc = some .DEAD →
    ∃ name, (instrCode prog pc).bind getSymbolName = some name
  /-- GOTO has a target. -/
  goto : instrType prog pc = some .GOTO →
    ∃ tgt, instrTarget prog pc = some (some tgt)
  /-- Every GOTO target resolves to an instruction index. -/
  gotoResolve : ∀ tgt, instrTarget prog pc = some (some tgt) →
    ∃ idx, findLocIdx prog.instructions tgt = some idx
  /-- FUNCTION_CALL names a callee the call relation can execute. -/
  fcall : instrType prog pc = some .FUNCTION_CALL →
    (∃ name, (instrCode prog pc).bind getCallCallee = some name) ∧
    (∀ name, (instrCode prog pc).bind getCallCallee = some name →
      ∃ σ' rv, callResult eval fenv name σ σ' rv)

/-- Well-formed program progress: if the PC is in bounds, the instruction is not
    a terminator, and it is `WellFormedAt` the current store, then either
    execution steps or ASSUME blocks. Composes the per-instruction progress
    lemmas above. -/
theorem progress_wellformed
    ...
    (hwf : WellFormedAt callResult eval fenv prog pc σ) :
    (∃ pc' σ', StepInstr callResult eval fenv prog pc σ pc' σ') ∨
    AssumeBlocks eval prog pc σ := by
  obtain ⟨ty, hty_eq, hsupp⟩ := hwf.supported
  ...
  · left; obtain ⟨hl, hr, hev⟩ := hwf.assign hty_eq
    exact progress_assign hty_eq hl hr hev
  · left; exact progress_decl hty_eq (hwf.decl hty_eq)
  · left; exact progress_dead hty_eq (hwf.dead hty_eq)
  · left
    exact progress_goto hty_eq hwf.guardBool (hwf.goto hty_eq) hwf.gotoResolve
  · exact progress_assume hty_eq hwf.guardBool
  · left; exact progress_assert hty_eq hwf.guardBool
  · left; obtain ⟨hcallee, hcall⟩ := hwf.fcall hty_eq
