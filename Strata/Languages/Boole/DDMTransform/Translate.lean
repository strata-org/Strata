...
open Strata Lambda Imperative Lean.Parser
...
partial def translateLMonoTy (bindings : TransBindings) (arg : Arg) :
  ...
  | .fvar _ i argst =>
    ...
    let ty_boole ← match decl with
                  | .type (.con tcons) _md =>
                    -- Type Declaration
                    let ty := tcons.toType
                    -- While the "unsafe" below looks scary, we should be alright as far as
                    -- Boole is concerned. See `Core.TypeConstructor`, where there is no
...
def translateTypeSynonym (bindings : TransBindings) (op : Operation) :
  ...
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_typesynonym 4
  ...
  let decl := Boole.Decl.type (.syn { name := name, typeArgs := targs, type := typedef }) md
  return (decl, { bindings with freeVars := bindings.freeVars.push decl })


def translateTypeDecl (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_typedecl 2
  let name ← translateIdent TyIdentifier op.args[0]!
  let numargs ←
    translateOption
      (fun maybearg =>
  ...
  -- irrelevant.
  let decl := Boole.Decl.type (.con { name := name, numargs := numargs }) md
  return (decl, { bindings with freeVars := bindings.freeVars.push decl })
...
def translateLhs (arg : Arg) : TransM BooleIdent := do
  ...
  | q`Core.lhsIdent, #[id] => translateIdent BooleIdent id
  ...
  | q`Core.mkBindings =>
    ...
    let bindings ← bindingSeq.mapM (fun bindingArg => do
      ...
      if bindingOp.name == q`Core.mkBinding then
        ...
        let id ← translateIdent BooleIdent bindingArgs[0]!
...
private def invariantsToOption (invs : List Boole.Expression.Expr) : Option Boole.Expression.Expr :=
  match invs with
  | [] => none
  | i :: is =>
    -- ((i ∧ i2) ∧ i3) ∧ ...
    some <| is.foldl
      (fun acc j => .app () (.app () Boole.boolAndOp acc) j)
      i
...
partial def translateStmt (p : Strata.Program) (bindings : TransBindings) (arg : Arg) :
  ...
  | q`Boole.for_statement, #[va, ina, ga, sa, ia, ba] =>
    ...
    let md ← getOpMetaData op
    return ([.for id (.forAll [] mty) init g step .none inv? bodyss md], bindings)
  | q`Boole.for_down_to_by_statement, #[va, ina, il, sa, ia, ba] =>
    ...
    let bindings := { bindings with boundVars := bindings.boundVars.pop }
    let md ← getOpMetaData op
    return ([.forto false id (.forAll [] mty) init limit step .none inv? bodyss md], bindings)
  | q`Boole.for_to_by_statement, #[va, ina, il, sa, ia, ba] =>
    ...
    let bindings := { bindings with boundVars := bindings.boundVars.pop }
    let md ← getOpMetaData op
    return ([.forto true id (.forAll [] mty) init limit step .none inv? bodyss md], bindings)
  ...
  | q`Core.funcDecl_statement, #[namea, _typeArgsa, bindingsa, returna, _axiomsa, bodya] =>
    let name ← translateIdent BooleIdent namea
    ...
    let funcBinding : LExpr BooleLParams.mono := .op () name (some outputMono)
    ...
    let func := { name := name,
                  ...
                  body := body,
                  attr := #[] }
    let funcDecl := Boole.Decl.func func md
...
def translateProcedure (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_procedure 6
...
def translateBlockCommand (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_block 1
...
def translateConstant (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_constdecl 3
...
def translateAxiom (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_axiom 2
...
def translateDistinct (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_distinct 2
...
def translateFunction (status : FnInterp) (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
  ...
  let _ ←
    match status with
    | .Definition  => @checkOp (Boole.Decl × TransBindings) op q`Core.command_fndef  6
    | .Declaration => @checkOp (Boole.Decl × TransBindings) op q`Core.command_fndecl 4
...
This is the Strata Boole-specific version of `ConstructorInfo` from AST.lean,
...
structure TransConstructorInfo where
  ...
  fields : Array (BooleIdent × LMonoTy)
  deriving Repr
...
-/
def translateConstructorList (p : Strata.Program) (bindings : TransBindings) (arg : Arg) :
...
/--
Translate a datatype declaration to Core declarations, updating bindings
appropriately.
...
-/
def translateDatatype (p : Strata.Program) (bindings : TransBindings) (op : Operation) :
    TransM (Boole.Decls × TransBindings) := do
  -- Check operation has correct name and argument count
  let _ ← @checkOp (Boole.Decls × TransBindings) op q`Core.command_datatype 3
  ...
  let placeholderLDatatype : LDatatype Visibility :=
    { name := datatypeName
      ...
      constrs_ne := by simp }
  let placeholderDecl := Boole.Decl.type (.data [placeholderLDatatype])
  ...
  if h : constructors.size == 0 then
    ...
    -- Generate factory from LDatatype and convert to Boole.Decl
    -- (used only for bindings.freeVars, not for allDecls)
    let factory ← match genBlockFactory [ldatatype] (T := BooleLParams) with
      | .ok f => pure f
      | .error e => TransM.error s!"Failed to generate datatype factory: {e}"
    let funcDecls : List Boole.Decl := factory.toList.map fun func =>
      Boole.Decl.func func

    -- Only includes typeDecl, factory functions generated later
    let md ← getOpMetaData op
    let typeDecl := Boole.Decl.type (.data [ldatatype]) md
...
def translateGlobalVar (bindings : TransBindings) (op : Operation) :
  TransM (Boole.Decl × TransBindings) := do
  let _ ← @checkOp (Boole.Decl × TransBindings) op q`Core.command_var 1
