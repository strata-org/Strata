...
def bvBinaryOp (fn:∀ {n}, BitVec n → BitVec n → BitVec n)
  ...
  (m:BooleLParams.Metadata)
  (ops:List (LExpr BooleLParams.mono))
    : Option (LExpr BooleLParams.mono) :=
  match ops with
...
scoped elab "ExpandBVOpFuncDefs'" "[" sizes:num,* "]" : command => do
  for size in sizes.getElems do
    let s := size.getNat.repr
    for (op, arity) in List.zip BVOpNames BVOpAritys do
      let funcName := Lean.mkIdent (.str .anonymous s!"bv{s}{op}Func")
      let funcArity := Lean.mkIdent (.str (.str .anonymous "Lambda") arity)
      let opName := Lean.Syntax.mkStrLit s!"Bv{s}.{op}"
      let bvTypeName := Lean.Name.mkSimple s!"bv{s}"
      let opStr := Lean.Syntax.mkStrLit op
      Lean.Elab.Command.elabCommand (← `(def $funcName : LFunc BooleLParams :=
        $funcArity $opName mty[$(Lean.mkIdent bvTypeName):ident]
...
scoped macro "ExpandBVOpFuncNames'" "[" sizes:num,* "]" : term => do
  ...
  for size in sizes.getElems do
    let s := size.getNat.repr
    let ops := BVOpNames.map (Lean.mkIdent ∘ (.str (.str .anonymous "Boole")) ∘ (s!"bv{s}" ++ · ++ "Func"))
...
def Factory : @Factory BooleLParams := #[
  @intAddFunc BooleLParams _,
  @intSubFunc BooleLParams _,
  @intMulFunc BooleLParams _,
  @intDivFunc BooleLParams _,
  @intModFunc BooleLParams _,
  @intNegFunc BooleLParams _,
...
scoped elab "DefBVOpFuncExprs" "[" sizes:num,* "]" : command => do
  for size in sizes.getElems do
    let s := size.getNat.repr
    for op in BVOpNames do
      let opName := Lean.mkIdent (.str .anonymous s!"bv{s}{op}Op")
      let funcName := Lean.mkIdent (.str (.str .anonymous "Boole") s!"bv{s}{op}Func")
      Lean.Elab.Command.elabCommand (← `(def $opName : Expression.Expr := ($funcName).opExpr))
...
def intAddOp : Expression.Expr := (@intAddFunc BooleLParams _).opExpr
def intSubOp : Expression.Expr := (@intSubFunc BooleLParams _).opExpr
def intMulOp : Expression.Expr := (@intMulFunc BooleLParams _).opExpr
def intDivOp : Expression.Expr := (@intDivFunc BooleLParams _).opExpr
def intModOp : Expression.Expr := (@intModFunc BooleLParams _).opExpr
def intNegOp : Expression.Expr := (@intNegFunc BooleLParams _).opExpr
def intLtOp : Expression.Expr := (@intLtFunc BooleLParams _).opExpr
def intLeOp : Expression.Expr := (@intLeFunc BooleLParams _).opExpr
def intGtOp : Expression.Expr := (@intGtFunc BooleLParams _).opExpr
def intGeOp : Expression.Expr := (@intGeFunc BooleLParams _).opExpr
...
def boolAndOp : Expression.Expr := (@boolAndFunc BooleLParams _).opExpr
def boolOrOp : Expression.Expr := (@boolOrFunc BooleLParams _).opExpr
def boolImpliesOp : Expression.Expr := (@boolImpliesFunc BooleLParams _).opExpr
def boolEquivOp : Expression.Expr := (@boolEquivFunc BooleLParams _).opExpr
def boolNotOp : Expression.Expr := (@boolNotFunc BooleLParams _).opExpr
...
/--
Get all the built-in functions supported by Strata Boole.
-/
