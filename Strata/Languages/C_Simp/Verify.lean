...
def translate_expr (e : C_Simp.Expression.Expr) : Lambda.LExpr Core.CoreLParams.mono :=
  ...
  | .op _ o ty => .op Strata.SourceRange.none ⟨o.name, ()⟩ ty
  | .bvar _ n => .bvar Strata.SourceRange.none n
  | .fvar _ n ty => .fvar Strata.SourceRange.none ⟨n.name, ()⟩ ty
  | .abs _ name ty e => .abs Strata.SourceRange.none name ty (translate_expr e)
