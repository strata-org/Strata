...
def loop_elimination_statement(s : C_Simp.Statement) : Core.Statement :=
  ...
  | .loop guard measure invList body _ =>
    ...
    | .det guard_expr, .some measure, _ =>
      ...
      let measure_pos := (.app () (.app () (.op () (Core.CoreOp.toString (.numeric ⟨.int, .Ge⟩)) none) (translate_expr measure)) (.intConst () 0))
      ...
      let measure_decreases : Core.Statement := .assert "measure_decreases" (.app () (.app () (.op () (Core.CoreOp.toString (.numeric ⟨.int, .Lt⟩)) none) (translate_expr measure)) (.fvar () "special-name-for-old-measure-value" none)) {}
      let measure_imp_not_guard : Core.Statement := .assert "measure_imp_not_guard" (.ite () (.app () (.app () (.op () (Core.CoreOp.toString (.numeric ⟨.int, .Le⟩)) none) (translate_expr measure)) (.intConst () 0)) (.app () (.op () (Core.CoreOp.toString (.bool .Not)) none) (translate_expr guard_expr)) (.true ())) {}
      ...
      let not_guard : Core.Statement := .assume "not_guard" (.app () (.op () (Core.CoreOp.toString (.bool .Not)) none) (translate_expr guard_expr)) {}
