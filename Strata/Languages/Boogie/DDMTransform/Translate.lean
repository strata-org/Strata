...
def SourceRange.toMetaData (ictx : InputContext) (sr : SourceRange) : Imperative.MetaData Boogie.Expression :=
  ...
  let uri: MetaData.Uri := .file file
...
instance : Inhabited Boogie.Decl where
  default := .var "badguy" (.forAll [] (.tcons "bool" [])) (.false .empty)
...
def translateQuantifier
  ...
  TransM Boogie.Expression.Expr := do
    ...
    let newBoundVars := List.toArray (xsArray.mapIdx (fun i _ => LExpr.bvar MetaData.empty i))
    ...
    let triggers ← match triggersa with
      | none => pure (LExpr.noTrigger MetaData.empty)
    ...
    let buildQuantifier := fun (_, ty) (e, first) =>
      ...
      | .forAll [] mty =>
        let triggers := if first then
            ...
            LExpr.noTrigger .empty
        (.quant .empty qk (.some mty) triggers e, false)
...
def translateTriggerGroup (p: Program) (bindings : TransBindings) (arg : Arg) :
  ...
  | q`Boogie.trigger, #[tsa] => do
   ...
   return ts.foldl (fun g t => .app .empty (.app .empty addTriggerOp t) g) emptyTriggerGroupOp
...
def translateTriggers (p: Program) (bindings : TransBindings) (arg : Arg) :
  ...
  | q`Boogie.triggersAtom, #[group] =>
    ...
    return .app .empty (.app .empty addTriggerGroupOp g) emptyTriggersOp
  | q`Boogie.triggersPush, #[triggers, group] => do
    let ts ← translateTriggers p bindings triggers
    let g ← translateTriggerGroup p bindings group
    return .app .empty (.app .empty addTriggerGroupOp g) ts
...
partial def translateExpr (p : Program) (bindings : TransBindings) (arg : Arg) :
  ...
  | .fn _ q`Boogie.not_equal, [_tpa, xa, ya] =>
    let x ← translateExpr p bindings xa
    let y ← translateExpr p bindings ya
    return (.app .empty Boogie.boolNotOp (.eq .empty x y))
  ...
  | .fn _ q`Boogie.map_get, [_ktp, _vtp, ma, ia] =>
     ...
     let fn : LExpr BoogieLParams.mono := (LExpr.op .empty "select" (.some (LMonoTy.mkArrow (mapTy kty vty) [kty, vty])))
     let m ← translateExpr p bindings ma
     let i ← translateExpr p bindings ia
     return .mkApp .empty fn [m, i]
  | .fn _ q`Boogie.map_set, [_ktp, _vtp, ma, ia, xa] =>
     let kty ← translateLMonoTy bindings _ktp
     let vty ← translateLMonoTy bindings _vtp
     -- TODO: use Boogie.mapUpdateOp, but specialized
     let fn : LExpr BoogieLParams.mono := (LExpr.op .empty "update" (.some (LMonoTy.mkArrow (mapTy kty vty) [kty, vty, mapTy kty vty])))
     let m ← translateExpr p bindings ma
     let i ← translateExpr p bindings ia
     let x ← translateExpr p bindings xa
     return .mkApp .empty fn [m, i, x]
  -- Quantifiers
  ...
  | .fvar _ i, [] =>
    ...
    | .var name _ty _expr =>
      -- Global Variable
      return (.fvar .empty name ty?)
    | .func func =>
      -- 0-ary Function
      return (.op .empty func.name ty?)
    | _ =>
  ...
  | .fvar _ i, argsa =>
    ...
    | .func func =>
      ...
      return .mkApp .empty func.opExpr args.toList
    | _ =>
     TransM.error s!"translateExpr unimplemented fvar decl: {format decl}"
  | op, args =>
  ...
  if decls.size != 1 then
    ...
    let newVars ← tpids.mapM (fun (id, ty) =>
                    if h: ty.isMonoType then
                      return ((LExpr.fvar .empty id (ty.toMonoType h)): LExpr BoogieLParams.mono)
                    else
...
def translateInitStatement (p : Program) (bindings : TransBindings) (args : Array Arg)
  ...
  if args.size != 3 then
    ...
    let ty := (.forAll [] mty)
    let newBinding: LExpr BoogieLParams.mono := LExpr.fvar .empty lhs mty
...
def translateProcedure (p : Program) (bindings : TransBindings) (op : Operation) :
  ...
  let in_bindings := (sig.map (fun (v, ty) => (LExpr.fvar .empty v ty))).toArray
  let out_bindings := (ret.map (fun (v, ty) => (LExpr.fvar .empty v ty))).toArray
