...
def dischargeObligation
  (options : Strata.SMT.Options)
  (vars : List (IdentT BoogieIdent)) (filename : String)
...
def verifySingleEnv (pE : Program × Env) (options : Strata.SMT.Options) : EIO Format VCResults := do
  let (p, E) := pE
  match E.error with
  | some err =>
    ...
    for obligation in E.deferred do
      ...
      | .ok (terms, ctx) =>
        ...
        let ans ←
            IO.toEIO
              ...
              (dischargeObligation options
                (ProofObligation.getVars obligation) (Imperative.smt2_filename obligation.label)
                terms ctx)
        match ans with
        | .ok (result, estate) =>
        ...
        | .error e =>
           ...
           let prog := f!"\n\nEvaluated program:\n{p}"
           dbg_trace f!"\n\nObligation {obligation.label}: solver error, possibly a timeout!\
                        {e}
                        {if options.verbose then prog else ""}"
           break
    return results

def verify (program : Program) (options : Options := Options.default) : EIO Format VCResults := do
  ...
  | .ok pEs =>
    let VCss ← if options.checkOnly then
                 ...
                 (List.mapM (fun pE => verifySingleEnv pE options) pEs)
...
def verify (env : Environment) (options : SMT.Options := SMT.Options.default) : IO Boogie.VCResults := do
  let (program, errors) := TransM.run (translateProgram env.commands)
  if errors.isEmpty then
    -- dbg_trace f!"AST: {program}"
    EIO.toIO (fun f => IO.Error.userError (toString f))
                (Boogie.verify program options)
  else
