...
structure SMT.Context where
  ...
  typeFactory : @Lambda.TypeFactory BoogieLParams.IDMeta := #[]
...
def SMT.Context.withTypeFactory (ctx : SMT.Context) (tf : @Lambda.TypeFactory BoogieLParams.IDMeta) : SMT.Context :=
...
private def datatypeConstructorsToSMT (d : LDatatype BoogieLParams.IDMeta) : List String :=
  d.constrs.map fun c =>
