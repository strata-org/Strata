...
def callConditions (proc : Procedure)
                   ...
                   (subst :  Map (Lambda.IdentT BoogieIdent Lambda.LTy) Expression.Expr) :
...
def Command.evalCall (E : Env) (old_var_subst : SubstMap)
  ...
  | some proc =>
    ...
    let formal_tys := proc.header.inputs.keys.map (fun k => ((k, none) : (Lambda.IdentT BoogieIdent Lambda.LTy)))
    ...
    let return_tys := proc.header.outputs.keys.map (fun k => ((k, none) : (Lambda.IdentT BoogieIdent Lambda.LTy)))
