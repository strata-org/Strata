...
instance : DecidableEq BoogieExprMetadata :=
  show DecidableEq (MetaData.MetaData { Identifier := BoogieIdent, Expr := Unit }) from inferInstance

instance : Repr BoogieExprMetadata :=
  show Repr (MetaData.MetaData { Identifier := BoogieIdent, Expr := Unit }) from inferInstance
