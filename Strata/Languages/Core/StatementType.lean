...
TODO: `Stmt.defUseWellFormed` (in `Strata/DL/Imperative/Stmt.lean`) must be
proven correct: successful type checking here should imply
`Stmt.defUseWellFormed` / `Block.defUseWellFormed` holds on the checked
statements.
-/
