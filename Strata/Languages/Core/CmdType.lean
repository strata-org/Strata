...
def preprocess (C: LContext CoreLParams) (Env : TEnv Visibility) (ty : LTy) :
    Except Strata.DiagnosticModel (LTy × TEnv Visibility) := do
  let (mty, Env) ← ty.instantiateWithCheck C Env |>.mapError Strata.DiagnosticModel.fromFormat
...
def postprocess (_: LContext CoreLParams) (Env: TEnv Visibility) (ty : LTy) :
  ...
  if h: ty.isMonoType then
    ...
    .error (Strata.DiagnosticModel.fromFormat f!"[postprocess] Expected mono-type; instead got {ty}")
...
def inferType (C: LContext CoreLParams) (Env: TEnv Visibility) (c : Cmd Expression) (e : LExpr CoreLParams.mono) :
    Except Strata.DiagnosticModel ((LExpr CoreLParams.mono) × LTy × TEnv Visibility) := do
  ...
  let T ← match c with
    ...
    | _ =>
      let _ ← Env.freeVarCheck e f!"[{c}]" |>.mapError Strata.DiagnosticModel.fromFormat
      .ok Env
  let e := OldExpressions.normalizeOldExpr e
  let (ea, T) ← LExpr.resolve C T e |>.mapError Strata.DiagnosticModel.fromFormat
...
def canonicalizeConstraints (constraints : List (LTy × LTy)) :
    Except Strata.DiagnosticModel Constraints := do
  ...
  | (t1, t2) :: c_rest =>
    if h: t1.isMonoType && t2.isMonoType then
      ...
      .error (Strata.DiagnosticModel.fromFormat f!"[canonicalizeConstraints] Expected to see only mono-types in \
                type constraints, but found the following instead:\n\
                t1: {t1}\nt2: {t2}\n")

def unifyTypes (Env: TEnv Visibility) (constraints : List (LTy × LTy)) :
    Except Strata.DiagnosticModel (TEnv Visibility) := do
  let constraints ← canonicalizeConstraints constraints
  let S ← Constraints.unify constraints Env.stateSubstInfo |> .mapError (fun f => Strata.DiagnosticModel.fromFormat (format f))
  let Env := Env.updateSubst S
  return Env

def typeErrorFmt (e : Strata.DiagnosticModel) : Format :=
  e.format none

---------------------------------------------------------------------

instance : Imperative.TypeContext Expression (LContext CoreLParams) (TEnv Visibility) Strata.DiagnosticModel where
