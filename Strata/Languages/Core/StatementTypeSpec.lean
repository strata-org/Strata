...
inductive StmtHasType' (τ : Type) (P : Program) [S : ExprTypingSpec τ] :
  ...
  /-- An atomic (extended) command. Delegates to `CmdExtHasType'`; `C` is
      ...
      The output type-scope `Δ` is required only up to `TContext.Equiv` (see the
      `nil` note on `StmtsHasType'`): under the `HMap`-backed context, the
      typechecker's output is only find?/alias-equivalent to the canonical one. -/
  | cmd : ∀ C Γ Γ' L c Δ,
      CmdExtHasType' (τ := τ) C P Γ c Γ' →
      TContext.Equiv (T := CoreLParams) Δ Γ' →
      StmtHasType' τ P C Γ L (.cmd c) C Δ
  ...
  | block : ∀ C Γ C_body Γ_body L label body md Δ,
      ...
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtHasType' τ P C Γ L (.block label body md) C Δ
  ...
  | ite_det : ∀ C Γ C_t Γ_t C_e Γ_e L cond thenb elseb md Δ,
      ...
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtHasType' τ P C Γ L (.ite (.det cond) thenb elseb md) C Δ
  ...
  | ite_nondet : ∀ C Γ C_t Γ_t C_e Γ_e L thenb elseb md Δ,
      ...
      StmtsHasType' τ P C Γ L elseb C_e Γ_e →
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtHasType' τ P C Γ L (.ite .nondet thenb elseb md) C Δ
  ...
  | loop : ∀ C Γ C_body Γ_body L guard measure invariants body md Δ,
      ...
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtHasType' τ P C Γ L (.loop guard measure invariants body md) C Δ

  /-- Exit statement: its target must name an enclosing block (`label ∈ L`). -/
  | exit : ∀ C Γ L label md Δ,
      label ∈ L →
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtHasType' τ P C Γ L (.exit label md) C Δ
  ...
  | funcDecl : ∀ C Γ L decl func md Δ,
      ...
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtHasType' τ P C Γ L (.funcDecl decl md) (C.addFactoryFunction func.toLFunc) Δ
  ...
  | typeDecl : ∀ C C' Γ L tc md Δ,
      ...
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtHasType' τ P C Γ L (.typeDecl tc md) C' Δ
...
inductive StmtsHasType' (τ : Type) (P : Program) [S : ExprTypingSpec τ] :
  ...
  /-- The empty statement list leaves the context unchanged, up to
      `TContext.Equiv`: the typechecker threads an `HMap`-backed context whose
      output is only find?/alias-equivalent (not structurally equal) to the
      input, so `nil` allows the output `Δ` to differ from `Γ` up to `Equiv`. -/
  | nil : ∀ C Γ L Δ,
      TContext.Equiv (T := CoreLParams) Δ Γ →
      StmtsHasType' τ P C Γ L [] C Δ
