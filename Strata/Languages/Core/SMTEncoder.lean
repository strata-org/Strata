...
def smtTermToLExpr (t : Strata.SMT.Term)
  ...
  | .var v                =>
    ...
    if constructorNames.contains v.id then
      .op () (CoreIdent.unres v.id) none
    else
      .fvar () (CoreIdent.unres v.id) none
  | .app (.core (.uf uf)) args _retTy =>
    ...
    let fnExpr : LExpr CoreLParams.mono :=
      if constructorNames.contains uf.id then
        .op () (CoreIdent.unres uf.id) none
      else
        .fvar () (CoreIdent.unres uf.id) none
  ...
  | .some inner           => .app () (.op () (CoreIdent.unres "some") none) (smtTermToLExpr inner constructorNames)
  | .quant _ _ _ _        =>
