...
/-- `update` with a gen-fresh monomorphic binding preserves `TEnvWF`. Direct
    application of `TEnvWF.of_addInNewestContext_mono` (`update = addInNewestContext`). -/
theorem update_TEnvWF (Env : TEnv Unit) (x : CoreIdent) (mty : LMonoTy)
  ...
  Lambda.LExpr.TEnvWF.of_addInNewestContext_mono (T := CoreLParams) Env x mty h_wf h_fresh
...
theorem preprocess_genState_mono (C : LContext CoreLParams) (Env : TEnv Unit)
  ...
  have h_iwc' : LTy.instantiateWithCheck ty C Env = .ok (mty, Env1) := by
    revert h_iwc; cases LTy.instantiateWithCheck ty C Env <;> simp
  exact LTy_instantiateWithCheck_tyGen_mono ty C Env mty Env1 h_iwc'
...
theorem preprocess_output_fresh (C : LContext CoreLParams) (Env : TEnv Unit)
  ...
  · simp at h
  · rename_i v h_iwc
    obtain ⟨mty_inst, Env1⟩ := v
    simp only [pure, Except.pure, Except.ok.injEq, Prod.mk.injEq, LTy.forAll.injEq] at h
    obtain ⟨⟨_, h_mty⟩, h_env⟩ := h; subst h_env
    have h_iwc' : LTy.instantiateWithCheck ty C Env = .ok (mty_inst, Env1) := by
      revert h_iwc; cases LTy.instantiateWithCheck ty C Env <;> simp
    have h_envwf' : TEnvWF (T := CoreLParams) Env1 :=
      LTy_instantiateWithCheck_TEnvWF ty C Env mty_inst Env1 h_iwc' h_wf
    have h_inst_fresh := Lambda.LExpr.LTy_instantiateWithCheck_freeVars_fresh
      ty C Env mty_inst Env1 h_iwc'
    -- `mty = subst Env1.subst mty_inst`.
    rw [← h_mty]
    exact Lambda.LExpr.freeVars_subst_genFresh Env1.stateSubstInfo mty_inst
      Env1.genEnv.genState h_envwf'.substFreshForGen h_inst_fresh
