...
-- ExprSourceLoc.none because they are generated by the evaluator, not parsed from source.
...
instance : Lambda.Traceable Lambda.LExpr.EvalProvenance ExpressionMetadata where
  combine reasons :=
    ...
    | some primaryLoc =>
      ...
      -- Note: SourceRange.eq_trivial makes == always true, so compare fields directly.
...
def PathCondition.merge (cond : Expression.Expr) (pc1 pc2 : PathCondition Expression) : PathCondition Expression :=
  ...
  let pc2' := pc2.map (fun (label, e) => (label, mkImplies (LExpr.ite ExprSourceLoc.none cond (LExpr.boolConst ExprSourceLoc.none false) (LExpr.boolConst ExprSourceLoc.none true)) e))
  pc1' ++ pc2'
