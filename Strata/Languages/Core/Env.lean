...
instance : ToFormat ExpressionMetadata :=
  show ToFormat Strata.SourceRange from inferInstance
...
instance : Inhabited (Lambda.LExpr ⟨⟨ExpressionMetadata, CoreIdent⟩, LMonoTy⟩) :=
  show Inhabited (Lambda.LExpr ⟨⟨Strata.SourceRange, CoreIdent⟩, LMonoTy⟩) from inferInstance
...
def PathCondition.merge (cond : Expression.Expr) (pc1 pc2 : PathCondition Expression) : PathCondition Expression :=
  ...
  let pc2' := pc2.map (fun (label, e) => (label, mkImplies (LExpr.ite Strata.SourceRange.none cond (LExpr.boolConst Strata.SourceRange.none false) (LExpr.boolConst Strata.SourceRange.none true)) e))
  pc1' ++ pc2'
  where mkImplies (ant con : Expression.Expr) : Expression.Expr :=
  LExpr.ite Strata.SourceRange.none ant con (LExpr.boolConst Strata.SourceRange.none true)
