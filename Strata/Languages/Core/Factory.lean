...
def realAddFunc : WFLFunc CoreLParams := binaryOpUneval "Real.Add" mty[real]
def realSubFunc : WFLFunc CoreLParams := binaryOpUneval "Real.Sub" mty[real]
def realMulFunc : WFLFunc CoreLParams := binaryOpUneval "Real.Mul" mty[real]
def realDivFunc : WFLFunc CoreLParams := binaryOpUneval "Real.Div" mty[real]
def realNegFunc : WFLFunc CoreLParams := unaryOpUneval "Real.Neg" mty[real]

/- Real Comparison Operations -/

def realLtFunc : WFLFunc CoreLParams := binaryPredicateUneval "Real.Lt" mty[real]
def realLeFunc : WFLFunc CoreLParams := binaryPredicateUneval "Real.Le" mty[real]
def realGtFunc : WFLFunc CoreLParams := binaryPredicateUneval "Real.Gt" mty[real]
def realGeFunc : WFLFunc CoreLParams := binaryPredicateUneval "Real.Ge" mty[real]
...
def strToRegexFunc : WFLFunc CoreLParams :=
  unaryMixedUneval "Str.ToRegEx" mty[string] mty[regex]

def strInRegexFunc : WFLFunc CoreLParams :=
  binaryHetUneval "Str.InRegEx" mty[string] mty[regex] mty[bool]
...
def reRangeFunc : WFLFunc CoreLParams :=
  binaryMixedUneval "Re.Range" mty[string] mty[regex]

def reConcatFunc : WFLFunc CoreLParams :=
  binaryOpUneval "Re.Concat" mty[regex]

def reStarFunc : WFLFunc CoreLParams :=
  unaryOpUneval "Re.Star" mty[regex]

def rePlusFunc : WFLFunc CoreLParams :=
  unaryOpUneval "Re.Plus" mty[regex]
...
def reUnionFunc : WFLFunc CoreLParams :=
  binaryOpUneval "Re.Union" mty[regex]

def reInterFunc : WFLFunc CoreLParams :=
  binaryOpUneval "Re.Inter" mty[regex]

def reCompFunc : WFLFunc CoreLParams :=
  unaryOpUneval "Re.Comp" mty[regex]
...
def bvConcatFunc (size : Nat) : WFLFunc CoreLParams :=
  binaryMixedUneval s!"Bv{size}.Concat"
    (.bitvec size) (.bitvec (size * 2)) rfl rfl

def bvExtractFunc (size hi lo : Nat) : WFLFunc CoreLParams :=
  unaryMixedUneval s!"Bv{size}.Extract_{hi}_{lo}"
...
def WFFactory : Lambda.WFLFactory CoreLParams :=
  -- (T := CoreLParams) annotations needed where Lean cannot infer T
  -- from array context across combinator shape changes.
