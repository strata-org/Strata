...
def corePipelinePhases : List PipelinePhase :=
  [callElimPipelinePhase, loopElimPipelinePhase]

/-- The abstracted phases derived from the Core pipeline phases. -/
def coreAbstractedPhases : List AbstractedPhase :=
  corePipelinePhases.map (·.phase)
...
def verify (program : Program)
  ...
  let finalProgram ← profileStep profile "  Program transformations" do
    ...
    | some procs =>
       -- Verify specific procedures. Apply the pipeline transforms from
       -- `corePipelinePhases` to inline contracts and eliminate loops.
       -- Each pipeline phase pairs its transform with its model validation,
       -- ensuring they stay in sync.
      let passes := fun prog => do
        let filtered ← FilterProcedures.run prog procs
        let mut current := filtered
        for pp in corePipelinePhases do
          let (_changed, next) ← pp.transform current
          current := next
        let transformed ← runPrecondElim current
        ...
        let result := { transformed with decls := transformed.decls.filter fun d =>
          match d with
