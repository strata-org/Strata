  ...
  let finalProgram ← profileStep profile "  Program transformations" do
    ...
    | some procs =>
      ...
      let passes := fun prog => do
        ...
        -- After CallElim (which maintains the call graph via decrementEdge)
        -- and PrecondElim (which registers WF procs as leaf nodes), the
        -- call-graph closure is accurate: target procs no longer reference
        -- prelude helpers. A second FilterProcedures pass prunes cleanly.
        let prog ← FilterProcedures.run prog
