  ...
  let finalProgram ← profileStep profile "  Program transformations" do
    ...
    | some procs =>
      ...
      let passes := fun prog => do
        ...
        -- Keep only the target procedures and their WF-checking procedures
        -- (generated by PrecondElim). Filter without call-graph closure
        -- because prelude helpers (List_take, List_drop, …) would
        -- otherwise be pulled back in.
