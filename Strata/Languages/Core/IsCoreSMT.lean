...
/-- Boolean version of `checkCoreSMTExpr`. -/
...
/-- Boolean version of `checkCoreSMTCmd`. -/
...
/-- Boolean version of `checkCoreSMTStmt`. -/
def isCoreSMTStmt (s : Core.Statement) : Bool :=
  (checkCoreSMTStmt s).isOk

/-- Boolean version of `checkCoreSMTStmts`. -/
