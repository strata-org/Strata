...
/-- expressions that can't be reduced when evaluating
  TODO: This must retire & be replaced with isCanonicalValue.
  It will require wiring the Factory object.
-/
...
/-- Syntactic int-type predicate for Core expressions.

    Recognizes:
    - integer literals `.intConst _`,
    - applications headed by an integer-yielding factory operator
      (e.g., `Int.Add`, `Int.Sub`, `Int.Mul`, `Int.Neg`, …) with
      sub-arguments themselves int-typed,
    - free variables `.fvar` annotated with `LMonoTy.int`.

    TODO: this predicate should ideally be defined in terms of a
    `HasType`-style typing judgment carried by the language, so that the
    Core int-shape obligation flows from the type-checker rather than from
    a hand-rolled syntactic over-approximation.  The current shape suffices
    for `loopElim_overapproximatesAggressive` because measure expressions
    in well-typed programs reduce to closed integer values, and these
    cases together cover the common arithmetic forms generated by Core. -/
inductive Core.IsIntTy : Core.Expression.Expr → Prop where
  /-- Integer literals are int-typed. -/
  | intConst (m : Unit) (i : Int) : Core.IsIntTy (.const m (.intConst i))
  /-- An fvar annotated with `int` is int-typed. -/
  | fvarInt (m : Unit) (v : Lambda.Identifier Unit) :
      Core.IsIntTy (.fvar m v (some Lambda.LMonoTy.int))
  /-- Application of an int-yielding binary op to two int-typed args.
      Covers `Int.Add`, `Int.Sub`, `Int.Mul`, etc. -/
  | binOpInt (m m' : Unit) (op : Core.Expression.Expr) (e₁ e₂ : Core.Expression.Expr) :
      op = Core.intAddOp ∨ op = Core.intSubOp ∨ op = Core.intMulOp ∨
      op = Core.intDivOp ∨ op = Core.intModOp ∨ op = Core.intDivTOp ∨
      op = Core.intModTOp →
      Core.IsIntTy e₁ → Core.IsIntTy e₂ →
      Core.IsIntTy (.app m (.app m' op e₁) e₂)
  /-- Application of `Int.Neg` to an int-typed arg. -/
  | negOpInt (m : Unit) (e : Core.Expression.Expr) :
      Core.IsIntTy e → Core.IsIntTy (.app m Core.intNegOp e)
...
instance : HasInt Core.Expression where
  ...
  zeroIsNumeral := by
    ...
    rfl
  isIntTy     := Core.IsIntTy
  numeralIsIntTy n hn := by
    show Core.IsIntTy n
    cases n with
    | const m c =>
      cases c with
      | intConst i => exact Core.IsIntTy.intConst m i
      | _ => simp [Core.isNumeral] at hn
    | _ => simp [Core.isNumeral] at hn
  intValueIsClosed e h_val h_int := by
    -- Int-typed values are closed.  Open-able shapes (`.fvar _ _ _`,
    -- `.app _ _ _`, `.eq _ _ _`, `.ite _ _ _ _`, `.quant _ _ _ _ _ _`,
    -- `.bvar _ _`) are eliminated either by `IsIntTy` (no constructors
    -- for unannotated `.fvar`/general `.app`/etc.) or by `Value` (no
    -- constructors for `.app`/`.eq`/`.ite`/`.quant`/`.fvar`).
    -- The intersection is `.intConst _` (closed) and rejected for the
    -- shapes both predicates allow for opposite reasons.
    cases h_int with
    | intConst _ _ => rfl
    | fvarInt _ _ =>
      -- `Value` has no constructor for `.fvar _ _ _`.
      cases h_val
    | binOpInt _ _ _ _ _ _ _ _ =>
      -- `Value` has no constructor for `.app _ _ _`.
      cases h_val
    | negOpInt _ _ _ =>
      cases h_val
