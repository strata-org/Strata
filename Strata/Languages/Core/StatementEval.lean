...
def Statement.containsCmd (predicate : Imperative.Cmd Expression → Bool) (s : Statement) : Bool :=
  ...
  | .funcDecl _ _ | .exit _ _ | .typeDecl _ _ _ => false  -- Function/type declarations and exits don't contain commands
...
def Statement.collectCovers (s : Statement) : List (String × Imperative.MetaData Expression) :=
  ...
  | .funcDecl _ _ | .exit _ _ | .typeDecl _ _ _ => []  -- Function/type declarations and exits don't contain cover commands
  termination_by Imperative.Stmt.sizeOf s
/--
...
def Statement.collectAsserts (s : Statement) : List (String × Imperative.MetaData Expression) :=
  ...
  | .funcDecl _ _ | .exit _ _ | .typeDecl _ _ _ => []  -- Function/type declarations and exits don't contain assert commands
  termination_by Imperative.Stmt.sizeOf s
/--
