...
private def evalOneStmt (old_var_subst : SubstMap)
  ...
  | .ite cond then_ss else_ss _ =>
    ...
    | .nondet =>
      ...
      let initStmt := Statement.init freshName (.forAll [] (.tcons "bool" [])) .nondet (Imperative.MetaData.ofProvenance .nondetIte)
      let iteStmt := Imperative.Stmt.ite (.det freshVar) then_ss else_ss (Imperative.MetaData.ofProvenance .nondetIte)
