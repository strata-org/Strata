        ...
        match s with
          ...
          | .ite cond then_ss else_ss md =>
            ...
            | .true _ | .false _ =>
              ...
              -- Collect deferred obligations from the dead branch (covers
              -- that are unreachable, asserts that are vacuously true).
              let deadDeferred :=
                if Statements.containsCovers ss_f || Statements.containsAsserts ss_f then
                  ...
                  deferred ++ createUnreachableAssertObligations deadPathConds ss_f_asserts
                else
                  #[]
              -- Process the live branch and fold dead-branch deferred
              -- obligations into each result to avoid creating extra paths.
              let Ewns := go' Ewn ss_t .none
              Ewns.map (fun (ewn : EnvWithNext) =>
                let ss' := ewn.stk.top
                let s' := Imperative.Stmt.ite cond' ss' [] md
                { ewn with stk := orig_stk.appendToTop [s'],
                           env.deferred := ewn.env.deferred ++ deadDeferred })
            | _ =>
