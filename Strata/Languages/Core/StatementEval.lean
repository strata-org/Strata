...
---------------------------------------------------------------------
-- nosourcerange-file: expressions synthesized during statement evaluation (fresh variables,
-- path conditions, proof obligations) carry ExprSourceLoc.none because
-- they are generated by the evaluator, not parsed from source.
---------------------------------------------------------------------
...
private def mkReturnSubst (proc : Procedure) (lhs : List Expression.Ident) (E : Env) :
  ...
  let lhs_tys := lhs.map (fun l => (E.exprEnv.state.findD l (none, .fvar ExprSourceLoc.none l none)).fst)
...
private def computeTypeSubst (input_tys output_tys: List LMonoTy)
  ...
  let lhs_tys := lhs.filterMap (fun l =>
    (E.exprEnv.state.findD l (none, .fvar ExprSourceLoc.none l none)).fst)
...
private def createUnreachableCoverObligations
  ...
  covers.toArray.map
    (fun (label, md) =>
      (Imperative.ProofObligation.mk label .cover pathConditions (LExpr.boolConst ExprSourceLoc.none false) md))
  ...
  asserts.toArray.map
    (fun (label, md) =>
      let propType := match md.getPropertyType with
        ...
        | _ => .assert
      (Imperative.ProofObligation.mk label propType pathConditions (LExpr.boolConst ExprSourceLoc.none true) md))
...
private def collectDeadBranchDeferred
  ...
  if Statements.containsCovers ss_f || Statements.containsAsserts ss_f then
    ...
    let deadPathConds := pathConditions.push [.assumption deadLabel (LExpr.boolConst ExprSourceLoc.none false)]
...
private def evalOneStmt (old_var_subst : SubstMap)
  ...
  | .ite cond then_ss else_ss _ =>
    ...
    | .nondet =>
      ...
      let freshVar : Expression.Expr := .fvar ExprSourceLoc.none freshName none
...
def processIteBranches (steps : Nat) (old_var_subst : SubstMap) (Ewn : EnvWithNext)
  ...
  let path_conds_false := Ewn.env.pathConditions.push
                            [.assumption label_false (Lambda.LExpr.ite ExprSourceLoc.none cond' (LExpr.boolConst ExprSourceLoc.none false) (LExpr.boolConst ExprSourceLoc.none true))]
        ...
        | .ok argVals =>
          ...
          if argVals.length != proc.header.inputs.keys.length then
            ...
            let outputBindings : List (CoreIdent × (Option LMonoTy × Expression.Expr)) :=
              ...
              |>.map fun (name, ty) => (name, (some ty, LExpr.fvar ExprSourceLoc.none name none))
              ...
              | none =>
                if lhs.length != proc.header.outputs.keys.length then
                  ...
                  let outputVals := proc.header.outputs.keys.map fun name =>
                    (callEnv'.exprEnv.state.findD name (none, LExpr.fvar ExprSourceLoc.none name none)).snd
