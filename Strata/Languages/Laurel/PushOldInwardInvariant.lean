...
> ⚠️ **BROKEN / STUBBED WITH `sorry` — DO NOT TRUST THESE PROOFS.**
>
> This file was generated against an older Laurel AST and no longer type-checks
> after the `main2` merge (notably `StmtExpr.PrimitiveOp` gained a `skipProof`
> field, and other APIs shifted). Rather than hand-repair ~250 machine-generated
> lemmas, the broken proofs below have been stubbed with `sorry` (and
> `decreasing_by all_goals sorry`) purely to keep a *compiling* copy on record.
>
> Consequences:
> - **None of the statements below are currently proven.** Treat this file as a
>   preserved draft, not as a verified result.
> - This module is intentionally hard to hand-maintain; per the maintenance note
>   below, the plan is to **regenerate or drop it**, not repair it by hand.
> - Nothing in the compiler imports this module, so its broken/stubbed state does
>   not affect any product build.
...
it was produced. **Update (post-`main2` merge):** this file no longer compiles
as written, so its proofs have been stubbed with `sorry` to keep a building copy.
The original guarantee — that it went through the kernel with no `axiom`,
`sorry`, `admit`, or `native_decide` — **no longer holds**;
`#print axioms pushOldInward_old_normalized` will now report `sorryAx`.
...
decreasing_by
  -- Stubbed after the main2 merge: the termination goals changed when the Laurel
  -- AST was reshaped. See the BROKEN disclaimer at the top of this file.
  all_goals sorry
...
theorem subOlds_literal_decimal (d : StrataDDM.Decimal) (src : Option FileRange) :
...
private theorem mapStmtExprM_insideOld_literalDecimal_id
    (s : PushOldState) (b : Bool) (d : StrataDDM.Decimal) (src : Option FileRange) :
...
private theorem pushOldInwardExpr_canonical_literalDecimal
    (s : PushOldState) (d : StrataDDM.Decimal) (src : Option FileRange) :
    AllSubOldsCanonical s.inoutNames
