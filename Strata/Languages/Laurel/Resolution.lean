...
/-- Native arithmetic typing: all operands must be numeric; the result is their
    LUB under the consistency relation. -/
...
/-- Native typing for the boolean / comparison / equality / string-concat
    families: a fixed result type with each family's operand constraint. -/
...
private def synthArith (g : GradualConfig) (op : Operation) (expr : StmtExpr) (args' : List StmtExprMd)
  ...
  firstElab
    [ elaborateDynamicOp g op args' argTypes source,
      strConcatAlt g op args' argTypes source,
...
private def gradualOp (g : GradualConfig) (op : Operation) (expr : StmtExpr)
    (args' : List StmtExprMd) (argTypes : List HighTypeMd) (source : Option FileRange) :
    ResolveM (StmtExpr × HighTypeMd) :=
  if isArithmeticOp op then synthArith g op expr args' argTypes source
  else synthOther g op expr args' argTypes source
...
def Check.resolveStmtExpr (exprMd : StmtExprMd) (expected : HighTypeMd) : ResolveM StmtExprMd := do
  ...
  if (← get).gradualActive && (exprMd.val matches .PrimitiveOp _ _) then
    ...
    match ← tryCoerceActive expected.val actual.val e' exprMd.source with
    | some c => return c
    | none =>
...
def Synth.primitiveOp (exprMd : StmtExprMd) (expr : StmtExpr)
  ...
  match ← activeGradual with
  | some g => gradualOp g op expr args' argTypes source
  | none   => nativeOp op expr args' argTypes source
  termination_by (exprMd, 1)
  decreasing_by
    all_goals
      apply Prod.Lex.left
