  ...
  | .op op => match op.name, op.args with
    ...
    | q`Laurel.assign, #[arg0, arg1] =>
      ...
      | .Subscript subTarget index none =>
        -- `a[i] := v` is lowered to `.Subscript subTarget index (some value)`
        -- at statement position. SubscriptElim recognises this shape and
        -- rewrites based on whether subTarget is an Array<T> or Seq<T>.
        return mkStmtExprMd (.Subscript subTarget index (some value)) src
