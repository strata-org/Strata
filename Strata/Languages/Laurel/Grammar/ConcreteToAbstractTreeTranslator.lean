...
def parseProcedure (arg : Arg) : TransM Procedure := do
  ...
  let (isFunction, isOpaque, nameArg, paramArg, returnTypeArg, returnParamsArg,
       requiresArg, ensuresArg, modifiesArg, bodyArg) ← match op.name, op.args with
    | q`Laurel.procedure, #[nameArg, paramArg, returnTypeArg, returnParamsArg,
        requiresArg, ensuresArg, modifiesArg, bodyArg] =>
      pure (false, false, nameArg, paramArg, returnTypeArg, returnParamsArg,
            requiresArg, ensuresArg, modifiesArg, bodyArg)
    | q`Laurel.function, #[opaqueArg, nameArg, paramArg, returnTypeArg, returnParamsArg,
        requiresArg, ensuresArg, modifiesArg, bodyArg] =>
      let isOpaque := match opaqueArg with | .option _ (some _) => true | _ => false
      pure (true, isOpaque, nameArg, paramArg, returnTypeArg, returnParamsArg,
            requiresArg, ensuresArg, modifiesArg, bodyArg)
    | q`Laurel.procedure, args
    | q`Laurel.function, args =>
      TransM.error s!"parseProcedure expects correct number of arguments, got {args.size}"
    | _, _ =>
