...
2. Adding `ensures` for constrained-typed outputs (Core handles body checks and caller assumes)
   - For `isFunctional` procedures, clears the flag since function postconditions are not yet
     supported. TODO: restore `isFunctional` once function postconditions are supported.
...
def isConstrainedType (ptMap : ConstrainedTypeMap) (ty : HighType) : Bool :=
  match ty with | .UserDefined name => ptMap.contains name | _ => false
...
partial def substId (old new : Identifier) : StmtExprMd → StmtExprMd
  ...
  | ⟨.Forall n ty body, md⟩ =>
    if n == old then ⟨.Forall n ty body, md⟩
    else if n == new then
      let fresh := n ++ "$"
      ⟨.Forall fresh ty (substId old new (substId n fresh body)), md⟩
    else ⟨.Forall n ty (substId old new body), md⟩
  | ⟨.Exists n ty body, md⟩ =>
    if n == old then ⟨.Exists n ty body, md⟩
    else if n == new then
      let fresh := n ++ "$"
      ⟨.Exists fresh ty (substId old new (substId n fresh body)), md⟩
    else ⟨.Exists n ty (substId old new body), md⟩
  | e => e
...
def resolveExpr (ptMap : ConstrainedTypeMap) : StmtExprMd → StmtExprMd
  ...
  | ⟨.Forall n ty body, md⟩ =>
    ...
    ⟨.Forall n (resolveType ptMap ty) (injectQuantifierConstraint ptMap ty.val n body' true), md⟩
  | ⟨.Exists n ty body, md⟩ =>
    let body' := resolveExpr ptMap body
    ⟨.Exists n (resolveType ptMap ty) (injectQuantifierConstraint ptMap ty.val n body' false), md⟩
...
def elimStmt (ptMap : ConstrainedTypeMap)
  ...
  | .LocalVariable name ty init =>
    ...
    if isPred then modify fun pv => pv.insert name ty.val
    ...
    let init' := match init with
      | none => match ty.val with
        | .UserDefined n => (ptMap.get? n).map (·.witness)
        | _ => none
      | some _ => init
...
def elimProc (ptMap : ConstrainedTypeMap) (proc : Procedure) : Procedure :=
  ...
  let initVars : PredVarMap := proc.inputs.foldl (init := {}) fun s p =>
    if isConstrainedType ptMap p.type.val then s.insert p.name p.type.val else s
...
private def mkWitnessProc (ptMap : ConstrainedTypeMap) (ct : ConstrainedType) : Procedure :=
  ...
  let witnessInit : StmtExprMd :=
    ⟨.LocalVariable "$witness" (resolveType ptMap ct.base) (some ct.witness), md⟩
  let asserts := (getAllConstraints ptMap (.UserDefined ct.name)).map fun (vn, pred) =>
    ⟨.Assert (substId vn "$witness" pred), md⟩
  { name := s!"$witness_{ct.name}"
    inputs := []
    outputs := []
...
def constrainedTypeElim (program : Program) : Program :=
