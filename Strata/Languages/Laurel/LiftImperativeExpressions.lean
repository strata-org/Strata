...
/-- Check if an expression contains a LocalVariable with no initializer (recursively).
    These are generated by HeapParameterization when procedure calls appear in
    expression position and must be lifted to statement position. -/
private def containsUninitLocalVar (expr : StmtExprMd) : Bool :=
  match expr with
  | AstNode.mk val _ _ =>
  match val with
  | .LocalVariable _ _ none => true
  | .PrimitiveOp _ args => args.attach.any (fun x => containsUninitLocalVar x.val)
  | .StaticCall _ args => args.attach.any (fun x => containsUninitLocalVar x.val)
  | .Block stmts _ => stmts.attach.any (fun x => containsUninitLocalVar x.val)
  | .IfThenElse cond th el =>
      containsUninitLocalVar cond || containsUninitLocalVar th ||
      match el with | some e => containsUninitLocalVar e | none => false
  | _ => false
  termination_by expr
  decreasing_by
    all_goals ((try cases x); simp_all; try term_by_mem)
...
def transformStmt (stmt : StmtExprMd) : LiftM (List StmtExprMd) := do
  ...
  match val with
  | .Assert cond =>
      -- Transform the condition to lift imperative calls and nondet holes.
      -- After HeapParameterization, procedure calls in assert conditions are
      -- wrapped in blocks with LocalVariable/Assign nodes that must be lifted
      -- to statement position. We also lift when there are nondet holes.
      -- For user-written Laurel asserts with procedure calls or assignments
      -- (no LocalVariable-none or nondet holes), we leave them as-is so the
      -- Laurel-to-Core translator can reject them.
      if containsNondetHole cond || containsUninitLocalVar cond then
        let seqCond ← transformExpr cond
        let prepends ← takePrepends
        modify fun s => { s with subst := [] }
        return prepends ++ [⟨.Assert seqCond, source, md⟩]
      else
        return [stmt]
