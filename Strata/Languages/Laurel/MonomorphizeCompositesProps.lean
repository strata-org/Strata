...
public import Strata.Languages.Laurel.MonomorphizeComposites
import all Strata.Languages.Laurel.MonomorphizeComposites

/-! # MonomorphizeComposites Properties

Key theorem: `matchTypeArg_monotone` — type-argument inference only ever EXTENDS the
accumulator; a binding already present in `acc` is preserved in the result. Formally,
`acc[k]? = some v → matchTypeArg d a acc = some m → m[k]? = some v`.
