...
def translateExpr (fieldNames : List Identifier) (env : TypeEnv) (expr : StmtExprMd)
  ...
  | .PrimitiveOp op [e] =>
    match op with
    | .Not => .app Strata.SourceRange.none boolNotOp (translateExpr fieldNames env e boundVars)
    | .Neg => .app Strata.SourceRange.none intNegOp (translateExpr fieldNames env e boundVars)
  ...
  | .PrimitiveOp op [e1, e2] =>
    let binOp (bop : Core.Expression.Expr): Core.Expression.Expr :=
      LExpr.mkApp Strata.SourceRange.none bop [translateExpr fieldNames env e1 boundVars, translateExpr fieldNames env e2 boundVars]
    match op with
    | .Eq => .eq Strata.SourceRange.none (translateExpr fieldNames env e1 boundVars) (translateExpr fieldNames env e2 boundVars)
    | .Neq => .app Strata.SourceRange.none boolNotOp (.eq Strata.SourceRange.none (translateExpr fieldNames env e1 boundVars) (translateExpr fieldNames env e2 boundVars))
    | .And => binOp boolAndOp
    | .Or => binOp boolOrOp
  ...
  | .StaticCall name args =>
      ...
      args.foldl (fun acc arg => .app Strata.SourceRange.none acc (translateExpr fieldNames env arg boundVars)) fnOp
  ...
  | .Hole => .fvar Strata.SourceRange.none (Core.CoreIdent.locl s!"DUMMY_VAR_{env.length}") none
