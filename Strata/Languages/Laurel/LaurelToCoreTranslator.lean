...
def translateWithLaurel (options: LaurelTranslateOptions) (program : Program): TranslateResultWithLaurel :=
  ...
  let (program, heapParamDiags) := heapParameterization model program
  ...
  let allDiagnostics := resolutionErrors ++ diamondErrors ++ heapParamDiags ++ modifiesDiags ++ constrainedTypeDiags ++ translateState.diagnostics
