...
-/
protected def instType (d : TypeExpr) (bindings : Array TypeExpr) : TypeExpr := Id.run <|
  d.instTypeM fun idx =>
    if p : idx < bindings.size then
      pure <| bindings[bindings.size - (idx+1)]
    else
      .bvar (idx - bindings.size)
...
partial def expandMacros (dm : DialectMap) (f : PreType) (args : Nat → Option Arg) : Except Unit TypeExpr :=
  match f with
  | .ident i a => .ident i <$> a.mapM fun e => expandMacros dm e args
  | .arrow a b => .arrow <$> expandMacros dm a args <*> expandMacros dm b args
  | .fvar i a => .fvar i <$> a.mapM fun e => expandMacros dm e args
  ...
  | .funMacro i r => do
    ...
    | none =>
      .error ()
    | some a =>
      let addType (tps : Array TypeExpr) _ s args : Array TypeExpr :=
  ...
  | .syncat decl =>
    ...
    for a in args do
      ...
      | _ =>
        ...
        return default
    let sc : SyntaxCat := SyntaxCat.mk ident scArgs
...
partial def translateBindingKind (tree : Tree) : ElabM BindingKind := do
  ...
  | q`Init.TypeIdent, 1 => do
    ...
    | .syncat decl =>
      ...
      let scArgs ← args.mapM translateSyntaxCat
      return .cat { name := name, args := scArgs }
