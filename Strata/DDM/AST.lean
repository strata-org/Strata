...
-- Start of dialect related definitions.
...
def none : SourceLoc := { start := 0, stop := 0 }

def isNone (loc : SourceLoc) : Bool := loc.start = 0 ∧ loc.stop = 0
...
inductive PreType where
  /-- A dialect defined type. -/
| ident (ann : SourceLoc) (name : QualifiedIdent) (args : Array PreType)
  /-- A bound type variable at the given deBruijn index in the context. -/
| bvar (ann : SourceLoc) (index : Nat)
  /-- A reference to a global variable along with any arguments to ensure it is well-typed. -/
| fvar (ann : SourceLoc) (fvar : FreeVarIndex) (args : Array PreType)
  /-- A function type. -/
