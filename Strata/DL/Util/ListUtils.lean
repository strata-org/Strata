...
public def PredDisjoint (P Q : α → Prop) : Prop := ∀ a, P a → Q a → False

/-- Predicate `P` implies predicate `Q` -/
public def PredImplies (P Q : α → Prop) : Prop := ∀ a, P a → Q a
...
/--
 ...
 -/
public class ListP {α : Type} (π : α → Prop) (πs : List α → Prop) where
...
-/
public class Wrapper (α : Type) where
  self : α
...
@[simp]
public theorem List.Forall_cons (p : α → Prop) (x : α) : ∀ l : List α, Forall p (x :: l) ↔ p x ∧ Forall p l
  | [] => (and_iff_left_of_imp fun _ ↦ trivial).symm
  | _ :: _ => Iff.rfl

public theorem List.Forall_mem_iff : ∀ {l : List α}, Forall p l ↔ ∀ x ∈ l, p x
  | [] => (iff_true_intro <| forall_mem_nil _).symm
  | x :: l => by rw [List.forall_mem_cons, List.Forall_cons, List.Forall_mem_iff]

public theorem List.Forall_append : Forall P (a ++ b) ↔ Forall P a ∧ Forall P b := by
  apply Iff.intro
...
-/
public def List.replaceAll [BEq α] : List α → α → α → List α
  | [],    _, _ => []
...
public theorem List.removeAll_Sublist [BEq α] {xs ys : List α}:
  (xs.removeAll ys).Sublist xs := by
  induction xs
  case nil => simp_all
  case cons h t ih => simp [List.removeAll]

public theorem List.removeAll_Disjoint  [BEq α] [LawfulBEq α] {xs ys : List α}:
  (xs.removeAll ys).Disjoint ys := by
  induction xs <;> simp [removeAll, Disjoint] at *

public theorem List.Disjoint.mono (h₁ : a.Sublist b) (h₂ : c.Sublist d) :
  Disjoint b d → Disjoint a c := λ Hdis _ Hin1 Hin2 ↦
  Hdis (Sublist.mem Hin1 h₁) (Sublist.mem Hin2 h₂)

public theorem List.Disjoint.mono_left (h : a.Sublist b) :
  Disjoint b c → Disjoint a c := λ Hdis ↦ mono h (Sublist.refl c) Hdis

public theorem List.Disjoint.mono_right (h : c.Sublist d) :
  Disjoint a d → Disjoint a c := λ Hdis ↦ mono (Sublist.refl a) h Hdis

public theorem List.Disjoint.removeAll [BEq α] [LawfulBEq α ] {xs ys zs: List α} :
...
public theorem List.Disjoint_cons_head : (h :: t).Disjoint l → ¬h ∈ l := by
  intros Hdis Hin
  simp [Disjoint] at Hdis
  exact Hdis.1 Hin

public theorem List.Disjoint_cons_tail : (h :: t).Disjoint l → t.Disjoint l := by
  intros Hdis Hin
  simp [Disjoint] at Hdis
  exact Hdis.2 Hin

public theorem List.Disjoint_app :
...
public theorem List.Disjoint_Nodup_iff :
...
/-- From Mathlib4
    ...
    -/
public theorem List.Disjoint.symm : Disjoint a b → Disjoint b a := fun H _ Hin1 Hin2 => H Hin2 Hin1
...
public theorem List.Disjoint_Subset_right : Disjoint vs ks → ks'.Subset ks → vs.Disjoint ks' := by
  intros Hdis Hsub
...
public theorem List.Disjoint_Subset_left : Disjoint vs ks → List.Subset vs' vs → vs'.Disjoint ks := by
  intros Hdis Hsub
  apply List.Disjoint.symm
  apply Disjoint_Subset_right (Disjoint.symm Hdis) Hsub

public theorem List.Disjoint_Subsets : Disjoint vs ks → List.Subset vs' vs → List.Subset ks' ks → vs'.Disjoint ks' := by
  intros Hdis Hsub1 Hsub2
  exact List.Disjoint_Subset_left (Disjoint_Subset_right Hdis Hsub2) Hsub1

public theorem List.DisjointAppLeft' :
...
public theorem List.DisjointAppRight' :
  List.Disjoint vs (ks ++ ks') → List.Disjoint vs ks := by
  intros Hdist
  have Hdist' := List.Disjoint.symm_app Hdist
  exact List.DisjointAppLeft' Hdist'

public theorem List.Subset.subset_app_of_or_2 {l: List α}: l ⊆ l1 ∨ l ⊆ l2 → l ⊆ l1 ++ l2  := by
  simp [Subset, List.Subset]
  intro H a Ha
  cases H <;> simp_all

public theorem List.Subset.subset_app_of_or_3 {l: List α}: l ⊆ l1 ∨ l ⊆ l2 ∨ l ⊆ l3 → l ⊆ l1 ++ l2 ++ l3  := by
  simp [Subset, List.Subset]
  intro H a Ha
  cases H <;> try (rename_i H; cases H)
  any_goals simp_all

public theorem List.Subset.subset_app_of_or_4 {l: List α}: l ⊆ l1 ∨ l ⊆ l2 ∨ l ⊆ l3 ∨ l ⊆ l4 → l ⊆ l1 ++ l2 ++ l3 ++ l4 := by
  simp [Subset, List.Subset]
  intro H a Ha
  cases H <;> try (rename_i H; cases H <;> try (rename_i H; cases H))
  any_goals simp_all

public theorem List.Subset.assoc {l: List α}: l ⊆ l1 ++ l2 ++ l3 ↔ l ⊆ l1 ++ (l2 ++ l3) := by
  simp [Subset, List.Subset]
...
public theorem List.removeAll_app {α : Type} [BEq α] {xs₁ xs₂ ys : List α}:
  ys.removeAll (xs₁ ++ xs₂) =
...
public theorem List.removeAll_comm {α : Type} [BEq α] {xs₁ xs₂ ys : List α}:
...
public theorem List.mem_zip_1 {l₁ : List α} {l₂ : List β}  :
l₁.length = l₂.length →
...
public theorem List.mem_zip_2 {l₁ : List α} {l₂ : List β}  :
l₁.length = l₂.length →
...
public theorem List.PredDisjoint_Disjoint :
  Forall P as →
  Forall Q bs →
  PredDisjoint P Q →
  Disjoint as bs := by
...
public theorem List.Forall_PredImplies :
...
public theorem List.Forall_flatMap :
