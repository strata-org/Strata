...
structure FuncWF {IdentT ExprT TyT MetadataT : Type}
  ...
  precond_freevars:
    ∀ p, p ∈ f.preconditions →
      getVarNames p ⊆ f.inputs.map (getName ·.1)
...
instance FuncWF.precond_freevars_decidable
    ...
    Decidable (∀ p, p ∈ f.preconditions →
      getVarNames p ⊆ f.inputs.map (getName ·.1)) := by
  exact List.decidableBAll (fun x => getVarNames x ⊆ f.inputs.map (getName ·.1))
    f.preconditions
