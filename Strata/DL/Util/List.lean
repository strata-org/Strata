...
-/
def deduplicate {α : Type} [DecidableEq α] : List α → List α
  | [] => []
  | a :: as =>
    let as := as.deduplicate
    if a ∈ as then as else a :: as

/--
A deduplicated list satisfies `Nodup`.
-/
theorem nodup_deduplicate {α : Type} [DecidableEq α] (l : List α) :
  l.deduplicate.Nodup := by
  induction l with
...
theorem length_dedup_le {α : Type} [DecidableEq α] (l : List α) :
  l.deduplicate.length ≤ l.length := by
  induction l with
  | nil => simp [deduplicate]
  | cons a as ih =>
    simp [deduplicate]
...
theorem length_dedup_cons_le {α : Type} [DecidableEq α] (a : α) (l : List α) :
  l.deduplicate.length ≤ (a :: l).deduplicate.length := by
  induction l with
  | nil => simp [deduplicate]
  | cons a as ih =>
    simp [deduplicate]
    ...
    · rename_i a' h
      ...
      by_cases a' = a
      · simp_all
      · by_cases a' ∈ as.deduplicate <;> simp_all

theorem mem_dedup_of_mem {α : Type} [DecidableEq α]
  (l : List α) (a : α) : a ∈ l.deduplicate → a ∈ l := by
  induction l with
  | nil => simp [deduplicate]
  | cons b bs ih =>
    simp [deduplicate]
...
theorem mem_of_mem_deduplicate {α : Type} [DecidableEq α]
  (l : List α) (a : α) : a ∈ l → a ∈ l.deduplicate := by
  induction l with
  | nil => simp [deduplicate]
  | cons b bs ih =>
    simp [deduplicate]
    intro h; cases h
    · subst a
      by_cases b ∈ bs.deduplicate <;> simp_all
    · by_cases b ∈ bs.deduplicate <;> simp_all
...
of `l`.
-/
theorem mem_of_deduplicate {α : Type} [DecidableEq α]
  (l : List α) (a : α) : a ∈ l ↔ a ∈ l.deduplicate := by
  apply Iff.intro
  exact fun h => mem_of_mem_deduplicate l a h
  exact fun h => mem_dedup_of_mem l a h

theorem length_dedup_cons_of_mem {α : Type} [DecidableEq α] (a : α) (l : List α)
  (h : a ∈ l) : (a :: l).deduplicate.length = l.deduplicate.length := by
  simp [deduplicate]
  have : a ∈ l.deduplicate := mem_of_mem_deduplicate l a h
  simp [this]

theorem length_dedup_cons_of_not_mem {α : Type} [DecidableEq α] (a : α) (l : List α)
  (h : a ∉ l) : (a :: l).deduplicate.length = 1 + l.deduplicate.length := by
  induction l
  · simp_all [deduplicate]
...
theorem mem_append_left_of_mem_deduplicate {α : Type} [DecidableEq α] (a : α) (l₁ l₂ : List α)
  (h1 : ¬a ∈ l₂.deduplicate) (h2 : a ∈ (l₁ ++ l₂).deduplicate) :
  a ∈ l₁ := by
  have := @mem_dedup_of_mem _ _ (l₁ ++ l₂) a (by assumption)
  have := @mem_dedup_of_mem _ _ l₂ a
  simp_all; cases this
  · assumption
  · have := @mem_of_mem_deduplicate _ _ l₂ a (by assumption)
    contradiction

theorem mem_append_right_of_mem_deduplicate {α : Type} [DecidableEq α] (a : α) (l₁ l₂ : List α)
  (h1 : ¬a ∈ l₁.deduplicate) (h2 : a ∈ (l₁ ++ l₂).deduplicate) :
  a ∈ l₂ := by
  have := @mem_dedup_of_mem _ _ (l₁ ++ l₂) a (by assumption)
  have := @mem_dedup_of_mem _ _ l₁ a
...
theorem length_dedup_append_le_sum {α : Type} [DecidableEq α] (l₁ l₂ : List α) :
  (l₁ ++ l₂).deduplicate.length ≤ l₁.deduplicate.length + l₂.deduplicate.length := by
  induction l₁ generalizing l₂
  · simp_all
  · rename_i head tail ih
    simp [deduplicate]
    by_cases h1 : head ∈ tail.deduplicate
    · have : head ∈ (tail ++ l₂).deduplicate := by
        have := @mem_dedup_of_mem _ _ tail head h1
        have := @mem_of_mem_deduplicate _ _ (tail ++ l₂) head
        simp_all
      simp_all
    · simp_all
      by_cases h2 : head ∈ l₂.deduplicate
      · have : head ∈ (tail ++ l₂).deduplicate := by
      ...
      · have : head ∉ (tail ++ l₂).deduplicate := by
          ...
          intro h
          simp_all
          have := @mem_of_mem_deduplicate _ _ tail head
          have := @mem_of_mem_deduplicate _ _ l₂ head
          simp_all
        simp_all
        have := ih l₂
        omega
...
theorem length_dedup_of_removeAll {α : Type} [DecidableEq α] (a : α) (l : List α)
  (h : a ∈ l) :
  l.deduplicate.length = 1 + (l.removeAll [a]).deduplicate.length := by
  induction l
  case nil => simp_all
  case cons x xs ih =>
    simp [deduplicate]
    ...
    case pos =>
      ...
      split
      · rename_i h_x_xs
        have : x ∈ xs := by exact (mem_of_deduplicate xs x).mpr h_x_xs
        have ih' := ih this
        simp_all [removeAll]
      · simp [removeAll]
        have : x ∉ xs := by
          have := @mem_of_deduplicate _ _ xs x
          simp_all
    ...
    case neg =>
      ...
      · rename_i hx
        ...
        have : x ∈ xs.removeAll [a] := by
          simp [removeAll, h']
          exact (mem_of_deduplicate xs x).mpr hx
        simp_all
...
theorem length_dedup_append_le_left {α : Type} [DecidableEq α] (l₁ l₂ : List α) :
  l₁.deduplicate.length ≤ (l₁ ++ l₂).deduplicate.length := by
  induction l₁ generalizing l₂
  case nil => simp [deduplicate]
  case cons a as ih =>
    simp [deduplicate]
    split
    · rename_i h
      have : a ∈ as := by exact (mem_of_deduplicate as a).mpr h
      have : a ∈ (as ++ l₂).deduplicate := by
        have : a ∈ as ++ l₂ := by simp_all
        exact (mem_of_deduplicate (as ++ l₂) a).mp this
      simp_all
    · by_cases ha : a ∈ (as ++ l₂).deduplicate
      case pos =>
        rename_i h_a_as
        simp_all
...
theorem length_dedup_append_all_in_right {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h : l₁.all (fun e => e ∈ l₂)) :
  (l₁ ++ l₂).deduplicate.length = l₂.deduplicate.length := by
  induction l₁
  · simp_all
  · rename_i head tail ih
    simp_all
    ...
    have : x ∈ (xrest ++ l₂).deduplicate := by
      exact @mem_of_mem_deduplicate _ _ (xrest ++ l₂) x (by assumption)
    simp_all
    done
...
theorem length_dedup_all_in_eq {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h1 : l₁.all (fun e => e ∈ l₂))
  (h2 : l₂.all (fun e => e ∈ l₁)) :
  l₁.deduplicate.length = l₂.deduplicate.length := by
...
theorem length_dedup_of_all_in_not_mem_lt {α : Type} [DecidableEq α] (l₁ l₂ : List α) (a : α)
  ...
  | nil =>
    simp_all [deduplicate]
    have : a ∈ l₂.deduplicate := by
      have := @mem_of_deduplicate _ _ l₂ a
      simp_all
  ...
  | cons head tail ih =>
    ...
    · rename_i h_head_not_in_tail
      ...
      case pos =>
        simp_all [@mem_of_deduplicate _ _ tail head]
      case neg =>
...
theorem length_dedup_of_subset_le {α : Type} [DecidableEq α] (l₁ l₂ : List α)
  (h : l₁ ⊆ l₂) : l₁.deduplicate.length ≤ l₂.deduplicate.length := by
  induction l₁ with
  | nil => simp_all [deduplicate]
  | cons head tail ih =>
    ...
    case neg =>
      ...
      have h_head_deduplicate : head ∉ tail.deduplicate := by
        have := @mem_of_deduplicate _ _ tail head
        simp_all
      simp_all [deduplicate]
      omega
