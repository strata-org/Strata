...
variable {Identifier : Type} [DecidableEq Identifier] {ExtraRestrict: Type} [DecidableEq ExtraRestrict]

/--
Compute the free variables in an `LExpr`, which are simply all the `LExpr.fvar`s
in it.
-/
def freeVars (e : LExpr (LMonoTy ExtraRestrict) Identifier) : IdentTs Identifier ExtraRestrict :=
  match e with
  | .const _ _ => []
  | .op _ _ => []
  | .bvar _ => []
...
-/
def fresh (x : IdentT Identifier ExtraRestrict) (e : LExpr (LMonoTy ExtraRestrict) Identifier) : Bool :=
  x ∉ (freeVars e)

/-- An expression `e` is closed if has no free variables. -/
def closed (e : LExpr (LMonoTy ExtraRestrict) Identifier) : Bool :=
  freeVars e |>.isEmpty

@[simp]
theorem fresh_abs :
  fresh (Identifier:=Identifier) (ExtraRestrict:=ExtraRestrict) x (.abs ty e) = fresh x e := by
  simp [fresh, freeVars]

@[simp]
theorem fresh_mdata :
  fresh (Identifier:=Identifier) (ExtraRestrict:=ExtraRestrict) x (.mdata info e) = fresh x e := by
  simp [fresh, freeVars]
...
-/
def substK (k : Nat) (s : LExpr (LMonoTy ExtraRestrict) Identifier) (e : LExpr (LMonoTy ExtraRestrict) Identifier) : LExpr (LMonoTy ExtraRestrict) Identifier :=
  match e with
...
-/
def subst (s : LExpr (LMonoTy ExtraRestrict) Identifier) (e : LExpr (LMonoTy ExtraRestrict) Identifier) : LExpr (LMonoTy ExtraRestrict) Identifier :=
  substK 0 s e
...
-/
def varOpen (k : Nat) (x : IdentT Identifier ExtraRestrict) (e : LExpr (LMonoTy ExtraRestrict) Identifier) : LExpr (LMonoTy ExtraRestrict) Identifier :=
...
`(.bvar k)`.
-/
def varClose (k : Nat) (x : IdentT Identifier ExtraRestrict) (e : LExpr (LMonoTy ExtraRestrict) Identifier) : LExpr (LMonoTy ExtraRestrict) Identifier :=
  match e with
  | .const c ty => .const c ty
  | .op o ty => .op o ty
  | .bvar i => .bvar i
...
theorem varClose_of_varOpen (h : fresh x e) :
  varClose (Identifier:=Identifier) (ExtraRestrict:=ExtraRestrict) i x (varOpen i x e) = e := by
...
-/
def lcAt (k : Nat) (e : LExpr (LMonoTy ExtraRestrict) Identifier) : Bool :=
  match e with
  | .const _ _ => true
  | .op _ _ => true
  | .bvar i => i < k
...
theorem varOpen_varClose_when_lcAt
  (h1 : lcAt k e) (h2 : k <= i) :
  (varOpen i x (varClose (Identifier:=Identifier) (ExtraRestrict:=ExtraRestrict) i x e)) = e := by
...
-/
def WF (e : LExpr (LMonoTy ExtraRestrict) Identifier) : Bool :=
  lcAt 0 e
...
-/
def substFvar {Identifier: Type} [DecidableEq Identifier] (e : LExpr (LMonoTy ExtraRestrict) Identifier) (fr : Identifier) (to : LExpr (LMonoTy ExtraRestrict) Identifier)
  : (LExpr (LMonoTy ExtraRestrict) Identifier) :=
  match e with
...
def substFvars {Identifier: Type} [DecidableEq Identifier] (e : LExpr (LMonoTy ExtraRestrict) Identifier) (sm : Map Identifier (LExpr (LMonoTy ExtraRestrict) Identifier))
  : LExpr (LMonoTy ExtraRestrict) Identifier :=
