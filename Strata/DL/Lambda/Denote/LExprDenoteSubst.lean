...
theorem denoteArgs_get
    ...
    (h_wt : List.Rel₂ (LExpr.HasTypeA Δ) exprs tys)
    (i : Nat)
    ...
    : (denoteArgs (tcInterp := tcInterp) (opInterp := opInterp) fvarVal vt bvarVal exprs tys h_wt).get i h_sort
      = LExpr.denote tcInterp opInterp fvarVal vt bvarVal e ty (List.Rel₂.get? h_wt i h_e h_ty) := by
...
private theorem go_typeCheck [DecidableEq T.IDMeta]
    ...
    {tys : List LMonoTy}
    (h_wt : List.Rel₂ (LExpr.HasTypeA Δ_outer) (bindings.map Prod.snd) tys)
  ...
  | fvar m name ty =>
    ...
    | some ty =>
      ...
      | some e' =>
        ...
        -- From h_annot: ty = ty'
        -- From Rel₂.get?: HasTypeA Δ_outer e' ty'
...
theorem substFvarsLifting_typeCheck [DecidableEq T.IDMeta]
    ...
    {Δ : List LMonoTy}
    {tys : List LMonoTy}
    (h_wt : List.Rel₂ (LExpr.HasTypeA Δ) (bindings.map Prod.snd) tys)
    {e : LExpr T.mono} {τ : LMonoTy}
    (h_annot : fvars_annotated_by (bindings.map Prod.fst |>.zip tys) e)
    ...
    | some ty =>
      ...
      | some e =>
        ...
        have h_sort : (sortBindings.map Prod.snd)[i]? = some (LMonoTy.substTyVars vt τ) := by
          ...
          have h_wt_i := List.Rel₂.get? h_wt i h_val_b h_tys_eq
        ...
        · have h_tys_eq : tys[i]? = some τ := by
            ...
            have h_wt_i := List.Rel₂.get? h_wt i h_val_b h_tys_i
