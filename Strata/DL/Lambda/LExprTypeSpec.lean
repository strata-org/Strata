...
inductive HasType {T: LExprParams} [DecidableEq T.IDMeta] (C: LContext T):
 ...
 `a` is not in the context.
  ...
  /-- If an expression has type `tcons name args` and `name` is a type alias in
  `Γ.aliases`, then the expression also has the expanded alias type. This rule
  captures the semantic equivalence between a type alias and its definition. -/
  | talias : ∀ Γ e name args mty,
            _root_.Lambda.TypeAlias.expandsTo Γ.aliases name args mty →
            HasType C Γ e (.forAll [] (.tcons name args)) →
      ...
      · rename_i v2 h_tcons
        ...
        have h_keys_fresh :=
          LMonoTys.resolveAliases_allKeysFresh args Env args' Env1 h_args
            h_fresh h_vals_fresh h_alias_wf h_args_fvs
        have h_vals_fresh' :=
        ...
        have h_args_fresh' : ∀ tv, tv ∈ LMonoTys.freeVars args' →
            TContext.isFresh (T := T) tv Env.context :=
          LMonoTys.resolveAliases_fvs_fresh args Env args' Env1 h_args h_vals_fresh h_alias_wf h_args_fvs
  ...
  | .tcons name args =>
    ...
    · rename_i v1 h_args_ra
      ...
      · rename_i v2 h_tcons
        ...
        have h_vals_fresh_mid :=
          LMonoTys.resolveAliases_vals_fresh args Env args' Env1 h_args_ra
            h_vals_fresh h_alias_wf h_args_fvs
        -- tconsAlias: if no alias, output = tcons name args' (fresh by h_args'_fresh)
        -- if alias, output = subst S (instantiatedDefinition) where S and instantiatedDefinition
        -- only contain fresh generated vars and args' vars (all fresh)
        exact tconsAlias_fvs_fresh name args' Env1 mty'' Env2 Env.context
          h_tcons h_ctx_eq h_args'_fresh h_vals_fresh_mid h_alias_wf
    ...
    · rename_i v1 h_args
      ...
      · rename_i v2 h_tcons
        ...
        exact Subst.absorbs_trans
          ...
          (LMonoTys.resolveAliases_absorbs args Env args' Env1 h_args)
          (tconsAlias_absorbs name args' Env1 mty'' Env2 h_tcons)
  ...
  | some alias =>
    ...
    | .ok S =>
      ...
      exact unify_preserves_SubstFreshForGen h_u
        ...
        (by sorry) -- constraint fvs freshness (needs _freeVars_fresh for alias types)
...
private theorem LMonoTy_resolveAliases_preserves_SubstFreshForGen
  ...
  | .tcons name args =>
    ...
    split at h; · simp at h
    rename_i v2 h_tcons; obtain ⟨mty'', Env2⟩ := v2
    simp [Pure.pure, Except.pure] at h; obtain ⟨_, h2⟩ := h; rw [← h2]
    exact tconsAlias_preserves_SubstFreshForGen name args' Env1 mty'' Env2 h_tcons
      (LMonoTys_resolveAliases_preserves_SubstFreshForGen args Env args' Env1 h_args h_fresh)

/-- `LMonoTys.resolveAliases` preserves `SubstFreshForGen`. -/
private theorem LMonoTys_resolveAliases_preserves_SubstFreshForGen
  ...
  | mty :: mrest =>
    ...
    simp [Pure.pure, Except.pure] at h; obtain ⟨_, h2⟩ := h; rw [← h2]
    exact LMonoTys_resolveAliases_preserves_SubstFreshForGen mrest Env1 mrest' Env2 h_tl
      (LMonoTy_resolveAliases_preserves_SubstFreshForGen mty Env mty' Env1 h_hd h_fresh)
end
...
private theorem LTy_resolveAliases_preserves_SubstFreshForGen
  ...
  -- ty.instantiate only changes genEnv, not stateSubstInfo
  ...
  exact LMonoTy_resolveAliases_preserves_SubstFreshForGen mty0 _ mty Env' h
    (h_eq ▸ SubstFreshForGen.mono _ _ _  h_fresh (by
...
private theorem inferFVar_preserves_SubstFreshForGen
  ...
  | none =>
    simp at h; obtain ⟨_, h2⟩ := h; rw [← h2]
    exact LTy_instantiateWithCheck_preserves_SubstFreshForGen _ C Env mty Env1 h_inst h_fresh
  | some fty_val =>
    simp only [Except.mapError] at h
    split at h; · simp at h
    ...
    have h_fresh1 := LTy_instantiateWithCheck_preserves_SubstFreshForGen
      _ C Env mty Env1 h_inst h_fresh
    have h_fresh2 := LMonoTy_instantiateWithCheck_preserves_SubstFreshForGen
      fty_val C Env1 fty_inst Env2 h_inst2 h_fresh1
    have h_unify := unify_of_mapError h_mapError
...
private theorem resolveAux_preserves_SubstFreshForGen :
  ...
  suffices ∀ (n : Nat) (e : LExpr T.mono), e.sizeOf = n →
      ...
      (SubstFreshForGen Env.stateSubstInfo Env.genEnv.genState →
       ContextFreshForGen Env.context Env.genEnv.genState →
       SubstFreshForGen Env'.stateSubstInfo Env'.genEnv.genState) by
    intro et C Env Env' h h_sf h_cf h_ne
    exact (this e.sizeOf e rfl et C Env Env' h h_ne).2 h_sf h_cf
  intro n
  induction n using Nat.strongRecOn with
  | _ n ih =>
  ...
  | .app m e1 e2 =>
    ...
    have h_fresh1 := (ih e1.sizeOf h_sz1 e1 rfl e1t C Env Env1 h_res1 h_ne).2 h_fresh h_ctx
    have h_ne1 := (ih_context e1.sizeOf h_sz1 e1 rfl e1t C Env Env1 h_res1 h_ne) ▸ h_ne
    have h_ctx1 := ih_ctx e1.sizeOf h_sz1 e1 rfl e1t C Env Env1 h_res1 h_ne h_ctx
    have h_fresh2 := (ih e2.sizeOf h_sz2 e2 rfl e2t C Env1 Env2 h_res2 h_ne1).2 h_fresh1 h_ctx1
  ...
  | .quant m qk bty tr body =>
    ...
    · simp at h; obtain ⟨_, h_env⟩ := h; rw [← h_env]; simp [TEnv.eraseFromContext, TEnv.updateContext]
      ...
      exact (ih _ h_sz_tr _ rfl trT C Env2 Env3 h_rec_tr h_ne2).2 h_fresh2 h_ctx2
    · simp at h
  | .eq m e1 e2 =>
    simp only [resolveAux, Bind.bind, Except.bind, Except.mapError] at h
    split at h; · simp at h
  ...
  | .ite m c t e =>
    ...
    have h_fresh1 := (ih c.sizeOf h_sz_c c rfl ct C Env Env1 h_res_c h_ne).2 h_fresh h_ctx
    have h_ne1 := (ih_context c.sizeOf h_sz_c c rfl ct C Env Env1 h_res_c h_ne) ▸ h_ne
    have h_ctx1 := ih_ctx c.sizeOf h_sz_c c rfl ct C Env Env1 h_res_c h_ne h_ctx
    have h_fresh2 := (ih t.sizeOf h_sz_t t rfl tht C Env1 Env2 h_res_t h_ne1).2 h_fresh1 h_ctx1
    have h_ne2 := (ih_context t.sizeOf h_sz_t t rfl tht C Env1 Env2 h_res_t h_ne1) ▸ h_ne1
    have h_ctx2 := ih_ctx t.sizeOf h_sz_t t rfl tht C Env1 Env2 h_res_t h_ne1 h_ctx1
    have h_fresh3 := (ih e.sizeOf h_sz_e e rfl elt C Env2 Env3 h_res_e h_ne2).2 h_fresh2 h_ctx2
    have h_unify := unify_of_mapError h_mapError
...
theorem resolveAux_absorbs :
  ...
  induction n using Nat.strongRecOn with
  | _ n ih =>
  intro e h_eq et C Env Env' h h_env_fresh h_ne
  match e with
  | .const m c =>
    simp [resolveAux, inferConst] at h
    split at h
  ...
  | .app m e1 e2 =>
    ...
    have h_fresh1 := resolveAux_preserves_SubstFreshForGen
      e1 e1t C Env Env1 h_res1 h_fresh h_env_fresh.2 h_ne
    have h_ne1 := (resolveAux_context e1 e1t C Env Env1 h_res1 h_ne) ▸ h_ne
    -- Absorption from IHs
    have h_abs1 := ih e1.sizeOf (by subst h_eq; simp [LExpr.sizeOf]; omega) e1 rfl e1t C Env Env1 h_res1 h_env_fresh h_ne
    have h_abs2 := ih e2.sizeOf (by subst h_eq; simp [LExpr.sizeOf]; omega) e2 rfl e2t C Env1 Env2 h_res2
      ⟨h_fresh1, (resolveAux_context e1 e1t C Env Env1 h_res1 h_ne) ▸
        ContextFreshForGen.mono _ _ _ h_env_fresh.2
          (resolveAux_genState_mono e1 e1t C Env Env1 h_res1)⟩ h_ne1
  ...
  | .abs m bty body =>
    ...
    exact Subst.absorbs_trans
      Env.stateSubstInfo.subst Env1.stateSubstInfo.subst Env2.stateSubstInfo.subst
      h_abs1
      (ih _ h_sz _ rfl et' C Env1 Env2 h_rec h_env_fresh1 h_ne1)
  | .quant m qk bty tr body =>
    simp only [resolveAux, Bind.bind, Except.bind] at h
    split at h; · simp at h
    rename_i v1 h_tbv; obtain ⟨xv, xty, Env1⟩ := v1; simp at h h_tbv
    ...
    · simp at h; obtain ⟨_, h_env⟩ := h; rw [← h_env]
      ...
      have h_env_fresh2 : EnvFreshForGen Env2 :=
        ⟨resolveAux_preserves_SubstFreshForGen _ et' C Env1 Env2 h_rec_e h_env_fresh1.1 h_env_fresh1.2 h_ne1,
         (resolveAux_context _ et' C Env1 Env2 h_rec_e h_ne1) ▸
      ...
      exact Subst.absorbs_trans
        ...
        (Subst.absorbs_trans
          Env.stateSubstInfo.subst Env1.stateSubstInfo.subst Env2.stateSubstInfo.subst
          h_abs1
          (ih _ h_sz_e _ rfl et' C Env1 Env2 h_rec_e h_env_fresh1 h_ne1))
        (ih _ h_sz_tr _ rfl trT C Env2 Env3 h_rec_tr h_env_fresh2 h_ne2)
    · simp at h
  | .eq m e1 e2 =>
    simp only [resolveAux, Bind.bind, Except.bind, Except.mapError] at h
    split at h; · simp at h
    ...
    have h_unify := unify_of_mapError h_mapError
    have h_fresh1 := resolveAux_preserves_SubstFreshForGen
      e1 e1t C Env Env1 h_res1 h_env_fresh.1 h_env_fresh.2 h_ne
    have h_ne1 := (resolveAux_context e1 e1t C Env Env1 h_res1 h_ne) ▸ h_ne
    exact Subst.absorbs_trans
      Env.stateSubstInfo.subst Env2.stateSubstInfo.subst v3.subst
      (Subst.absorbs_trans
        Env.stateSubstInfo.subst Env1.stateSubstInfo.subst Env2.stateSubstInfo.subst
        (ih e1.sizeOf (by subst h_eq; simp [LExpr.sizeOf]; omega) e1 rfl e1t C Env Env1 h_res1 h_env_fresh h_ne)
  ...
  | .ite m c t e =>
    ...
    simp at h; obtain ⟨_, h_env⟩ := h; rw [← h_env]; simp [TEnv.updateSubst]
    have h_unify := unify_of_mapError h_mapError
    have h_ne1 := (resolveAux_context c ct C Env Env1 h_res_c h_ne) ▸ h_ne
    have h_ctx1 : ContextFreshForGen Env1.context Env1.genEnv.genState :=
      (resolveAux_context c ct C Env Env1 h_res_c h_ne) ▸
        ContextFreshForGen.mono _ _ _ h_env_fresh.2 (resolveAux_genState_mono c ct C Env Env1 h_res_c)
    have h_fresh1 := resolveAux_preserves_SubstFreshForGen
      c ct C Env Env1 h_res_c h_env_fresh.1 h_env_fresh.2 h_ne
    have h_ne2 := (resolveAux_context t tht C Env1 Env2 h_res_t h_ne1) ▸ h_ne1
    have h_ctx2 : ContextFreshForGen Env2.context Env2.genEnv.genState :=
      (resolveAux_context t tht C Env1 Env2 h_res_t h_ne1) ▸
    ...
    have h_fresh2 := resolveAux_preserves_SubstFreshForGen
      t tht C Env1 Env2 h_res_t h_fresh1 h_ctx1 h_ne1
    exact Subst.absorbs_trans
      ...
      (Subst.absorbs_trans
        ...
        (Subst.absorbs_trans
          Env.stateSubstInfo.subst Env1.stateSubstInfo.subst Env2.stateSubstInfo.subst
          (ih c.sizeOf (by subst h_eq; simp [LExpr.sizeOf]; omega) c rfl ct C Env Env1 h_res_c h_env_fresh h_ne)
          (ih t.sizeOf (by subst h_eq; simp [LExpr.sizeOf]; omega) t rfl tht C Env1 Env2 h_res_t ⟨h_fresh1, h_ctx1⟩ h_ne1))
        (ih e.sizeOf (by subst h_eq; simp [LExpr.sizeOf]; omega) e rfl elt C Env2 Env3 h_res_e ⟨h_fresh2, h_ctx2⟩ h_ne2))
      (unify_absorbs _ _ _ h_unify)
...
/-- `LTy.resolveAliases` preserves `allKeysFresh`.
    The subst is unchanged by `instantiate` (which only modifies genEnv), so
    preservation reduces to `LMonoTy.resolveAliases_allKeysFresh`. -/
private theorem LTy_resolveAliases_allKeysFresh
    (ty : LTy) (Env : TEnv T.IDMeta) (mty : LMonoTy) (Env' : TEnv T.IDMeta)
    (h : LTy.resolveAliases ty Env = .ok (mty, Env'))
    (h_sf : Subst.allKeysFresh Env.stateSubstInfo.subst Env.context)
    (h_vf : Subst.valsFresh Env.stateSubstInfo.subst Env.context)
    (h_aw : TContext.AliasesWF Env.context) :
    Subst.allKeysFresh Env'.stateSubstInfo.subst Env.context := by
  simp only [LTy.resolveAliases, Bind.bind, Except.bind] at h
  split at h; · simp at h
  rename_i v1 h_inst; obtain ⟨mty0, genEnv'⟩ := v1; simp at h h_inst
  -- h_inst : ty.instantiate Env.genEnv = .ok (mty0, genEnv')
  -- h : LMonoTy.resolveAliases mty0 {Env with genEnv := genEnv'} = .ok (mty, Env')
  -- {Env with genEnv := genEnv'}.stateSubstInfo = Env.stateSubstInfo
  -- {Env with genEnv := genEnv'}.context = Env.context
  have h_inst_ctx : genEnv'.context = Env.genEnv.context :=
    LTy.instantiate_context ty Env.genEnv mty0 genEnv' h_inst
  -- The intermediate env has same subst and (provably) same context as Env
  have h_ra := h  -- keep a copy
  -- resolveAliases_allKeysFresh gives us allKeysFresh w.r.t. Env_mid.context = genEnv'.context
  -- We need it w.r.t. Env.context = Env.genEnv.context. Use h_inst_ctx to bridge.
  have h_goal := LMonoTy.resolveAliases_allKeysFresh (T := T) mty0
    {Env with genEnv := genEnv'} mty Env' h
  -- The subst didn't change: {Env with genEnv := genEnv'}.stateSubstInfo = Env.stateSubstInfo
  -- The context: {Env with genEnv := genEnv'}.context = genEnv'.context = Env.genEnv.context = Env.context
  simp only [TEnv.context] at h_goal
  rw [h_inst_ctx] at h_goal
  exact h_goal h_sf h_vf h_aw
    (by -- h_fvs: freeVars of mty0 are fresh in context
        sorry)

/-- `LTy.instantiateWithCheck` preserves `allKeysFresh`. -/
private theorem LTy_instantiateWithCheck_allKeysFresh
    (ty : LTy) (C : LContext T) (Env : TEnv T.IDMeta) (mty : LMonoTy) (Env' : TEnv T.IDMeta)
    (h : LTy.instantiateWithCheck ty C Env = .ok (mty, Env'))
    (h_sf : Subst.allKeysFresh Env.stateSubstInfo.subst Env.context)
    (h_vf : Subst.valsFresh Env.stateSubstInfo.subst Env.context)
  ...
  split at h
  · simp [Pure.pure, Except.pure] at h; obtain ⟨_, h2⟩ := h; rw [← h2]
    exact LTy_resolveAliases_allKeysFresh ty Env mty0 Env1 h_res h_sf h_vf h_aw
  · simp at h

/-- `LMonoTy.instantiateWithCheck` preserves `allKeysFresh`. -/
private theorem LMonoTy_instantiateWithCheck_allKeysFresh
    (mty_in : LMonoTy) (C : LContext T) (Env : TEnv T.IDMeta) (mty : LMonoTy) (Env' : TEnv T.IDMeta)
    (h : LMonoTy.instantiateWithCheck mty_in C Env = .ok (mty, Env'))
    (h_sf : Subst.allKeysFresh Env.stateSubstInfo.subst Env.context)
    (h_vf : Subst.valsFresh Env.stateSubstInfo.subst Env.context)
  ...
  split at h
  · simp [Pure.pure, Except.pure] at h; obtain ⟨_, h2⟩ := h; rw [← h2]
    -- Env1 from LMonoTys.instantiateEnv: subst unchanged, context preserved
    have h_subst_eq : Env1.stateSubstInfo = Env.stateSubstInfo :=
      LMonoTys.instantiateEnv_stateSubstInfo _ _ Env _ Env1 h_inst
    have h_ctx_eq : Env1.context = Env.context :=
      LMonoTys.instantiateEnv_context _ _ Env _ Env1 h_inst
    have h_goal := LMonoTy.resolveAliases_allKeysFresh (T := T)
      (instTypes[0]'(by have := LMonoTys.instantiateEnv_length _ _ _ _ _ h_inst; simp at this; omega))
      Env1 mtyi Env2 h_res
    rw [h_subst_eq, h_ctx_eq] at h_goal
    exact h_goal h_sf h_vf h_aw
      (by -- h_fvs: freeVars of instTypes[0] are fresh in context
          -- After instantiateEnv, the mono type's free vars include generated (fresh) vars
          -- and original free vars which may not be fresh.
          sorry)
  · simp at h

/-- `inferFVar` preserves `allKeysFresh`. -/
private theorem inferFVar_allKeysFresh
    (C : LContext T) (Env : TEnv T.IDMeta) (x : T.Identifier) (fty : Option LMonoTy)
    (ty_res : LMonoTy) (Env' : TEnv T.IDMeta)
    (h : inferFVar C Env x fty = .ok (ty_res, Env'))
    (h_sf : Subst.allKeysFresh Env.stateSubstInfo.subst Env.context)
  ...
  · rename_i ty h_find
    ...
    · rename_i v1 h_inst
      obtain ⟨mty, Env1⟩ := v1; dsimp at h h_inst
      cases fty with
      | none =>
        simp at h; obtain ⟨_, h2⟩ := h; rw [← h2]
        exact LTy_instantiateWithCheck_allKeysFresh ty C Env mty Env1 h_inst h_sf h_vf h_aw
      | some fty_val =>
        simp only [Except.mapError] at h
        split at h; · simp at h
        rename_i v2 h_inst2; obtain ⟨fty_inst, Env2⟩ := v2; dsimp at h h_inst2
        ...
        simp at h; obtain ⟨_, h2⟩ := h; rw [← h2]; simp [TEnv.updateSubst]
        have h_unify := unify_of_mapError h_mapError
        have h_sf1 := LTy_instantiateWithCheck_allKeysFresh
          ty C Env mty Env1 h_inst h_sf h_vf h_aw
        have h_ctx1 := LTy_instantiateWithCheck_context ty C Env mty Env1 h_inst
        have h_sf2 := LMonoTy_instantiateWithCheck_allKeysFresh
          fty_val C Env1 fty_inst Env2 h_inst2
          (h_ctx1 ▸ h_sf1)
          (h_ctx1 ▸ sorry) -- valsFresh for Env1 w.r.t. Env.context
          (h_ctx1 ▸ h_aw)
        have h_ctx2 := LMonoTy_instantiateWithCheck_context fty_val C Env1 fty_inst Env2 h_inst2
        rw [h_ctx1] at h_sf2
        exact Constraints.unify_allKeysFresh h_unify h_sf2
          (by sorry) -- constraint fvs freshness
          (by sorry) -- subst value fvs freshness for Env2
...
-/
theorem resolveAux_keys_vals_fresh :
    ∀ (e : LExpr T.mono) (et : LExprT T.mono) (C : LContext T)
      (Env Env' : TEnv T.IDMeta),
      resolveAux C Env e = .ok (et, Env') →
      TEnvWF Env →
      ...
      Subst.valsFresh Env'.stateSubstInfo.subst Env.context := by
  intro e
  induction e with
  | const m c =>
    ...
    | .ok (ty, Env1) =>
      simp at h; obtain ⟨_, h2⟩ := h; rw [← h2]
      simp [inferConst] at h_ic; split at h_ic
      · simp at h_ic; obtain ⟨_, h_env⟩ := h_ic; subst h_env; exact ⟨h_sf, h_vf⟩
      · simp at h_ic
  | bvar m i =>
    intro et C Env Env' h _ _; simp [resolveAux] at h
  | fvar m x fty =>
    intro et C Env Env' h h_envwf _
    have h_sf := h_envwf.keysFresh; have h_vf := h_envwf.valsFresh; have h_aw := h_envwf.aliasesWF
    ...
    · rename_i v1 h_infer
      obtain ⟨ty_res, Env_res⟩ := v1; simp at h
      obtain ⟨_, h_env⟩ := h; rw [← h_env]
      exact ⟨inferFVar_allKeysFresh C Env x fty ty_res Env_res h_infer h_sf h_vf h_aw,
             sorry⟩ -- valsFresh for inferFVar
  | op m o oty =>
    intro et C Env Env' h h_envwf _
    have h_sf := h_envwf.keysFresh; have h_vf := h_envwf.valsFresh; have h_aw := h_envwf.aliasesWF
    simp only [resolveAux, Bind.bind, Except.bind] at h
    ...
    | none =>
      simp at h; obtain ⟨_, h2⟩ := h; rw [← h2]
      exact ⟨LTy_instantiateWithCheck_allKeysFresh type_val C Env ty_inst Env1 h_inst h_sf h_vf h_aw,
             sorry⟩ -- valsFresh for instantiateWithCheck
    | some oty_val =>
      simp only [Except.mapError] at h
      split at h; · simp at h
      rename_i v2 h_inst2; obtain ⟨oty_inst, Env2⟩ := v2; dsimp at h h_inst2
      ...
      have h_unify := unify_of_mapError h_mapError
      have h_sf1 := LTy_instantiateWithCheck_allKeysFresh
        type_val C Env ty_inst Env1 h_inst h_sf h_vf h_aw
      have h_ctx1 := LTy_instantiateWithCheck_context type_val C Env ty_inst Env1 h_inst
      have h_sf2 := LMonoTy_instantiateWithCheck_allKeysFresh
        oty_val C Env1 oty_inst Env2 h_inst2
        (h_ctx1 ▸ h_sf1)
        (h_ctx1 ▸ sorry) -- valsFresh for Env1 w.r.t. Env.context
        (h_ctx1 ▸ h_aw)
      ...
      exact ⟨Constraints.unify_allKeysFresh h_unify h_sf2
        (by sorry) -- constraint fvs freshness
        (by sorry), -- subst value fvs freshness for Env2
        sorry⟩ -- valsFresh for unify output
  | app m e1 e2 ih1 ih2 =>
    intro et C Env Env' h h_envwf h_ne
    have h_sf := h_envwf.keysFresh; have h_vf := h_envwf.valsFresh; have h_aw := h_envwf.aliasesWF
    -- Decompose resolveAux for .app
    simp only [resolveAux, Bind.bind, Except.bind, Except.mapError] at h
    split at h; · simp at h
    rename_i v1 h_res1; obtain ⟨e1t, Env1⟩ := v1; dsimp at h h_res1
    split at h; · simp at h
    ...
    have h_gen_fresh := TEnv.genTyVar_isFresh Env2 fresh_name Env3 h_gen
    have h_ctx1 := resolveAux_context e1 e1t C Env Env1 h_res1 h_ne
    have h_ne1 := h_ctx1 ▸ h_ne
    -- IH for e1: allKeysFresh ∧ valsFresh for Env1.subst w.r.t. Env.context
    have h_ih1 := ih1 e1t C Env Env1 h_res1 h_envwf h_ne
    have h_ctx2 := resolveAux_context e2 e2t C Env1 Env2 h_res2 h_ne1
    -- Build TEnvWF for Env1 (using both components from IH)
    have h_envwf1 : TEnvWF Env1 :=
      { keysFresh := h_ctx1 ▸ h_ih1.1
        valsFresh := h_ctx1 ▸ h_ih1.2
        ...
        boundVarsWF := fun y ty hf => by
          have hf' := h_ctx1 ▸ hf
          obtain ⟨h1, h2⟩ := h_envwf.boundVarsWF y ty hf'
          exact ⟨h1, fun v hv => by rw [show Env1.genEnv.context = Env.genEnv.context from h_ctx1]; exact h2 v hv⟩ }
    -- IH for e2: allKeysFresh ∧ valsFresh for Env2.subst w.r.t. Env1.context
    have h_ih2 := ih2 e2t C Env1 Env2 h_res2 h_envwf1 h_ne1
    -- allKeysFresh Env3.subst = allKeysFresh Env2.subst (genTyVar preserves subst)
    rw [h_gen_subst] at h_unify
    -- allKeysFresh of unify output
    have h_fresh_v4 : Subst.allKeysFresh v4.subst Env.context := by
      rw [h_ctx1] at h_ih2
      exact Constraints.unify_allKeysFresh h_unify h_ih2.1
        (by sorry) -- constraint fvs freshness
        (h_ih2.2) -- subst value fvs freshness for Env2 (from IH!)
    -- allKeysFresh of remove: weaker than allKeysFresh of original
    exact ⟨Subst.allKeysFresh_of_remove h_fresh_v4, sorry⟩ -- valsFresh for remove
  | abs m bty e ih =>
    intro et C Env Env' h h_envwf _
    exact ⟨sorry, sorry⟩ -- abs case: typeBoundVar + recursive + eraseFromContext
  | quant m qk bty triggers e ih_e ih_t =>
    intro et C Env Env' h h_envwf _
    exact ⟨sorry, sorry⟩ -- quant case: similar to abs
  | eq m e1 e2 ih1 ih2 =>
    intro et C Env Env' h h_envwf h_ne
    have h_sf := h_envwf.keysFresh; have h_vf := h_envwf.valsFresh; have h_aw := h_envwf.aliasesWF
    ...
    simp [resolveAux, Bind.bind, Except.bind] at h
    split at h; · simp at h
    · rename_i v1 h_res1; obtain ⟨e1t, Env1⟩ := v1; simp at h h_res1
      split at h; · simp at h
      · rename_i v2 h_res2; obtain ⟨e2t, Env2⟩ := v2; simp at h h_res2
        split at h; · simp at h
        · rename_i v3 h_mapError
          simp at h; obtain ⟨_, h_env⟩ := h; rw [← h_env]
          simp [TEnv.updateSubst]
          ...
          have h_unify := unify_of_mapError h_mapError
          -- constraint fvs are fresh (from resolveAux output types)
          have h_cs_fresh : ∀ tv, tv ∈ Constraints.freeVars [(e1t.toLMonoTy, e2t.toLMonoTy)] →
              TContext.isFresh (T := T) tv Env.context := by sorry
          rw [h_ctx1] at h_ih2
          exact ⟨Constraints.unify_allKeysFresh h_unify h_ih2.1 h_cs_fresh h_ih2.2,
                 Constraints.unify_vals_fresh h_unify h_cs_fresh h_ih2.2⟩
  | ite m c th el ih_c ih_t ih_e =>
    intro et C Env Env' h h_envwf h_ne
    have h_sf := h_envwf.keysFresh; have h_vf := h_envwf.valsFresh; have h_aw := h_envwf.aliasesWF
    -- Decompose resolveAux for .ite
    simp [resolveAux, Bind.bind, Except.bind] at h
    split at h; · simp at h
    · rename_i v1 h_res_c; obtain ⟨ct, Env1⟩ := v1; simp at h h_res_c
      split at h; · simp at h
      · rename_i v2 h_res_t; obtain ⟨tht, Env2⟩ := v2; simp at h h_res_t
        split at h; · simp at h
        · rename_i v3 h_res_e; obtain ⟨elt, Env3⟩ := v3; simp at h h_res_e
          split at h; · simp at h
          · rename_i v4 h_mapError
            simp at h; obtain ⟨_, h_env⟩ := h; rw [← h_env]
            simp [TEnv.updateSubst]
            ...
            have h_ne1 := h_ctx1 ▸ h_ne
            have h_ih_c := ih_c ct C Env Env1 h_res_c h_envwf h_ne
            have h_ctx2 := resolveAux_context th tht C Env1 Env2 h_res_t h_ne1
            have h_ne2 := h_ctx2 ▸ h_ne1
            -- Build TEnvWF for Env1 (using both components from IH)
            have h_envwf1 : TEnvWF Env1 :=
              { keysFresh := h_ctx1 ▸ h_ih_c.1
                valsFresh := h_ctx1 ▸ h_ih_c.2
                aliasesWF := h_ctx1 ▸ h_aw
                ...
                boundVarsWF := fun y ty hf => by
                  ...
                  obtain ⟨h1, h2⟩ := h_envwf.boundVarsWF y ty hf'
                  exact ⟨h1, fun v hv => by rw [show Env1.genEnv.context = Env.genEnv.context from h_ctx1]; exact h2 v hv⟩ }
            have h_ih_t := ih_t tht C Env1 Env2 h_res_t h_envwf1 h_ne1
            have h_ctx3 := resolveAux_context el elt C Env2 Env3 h_res_e h_ne2
            -- Build TEnvWF for Env2 (using both components from IH)
            have h_envwf2 : TEnvWF Env2 :=
              { keysFresh := h_ctx2 ▸ h_ih_t.1
                valsFresh := h_ctx2 ▸ h_ih_t.2
                aliasesWF := h_ctx2 ▸ h_ctx1 ▸ h_aw
                substFreshForGen := resolveAux_preserves_SubstFreshForGen th tht C Env1 Env2 h_res_t h_envwf1.substFreshForGen h_envwf1.ctxFreshForGen h_ne1
                ctxFreshForGen := h_ctx2 ▸ ContextFreshForGen.mono _ _ _ h_envwf1.ctxFreshForGen (resolveAux_genState_mono th tht C Env1 Env2 h_res_t)
                boundVarsWF := fun y ty hf => by
                  have hf' := h_ctx2 ▸ hf
                  obtain ⟨h1, h2⟩ := h_envwf1.boundVarsWF y ty hf'
                  exact ⟨h1, fun v hv => by rw [show Env2.genEnv.context = Env1.genEnv.context from h_ctx2]; exact h2 v hv⟩ }
            have h_ih_e := ih_e elt C Env2 Env3 h_res_e h_envwf2 h_ne2
            have h_unify := unify_of_mapError h_mapError
            have h_ih_e_ctx : Subst.allKeysFresh Env3.stateSubstInfo.subst Env.context ∧
                Subst.valsFresh Env3.stateSubstInfo.subst Env.context := by
              constructor
              · rw [← h_ctx1, ← h_ctx2]; exact h_ih_e.1
              · rw [← h_ctx1, ← h_ctx2]; exact h_ih_e.2
            have h_cs_fresh : ∀ tv, tv ∈ Constraints.freeVars [(toLMonoTy ct, LMonoTy.bool), (toLMonoTy tht, toLMonoTy elt)] →
                TContext.isFresh (T := T) tv Env.context := by sorry
            exact ⟨Constraints.unify_allKeysFresh h_unify h_ih_e_ctx.1 h_cs_fresh h_ih_e_ctx.2,
                   Constraints.unify_vals_fresh h_unify h_cs_fresh h_ih_e_ctx.2⟩

/-- Corollary: extract just `allKeysFresh` from the combined theorem. -/
theorem resolveAux_keys_fresh
    ...
    Subst.allKeysFresh Env'.stateSubstInfo.subst Env.context :=
  (resolveAux_keys_vals_fresh e et C Env Env' h h_wf h_ne).1
...
mutual
private theorem resolveAliases_subst_eq
    (mty_in : LMonoTy) (Env : TEnv T.IDMeta) (mty_out : LMonoTy) (Env' : TEnv T.IDMeta)
    (h : LMonoTy.resolveAliases mty_in Env = .ok (mty_out, Env')) :
    LMonoTy.subst Env'.stateSubstInfo.subst mty_in =
    LMonoTy.subst Env'.stateSubstInfo.subst mty_out := by
  match mty_in with
  | .ftvar _ | .bitvec _ =>
    simp [LMonoTy.resolveAliases, Pure.pure, Except.pure] at h
  ...
  | .tcons name args =>
    ...
    split at h; · simp at h
    rename_i v2 h_tcons; obtain ⟨mty', Env2⟩ := v2
    simp [Pure.pure, Except.pure] at h; obtain ⟨h1, h2⟩ := h; rw [← h1, ← h2]
    -- Need: subst S (tcons name args) = subst S mty'
    -- Decompose tconsAlias: no-alias vs alias
    unfold LMonoTy.tconsAlias at h_tcons
    generalize h_ma : List.find? _ _ = ma at h_tcons
    match ma with
    | none =>
      -- No alias: mty' = tcons name args', Env2 = Env1
      simp [Pure.pure, Except.pure] at h_tcons
      obtain ⟨h_mty, h_env⟩ := h_tcons; rw [← h_mty, ← h_env]
      -- Need: subst Env1.subst (tcons name args) = subst Env1.subst (tcons name args')
      -- Follows from resolveAliasList_subst_eq (mutual recursion).
      have h_list_eq : LMonoTys.subst Env1.stateSubstInfo.subst args =
          LMonoTys.subst Env1.stateSubstInfo.subst args' :=
        resolveAliasList_subst_eq args Env args' Env1 h_args
      -- Show subst S (tcons name xs) = tcons name (LMonoTys.subst S xs)
      -- for both args and args', then use h_list_eq
      show LMonoTy.subst Env1.stateSubstInfo.subst (.tcons name args) =
           LMonoTy.subst Env1.stateSubstInfo.subst (.tcons name args')
      by_cases h_e : Subst.hasEmptyScopes Env1.stateSubstInfo.subst
      · rw [LMonoTy.subst_emptyS h_e, LMonoTy.subst_emptyS h_e]
        -- Under empty subst, h_list_eq gives args = args'
        simp [LMonoTy.subst, LMonoTys.subst, h_e] at h_list_eq
        rw [h_list_eq]
      · have h_ne : Subst.hasEmptyScopes Env1.stateSubstInfo.subst = false := by
          revert h_e; cases Subst.hasEmptyScopes Env1.stateSubstInfo.subst <;> simp
        simp [LMonoTy.subst, h_ne]; exact h_list_eq
    | some alias =>
      -- Alias expansion: tconsAlias unifies (tcons name args') with the
      -- instantiated alias pattern, then returns the substituted alias definition.
      -- After unification: subst S (tcons name args') = subst S instantiatedPattern.
      -- The output mty' = subst S instantiatedDefinition.
      -- So need: subst S (tcons name args) = subst S (subst S instantiatedDefinition).
      --
      -- This requires subst S instantiatedPattern = subst S instantiatedDefinition,
      -- which is NOT syntactically true: the pattern is e.g. `Pair a b` while the
      -- definition is e.g. `Tuple a b`. These are semantically equal by the alias
      -- definition but structurally different.
      --
      -- NOTE: This theorem (subst S mty_in = subst S mty_out) is FALSE for the
      -- alias case, since alias expansion changes the type constructor name.
      -- HasType_resolveAliases now handles alias expansion via the HasType.talias
      -- rule instead of relying on this syntactic equality. This sorry only
      -- propagates through resolveAliasList_subst_eq, which is used in the
      -- no-alias branch of HasType_resolveAliases. If args themselves contain
      -- aliases, this sorry would be reached; however, in practice, the full
      -- soundness path goes through HasType_resolveAliases which handles aliases
      -- via talias at each level.
      sorry

/-- List version: after resolving aliases in a type list, the output substitution
    equates input and output lists element-wise. Uses `resolveAliases_subst_eq`
    per element + absorption to lift to the final substitution. -/
private theorem resolveAliasList_subst_eq
    (mtys_in : LMonoTys) (Env : TEnv T.IDMeta) (mtys_out : LMonoTys) (Env' : TEnv T.IDMeta)
    (h : LMonoTys.resolveAliases mtys_in Env = .ok (mtys_out, Env')) :
    LMonoTys.subst Env'.stateSubstInfo.subst mtys_in =
    LMonoTys.subst Env'.stateSubstInfo.subst mtys_out := by
  match mtys_in with
  | [] =>
    simp [LMonoTys.resolveAliases, Pure.pure, Except.pure] at h
    obtain ⟨h1, h2⟩ := h; rw [← h1, ← h2]
  | mty :: mrest =>
    ...
    rename_i v2 h_tl; obtain ⟨mrest', Env2⟩ := v2
    simp [Pure.pure, Except.pure] at h; obtain ⟨h1, h2⟩ := h; rw [← h1, ← h2]
    -- Per-element: subst Env1.subst mty = subst Env1.subst mty'
    have h_hd_eq := resolveAliases_subst_eq mty Env mty' Env1 h_hd
    -- Lift to final subst via absorption (Env2 absorbs Env1):
    have h_abs := LMonoTys.resolveAliases_absorbs mrest Env1 mrest' Env2 h_tl
    have h_hd_final : LMonoTy.subst Env2.stateSubstInfo.subst mty =
        LMonoTy.subst Env2.stateSubstInfo.subst mty' := by
      rw [← LMonoTy.subst_absorbs Env2.stateSubstInfo.subst Env1.stateSubstInfo.subst mty h_abs,
          h_hd_eq,
          LMonoTy.subst_absorbs Env2.stateSubstInfo.subst Env1.stateSubstInfo.subst mty' h_abs]
    -- IH on tail
    have h_tl_eq := resolveAliasList_subst_eq mrest Env1 mrest' Env2 h_tl
    -- Combine: subst S (hd :: tl) = subst S hd :: subst S tl (for both in/out)
    -- Use the fact that LMonoTys.subst maps over the list
    show LMonoTys.subst Env2.stateSubstInfo.subst (mty :: mrest) =
         LMonoTys.subst Env2.stateSubstInfo.subst (mty' :: mrest')
    -- Use substLogic which distributes over cons
    rw [LMonoTys.subst_eq_substLogic, LMonoTys.subst_eq_substLogic]
    simp only [LMonoTys.substLogic]; split
    · -- hasEmptyScopes: subst is identity; need mty :: mrest = mty' :: mrest'
      rename_i h_e
      simp [LMonoTy.subst_emptyS h_e] at h_hd_final
      simp [LMonoTys.subst, h_e] at h_tl_eq
      rw [h_hd_final, h_tl_eq]
    · -- non-empty: cons distributes
      rw [h_hd_final]
      congr 1
      rwa [LMonoTys.subst_eq_substLogic, LMonoTys.subst_eq_substLogic] at h_tl_eq
end
...
private theorem HasType_resolveAliases
  ...
  -- Case split on mty_in to distinguish alias vs non-alias
  match mty_in with
  | .ftvar _ =>
    -- resolveAliases is identity for ftvar
    simp [LMonoTy.resolveAliases, Pure.pure, Except.pure] at h_ra
    obtain ⟨h1, h2⟩ := h_ra; subst h1; subst h2
    exact HasType_subst_fresh_all C Γ e _ Env.stateSubstInfo.subst
      h_ty (fun a hk hfv => h_fresh a hk) Env.stateSubstInfo.isWF
  | .bitvec _ =>
    -- resolveAliases is identity for bitvec
    simp [LMonoTy.resolveAliases, Pure.pure, Except.pure] at h_ra
    obtain ⟨h1, h2⟩ := h_ra; subst h1; subst h2
    exact HasType_subst_fresh_all C Γ e _ Env.stateSubstInfo.subst
      h_ty (fun a hk hfv => h_fresh a hk) Env.stateSubstInfo.isWF
  | .tcons name args =>
    -- resolveAliases first resolves args, then tries alias expansion
    simp [LMonoTy.resolveAliases, Bind.bind, Except.bind] at h_ra
    split at h_ra; · simp at h_ra
    rename_i v1 h_args; obtain ⟨args', Env1⟩ := v1; simp at h_ra h_args
    split at h_ra; · simp at h_ra
    rename_i v2 h_tcons; obtain ⟨mty', Env2⟩ := v2
    simp [Pure.pure, Except.pure] at h_ra; obtain ⟨h1, h2⟩ := h_ra
    subst h1; subst h2
    -- Decompose tconsAlias: no-alias vs alias
    unfold LMonoTy.tconsAlias at h_tcons
    generalize h_ma : List.find? _ _ = ma at h_tcons
    ...
    | none =>
      -- No alias: mty' = tcons name args', Env2 = Env1
      simp [Pure.pure, Except.pure] at h_tcons
      obtain ⟨h_mty, h_env⟩ := h_tcons; subst h_mty; subst h_env
      -- Use resolveAliasList_subst_eq (which works for the no-alias case)
      have h_eq : LMonoTy.subst Env1.stateSubstInfo.subst (.tcons name args) =
          LMonoTy.subst Env1.stateSubstInfo.subst (.tcons name args') := by
        have h_list_eq := resolveAliasList_subst_eq args Env args' Env1 h_args
        by_cases h_e : Subst.hasEmptyScopes Env1.stateSubstInfo.subst
        · rw [LMonoTy.subst_emptyS h_e, LMonoTy.subst_emptyS h_e]
          simp [LMonoTys.subst, h_e] at h_list_eq; rw [h_list_eq]
        · have h_ne : Subst.hasEmptyScopes Env1.stateSubstInfo.subst = false := by
            revert h_e; cases Subst.hasEmptyScopes Env1.stateSubstInfo.subst <;> simp
          simp [LMonoTy.subst, h_ne]; exact h_list_eq
      have h_ty_subst := HasType_subst_fresh_all C Γ e (.tcons name args)
        Env1.stateSubstInfo.subst h_ty (fun a hk hfv => h_fresh a hk) Env1.stateSubstInfo.isWF
      rw [h_eq] at h_ty_subst; exact h_ty_subst
    | some alias =>
      -- Alias expansion: use HasType.talias + tconsAlias_expand_eq
      --
      -- Step 1: Get HasType for subst S (tcons name args)
      have h_ty_subst := HasType_subst_fresh_all C Γ e (.tcons name args)
        Env2.stateSubstInfo.subst h_ty (fun a hk hfv => h_fresh a hk) Env2.stateSubstInfo.isWF
      -- Step 2: Decompose subst S (tcons name args) into tcons name (subst S args)
      -- and show subst S args = subst S args' via absorption
      have h_abs := LMonoTys.resolveAliases_absorbs args Env args' Env1 h_args
      have h_abs2 := tconsAlias_absorbs name args' Env1 mty' Env2
        (by unfold LMonoTy.tconsAlias; rw [h_ma]; exact h_tcons)
      have h_abs_full : Subst.absorbs Env2.stateSubstInfo.subst Env.stateSubstInfo.subst :=
        Subst.absorbs_trans _ _ _ h_abs h_abs2
      -- Step 3: The alias is in Γ.aliases (via context preservation)
      ...
      have h_alias_in_ctx : alias ∈ Γ.aliases := by
        rw [h_aliases, ← h_ctx_pres]; exact h_alias_mem
      -- Step 4: alias.name = name and arity matches (from the find? predicate)
      have h_pred := List.find?_some h_ma
      simp [BEq.beq, decide_eq_true_eq] at h_pred
      obtain ⟨h_name, h_len⟩ := h_pred
      -- Step 5: Apply talias to get HasType for expand alias (subst S args)
      have h_expand : TypeAlias.expandsTo Γ.aliases name (LMonoTys.subst Env2.stateSubstInfo.subst args) (TypeAlias.expand alias (LMonoTys.subst Env2.stateSubstInfo.subst args)) := by
        refine ⟨alias, h_alias_in_ctx, h_name, ?_, rfl⟩
        -- alias.typeArgs.length = args'.length = (subst S args).length = args.length
        -- subst preserves length (via substLogic)
        -- First show args'.length = args.length (resolveAliases preserves length)
        have h_ra_len : ∀ (xs : LMonoTys) (E : TEnv T.IDMeta) (xs' : LMonoTys) (E' : TEnv T.IDMeta),
            LMonoTys.resolveAliases xs E = .ok (xs', E') → xs'.length = xs.length := by
          intro xs; induction xs with
          | nil => intros; simp_all [LMonoTys.resolveAliases, Pure.pure, Except.pure]
          | cons hd tl ih_len =>
            intro E xs' E' h
            simp [LMonoTys.resolveAliases, Bind.bind, Except.bind] at h
            split at h; · simp at h
            rename_i v1 h_hd; obtain ⟨_, E_hd⟩ := v1; simp at h h_hd
            split at h; · simp at h
            rename_i v2 h_tl; obtain ⟨tl', E_tl⟩ := v2
            simp [Pure.pure, Except.pure] at h
            obtain ⟨h1, _⟩ := h; rw [← h1]; simp
            exact ih_len E_hd tl' E_tl h_tl
        suffices h_subst_len : (LMonoTys.subst Env2.stateSubstInfo.subst args).length = args.length by
          rw [h_subst_len, ← h_ra_len args Env args' Env1 h_args]; exact h_len
        rw [LMonoTys.subst_eq_substLogic]
        ...
        exact this _ _
      -- Step 6: subst S (tcons name args) = tcons name (subst S args) (for non-empty S)
      -- and apply talias to get HasType for TypeAlias.expand alias (subst S args)
      have h_ty_alias := HasType.talias (C := C) Γ e name (LMonoTys.subst Env2.stateSubstInfo.subst args)
        (TypeAlias.expand alias (LMonoTys.subst Env2.stateSubstInfo.subst args))
        h_expand
      -- Need: HasType C Γ e (.forAll [] (.tcons name (subst S args)))
      -- From h_ty_subst: HasType C Γ e (.forAll [] (subst S (.tcons name args)))
      -- subst S (.tcons name args) = .tcons name (subst S args) when not hasEmptyScopes
      -- or = .tcons name args when hasEmptyScopes
      have h_subst_tcons : LMonoTy.subst Env2.stateSubstInfo.subst (.tcons name args) =
          .tcons name (LMonoTys.subst Env2.stateSubstInfo.subst args) := by
        by_cases hS : Subst.hasEmptyScopes Env2.stateSubstInfo.subst
        · simp [LMonoTy.subst, LMonoTys.subst, hS]
        · have hS_ne : Subst.hasEmptyScopes Env2.stateSubstInfo.subst = false := by
            revert hS; cases Subst.hasEmptyScopes Env2.stateSubstInfo.subst <;> simp
          simp [LMonoTy.subst, hS_ne]
      rw [h_subst_tcons] at h_ty_subst
      have h_ty_expanded := h_ty_alias h_ty_subst
      -- Step 7: Use tconsAlias_expand_eq to show
      ...
      -- subst S args = subst S args' via absorption chain
      have h_list_eq := resolveAliasList_subst_eq args Env args' Env1 h_args
      have h_args_eq : LMonoTys.subst Env2.stateSubstInfo.subst args =
          LMonoTys.subst Env2.stateSubstInfo.subst args' := by
        -- Lift LMonoTy.subst_absorbs to lists via tcons wrapper:
        -- subst S2 (subst S1 (tcons "" xs)) = subst S2 (tcons "" xs)
        -- ↔ tcons "" (subst S2 (subst S1 xs)) = tcons "" (subst S2 xs)
        -- ↔ subst S2 (subst S1 xs) = subst S2 xs
        -- Combined with h_list_eq: subst S1 xs = subst S1 xs'
        -- gives subst S2 xs = subst S2 (subst S1 xs) = subst S2 (subst S1 xs') = subst S2 xs'
        -- Lift to lists via tcons wrapper + LMonoTy.subst_absorbs
        have h_extract : ∀ (S : Subst) (xs : LMonoTys),
            LMonoTy.subst S (.tcons "" xs) = .tcons "" (LMonoTys.subst S xs) := by
          intro S' xs'
          by_cases hS' : Subst.hasEmptyScopes S'
          · simp [LMonoTy.subst, LMonoTys.subst, hS']
        ...
        -- subst S2 (subst S1 (tcons "" xs)) = subst S2 (tcons "" xs) [by absorption]
        have h_abs_args := LMonoTy.subst_absorbs Env2.stateSubstInfo.subst
            Env1.stateSubstInfo.subst (.tcons "" args) h_abs2
        have h_abs_args' := LMonoTy.subst_absorbs Env2.stateSubstInfo.subst
            Env1.stateSubstInfo.subst (.tcons "" args') h_abs2
        -- Extract list equality from tcons equality
        -- h_abs_args : subst S2 (subst S1 (tcons "" args)) = subst S2 (tcons "" args)
        -- Expand both sides using h_extract
        -- LHS of h_abs_args = tcons "" (subst S2 (subst S1 args))  [via h_extract on subst S1, then S2]
        -- RHS of h_abs_args = tcons "" (subst S2 args)
        -- So subst S2 (subst S1 args) = subst S2 args
        -- Similarly for args'
        -- Then h_list_eq connects them
        -- Clean approach: just use congrArg at the LMonoTy level
        -- h_list_eq : subst S1 args = subst S1 args'
        -- Lift to tcons level: subst S1 (tcons "" args) wraps subst S1 args in tcons ""
        -- So we need to show subst S1 (tcons "" args) and subst S1 (tcons "" args') have same args part
        have h_step : LMonoTy.subst Env2.stateSubstInfo.subst
            (LMonoTy.subst Env1.stateSubstInfo.subst (.tcons "" args)) =
            LMonoTy.subst Env2.stateSubstInfo.subst
            (LMonoTy.subst Env1.stateSubstInfo.subst (.tcons "" args')) := by
          -- subst S1 (tcons "" args) = tcons "" (subst S1 args)
          rw [h_extract Env1.stateSubstInfo.subst args,
              h_extract Env1.stateSubstInfo.subst args',
              h_list_eq]
        -- h_step : subst S2 (subst S1 (tcons "" args)) = subst S2 (subst S1 (tcons "" args'))
        -- h_abs_args  : subst S2 (subst S1 (tcons "" args))  = subst S2 (tcons "" args)
        -- h_abs_args' : subst S2 (subst S1 (tcons "" args')) = subst S2 (tcons "" args')
        have h_tcons_eq := h_abs_args.symm.trans (h_step.trans h_abs_args')
        -- h_tcons_eq : subst S2 (tcons "" args) = subst S2 (tcons "" args')
        rw [h_extract, h_extract] at h_tcons_eq
        exact (LMonoTy.tcons.inj h_tcons_eq).2
      rw [h_bridge, ← h_args_eq]; exact h_ty_expanded
...
theorem resolveAux_HasType :
  ...
  | .fvar m x fty =>
    ...
    · rename_i v1 h_infer
      ...
      rw [← h_et, ← h_env']
      simp [toLMonoTy]
      exact inferFVar_HasType C Env x fty ty_res Env_res m h_infer h_envwf.boundVarsWF h_envwf.aliasesWF
  | .op m o oty =>
    intro et C Env Env' h h_envwf h_ne
    have h_sf := h_envwf.keysFresh; have h_vf := h_envwf.valsFresh; have h_aw := h_envwf.aliasesWF
        ...
        · rename_i v3 h_genTyVar
          ...
          · rename_i v4 h_mapError
            ...
            have h_sf1 : Subst.allKeysFresh Env1.stateSubstInfo.subst Env1.context := by
              rw [h_ctx1]; exact resolveAux_keys_fresh e1 e1t C Env Env1 h_res1 h_envwf h_ne
            -- Build TEnvWF for Env1 (context preserved, subst/gen extended)
            have h_envwf1 : TEnvWF Env1 :=
              { keysFresh := h_ctx1 ▸ h_sf1
                valsFresh := h_ctx1 ▸ sorry -- needs resolveAux_vals_fresh
                aliasesWF := h_ctx1 ▸ h_aw
                substFreshForGen := resolveAux_preserves_SubstFreshForGen e1 e1t C Env Env1 h_res1 h_envwf.substFreshForGen h_envwf.ctxFreshForGen h_ne
                ctxFreshForGen := h_ctx1 ▸ ContextFreshForGen.mono _ _ _ h_envwf.ctxFreshForGen (resolveAux_genState_mono e1 e1t C Env Env1 h_res1)
                boundVarsWF := transfer_wf h_envwf.boundVarsWF h_ctx1 }
            ...
            have h_abs_Env2_Env1 := resolveAux_absorbs e2 e2t C Env1 Env2 h_res2 h_envwf1.toEnvFreshForGen h_ne1
            have h_abs_Env1_Env := resolveAux_absorbs e1 e1t C Env Env1 h_res1 h_envwf.toEnvFreshForGen h_ne
            ...
            · -- Typing: apply HasType.tapp
              ...
              have h_ty1_up := HasType_subst_upgrade C Env.context e1
                ...
                h_ty1 h_abs_v4_Env1 (fun a hk hfv => h_fresh_v4 a hk) v4.isWF
              ...
              have h_ty2_up := HasType_subst_upgrade C Env.context e2
                ...
                h_ty2 h_abs_v4_Env3 (fun a hk hfv => h_fresh_v4 a hk) v4.isWF
      ...
      · rename_i v2 h_res_t
        ...
        · rename_i v3 h_res_e
          ...
          · rename_i v4 h_mapError
            ...
            have h_ne1 := h_ctx1 ▸ h_ne
            have h_sf1 : Subst.allKeysFresh Env1.stateSubstInfo.subst Env1.context := by
              rw [h_ctx1]; exact resolveAux_keys_fresh c ct C Env Env1 h_res_c h_envwf h_ne
            ...
            have h_sf2 : Subst.allKeysFresh Env2.stateSubstInfo.subst Env2.context := by
              rw [h_ctx2]; exact resolveAux_keys_fresh t tht C Env1 Env2 h_res_t h_envwf1 h_ne1
            have h_envwf2 : TEnvWF Env2 :=
              { keysFresh := h_ctx2 ▸ h_sf2
                ...
                boundVarsWF := transfer_wf h_envwf1.boundVarsWF h_ctx2 }
            ...
            have h_abs_Env3_Env2 := resolveAux_absorbs e elt C Env2 Env3 h_res_e h_envwf2.toEnvFreshForGen h_ne2
            have h_abs_Env2_Env1 := resolveAux_absorbs t tht C Env1 Env2 h_res_t h_envwf1.toEnvFreshForGen h_ne1
            have h_abs_Env1_Env := resolveAux_absorbs c ct C Env Env1 h_res_c h_envwf.toEnvFreshForGen h_ne
            ...
            · -- Typing: result type is tht.toLMonoTy, rule is HasType.tif
              ...
              have h_ty_c_up := HasType_subst_upgrade C Env.context c
                ...
                h_ty_c h_abs_v4_Env1 (fun a hk hfv => h_fresh_v4 a hk) v4.isWF
              ...
              have h_ty_t_up := HasType_subst_upgrade C Env.context t
                ...
                h_ty_t h_abs_v4_Env2 (fun a hk hfv => h_fresh_v4 a hk) v4.isWF
              ...
              have h_ty_e_up := HasType_subst_upgrade C Env.context e
                ...
                h_ty_e h_abs_v4_Env3 (fun a hk hfv => h_fresh_v4 a hk) v4.isWF
        ...
        · rename_i v3 h_mapError
          ...
          · -- Typing: result type is bool (ground)
            ...
            have h_ty1_upgraded := HasType_subst_upgrade C Env.context e1
              ...
              h_ty1 h_abs_v3_Env1 (fun a hk hfv => h_fresh_v3 a hk) v3.isWF
            ...
            have h_ty2_upgraded := HasType_subst_upgrade C Env.context e2
              ...
              h_ty2 h_abs_v3_Env2 (fun a hk hfv => h_fresh_v3 a hk) v3.isWF
