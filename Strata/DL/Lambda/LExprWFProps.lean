...
/-- The names of `e`'s free variables (first components of `freeVars`) are exactly
    `LExpr.getVars e`. Symmetric form of `getVars_eq_freeVars_idents`. -/
theorem LExpr.freeVars_map_fst_eq_getVars {T : LExprParams} (e : LExpr T.mono) :
    (LExpr.freeVars e).map Prod.fst = LExpr.LExpr.getVars e :=
  (getVars_eq_freeVars_idents e).symm
