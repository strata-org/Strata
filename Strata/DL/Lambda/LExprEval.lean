...
variable {TypeType : Type} [DecidableEq TypeType] [ToFormat TypeType]

/--
Check for boolean equality of two expressions `e1` and `e2` after erasing any
type annotations.
-/
def eqModuloTypes (e1 e2 : (LExpr Identifier)) : Bool :=
...
-/
def isCanonicalValue (e : (LExpr Identifier)) : Bool :=
  match e with
  | .const _ _ => true
  | .abs _ _ =>
...
-/
def eql (e1 e2 : (LExpr Identifier))
...
instance : ToFormat (Except Format (LExpr Identifier)) where
...
-/
def mkAbsOfArity (arity : Nat) (core : (LExpr Identifier)) :
    (LExpr Identifier) :=
  go 0 arity core
  where go (bvarcount arity : Nat) (core : (LExpr Identifier)) :=
  match arity with
  | 0 => core
  | n + 1 =>
...
-/
def eval (n : Nat) (σ : (LState Identifier)) (e : (LExpr Identifier)) :
    (LExpr Identifier) :=
  match n with
  | 0 => e
  | n' + 1 =>
...
def evalCore (n' : Nat) (σ : (LState Identifier)) (e : (LExpr Identifier)) : (LExpr Identifier) :=
  match e with
  | .const _ _  => e
  | .op _ _     => e
  | .bvar _     => e
...
def evalIte (n' : Nat) (σ : (LState Identifier)) (c t f : (LExpr Identifier)) : (LExpr Identifier) :=
  let c' := eval n' σ c
  match c' with
...
def evalEq (n' : Nat) (σ : (LState Identifier)) (e1 e2 : (LExpr Identifier)) : (LExpr Identifier) :=
  open LTy.Syntax in
  let e1' := eval n' σ e1
  let e2' := eval n' σ e2
  if eqModuloTypes e1' e2' then
...
def evalApp (n' : Nat) (σ : (LState Identifier)) (e e1 e2 : (LExpr Identifier)) : (LExpr Identifier) :=
  let e1' := eval n' σ e1
  let e2' := eval n' σ e2
  match e1' with
  | .abs _ e1' =>
