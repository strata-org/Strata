...
-/
def mkAbsOfArity (arity : Nat) (core : (LExpr Identifier)) : (LExpr Identifier) :=
  go 0 arity core
  where go (bvarcount arity : Nat) (core : (LExpr Identifier)) : (LExpr Identifier) :=
  match arity with
  | 0 => core
  | n + 1 =>
...
-/
def eval (n : Nat) (σ : (LState Identifier)) (e : (LExpr Identifier)) : (LExpr Identifier) :=
  match n with
  | 0 => e
  | n' + 1 =>
