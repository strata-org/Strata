...
theorem TContext_types_subst_find_none
    (types : HMaps (Identifier IDMeta) LTy) (S : Subst) (x : Identifier IDMeta)
    (h : HMaps.find? types x = none) :
    HMaps.find? (TContext.types.subst types S) x = none := by
  simp only [TContext.types.subst, HMaps.find?_mapValues, h, Option.map_none]

/-- A name absent from a context's type scope stays absent after substitution. -/
theorem TContext.subst_find_none
...
theorem TContext.subst_find_some
...
def LContext.addFactoryFunctionWithError [Inhabited T.Metadata] [ToFormat T.IDMeta]
    (C : LContext T) (fn : LFunc T) : Except Message (LContext T) := do
...
theorem TEnv.addInNewestContext_stateSubstInfo (Env : TEnv T.IDMeta) (map : HMap T.Identifier LTy) :
...
def LMonoTys.instantiateEnvWithSubst [ToFormat IDMeta] (ids : List TyIdentifier) (mtys : LMonoTys) (T : (TEnv IDMeta)) :
  ...
  let S : Subst := Strata.Util.HMaps.ofScopes [List.zip ids (freshtvs.map LMonoTy.ftvar)]
  .ok (LMonoTys.subst S mtys, {T with genEnv := genEnv'}, S)

theorem LMonoTys.subst_length (S : Subst) (mtys : LMonoTys) :
    (LMonoTys.subst S mtys).length = mtys.length := by
  rw [LMonoTys.subst_eq_map]; exact List.length_map _
...
theorem TGenEnv.genTyVars_prefixed {IDMeta : Type} [DecidableEq IDMeta] [Hashable IDMeta] [ToFormat IDMeta]
