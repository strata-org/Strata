...
We leave placeholders for (poly)type annotations only for constants
(`.const`), operations (`.op`), binders (`.abs`, `.quant`), and free
variables (`.fvar`). For a fully (mono-type)annotated AST, see `LExprT` that is
created after the type inference transform.

`LExpr` is parameterized by `Identifier` for allowed identifiers.
-/
inductive LExpr (Identifier : Type) : Type where
  /-- `.const c ty`: constants (in the sense of literals). -/
  | const   (c : String) (ty : Option LTy)
  /-- `.op c ty`: operation names. -/
  | op      (o : Identifier) (ty : Option LTy)
  /-- `.bvar deBruijnIndex`: bound variable. -/
  | bvar    (deBruijnIndex : Nat)
  /-- `.fvar name ty`: free variable, with an option (mono)type annotation. -/
  | fvar    (name : Identifier) (ty : Option LTy)
  | mdata   (info : Info) (e : LExpr Identifier)
  /-- `.abs ty e`: abstractions; `ty` the is type of bound variable. -/
  | abs     (ty : Option LTy) (e : LExpr Identifier)
  /-- `.quant k ty tr e`: quantified expressions; `ty` the is type of bound variable, and `tr` the trigger. -/
  | quant   (k : QuantifierKind) (ty : Option LTy) (trigger: LExpr Identifier) (e : LExpr Identifier)
...
def LExpr.allTr {Identifier : Type} (ty : Option LTy) := @LExpr.quant Identifier .all ty
def LExpr.all {Identifier : Type} (ty : Option LTy) := @LExpr.quant Identifier .all ty LExpr.noTrigger
def LExpr.existTr {Identifier : Type} (ty : Option LTy) := @LExpr.quant Identifier .exist ty
def LExpr.exist {Identifier : Type} (ty : Option LTy) := @LExpr.quant Identifier .exist ty LExpr.noTrigger

abbrev LExpr.absUntyped {Identifier : Type} := @LExpr.abs Identifier .none
abbrev LExpr.allUntypedTr {Identifier : Type} := @LExpr.quant Identifier .all .none
abbrev LExpr.allUntyped {Identifier : Type} := @LExpr.quant Identifier .all .none LExpr.noTrigger
abbrev LExpr.existUntypedTr {Identifier : Type} := @LExpr.quant Identifier .exist .none
abbrev LExpr.existUntyped {Identifier : Type} := @LExpr.quant Identifier .exist .none LExpr.noTrigger
...
@[match_pattern]
protected def true : (LExpr Identifier) := .const "true" LTy.bool

@[match_pattern]
protected def false : (LExpr Identifier) := .const "false" LTy.bool
...
def denoteBool (e : (LExpr Identifier)) : Option Bool :=
  match e with
  | .const "true"  (some .bool) => some true
  | .const "true"  none => some true
  | .const "false" (some .bool) => some false
...
instance (Identifier : Type) [Repr Identifier] : ToString (LExpr Identifier) where
   toString a := toString (repr a)

private def formatLExpr [ToFormat Identifier] (e : (LExpr Identifier)) :
    Format :=
  match e with
  | .const c ty =>
    match ty with
...
def elabLConst (Identifier : Type) [MkIdent Identifier] : Lean.Syntax → MetaM Expr
  ...
  | `(lconst| (#$n:num : $ty:lty)) => do
    ...
    return mkAppN (.const ``LExpr.const []) #[MkIdent.toExpr Identifier, mkStrLit (toString n.getNat), lty]
  | `(lconst| #-$n:num) => do
    let none ← mkNone (mkConst ``LTy)
    return mkAppN (.const ``LExpr.const []) #[MkIdent.toExpr Identifier, mkStrLit ("-" ++ (toString n.getNat)), none]
  | `(lconst| (#-$n:num : $ty:lty)) => do
  ...
  | `(lconst| (#true : $ty:lty))    => do
    let lty ← Lambda.LTy.Syntax.elabLTy ty
    let lty ← mkSome (mkConst ``LTy) lty
    return mkAppN (.const ``LExpr.const []) #[MkIdent.toExpr Identifier, mkStrLit "true", lty]
  | `(lconst| #false)   =>  do
    let none ← mkNone (mkConst ``LTy)
  ...
  | `(lconst| (#false : $ty:lty))    => do
    let lty ← Lambda.LTy.Syntax.elabLTy ty
    let lty ← mkSome (mkConst ``LTy) lty
    return mkAppN (.const ``LExpr.const []) #[MkIdent.toExpr Identifier, mkStrLit "false", lty]
  | `(lconst| #$s:ident) => do
    let none ← mkNone (mkConst ``LTy)
  ...
  | `(lconst| (#$s:ident : $ty:lty)) => do
    ...
    let lty ← mkSome (mkConst ``LTy) lty
    let s := toString s.getId
    return mkAppN (.const ``LExpr.const []) #[MkIdent.toExpr Identifier, mkStrLit s, lty]
...
def elabLOp (Identifier : Type) [MkIdent Identifier] : Lean.Syntax → MetaM Expr
  | `(lop| ~$s:lident)  => do
    ...
    return mkAppN (.const ``LExpr.op []) #[MkIdent.toExpr Identifier, ident, none]
...
partial def elabLExpr (Identifier : Type) [MkIdent Identifier] : Lean.Syntax → MetaM Expr
  ...
  | `(lexpr| λ $e:lexpr) => do
     ...
     return mkAppN (.const ``LExpr.absUntyped []) #[MkIdent.toExpr Identifier, e']
  | `(lexpr| λ ($mty:lty): $e:lexpr) => do
     let lty ← Lambda.LTy.Syntax.elabLTy mty
     let lty ← mkSome (mkConst ``LTy) lty
     let e' ← elabLExpr Identifier e
     return mkAppN (.const ``LExpr.abs []) #[MkIdent.toExpr Identifier, lty, e']
  | `(lexpr| ∀ $e:lexpr) => do
     let e' ← elabLExpr Identifier e
     return mkAppN (.const ``LExpr.allUntyped []) #[MkIdent.toExpr Identifier, e']
  | `(lexpr| ∀{$tr}$e:lexpr) => do
     let e' ← elabLExpr Identifier e
     let tr' ← elabLExpr Identifier tr
     return mkAppN (.const ``LExpr.allUntypedTr []) #[MkIdent.toExpr Identifier, tr', e']
  | `(lexpr| ∀ ($mty:lty): $e:lexpr) => do
     let lty ← Lambda.LTy.Syntax.elabLTy mty
     let lty ← mkSome (mkConst ``LTy) lty
     let e' ← elabLExpr Identifier e
     return mkAppN (.const ``LExpr.all []) #[MkIdent.toExpr Identifier, lty, e']
  | `(lexpr| ∀ ($mty:lty): {$tr}$e:lexpr) => do
     let lty ← Lambda.LTy.Syntax.elabLTy mty
     let lty ← mkSome (mkConst ``LTy) lty
     let e' ← elabLExpr Identifier e
     let tr' ← elabLExpr Identifier tr
     return mkAppN (.const ``LExpr.allTr []) #[MkIdent.toExpr Identifier, lty, tr', e']
  | `(lexpr| ∃ ($mty:lty): $e:lexpr) => do
     let lty ← Lambda.LTy.Syntax.elabLTy mty
     let lty ← mkSome (mkConst ``LTy) lty
     let e' ← elabLExpr Identifier e
     return mkAppN (.const ``LExpr.exist []) #[MkIdent.toExpr Identifier, lty, e']
  | `(lexpr| ∃ ($mty:lty): {$tr}$e:lexpr) => do
     let lty ← Lambda.LTy.Syntax.elabLTy mty
     let lty ← mkSome (mkConst ``LTy) lty
     let e' ← elabLExpr Identifier e
     let tr' ← elabLExpr Identifier tr
     return mkAppN (.const ``LExpr.existTr []) #[MkIdent.toExpr Identifier, lty, tr', e']
  | `(lexpr| ∃ $e:lexpr) => do
     let e' ← elabLExpr Identifier e
  ...
  | `(lexpr| ($e1:lexpr $e2:lexpr)) => do
     ...
     return mkAppN (.const ``LExpr.app []) #[MkIdent.toExpr Identifier, e1', e2']
  | `(lexpr| $e1:lexpr == $e2:lexpr) => do
     let e1' ← elabLExpr Identifier e1
     let e2' ← elabLExpr Identifier e2
     return mkAppN (.const ``LExpr.eq []) #[MkIdent.toExpr Identifier, e1', e2']
  | `(lexpr| if $e1:lexpr then $e2:lexpr else $e3:lexpr) => do
     let e1' ← elabLExpr Identifier e1
     let e2' ← elabLExpr Identifier e2
     let e3' ← elabLExpr Identifier e3
     return mkAppN (.const ``LExpr.ite []) #[MkIdent.toExpr Identifier, e1', e2', e3']
...
/-- info: (abs (some (LTy.forAll [] (LMonoTy.tcons "bool" []))) (bvar 0)).app (const "true" none) : LExpr String -/
#guard_msgs in
...
/-- info: exist (some (LTy.forAll [] (LMonoTy.tcons "int" []))) ((bvar 0).eq (const "5" none)) : LExpr String -/
#guard_msgs in
#check es[(∃ (int): %0 == #5)]

/-- info: fvar "x" (some (LTy.forAll [] (LMonoTy.tcons "bool" []))) : LExpr String -/
#guard_msgs in
#check es[(x : bool)]
