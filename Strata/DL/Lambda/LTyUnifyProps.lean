...
/-- A substitution's range is well-kinded: every stored type is well-kinded. -/
def Subst.RangeWellKinded (arity : String → Option Nat) (S : Subst) : Prop :=
  ∀ v sty, HMaps.find? S v = some sty → LMonoTy.WellKinded arity sty

/-- Looking up any variable through a range-well-kinded subst yields a
    well-kinded type (identity `ftvar` when absent is trivially well-kinded). -/
theorem Subst.RangeWellKinded.lookup (arity : String → Option Nat) {S : Subst}
    (h : Subst.RangeWellKinded arity S) (v : TyIdentifier) :
    LMonoTy.WellKinded arity (LMonoTy.subst S (.ftvar v)) := by
  have h_eq : LMonoTy.subst S (.ftvar v) = match HMaps.find? S v with
      | some sty => sty | none => .ftvar v := by
    by_cases hemp : Subst.hasEmptyScopes S
    · rw [LMonoTy.subst_of_hasEmptyScopes hemp, Subst.find?_none_of_hasEmptyScopes hemp]
    · rw [LMonoTy.subst_eq_substCore S _ (by simpa using hemp)]; rfl
  rw [h_eq]
  cases hf : HMaps.find? S v with
  | none => intro ref n h_mem; simp only [getTypeConsArities, List.not_mem_nil] at h_mem
  | some sty => exact h v sty hf

/-- `Subst.apply new S` preserves `RangeWellKinded` when the applied single-entry
    subst `[new]` is itself range-well-kinded (its lone value is well-kinded). -/
theorem Subst.RangeWellKinded.apply (arity : String → Option Nat)
    {new : SubstOne} {S : Subst}
    (hS : Subst.RangeWellKinded arity S)
    (hnew : Subst.RangeWellKinded arity [new]) :
    Subst.RangeWellKinded arity (Subst.apply new S) := by
  intro v sty h_find
  rw [Subst.find?_apply] at h_find
  cases hf : HMaps.find? S v with
  | none => rw [hf] at h_find; simp at h_find
  | some w =>
    rw [hf] at h_find; simp only [Option.map_some, Option.some.injEq] at h_find
    subst h_find
    have h_w : LMonoTy.WellKinded arity w := hS v w hf
    -- subst [new] w is WK: w is WK and each var of w maps through [new] to a WK type.
    exact LMonoTy.WellKinded_subst arity [new] w h_w
      (fun u _ => hnew.lookup arity u)

/-- Inserting a well-kinded binding preserves `RangeWellKinded`. -/
theorem Subst.RangeWellKinded.insert (arity : String → Option Nat)
    {S : Subst} {id : TyIdentifier} {lty : LMonoTy}
    (hS : Subst.RangeWellKinded arity S) (hlty : LMonoTy.WellKinded arity lty) :
    Subst.RangeWellKinded arity (HMaps.insert S id lty) := by
  intro v sty h_find
  by_cases hv : v = id
  · subst hv; rw [HMaps.find?_insert_self] at h_find
    simp only [Option.some.injEq] at h_find; subst h_find; exact hlty
  · rw [HMaps.find?_insert_ne _ _ _ _ (by simpa using hv)] at h_find
    exact hS v sty h_find

/-- `RangeWellKinded` is monotone under arity extension. -/
theorem Subst.RangeWellKinded.mono_arity {arity arity' : String → Option Nat} {S : Subst}
    (h_sub : ∀ ref n, arity ref = some n → arity' ref = some n)
    (h : Subst.RangeWellKinded arity S) : Subst.RangeWellKinded arity' S :=
  fun v sty h_find => (h v sty h_find).mono_arity h_sub

/-- A one-scope substitution whose entries all map to `.ftvar` has a well-kinded
    range (every looked-up value is an ftvar, trivially WK). Used for the
    fresh→user renaming built by `Function.typeCheck`. -/
theorem Subst.RangeWellKinded_ofScopes_ftvar (arity : String → Option Nat)
    (pairs : List (TyIdentifier × TyIdentifier)) :
    Subst.RangeWellKinded arity
      (Strata.Util.HMaps.ofScopes [pairs.map (fun x => (x.1, LMonoTy.ftvar x.2))]) := by
  intro v sty h_find
  have h_val : ∃ w, sty = LMonoTy.ftvar w := by
    simp only [Strata.Util.HMaps.ofScopes, List.map_cons, List.map_nil,
      Strata.Util.HMaps.find?] at h_find
    rw [Strata.Util.HMap.find?_ofList_map_snd pairs LMonoTy.ftvar v] at h_find
    cases h_pf : Strata.Util.HMap.find? (Strata.Util.HMap.ofList pairs) v with
    | none => rw [h_pf] at h_find; simp at h_find
    | some w =>
      rw [h_pf] at h_find; simp only [Option.map_some, Option.some.injEq] at h_find
      exact ⟨w, h_find.symm⟩
  obtain ⟨w, rfl⟩ := h_val
  exact LMonoTy.WellKinded_ftvar arity w

/-- A constraint is well-kinded when both its component types are. -/
def Constraint.WellKinded (arity : String → Option Nat) (c : Constraint) : Prop :=
  LMonoTy.WellKinded arity c.1 ∧ LMonoTy.WellKinded arity c.2

/-- All constraints in a list are well-kinded. -/
def Constraints.WellKinded (arity : String → Option Nat) (cs : Constraints) : Prop :=
  ∀ c ∈ cs, Constraint.WellKinded arity c

/-- **The crux.** `unifyOne` (and `unifyCore`, via the mutual induction)
    preserve `RangeWellKinded`: given well-kinded input constraints and a
    range-well-kinded incoming substitution, the resulting substitution's range
    is well-kinded. Mirrors `Constraint.unifyOne_sound`'s induct skeleton. -/
theorem Constraint.unifyOne_RangeWellKinded (arity : String → Option Nat)
    (c : Constraint) (S : SubstInfo)
    (relS : ValidSubstRelation [c] S)
    (h : Constraint.unifyOne c S = .ok relS)
    (h_c : Constraint.WellKinded arity c)
    (h_S : Subst.RangeWellKinded arity S.subst) :
    Subst.RangeWellKinded arity relS.newS.subst := by
  suffices H : ∀ relS, Constraint.unifyOne c S = .ok relS →
      Constraint.WellKinded arity c → Subst.RangeWellKinded arity S.subst →
      Subst.RangeWellKinded arity relS.newS.subst from H relS h h_c h_S
  clear h h_c relS
  apply Constraint.unifyOne.induct
    (motive1 := fun c S => ∀ relS, Constraint.unifyOne c S = .ok relS →
      Constraint.WellKinded arity c → Subst.RangeWellKinded arity S.subst →
      Subst.RangeWellKinded arity relS.newS.subst)
    (motive2 := fun cs S => ∀ relS, Constraints.unifyCore cs S = .ok relS →
      Constraints.WellKinded arity cs → Subst.RangeWellKinded arity S.subst →
      Subst.RangeWellKinded arity relS.newS.subst)
  -- Case 1: t1 == t2 → newS = S
  · intro S t1 t2 h_eq _ relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · simp only [Except.ok.injEq] at h; subst h; exact h_S
    · exact absurd h_eq ‹_›
  -- Case 2: ftvar id == lty → newS = S
  · intro S id orig_lty h_neq _lty _ _ h_eq_lty relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · simp only [Except.ok.injEq] at h; subst h; exact h_S
  -- Case 3: occurs check — error
  · intro S id orig_lty h_neq _lty _ _ h_neq_lty h_occurs relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h <;> grind
  -- Case 4: ftvar id, some sty — recursive
  · intro S id orig_lty h_neq _lty _ _ h_neq_lty h_not_occurs sty h_some ih_rec relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · split at h
      · rename_i sty' h_some'
        ...
        · rename_i relS' h_call
          simp only [Except.ok.injEq] at h; subst h
          have h_sty_wk : LMonoTy.WellKinded arity sty := h_S id sty h_some
          have h_lty_wk : LMonoTy.WellKinded arity (LMonoTy.subst S.subst orig_lty) :=
            LMonoTy.WellKinded_subst arity S.subst orig_lty h_c.2
              (fun v _ => h_S.lookup arity v)
          exact ih_rec relS' h_call ⟨h_sty_wk, h_lty_wk⟩ h_S
      · rename_i h_none'; rw [h_some] at h_none'; simp at h_none'
  -- Case 5: ftvar id, none — insert+apply (THE binding case)
  · intro S id orig_lty h_neq _lty _ _ h_neq_lty h_not_occurs h_none _ _ _ns h' _nS _ _ relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · split at h
      · rename_i sty h_some; rw [h_none] at h_some; simp at h_some
      · simp only [Except.ok.injEq] at h; subst h
        -- newS.subst = (apply (single id lty) S.subst).insert id lty, lty = subst S.subst orig_lty
        have h_lty_wk : LMonoTy.WellKinded arity (LMonoTy.subst S.subst orig_lty) :=
          LMonoTy.WellKinded_subst arity S.subst orig_lty h_c.2
            (fun v _ => h_S.lookup arity v)
        have h_single_wk : Subst.RangeWellKinded arity [HMap.single id (LMonoTy.subst S.subst orig_lty)] := by
          intro v sty h_find
          rw [HMaps.find?_single_scope] at h_find
          by_cases hv : v = id
          · subst hv; rw [HMap.find?_single_self] at h_find
            simp only [Option.some.injEq] at h_find; subst h_find; exact h_lty_wk
          · rw [HMap.find?_single_ne _ _ _ (by simp [bne, hv])] at h_find; simp at h_find
        exact (h_S.apply arity h_single_wk).insert arity h_lty_wk
  -- Case 6: orig_lty, ftvar id == lty → newS = S
  · intro S orig_lty id h_neq _ _lty _ _ h_eq_lty relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · simp only [Except.ok.injEq] at h; subst h; exact h_S
  -- Case 7: occurs check — error
  · intro S orig_lty id h_neq _ _lty _ _ h_neq_lty h_occurs relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · simp at h
  -- Case 8: orig_lty, ftvar id, some sty — recursive (symmetric to 4)
  · intro S orig_lty id h_neq _ _lty _ _ h_neq_lty h_not_occurs sty h_some ih_rec relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · split at h
      · rename_i sty' h_some'
        ...
        · rename_i relS' h_call
          ...
          have h_sty_wk : LMonoTy.WellKinded arity sty := h_S id sty h_some
          have h_lty_wk : LMonoTy.WellKinded arity (LMonoTy.subst S.subst orig_lty) :=
            LMonoTy.WellKinded_subst arity S.subst orig_lty h_c.1
              (fun v _ => h_S.lookup arity v)
          exact ih_rec relS' h_call ⟨h_sty_wk, h_lty_wk⟩ h_S
      · rename_i h_none'; rw [h_some] at h_none'; simp at h_none'
  -- Case 9: orig_lty, ftvar id, none — insert+apply (symmetric to 5)
  · intro S orig_lty id h_neq _ _lty _ _ h_neq_lty h_not_occurs h_none _ _ _ns h' _nS _ _ relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · split at h
      · rename_i sty h_some; rw [h_none] at h_some; simp at h_some
      · simp only [Except.ok.injEq] at h; subst h
        ...
        exact (h_S.apply arity h_single_wk).insert arity h_lty_wk
  -- Case 10: bitvec n1 == n2 contradiction
  · intro S n1 n2 h_neq h_eq_n relS h h_c h_S; grind
  -- Case 11: bitvec n1 ≠ n2 — error
  · intro S n1 n2 h_neq h_neq_n relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h <;> grind
  -- Case 12: tcons match — recursive unifyCore
  · intro S name1 args1 name2 args2 h_neq h_match _nc ih_core relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; simp only at h; split at h
    · exact absurd ‹_› h_neq
    · simp only [bind, Except.bind] at h
      split at h
      ...
      · rename_i relS' h_call
        simp only [Except.ok.injEq] at h; subst h
        -- Each argument of a WK tcons is WK (subterm).
        have h_arg_wk : ∀ (nm : String) (as : List LMonoTy), LMonoTy.WellKinded arity (.tcons nm as) →
            ∀ a ∈ as, LMonoTy.WellKinded arity a := by
          intro nm as h_wk a h_a_mem ref n hk
          exact h_wk ref n (by
            simp only [getTypeConsArities, List.mem_cons]; right
            rw [List.mem_flatMap]; exact ⟨a, h_a_mem, hk⟩)
        have h_zip_wk : Constraints.WellKinded arity (args1.zip args2) := by
          intro p hp
          obtain ⟨h_p1, h_p2⟩ := List.of_mem_zip hp
          exact ⟨h_arg_wk name1 args1 h_c.1 p.1 h_p1, h_arg_wk name2 args2 h_c.2 p.2 h_p2⟩
        exact ih_core relS' h_call h_zip_wk h_S
  -- Case 13: tcons mismatch — error
  · intro S name1 args1 name2 args2 h_neq h_mismatch relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; grind
  -- Case 14: bitvec, tcons — error
  · intro S size name args h_neq relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; grind
  -- Case 15: tcons, bitvec — error
  · intro S name args size h_neq relS h h_c h_S
    rw [Constraint.unifyOne.eq_def] at h; grind
  -- Case 16: unifyCore []
  · intro S relS h h_c h_S
    rw [Constraints.unifyCore.eq_def] at h; simp only at h
    simp only [Except.ok.injEq] at h; subst h; exact h_S
  -- Case 17: unifyCore c :: rest
  · intro S c c_rest ih1 ih2 relS h h_c h_S
    rw [Constraints.unifyCore.eq_def] at h; simp only at h
    simp only [Bind.bind, Except.bind, Except.mapError] at h
    split at h
    · simp at h
    · rename_i relS_one h_one_raw
      ...
      · rename_i relS_rest h_rest
        simp only [Except.ok.injEq] at h; subst h
        have h_c_head : Constraint.WellKinded arity c := h_c c List.mem_cons_self
        have h_c_rest : Constraints.WellKinded arity c_rest :=
          fun c' hc' => h_c c' (List.mem_cons_of_mem c hc')
        have h_one_wk := ih1 relS_one h_one h_c_head h_S
        exact ih2 relS_one relS_rest h_rest h_c_rest h_one_wk

/-- `unifyCore` preserves `RangeWellKinded` (exposed form via list induction,
    delegating to the combined `unifyOne` lemma's motive2). -/
theorem Constraints.unifyCore_RangeWellKinded (arity : String → Option Nat)
    (cs : Constraints) (S : SubstInfo) (relS : ValidSubstRelation cs S)
    (h : Constraints.unifyCore cs S = .ok relS)
  ...
  | cons c c_rest ih =>
    ...
    · rename_i relS_one h_one_raw
      ...
      · rename_i relS_rest h_rest
        simp only [Except.ok.injEq] at h; subst h
        have h_c_head : Constraint.WellKinded arity c := h_cs c List.mem_cons_self
        have h_c_rest : Constraints.WellKinded arity c_rest :=
          fun c' hc' => h_cs c' (List.mem_cons_of_mem c hc')
        have h_one_wk := Constraint.unifyOne_RangeWellKinded arity c S relS_one h_one h_c_head h_S
        exact ih relS_one.newS relS_rest h_rest h_c_rest h_one_wk

/-- **Top-level: `Constraints.unify` produces a range-well-kinded substitution**
    from well-kinded input constraints and a range-well-kinded incoming subst. -/
theorem Constraints.unify_RangeWellKinded (arity : String → Option Nat)
    (cs : Constraints) (S S' : SubstInfo)
    (h : Constraints.unify cs S = .ok S')
    (h_cs : Constraints.WellKinded arity cs)
    (h_S : Subst.RangeWellKinded arity S.subst) :
    Subst.RangeWellKinded arity S'.subst := by
  simp only [Constraints.unify, Bind.bind, Except.bind] at h
  split at h
  · simp at h
  · rename_i relS h_core
    simp only [Except.ok.injEq] at h; subst h
    exact Constraints.unifyCore_RangeWellKinded arity cs S relS h_core h_cs h_S

/-- Removing a key preserves `RangeWellKinded` (never introduces a new value). -/
theorem Subst.RangeWellKinded.remove (arity : String → Option Nat) {S : Subst}
    (h : Subst.RangeWellKinded arity S) (k : TyIdentifier) :
    Subst.RangeWellKinded arity (HMaps.remove S k) := by
  intro v sty h_find
  by_cases hv : v = k
  · subst hv; rw [HMaps.find?_remove_self] at h_find; simp at h_find
  · rw [HMaps.find?_remove_ne S k v (by simp [bne, hv])] at h_find
    exact h v sty h_find
