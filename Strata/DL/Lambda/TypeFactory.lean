...
/--
Add an `LDatatype` to an existing `TypeFactory`. This checks that no
types are duplicated. If there is a separate `Factory` containing all
constructors, the constructor duplication check will be performed
when the `Factory` generated by `LDatatype.getFactory` is added.
-/
