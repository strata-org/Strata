...
/--
A typeclass that bundles `mkConst` and `cevalTy` for a given `(LMonoTy, ValTy)` pair,
allowing eval combinators to infer them automatically.
-/
...
def polyUneval (n : T.Identifier) (typeArgs : List String)
    ...
    (h_output : output.freeVars ⊆ typeArgs := by first | decide | grind) : WFLFunc T :=
...
@[inline]
def unaryMixedUneval (n : T.Identifier) (inTy outTy : LMonoTy)
...
/-- Unary unevaluated function with same input/output type. -/
@[inline]
def unaryOpUneval (n : T.Identifier) (ty : LMonoTy)
    (hty : ty.freeVars = [] := by decide) : WFLFunc T :=
  unaryMixedUneval n ty ty hty hty

/-- Binary unevaluated function with heterogeneous input types. -/
@[inline]
def binaryHetUneval (n : T.Identifier) (inTy1 inTy2 outTy : LMonoTy)
...
/-- Binary unevaluated function with same-type inputs and mixed output type. -/
@[inline]
def binaryMixedUneval (n : T.Identifier) (inTy outTy : LMonoTy)
    (hInTy : inTy.freeVars = [] := by decide)
    (hOutTy : outTy.freeVars = [] := by decide) : WFLFunc T :=
  binaryHetUneval n inTy inTy outTy hInTy hInTy hOutTy

/-- Binary unevaluated function with same input/output type. -/
@[inline]
def binaryOpUneval (n : T.Identifier) (ty : LMonoTy)
    (hty : ty.freeVars = [] := by decide) : WFLFunc T :=
  binaryMixedUneval n ty ty hty hty

/-- Binary unevaluated predicate (same-type inputs, Bool output). -/
@[inline]
def binaryPredicateUneval (n : T.Identifier) (ty : LMonoTy)
    (hty : ty.freeVars = [] := by decide) : WFLFunc T :=
  binaryMixedUneval n ty .bool hty

/-- Ternary unevaluated function with mixed input types.
    First input has type `inTy1`; second and third share type `inTy2`. -/
@[inline]
def ternaryMixedUneval (n : T.Identifier) (inTy1 inTy2 outTy : LMonoTy)
    (hInTy1 : inTy1.freeVars = [] := by decide)
    (hInTy2 : inTy2.freeVars = [] := by decide)
    (hOutTy : outTy.freeVars = [] := by decide) : WFLFunc T :=
...
/-- Binary operation with mixed input/output types, optional guard, and concrete evaluation.
    The guard is checked on the second argument; `none` is returned if it fails. -/
@[inline]
  ...
  ⟨{ name := n,
     ...
     concreteEval := some (fun md args => match args with
       | [x, y] => match cevalInTy x, cevalInTy y with
         | some a, some b => if guard b then mkConst md (op a b) else .none
         | _, _ => .none
       | _ => none) }, {
    arg_nodup := by simp
    body_freevars := by intro b hb; simp at hb
