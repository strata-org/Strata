...
/-- Returns ALL operator/function names referenced in an expression
    (e.g., `.op` constructs in Lambda).  Used by `defUseWellFormed` to
    enforce that every function name applied in an expression has been
    introduced by a preceding `funcDecl`.  This in turn lets us conclude
    that subsequent `funcDecl`s (which add fresh names) cannot change the
    meaning of already-typechecked expressions. -/
