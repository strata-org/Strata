  ...
  | .init x xty e md =>
    ...
    | none =>
      if x ∈ TC.freeVars e then
        ...
        let (xty, τ) ← TC.preprocess ctx τ xty md
        let (e, ety, τ) ← TC.inferType ctx τ c e md
        -- let xty_md := (xty, md.pushElem (.label "UserTypeAnnotation") (.msg s!"{format x}"))
        -- let ety_md := (ety, md.pushElem (.label "InferredType") (.expr e))
        let τ ← TC.unifyTypes τ [(xty, ety)] md
