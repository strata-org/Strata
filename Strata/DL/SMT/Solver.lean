...
def spawn (path : String) (args : Array String) : IO Solver := do
  try
    ...
    let suggestion := if path == Core.Options.defaultSolver || path.endsWith Core.Options.defaultSolver then s!" Ensure {Core.Options.defaultSolver} is on your PATH or use --solver to specify another SMT solver." else ""
