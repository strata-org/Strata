...
structure EncoderState where
  /-- Maps a `Term` to its abbreviated `Term` (a `Term.var` with name like `$__t.0`).
      Caches terms that have been emitted as `define-fun` to avoid duplication. -/
...
/-- Emit a `define-fun` for a term. When not in a binder, emits a
...
def defineSet (ty : TermType) (tEncs : List Term) : EncoderM Term := do
  ...
  defineTerm false ty result

def defineRecord (ty : TermType) (tEncs : List Term) : EncoderM Term := do
  defineTerm false ty (.app (.datatype_op .constructor ty.mkName) tEncs ty)
