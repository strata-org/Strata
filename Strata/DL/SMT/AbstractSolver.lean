...
opaque term type `τ`, an opaque sort type `σ`, and a monad `m`. Failures are
signaled by throwing `IO.Error` through `MonadExceptOf IO.Error m`, so methods
return `m τ` directly without an explicit `Except String`. The monad `m`
captures any state or effects the backend needs.
...
Term constructors are fallible: solvers might not accept certain constructs
(e.g., wrong sorts, unsupported combinations). Failures are reported by
throwing an `IO.Error` through `MonadExceptOf IO.Error m`, so each method
returns `m τ` directly. -/
