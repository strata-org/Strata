...
private def genericCompositeCorpus : List Case := [
...
procedure u() opaque { var b: Box<int> := new Box; b#val := 1; assert b#val == 1 };"},
  ...
  { name := "new_typeargs_self_seed", outcome := .verifies,
    ...
    src := r"
composite Box<T> { var val: T }
procedure sink<T>(x: T): int { return 0 };
...
procedure u() opaque { var b: B<int> := new Box<int>; b#val := 5; assert b#val == 5 };"},
  ...
  { name := "generic_alias_cyclic", outcome := .rejectedExactly .userError,
...
/-- The corpus DSL pins a diagnostic's KIND but not its source location. This pins the
    ...
    name clash is reported AT the user's declaration, not the source-less synthetic name.
    A bare `!= <unknown>` check is too weak — it passes even if the range points at the wrong
    token or the whole program — so this pins the RESOLVED range of the located diagnostic
    (line 3, cols 10-20 = the `Box$a1$int` name), which regresses if the location drifts to
    another decl OR collapses to `<unknown>`. -/
private def monomorphCollisionIsLocated : IO Unit := do
  let src := "
composite Box<T> { var val: T }
composite Box$a1$int { var val: bool }
procedure u() opaque { var bi: Box<int> := new Box<int>; bi#val := 7; assert bi#val == 7 };"
  ...
  | .ok prog =>
    ...
    match diags.find? (fun d => d.kind == .userError && d.fileRange.file != Uri.file "<unknown>") with
    | none => IO.println "collision diagnostic located at user source: false"
    | some d => IO.println s!"collision diagnostic located at: {(d.fileRange.format (some fileMap)).pretty}"
