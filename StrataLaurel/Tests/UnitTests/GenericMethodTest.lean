...
meta import StrataLaurel.Implementation.Grammar.LaurelGrammar
meta import StrataLaurel.Implementation.Grammar.ConcreteToAbstractTreeTranslator
meta import StrataLaurel.Implementation.LaurelCompilationPipeline
meta import all StrataLaurel.Tests.Util.LaurelCorpusHarness
...
-- `substitutedAncestors`, which applies the `extends` remap; a non-substituting check would
-- accept the WRONG supertype (UNSOUND). These pin both the correct upcasts AND that the wrong
...
private def genericMethodCorpus : List Case := [
...
procedure u() opaque { var x: Box<bool> := new Box<bool>; x#b := 7; var p: Base<int> := x; assert p#b == 6 };"},
  ...
  { name := "generic_upcast_wrong_inst", outcome := .rejectedExactly .userError,
...
procedure u() opaque { var b: Box<int> := new Box<int>; var p: Base<bool> := b; assert 1 == 1 };"},
  ...
  { name := "generic_upcast_remap_wrong_target", outcome := .rejectedExactly .userError,
    why := "`P2<int,bool> extends Pair<B,A>` upcast to the WRONG `Pair<int,bool>` must be REJECTED — the true supertype is `Pair<bool,int>`, not `Pair<int,bool>`"
    src := r"
composite Pair<A,B> { var fst: A var snd: B }
composite P2<A,B> extends Pair<B,A> { var extra: int }
procedure u() opaque { var x: P2<int,bool> := new P2<int,bool>; var p: Pair<int,bool> := x; assert 1 == 1 };"},

  { name := "generic_upcast_concretization_wrong_target", outcome := .rejectedExactly .userError,
    why := "`Box<bool> extends Base<int>` upcast to `Base<bool>` must be REJECTED — the supertype is `Base<int>`, not `Base<bool>`"
    src := r"
composite Base<S> { var b: S }
composite Box<T> extends Base<int> { var v: T }
procedure u() opaque { var x: Box<bool> := new Box<bool>; var p: Base<bool> := x; assert 1 == 1 };"},

  { name := "generic_upcast_unrelated", outcome := .rejectedExactly .userError,
...
procedure u() opaque { var d: D<int> := new D<int>; var x: int := d#f; assert 1 == 1 };"},
  ...
  { name := "field_tvar_own_write_wrong", outcome := .rejectedExactly .userError,
...
procedure u(g: GHolder<Pair<int,bool>>) opaque modifies g { var o: Other := new Other; g#h := o };"},
  ...
  { name := "field_tvar_own_read_wrong", outcome := .rejectedExactly .userError,
...
procedure u(g: GHolder<Pair<int,bool>>) opaque { var o: Other := g#h };"},
  ...
  { name := "field_tvar_inherited_write_wrong", outcome := .rejectedExactly .userError,
...
procedure u(g: GHolder<Pair<int,bool>>) opaque modifies g { var p: Pair<int,bool> := new Pair<int,bool>; g#h := p; assert 1 == 1 };"},
  { name := "field_tvar_inherited_remap_write_wrong", outcome := .rejectedExactly .userError,
...
procedure u() opaque { var s: Super := new Super; s#val := 9; var b: Box<int> := s; assert b#val == 9 };"},
  ...
  { name := "concrete_extends_geninst_wrong_target", outcome := .rejectedExactly .userError,
...
procedure u() opaque { var ib: IntBox := new IntBox; var b: Box<bool> := ib; assert 1 == 1 };"},
  ...
  { name := "concrete_extends_geninst_remap_swap", outcome := .rejectedExactly .userError,
...
procedure u() opaque { var ib: IntBox := new IntBox; ib#val := 5; var b: Box<int> := ib; var got: int := b#val; assert got == 6 };"},
  ...
  { name := "as_guarded_downcast_heap_neutral", outcome := .verifies,
    why := "`if (p is Child) then ... p as Child ...` in a heap-neutral opaque proc on an opaque `Parent` param translates + verifies; the guard discharges the cast's is-obligation"
    src := r"
...
private def mapConstInternalRoundtripsAsTypeTag : IO Unit := do
  let src := "
composite Animal { var legs: int }
composite Dog extends Animal { var breed: int }
procedure u(d: Dog) opaque modifies d { d#legs := 4; assert d#legs == 4 };"
  let prog ← corpusParse "mapConst_typetag_roundtrip" src
  let (core?, _diags) ← Laurel.translate default prog
  match core? with
  | none => IO.println "no Core program produced"
  | some core =>
    let text := (Core.formatProgram core).pretty
    let hasTypeTag := (text.splitOn "mapConst<TypeTag>").length != 1
    let hasUnknown := (text.splitOn "$__unknown_type").length != 1
    IO.println s!"mapConst<TypeTag> present: {hasTypeTag}; $__unknown_type present: {hasUnknown}"

/-- info: mapConst<TypeTag> present: true; $__unknown_type present: false -/
#guard_msgs in
#eval mapConstInternalRoundtripsAsTypeTag
