...
partial def collectLocalsFromStmt (s : PythonStmt) : List (PythonIdentifier × PythonType) :=
  ...
  | .AnnAssign _ target annotation valueOpt _ =>
      -- Trust the user's annotation (v2 verbatim): an annotated local keeps its declared
      -- type. The coercion mechanism reconciles the boxed-`Any` RHS with it.
...
/-- All procedure inputs as `(Laurel.Identifier × PythonType)`. For instance
    ...
    The receiver MUST be typed as the class (not `Any`): the kbd Laurel resolver
    resolves `self.field` via the receiver's static type (`resolveFieldRef` →
    `targetTypeName` → type scope). With `self : Any` the field would instead resolve
    to a same-named parameter (e.g. `__init__(self, name)` writing `self.name`), giving
    "resolveQualifiedFieldName resolved to something other than a field". This matches
    the proven PythonToLaurel pipeline, which types `self` as the class/Composite. -/
