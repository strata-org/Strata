...
def dummyClient : Core.Expression.Expr := .fvar pySynthLoc "DUMMY_CLIENT" none

def dictStrAnyType : Core.Expression.Ty := .forAll [] (.tcons "DictStrAny" [])
def dummyDictStrAny : Core.Expression.Expr := .fvar pySynthLoc "DUMMY_DICT_STR_ANY" none

def strType : Core.Expression.Ty := .forAll [] (.tcons "string" [])
def dummyStr : Core.Expression.Expr := .fvar pySynthLoc "DUMMY_STR" none

def listStrType : Core.Expression.Ty := .forAll [] (.tcons "ListStr" [])
def dummyListStr : Core.Expression.Expr := .fvar pySynthLoc "DUMMY_LIST_STR" none

def datetimeType : Core.Expression.Ty := .forAll [] (.tcons "Datetime" [])
def dummyDatetime : Core.Expression.Expr := .fvar pySynthLoc "DUMMY_DATETIME" none

def dateType : Core.Expression.Ty := .forAll [] (.tcons "Date" [])
def dummyDate : Core.Expression.Expr := .fvar pySynthLoc "DUMMY_DATE" none

def timedeltaType : Core.Expression.Ty := .forAll [] (.tcons "int" [])
def dummyTimedelta : Core.Expression.Expr := .fvar pySynthLoc "DUMMY_Timedelta" none
...
def PyConstToCore (loc : ExprSourceLoc) (c: constant SourceRange) : Core.Expression.Expr :=
  match c with
...
def handleAdd (translation_ctx: TranslationContext) (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  ...
  | .fvar _ l _, .fvar _ r _ =>
    ...
    | some (_, .tcons "int" []), some (_, .tcons "int" []) =>
      .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Add⟩) (some mty[int → (int → int)])) lhs) rhs
    | some (_, .tcons "string" []), some (_, .tcons "string" []) =>
      .app pySynthLoc (.app pySynthLoc (.op pySynthLoc "Str.Concat" mty[string → (string → string)]) lhs) rhs
    | _, _ => panic! s!"Unsupported types for +. Exprs: {lhs} and {rhs}"
  | _, _ => .app pySynthLoc (.app pySynthLoc (.op pySynthLoc "Str.Concat" mty[string → (string → string)]) lhs) rhs
...
def handleSub (translation_ctx: TranslationContext) (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  ...
  | .fvar _ l _, .fvar _ r _ =>
    ...
    match l_ty, r_ty with
    | some (_, .tcons "int" []), some (_, .tcons "int" []) =>
      .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Sub⟩) (some mty[int → (int → int)])) lhs) rhs
    | some (_, .tcons "Datetime" []), some (_, .tcons "int" []) =>
      .app pySynthLoc (.app pySynthLoc (.op pySynthLoc "Datetime_sub" none) lhs) rhs
...
def handleMult (translation_ctx: TranslationContext) (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  match lhs, rhs with
  | .strConst _ s, .intConst _ i => .strConst pySynthLoc (String.join (List.replicate i.toNat s))
  | .intConst _ l, .intConst _ r => .intConst pySynthLoc (l * r)
  | .fvar _ l _, .fvar _ r _ =>
    ...
    | .some lty, .some rty =>
      ...
      | .tcons "int" [], .tcons "int" [] => .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Mul⟩) (some mty[int → (int → int)])) lhs) rhs
...
def handleFloorDiv (_translation_ctx: TranslationContext) (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Div⟩) (some mty[int → (int → int)])) lhs) rhs

def handleNot (arg: Core.Expression.Expr) : Core.Expression.Expr :=
  let ty : Lambda.LMonoTy := (.tcons "ListStr" [])
  match ty with
  | (.tcons "ListStr" []) => .eq pySynthLoc arg (.op pySynthLoc "ListStr_nil" none)
...
def handleLt (translation_ctx: TranslationContext) (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  ...
  | .fvar _ l _, .fvar _ r _ =>
    ...
    | some (_, .tcons "Datetime" []), some (_, .tcons "Datetime" []) =>
      .app pySynthLoc (.app pySynthLoc (.op pySynthLoc "Datetime_lt" none) lhs) rhs
    | _, _ => .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Lt⟩) (some mty[int → (int → bool)])) lhs) rhs
  | _, _ => .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Lt⟩) (some mty[int → (int → bool)])) lhs) rhs
...
def handleLtE (translation_ctx: TranslationContext) (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  ...
  | .fvar _ l _, .fvar _ r _ =>
    ...
    match l_ty, r_ty with
    | some (_, .tcons "Datetime" []), some (_, .tcons "Datetime" []) =>
      let eq := (.eq pySynthLoc lhs rhs)
      let lt := (.app pySynthLoc (.app pySynthLoc (.op pySynthLoc "Datetime_lt" none) lhs) rhs)
      (.app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.bool .Or)) eq) lt)
    | _, _ => .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Le⟩) (some mty[int → (int → bool)])) lhs) rhs
  | _, _ => .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Le⟩) (some mty[int → (int → bool)])) lhs) rhs

def handleGt (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Gt⟩) (some mty[int → (int → bool)])) lhs) rhs

def handleGtE (lhs rhs: Core.Expression.Expr) : Core.Expression.Expr :=
  .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Ge⟩) (some mty[int → (int → bool)])) lhs) rhs
...
def PyListStrToCore (names : Array (alias SourceRange)) : Core.Expression.Expr :=
  .app pySynthLoc (.app pySynthLoc (.op pySynthLoc "ListStr_cons" mty[string → (ListStr → ListStr)]) (PyAliasToCoreExpr names[0]!))
       (.op pySynthLoc "ListStr_nil" mty[ListStr])

def handleList (_elmts: Array (expr SourceRange)) (expected_type : Lambda.LMonoTy): PyExprTranslated :=
  match expected_type with
  | (.tcons "ListStr" _) => {stmts := [], expr := (.op pySynthLoc "ListStr_nil" expected_type)}
  | (.tcons "ListDictStrAny" _) => {stmts := [], expr := (.op pySynthLoc "ListDictStrAny_nil" expected_type)}
...
def noneOrExpr (translation_ctx : TranslationContext) (fname n : String) (e: Core.Expression.Expr) : Core.Expression.Expr :=
  ...
  | .ok type_str =>
    if type_str.endsWith "OrNone" then
      ...
      match type_str with
      | "IntOrNone" => .app pySynthLoc (.op pySynthLoc "IntOrNone_mk_int" none) e
      | "StrOrNone" => .app pySynthLoc (.op pySynthLoc "StrOrNone_mk_str" none) e
      | "BytesOrStrOrNone" => .app pySynthLoc (.op pySynthLoc "BytesOrStrOrNone_mk_str" none) e
      | _ => panic! "Unsupported type_str: "++ type_str
    else
      e

def handleCallThrow (jmp_target : String) : Core.Statement :=
  let cond := .app pySynthLoc (.op pySynthLoc "ExceptOrNone..isExceptOrNone_mk_code" none) (.fvar pySynthLoc "maybe_except" none)
...
partial def collectVarDecls (translation_ctx : TranslationContext) (stmts: Array (stmt SourceRange)) : List Core.Statement :=
  ...
  let toCore (p: String × String) : List Core.Statement :=
    ...
    let ty_name := p.snd
    match ty_name with
    | "bool" => [(.init name t[bool] (.det (.boolConst pySynthLoc false)) .empty), (.havoc name .empty)]
    | "str" => [(.init name t[string] (.det (.strConst pySynthLoc "")) .empty), (.havoc name .empty)]
    | "int" => [(.init name t[int] (.det (.intConst pySynthLoc 0)) .empty), (.havoc name .empty)]
    | "float" => [(.init name t[string] (.det (.strConst pySynthLoc "0.0")) .empty), (.havoc name .empty)] -- Floats as strs for now
    | "bytes" => [(.init name t[string] (.det (.strConst pySynthLoc "")) .empty), (.havoc name .empty)]
    ...
    | _ =>
      ...
      | .some i =>
        ...
        let user_defined_class_dummy := .fvar pySynthLoc ("DUMMY_" ++ i.name) none
...
partial def handleDict (translation_ctx: TranslationContext) (sr : SourceRange) (keys: Array (opt_expr SourceRange)) (values: Array (expr SourceRange)) : PyExprTranslated :=
  ...
  let dict := .app pySynthLoc (.op pySynthLoc "DictStrAny_mk" none) (.strConst pySynthLoc "DefaultDict") -- TODO: need to generate unique dict arg
  assert! keys.size == values.size
  let zipped := Array.zip keys values

  let res := zipped.toList.flatMap (λ (k, v) =>
    let n := PyOptExprToString k
    let in_dict := (.assume s!"assume_{n}_in_dict" (.app pySynthLoc (.app pySynthLoc (.op pySynthLoc "str_in_dict_str_any" none) (.strConst pySynthLoc n)) dict) md)
    match v with
    | .Call _ f args _ =>
      match f with
      | .Name _ {ann := _ , val := "str"} _ =>
        assert! args.val.size == 1
        let dt := (.app pySynthLoc (.op pySynthLoc "datetime_to_str" none) ((PyExprToCore default args.val[0]!).expr))
        let dict_of_v_is_k := (.assume s!"assume_{n}_key" (.eq pySynthLoc (.app pySynthLoc (.app pySynthLoc (.op pySynthLoc "dict_str_any_get_str" none) dict) (.strConst pySynthLoc n)) dt) md)
        [in_dict, dict_of_v_is_k]
      | _ => panic! "Unsupported function when constructing map"
    | _ =>
      let dict_of_v_is_k := (.assume s!"assume_{n}_key" (.eq pySynthLoc (.app pySynthLoc (.app pySynthLoc (.op pySynthLoc "dict_str_any_get_str" none) dict) (.strConst pySynthLoc n)) (.strConst pySynthLoc "DummyVal")) md)
      [in_dict, dict_of_v_is_k])
...
partial def initTmpParam (translation_ctx: TranslationContext) (p: expr SourceRange × String) : List Core.Statement :=
  ...
  | .Call _ f args _ =>
    ...
    | .Name _ n _ =>
      match n.val with
      | "json_dumps" => [(.init p.snd t[string] (.det (.strConst pySynthLoc "")) md), .call "json_dumps" ([.inArg (.app pySynthLoc (.op pySynthLoc "DictStrAny_mk" none) (.strConst pySynthLoc "DefaultDict")), .inArg (TypeStrToCoreExpr "IntOrNone")] ++ [.outArg p.snd, .outArg "maybe_except"]) md]
      | "str" =>
        assert! args.val.size == 1
        [(.init p.snd t[string] (.det (.strConst pySynthLoc "")) md), .set p.snd (.app pySynthLoc (.op pySynthLoc "datetime_to_str" none) ((PyExprToCore default args.val[0]!).expr)) md]
      | "int" => [(.init p.snd t[int] (.det (.intConst pySynthLoc 0)) md), .set p.snd (.op pySynthLoc "datetime_to_int" none) md]
...
partial def exceptHandlersToCore (jmp_targets: List String) (translation_ctx: TranslationContext) (h : excepthandler SourceRange) : List Core.Statement :=
  ...
  | .ExceptHandler sr ex_ty _ body =>
    ...
    | .some ex_ty =>
      ...
      let exception_ty : Core.Expression.Expr := .app pySynthLoc (.op pySynthLoc get_ex_tag none) (.fvar pySynthLoc "maybe_except" none)
      let rhs_curried : Core.Expression.Expr := .app pySynthLoc (.op pySynthLoc inherits_from none) exception_ty
      let res := PyExprToCore translation_ctx ex_ty
      let rhs : Core.Expression.Expr := .app pySynthLoc rhs_curried (res.expr)
      let call := .set "exception_ty_matches" rhs md
      res.stmts ++ [call]
    | .none =>
      [.set "exception_ty_matches" (.boolConst pySynthLoc false) md]
    let cond := .fvar pySynthLoc "exception_ty_matches" none
...
partial def handleFunctionCall (lhs: List Core.Expression.Ident)
  ...
  let substitution_records : List SubstitutionRecord := args_calls_to_tmps.toList.map (λ p => {pyExpr := p.fst, coreExpr := .fvar pySynthLoc p.snd none}) ++
                                                        kwords_calls_to_tmps.toList.map (λ p => {pyExpr := p.fst, coreExpr := .fvar pySynthLoc p.snd none})
...
partial def handleComprehension (translation_ctx: TranslationContext) (lhs: expr SourceRange) (gen: Array (comprehension SourceRange)) : List Core.Statement :=
  ...
  | .mk_comprehension sr _ itr _ _ =>
    ...
    let guard := .app pySynthLoc (Core.coreOpExpr (.bool .Not)) (.eq pySynthLoc (.app pySynthLoc (.op pySynthLoc "dict_str_any_length" none) res.expr) (.intConst pySynthLoc 0))
    let then_ss: List Core.Statement := [.havoc (PyExprToString lhs) md]
    let else_ss: List Core.Statement := [.set (PyExprToString lhs) (.op pySynthLoc "ListStr_nil" none) md]
...
partial def PyStmtToCore (jmp_targets: List String) (translation_ctx : TranslationContext) (s : stmt SourceRange) : List Core.Statement × TranslationContext :=
  ...
  let non_throw : List Core.Statement × Option (String × Lambda.LMonoTy) := match s with
    ...
    | .For _ tgt itr body _ _ =>
      -- Do one unrolling:
      let guard := .app pySynthLoc (Core.coreOpExpr (.bool .Not)) (.eq pySynthLoc (.app pySynthLoc (.op pySynthLoc "dict_str_any_length" none) (PyExprToCore default itr).expr) (.intConst pySynthLoc 0))
      match tgt with
      | .Name _ n _ =>
    ...
    | .While _ test body _ =>
      -- Do one unrolling:
      let guard := .app pySynthLoc (Core.coreOpExpr (.bool .Not)) (.eq pySynthLoc (.app pySynthLoc (.op pySynthLoc "dict_str_any_length" none) (PyExprToCore default test).expr) (.intConst pySynthLoc 0))
    ...
    | .AugAssign _ lhs op rhs =>
      ...
      | .Add _ =>
        ...
        | .Name _ n _ =>
          let rhs := PyExprToCore translation_ctx rhs
          let new_lhs := (.strConst pySynthLoc "DUMMY_FLOAT")
      ...
      | .FloorDiv _ =>
        ...
        | .Name _ n _ =>
          let lhs := PyExprToCore translation_ctx lhs
          let rhs := PyExprToCore translation_ctx rhs
          let new_lhs := .app pySynthLoc (.app pySynthLoc (Core.coreOpExpr (.numeric ⟨.int, .Div⟩) (some mty[int → (int → int)])) lhs.expr) rhs.expr
          (rhs.stmts ++ [.set n.val new_lhs md], none)
        | _ => panic! s!"Expected lhs to be name: {repr lhs}"
...
def pythonFuncToCore (name : String) (args: List (String × String)) (body: Array (stmt SourceRange)) (ret : Option (expr SourceRange)) (spec : Core.Procedure.Spec) (translation_ctx : TranslationContext) : Core.Procedure :=
  ...
  let varDecls := collectVarDecls translation_ctx body ++ [(.init "exception_ty_matches" t[bool] (.det (.boolConst pySynthLoc false)) .empty), (.havoc "exception_ty_matches" .empty)]
