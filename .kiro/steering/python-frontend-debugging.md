...
4. **Regenerate PySpecs if stale.** PySpec Ion files are produced from the
   per-library `.py` stubs via the `pySpecs` subcommand:
   ```
   lake exe strata pySpecs <python-source-dir> <output-dir>
   ```
   The `<output-dir>` is what `pyAnalyzeLaurel` consumes via `--spec-dir`.
   Rerun after editing any PySpec stub, or when a benchmark fails with
   resolver errors that look like they come from mismatched spec shapes.
