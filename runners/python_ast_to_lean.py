...
def parse_call(j):
    """Parse Python Call (function call)"""
    target_j = {
        "func": parse_expression(j['func']),
        "args": [parse_expression(arg) for arg in j['args']],
...
def parse_import(j):
    ...
    # Extract module names from the names array
    names = []
    for alias in j.get('names', []):
...
def parse_expr(j):
    """Parse Python Expr statement (expression statement)"""
    target_j = {
        "value": parse_expression(j['value'])
    }
    add_missing_node_info(j, target_j)
...
def parse_functiondef(j):
    """Parse Python FunctionDef statement"""
    # Extract parameter names from args
    args = []
    if 'args' in j and 'args' in j['args']:
        for arg in j['args']['args']:
    ...
    target_j = {
        ...
        "args": args,
        "body": [parse_statement(stmt) for stmt in j['body']],
        "returns": parse_expression(j['returns']) if j.get('returns') else None
    }
    add_missing_node_info(j, target_j)
    return target_j

def parse_return(j):
    """Parse Python Return statement"""
    target_j = {
        "value": parse_expression(j['value']) if j.get('value') else None
    }
    add_missing_node_info(j, target_j)
    return target_j
...
def parse_statement(j):
    ...
    match j.get('_type'):
        ...
        case "Import":
            return {"Py_Import": parse_import(j)}
        case "FunctionDef":
            return {"Py_FunctionDef": parse_functiondef(j)}
        case "Return":
            return {"Py_Return": parse_return(j)}
        case "Expr":
            # Expression statements (like function calls)
            return {"Py_Expr": parse_expr(j)}
        case _:
